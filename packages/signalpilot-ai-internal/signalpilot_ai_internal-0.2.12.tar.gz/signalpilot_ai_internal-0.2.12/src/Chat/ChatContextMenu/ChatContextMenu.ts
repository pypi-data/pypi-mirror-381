...
import {ChatContextLoaders, MENTION_CATEGORIES, MentionContext} from './ChatContextLoaders';
...
export class ChatContextMenu {
  ...
  private async refreshContextItems(categoryId: string): Promise<void> {
    console.log(`[ChatContextMenu] Refreshing context items for category: ${categoryId}`);
    ...
    try {
      switch (categoryId) {
        case 'data':
          ...
          console.log(`[ChatContextMenu] Loaded ${dataItems.length} data items from cache`);
          break;
        case 'snippets':
      ...
      console.error(`[ChatContextMenu] Error refreshing ${categoryId} category:`, error);
  ...
  private updateCategoryHeader(): void {
    ...
    if (this.currentView === 'items' && this.selectedCategory) {
      ...
      if (this.selectedCategory === 'data' && this.currentDataPath !== './data') {
   ...
   */
  private filterItemsForCurrentDirectory(items: MentionContext[], currentPath: string): MentionContext[] {
    ...
    if (normalizedCurrentPath === './data') {
      return items.filter(item => {
        ...
        const relativePath = itemPath.replace('./data/', '').replace('./data', '');
    ...
    return items.filter(item => {
      ...
      const expectedPrefix = normalizedCurrentPath === './data' ? './data/' : `${normalizedCurrentPath}/`;
  ...
  private async renderCategoryItems(
    ...
    // For data category, filter items to show only those in the current directory level
    if (categoryId === 'data') {
      // Filter items to only show those that belong to the current directory level
      const currentLevelItems = this.filterItemsForCurrentDirectory(items, this.currentDataPath);
      
      const directories = currentLevelItems.filter((item: MentionContext) => item.isDirectory);
      const files = currentLevelItems.filter((item: MentionContext) => !item.isDirectory);
      ...
      if (searchText) {
        const matchingDirectories: Array<{ item: MentionContext; score: number }> = [];
        const matchingFiles: Array<{ item: MentionContext; score: number }> = [];

        for (const item of directories) {
          const score = this.calculateRelevanceScore(item, searchText);
          if (score > 500) {
            matchingDirectories.push({ item, score });
          }
        }
        ...
        displayDirectories = matchingDirectories.slice(0, 5).map(({ item }) => item);
        displayFiles = matchingFiles.slice(0, 10).map(({ item }) => item);
      }

      // Render directories first with a different style
      displayDirectories.forEach((item: MentionContext) => {
        const itemElement = createDropdownItemElement(
          item,
          'sage-ai-mention-item sage-ai-mention-subcategory sage-ai-mention-directory'
        );
        this.contentContainer.appendChild(itemElement);
      });
  ...
  private async selectItem(itemId: string): Promise<void> {
    ...
    const currentMentionEnd = this.currentMentionStart + 1 + this.currentMentionText.length;
  ...
  private showLoadingState(message: string = 'Loading contexts...'): void {
    ...
    const existingLoader = this.contentContainer.querySelector('.sage-ai-mention-loading');
    if (existingLoader) {
