...
export class NotebookTools {
  ...
  /**
   * List database tables with fuzzy search capability
   * @param options Configuration options
   * @param options.query Search query to filter table names (optional)
   * @param options.limit Maximum number of results to return (default: 10)
   * @returns JSON string with list of matching tables and their metadata
   */
  async list_databases(options: {
    query?: string;
    limit?: number;
  }): Promise<string> {
    try {
      ...
      if (!activeConfig) {
        return JSON.stringify({
          error: 'No active database configuration found. Please connect to a database first.'
        });
      }

      // Get schema information
      const schemaInfo = DatabaseStateService.getSchemaInfo(activeConfig.id);
      if (!schemaInfo || !schemaInfo.schema) {
        return JSON.stringify({
          error: 'No schema information available. Please fetch schema first using database configuration.'
        });
      }
      ...
      let parsedSchema;
      try {
        parsedSchema = JSON.parse(schemaInfo.schema);
      } catch (e) {
        return JSON.stringify({
          error: 'Failed to parse schema information.'
        });
      }
      ...
      // Perform fuzzy search if query is provided
      ...
      // Limit results
      const limitedTables = filteredTables.slice(0, limit);

      return JSON.stringify({
        database: activeConfig.name,
        database_type: activeConfig.type,
        query_used: query,
        total_matches: filteredTables.length,
        tables: limitedTables.map((table: any) => ({
  ...
  async read_databases(options: {
    ...
    try {
      ...
      if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(tableName)) {
        return JSON.stringify({
          error: 'Invalid table name. Table names must contain only letters, numbers, and underscores.'
        });
      }
      ...
      } else if (activeConfig.connectionType === 'credentials' && activeConfig.credentials) {
