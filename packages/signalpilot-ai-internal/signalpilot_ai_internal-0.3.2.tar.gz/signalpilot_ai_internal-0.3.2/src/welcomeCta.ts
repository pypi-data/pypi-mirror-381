  ...
  app.commands.addCommand(addCtaDivCommand, {
    ...
    execute: async () => {
      try {
        ...
        const contentDiv = ctaContainer.querySelector(
          '.sage-ai-data-cta-content'
        ) as HTMLElement;
        ...
        if (outerPanelElement) {
          ...
          console.log('[WelcomeCTA] Data CTA interface added to jp-WindowedPanel-outer');
        } else {
          console.warn('[WelcomeCTA] Could not find jp-WindowedPanel-outer, trying fallback');
          ...
          if (notebookElement) {
            ...
            if (fallbackOuter) {
              fallbackOuter.insertBefore(ctaContainer, fallbackOuter.firstChild);
              console.log('[WelcomeCTA] Data CTA interface added via fallback');
            } else {
              currentNotebook.node.appendChild(ctaContainer);
              console.log('[WelcomeCTA] Data CTA interface added to notebook panel (final fallback)');
            }
          } else {
            currentNotebook.node.appendChild(ctaContainer);
            console.log('[WelcomeCTA] Data CTA interface added to notebook panel (final fallback)');
