...
export class DatabaseTools {
  ...
  private async createTemporaryKernel(): Promise<{
    ...
    try {
      ...
      // Set up database environments in the temporary kernel using the new multi-database approach
      // Import KernelUtils to use the new database environment setup
      const { KernelUtils } = await import('../utils/kernelUtils');
      
      // Set up all database environments in the temporary kernel
      const toolService = AppStateService.getToolService();
      const currentKernel = toolService?.getCurrentNotebook()?.kernel;
      
      // Temporarily switch the current kernel to our temporary one
      if (toolService && toolService.getCurrentNotebook) {
        const originalKernel = currentKernel;
        try {
          // Monkey patch to make our temporary kernel the "current" one
          const currentNotebook = toolService.getCurrentNotebook();
          if (currentNotebook) {
            const originalKernelRef = currentNotebook.kernel;
            currentNotebook.kernel = session.kernel;
            
            // Now call KernelUtils which will use our temporary kernel
            KernelUtils.setDatabaseEnvironmentsInKernel();
            
            // Restore the original kernel reference
            currentNotebook.kernel = originalKernelRef;
          }
        } catch (setupError) {
          console.warn('[DatabaseTools] Error setting up database environments in temporary kernel:', setupError);
          ...
          if (databaseUrl) {
            console.log('[DatabaseTools] Falling back to legacy DB_URL setup in temporary kernel');
            const setupCode = `
import os
os.environ['DB_URL'] = '${databaseUrl.replace(/'/g, "\\'")}'
print("[DatabaseTools] DB_URL environment variable set in temporary kernel (fallback)")
            `;
...
# Function to install packages and setup database dependencies
def setup_database_environment():
    ...
    if missing_packages:
        print(json.dumps({"error": "Required modules could not be installed: " + ", ".join(missing_packages)}))
        exit()
    
    return sqlalchemy, create_engine, text

# Setup database environment
sqlalchemy, create_engine, text = setup_database_environment()

try:
    # Use provided dbUrl or try to get from environment
    db_url = ${dbUrl ? `'${dbUrl.replace(/'/g, "\\'")}'` : "None"}
    
    # If no specific dbUrl provided, try to use from environment variables
    if not db_url:
        ...
        for key, value in os.environ.items():
            if key.endswith('_CONNECTION_JSON'):
                try:
                    ...
                    # Use the first available database configuration if no specific one requested
                    if not db_url_found:
                        if 'connectionUrl' in config:
                            ...
                            if config['type'] == 'postgresql':
                                ...
                                db_url = f"mysql://{config['username']}:{config['password']}@{config['host']}:{config['port']}/{config['database']}"
                            elif config['type'] == 'snowflake':
                                role_param = f"&role={config['role']}" if 'role' in config else ""
                                db_url = f"snowflake://{config['username']}:{config['password']}@{config['account']}/{config['database']}?warehouse={config['warehouse']}{role_param}"
                        
                        if db_url:
                            db_url_found = True
                            print(f"[DatabaseTools] Using database configuration: {config['name']}")
                            break
                except Exception as e:
                    print(f"[DatabaseTools] Error parsing database config {key}: {e}")
                    continue
        ...
        if not db_url:
            ...
            if db_url:
                print("[DatabaseTools] Using legacy DB_URL environment variable")
    
    if not db_url:
        print(json.dumps({"error": "No database URL provided and no database configurations found in environment"}))
    else:
        from sqlalchemy import create_engine, text
        ...
        with engine.connect() as conn:
            ...
            if not tables:
                ...
                # Return both markdown and individual table schemas
                result = {
                    "result": markdown_output.strip(),
                    "table_schemas": table_schemas
                }
                print(f"[DatabaseTools] Returning JSON with {len(table_schemas)} tables", file=sys.stderr)
...
`;
          ...
          .then(() => {
            try {
              ...
              if (result.error) {
                ...
                // Return the full JSON result (with both schema and table_schemas)
              ...
              reject(
                new Error(`Error parsing database query result: ${output}`)
              );
            } finally {
      ...
      throw `Error getting database schema info: ${error instanceof Error ? error.message : String(error)}`;
    }
  }
