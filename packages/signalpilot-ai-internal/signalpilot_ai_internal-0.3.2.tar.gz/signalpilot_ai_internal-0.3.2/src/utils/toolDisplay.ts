...
> = {
  ...
  'database-list_databases': ctx => {
    if (ctx.query) {
      return `SignalPilot is searching for database tables matching ${codeWrapper(ctx.query)}...`;
    }
    return 'SignalPilot is listing database tables...';
  },
