  ...
  app.commands.addCommand(addCtaDivCommand, {
    ...
    execute: async () => {
      try {
        ...
        const existingCta = currentNotebook.node.querySelector('.sage-ai-data-cta-container');
        if (existingCta) {
          console.log('[WelcomeCTA] CTA already exists in this notebook, skipping render');
          return;
        }
        ...
        const isCollapsed = localStorage.getItem(CTA_COLLAPSED_KEY) === 'true';
        ...
        ctaContainer.innerHTML = `
          ...
          <div class="sage-ai-data-cta-header">
            <div class="sage-ai-data-cta-title">Welcome to Your AI Data Assistant! 🚀</div>
        ...
        const inputField = ctaContainer.querySelector('.sage-ai-data-cta-input') as HTMLTextAreaElement;
        const sendButton = ctaContainer.querySelector('.sage-ai-data-cta-send-btn') as HTMLButtonElement;
        const filesCard = ctaContainer.querySelector('.sage-ai-data-files-card') as HTMLElement;
        const dbCards = ctaContainer.querySelectorAll('.sage-ai-data-db-card');
        const toggleButton = ctaContainer.querySelector('.sage-ai-data-cta-toggle') as HTMLButtonElement;
        const contentDiv = ctaContainer.querySelector('.sage-ai-data-cta-content') as HTMLElement;
        ...
        toggleButton.addEventListener('click', () => {
          const isCurrentlyCollapsed = ctaContainer.classList.contains('collapsed');
        ...
        const handleSendMessage = () => {
          ...
          if (message) {
            ...
            if (chatContainer?.chatWidget?.inputManager) {
              ...
              chatContainer.chatWidget.inputManager.sendMessage();
        ...
        const handleFileUpload = () => {
          const fileExplorerWidget = AppStateService.getState().fileExplorerWidget;
        ...
        const handleDatabaseConnect = (dbType: string) => {
          const databaseManagerWidget = AppStateService.getState().databaseManagerWidget;
          ...
          if (databaseManagerWidget) {
            ...
            console.log(`[WelcomeCTA] Opening database connection modal for ${dbType}`);
          } else {
        ...
        // Find the notebook element and add the container
        const notebookElement = currentNotebook.node.querySelector(
          '.jp-WindowedPanel-viewport[role="feed"][aria-label="Cells"]'
        );
        if (notebookElement) {
          // Add the CTA container at the top of the notebook
          notebookElement.insertBefore(ctaContainer, notebookElement.firstChild);
          console.log('[WelcomeCTA] Data CTA interface added to notebook');
        } else {
          // Fallback to notebook panel if viewport not found
          currentNotebook.node.appendChild(ctaContainer);
          console.log('[WelcomeCTA] Data CTA interface added to notebook panel (fallback)');
        }
      } catch (error) {
