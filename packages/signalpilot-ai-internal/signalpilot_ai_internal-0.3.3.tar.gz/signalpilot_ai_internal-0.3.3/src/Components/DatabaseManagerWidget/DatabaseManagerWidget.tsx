...
export interface IDatabaseManagerState {
  ...
  initialDatabaseType: DatabaseType | null; // Initial database type to pre-select when opening modal
}
...
function DatabaseManagerContent({
  ...
  onCloseModal: () => void;
  onCreateDatabase: (formData: DatabaseFormData) => Promise<void>;
  onValidateSchema: (
  ...
  const handleAddToContext = async (config: IDatabaseConfig): Promise<void> => {
    ...
    if (!chatContainer) {
      console.error('[DatabaseManagerWidget] Chat container not available');
      return;
    }
    ...
    try {
      ...
      const { getDatabaseById } = await import(
        '../../Chat/ChatContextMenu/databaseHelper'
      );
  ...
  const handleMenuAction = (
    ...
    switch (action) {
      ...
      case 'delete':
        ...
        break;
      case 'addToContext':
        handleAddToContext(config);
        break;
      case 'refreshSchema':
        onRefreshSchema(config);
        break;
      case 'viewSchema':
          ...
          {state.selectedDatabaseForSchema.database_schema ? (
            ...
            state.selectedDatabaseForSchema.type === DatabaseType.Snowflake ? (
              <SnowflakeSchemaViewer
                schemaData={state.selectedDatabaseForSchema.database_schema as SnowflakeSchemaData}
              ...
              <DatabaseSchemaExplorer
                schema={
                  (state.selectedDatabaseForSchema.database_schema as IMySQLPostgreSQLSchema)
                    .table_schemas
                }
                databaseName={state.selectedDatabaseForSchema.name}
                onBack={onBackToDatabaseList}
                      ...
                      <div className="database-detail">
                        ...
                        <span className="detail-value">
                          {database.type === DatabaseType.Snowflake
                            ? ((database.credentials as any)?.connectionUrl || 'N/A')
  ...
  private handleRefreshSchema = async (
    ...
    try {
      ...
      if (dbType === 'snowflake' && snowflakeConfig) {
        ...
        const { DatabaseType: DBToolsType } = await import('../../BackendTools/DatabaseTools');
        ...
        // Import DatabaseType enum
        const { DatabaseType: DBToolsType } = await import('../../BackendTools/DatabaseTools');
        const toolsDbType = dbType === 'postgresql' ? DBToolsType.PostgreSQL : DBToolsType.MySQL;
        schemaResult = await databaseTools.getDatabaseMetadata(databaseUrl, toolsDbType);
      }
  ...
  private handleCreateDatabase = async (
    ...
    try {
      ...
      if (formData.schema && formData.schemaLastUpdated) {
        ...
        let parsedSchema: IMySQLPostgreSQLSchema | SnowflakeSchemaData | null = null;
        try {
          parsedSchema = typeof formData.schema === 'string' 
            ? JSON.parse(formData.schema) 
            : formData.schema;
        } catch (error) {
          console.error('[DatabaseManagerWidget] Failed to parse schema:', error);
        }
