...
export function DatabaseSchemaExplorer({
  ...
  const handleAddTableToContext = async (tableSchema: ITableSchema): Promise<void> => {
    try {
...
${tableSchema.indices.length > 0 ? `Indices:\n${tableSchema.indices.map(idx => 
  `- ${idx.indexname}: ${idx.indexdef}`
      ...
      console.log(`[DatabaseSchemaExplorer] Added table ${tableSchema.table_name} to context`);
    } catch (error) {
      console.error('[DatabaseSchemaExplorer] Error adding table to context:', error);
    }
  };
                ...
                >
                  <div className="table-item-header">
                    <div className="table-name-section">
                      ...
                      <div className="table-full-name">{tableSchema.full_name}</div>
                    </div>
                    <button
                      className="add-to-context-button"
                      onClick={(e) => {
                        e.stopPropagation();
                        handleAddTableToContext(tableSchema);
                      }}
                      title={`Add ${tableSchema.table_name} to context`}
                    >
                      <span className="add-to-context-icon">+</span>
