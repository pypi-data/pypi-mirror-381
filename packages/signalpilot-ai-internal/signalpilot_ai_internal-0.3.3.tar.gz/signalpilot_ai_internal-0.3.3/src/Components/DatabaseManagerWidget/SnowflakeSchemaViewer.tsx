...
export function SnowflakeSchemaViewer({
  ...
  const formatColumnType = (column: Column): string => {
    switch (column.type) {
      ...
      case 'TEXT':
        const textCol = column as TextColumn;
  ...
  const handleAddTableToContext = async (table: TableEntry, database: string): Promise<void> => {
    try {
      const chatMessages = AppStateService.getState().chatContainer?.chatWidget.messageComponent;
      if (!chatMessages) {
...
${table.columns
  ...
  .map(col => {
    ...
    const descStr = 'description' in col && col.description ? ` - ${col.description}` : '';
  ...
  .join('\n')}`;
      ...
      console.log(`[SnowflakeSchemaViewer] Added table ${table.table} to context`);
    } catch (error) {
      console.error('[SnowflakeSchemaViewer] Error adding table to context:', error);
    }
  };
      ...
      <div className="database-schema-explorer snowflake-schema-viewer">
        <div className="schema-explorer-header">
          ...
          <button
            ...
            onClick={() => handleAddTableToContext(selectedTable, selectedDatabase)}
          ...
          <div className="schema-section">
              ...
              {selectedTable.columns
                ...
                .map((column, index) => (
                  <div key={index} className="column-row">
                    ...
                    <div className="column-type">{formatColumnType(column)}</div>
            ...
            {'description' in selectedTable.columns[0] && (
              <div className="column-descriptions">
                {selectedTable.columns.filter(col => 'description' in col && col.description).length > 0 && (
                  <>
                    ...
                    {selectedTable.columns
                      ...
                      .map((col, idx) => (
                        <div key={idx} className="description-item">
                          <strong>{col.name}:</strong> {(col as TextColumn).description}
                        </div>
                      ))}
                  </>
                )}
  ...
  if (selectedDatabase && selectedSchema) {
    const database = schemaData.databases.find(db => db.database === selectedDatabase);
                ...
                >
                  <div className="table-item-header">
                    ...
                    <button
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        handleAddTableToContext(table, selectedDatabase);
                      }}
    ...
    return (
      <div className="database-schema-explorer snowflake-schema-viewer">
        <div className="schema-explorer-header">
          ...
          <span className="tables-count">({filteredSchemas.length} schemas)</span>
        </div>
            ...
            filteredSchemas.map((schema, index) => {
              ...
              return (
                ...
                >
                  <div
                    className="table-item-content"
                    onClick={() => !hasError && setSelectedSchema(schema.schema)}
                  >
                    <div className="table-item-header">
                      <div className="table-name-section">
  ...
  return (
    <div className="database-schema-explorer snowflake-schema-viewer">
      <div className="schema-explorer-header">
        {onBack && (
          <button onClick={onBack} className="back-button" title="Back to databases">
            ← Back
          </button>
        )}
        <h3 className="database-title">{databaseName} - Select Database</h3>
        <span className="tables-count">({filteredDatabases.length} databases)</span>
      </div>
