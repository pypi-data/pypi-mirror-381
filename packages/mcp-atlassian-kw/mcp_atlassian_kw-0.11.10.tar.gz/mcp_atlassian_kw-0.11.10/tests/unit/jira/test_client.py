...
def test_init_from_env():
    ...
    with (
        patch("mcp_atlassian_kw.jira.config.JiraConfig.from_env") as mock_from_env,
        patch("mcp_atlassian_kw.jira.client.Jira") as mock_jira,
...
def _test_get_paged(method: Literal["get", "post"]):
    ...
    with (
        patch(
            "mcp_atlassian_kw.jira.client.Jira.get", new_callable=DeepcopyMock
        ) as mock_get,
        patch(
            "mcp_atlassian_kw.jira.client.Jira.post", new_callable=DeepcopyMock
        ) as mock_post,
        patch("mcp_atlassian_kw.jira.client.configure_ssl_verification"),
    ):
        config = JiraConfig(
...
def test_get_paged_without_cloud():
    ...
    with patch("mcp_atlassian_kw.jira.client.configure_ssl_verification"):
        config = JiraConfig(
            url="https://jira.example.com",
            auth_type="pat",
