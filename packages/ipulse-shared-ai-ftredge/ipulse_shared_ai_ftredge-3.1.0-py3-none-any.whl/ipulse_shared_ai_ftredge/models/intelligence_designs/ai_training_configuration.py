...
class AITrainingConfiguration(BaseModel):
    ...
    feature_engineering_strategy: Optional[Dict[str, Any]] = Field(None, description="High-level feature engineering strategy, not specific pipeline details.")
    data_splitting_strategy: Optional[Dict[str, Any]] = Field(None, description="General data splitting approach, e.g., {'method': 'temporal', 'validation_approach': 'rolling_window'}.")
    ...
    training_stopping_criteria: Optional[str] = Field(None, description="General stopping criteria approach, e.g., 'Early stopping based on validation loss'.")
    hyperparameters_template: Optional[Dict[str, Any]] = Field(None, description="Template hyperparameters for this training configuration lineage.")
    
    # --- Expected Performance and Cost ---
    training_config_overall_performance_score: Optional[float] = Field(None, description="Expected performance score for this config ")
    avg_monthly_cost_usd: Optional[float] = Field(None, description="Estimated monthly cost for this training configuration.")
    avg_training_duration_hours: Optional[float] = Field(None, description="Estimated training duration per run in hours.")
    
    # --- Metadata ---
    notes: Optional[str] = Field(None, description="Strategic notes about this training configuration approach.")
    strengths: Optional[str] = Field(None, description="Expected strengths of this training approach.")
    weaknesses: Optional[str] = Field(None, description="Known limitations of this training approach.")
    recommended_use_cases: Optional[List[str]] = Field(None, description="Recommended scenarios for using this training configuration.")
    tags: Dict[str, str] = Field(default_factory=dict, description="Tags for categorization, e.g., 'production', 'experimental'.")

    # --- Namespace and Identity ---
    pulse_namespace: Optional[str] = Field(None, description="Namespace for this AI training configuration.")
