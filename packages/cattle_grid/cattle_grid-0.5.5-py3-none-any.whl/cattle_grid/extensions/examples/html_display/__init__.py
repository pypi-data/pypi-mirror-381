...
from muck_out.cattle_grid import ParsedActivity
from muck_out.types import Object
...
async def html_publish_object(
    ...
    publisher = Publisher(actor, config, obj)
    session.add(
        PublishedObject(
            id=publisher.uuid, data=publisher.object_for_store, actor=actor.actor
        )
    )
...
async def html_display_incoming_create(
    ...
    embedded_object = Object.model_validate(
        message.data.get("parsed", {}).get("embeddedObject")
    )
    if embedded_object is None:
        logger.info("Unparsed create: " + json.dumps(message.data, indent=2))
        return
