...
class write_database():
    ...
    def __calc__(self, **kwargs):
        ...
        if self.sourceformat.lower() == 'gwb': # options for all default database included with PyGCC
            ...
            if self.kwargs["sourcedb"] == 'data0' or self.kwargs["sourcedb"] is None:
                ...
                self.sourcedb = self.kwargs["sourcedb"]
        elif self.sourceformat.lower() == 'phreeqc':
            if self.kwargs["sourcedb"] == 'PHREEQC' or self.kwargs["sourcedb"] is None:
    ...
    def write_PHREEQCdb(self, T, P):
        ...
        # extract periodic table symbols into a set
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'PeriodicTableJSON.json'), encoding='utf8') as periodic_table:
            data = json.load(periodic_table)
            element_symbols = {element["symbol"] for element in data["elements"]}
