...
select_filter = [ i for i,p in enumerate(particle_indices) if p == 12 ]


xvals,time_indices,particle_indices = xvals[select_filter],time_indices[select_filter],jnp.zeros_like(jnp.array(select_filter))
...
# Define the basis:
import OLI_bases
force_b, force_grad_b = OLI_bases.basis_selector({ 'type' : 'polynomial', 'order' : 2},data.d,output="vector")
