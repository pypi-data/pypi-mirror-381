...
# I. Load the data:
metadata, particle_indices, time_indices, xvals = SFI_utils.load_trajectory_csv('original_tracks_labeled.csv',particle_column=1, time_column=2, state_columns=[9,10,11]) 
...
# # find first time for each particle
# first_times = {}
# for p, t in zip(particle_indices, time_indices):
#     if p not in first_times or t < first_times[p]:
#         first_times[p] = int(t)

# vectorized shift
# shifted_time = np.array([t - first_times[p] for p, t in zip(particle_indices, time_indices)]) # 

#mask = (xvals[:,2] == 'seam')
freq = 1
mask = (xvals[:,2] == 'hyp') & (time_indices % freq == 0)
particle_indices, time_indices, xvals = particle_indices[mask], time_indices[mask]//freq, xvals[mask,:2].astype(float)
...
# Define the basis:
(force_b, force_grad_b),descr = OLI_bases.basis_selector({ 'type' : 'polynomial', 'order' : 1},data.d,output="vector")
...
import SFI
for i in range(data.Nmaxparticles):
    plt.subplot(5,6,i+1)
