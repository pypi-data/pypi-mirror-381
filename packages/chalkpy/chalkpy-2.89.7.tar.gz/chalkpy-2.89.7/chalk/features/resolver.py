...
def make_stream_resolver(
    *,
    ...
    if not valid_message_type:  # pyright: ignore[reportUnnecessaryIsInstance]
        error_builder.add_diagnostic(
            message=f"Invalid message_type for stream resolver '{name}': expected Pydantic BaseModel, protobuf Message, dataclass, str, or bytes, got {type(message_type).__name__}",
            code="195",
...
class Sink:
    ...
    feature_expressions: Mapping[FeatureWrapper | str, Underscore] | None
