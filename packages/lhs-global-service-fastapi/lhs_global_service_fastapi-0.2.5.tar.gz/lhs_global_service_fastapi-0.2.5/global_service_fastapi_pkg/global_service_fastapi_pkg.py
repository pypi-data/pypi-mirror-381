...
class GlobalServiceConfig:
    def __init__(self, GLOBAL_LOG_URL=None, PROJECT_TYPE=None, APP_CODE=None,
                 ...
                 REMOTE_ACTIVITY_FAILED_LOG_FILE=None, REMOTE_DEBUG_FAILED_LOG_FILE=None,
                 LOCAL_ACTIVITY_LOG_FILE=None, LOCAL_DEBUG_LOG_FILE=None, GLOBAL_CONFIG_PATH=None):
        ...
        # Local JSONL files for failed logs and local activity/debug logs
        self.REMOTE_ACTIVITY_FAILED_LOG_FILE = REMOTE_ACTIVITY_FAILED_LOG_FILE or os.getenv("REMOTE_ACTIVITY_FAILED_LOG_FILE") or os.path.abspath(os.path.join(os.getcwd(), "remote_activity_failed_logs.json"))
        self.REMOTE_DEBUG_FAILED_LOG_FILE = REMOTE_DEBUG_FAILED_LOG_FILE or os.getenv("REMOTE_DEBUG_FAILED_LOG_FILE") or os.path.abspath(os.path.join(os.getcwd(), "remote_debug_failed_logs.json"))
        self.LOCAL_ACTIVITY_LOG_FILE = LOCAL_ACTIVITY_LOG_FILE or os.getenv("LOCAL_ACTIVITY_LOG_FILE") or os.path.abspath(os.path.join(os.getcwd(), "local_activity_logs.json"))
        self.LOCAL_DEBUG_LOG_FILE = LOCAL_DEBUG_LOG_FILE or os.getenv("LOCAL_DEBUG_LOG_FILE") or os.path.abspath(os.path.join(os.getcwd(), "local_debug_logs.json"))
...
def get_global_service_local_storage_enabled():
    return _read_global_flag('GLOBAL_SERVICE_LOCAL_STORAGE', default='true')
...
def flush_failed_logs():
    ...
    while True:
        try:
            if config.REMOTE_ACTIVITY_FAILED_LOG_FILE and os.path.exists(config.REMOTE_ACTIVITY_FAILED_LOG_FILE):
                with open(config.REMOTE_ACTIVITY_FAILED_LOG_FILE, "r+", encoding="utf-8") as f:
                    logs = f.readlines()
                    if logs:
                        remaining_logs = []
                        for log in logs:
...
# Periodic debug log flushing thread
def flush_failed_debug_logs():
    """
    Periodically send debug logs stored in the JSON file to Oracle DB.
    Only delete logs from the file after successful DB insert.
    """
    while True:
        try:
            if config.REMOTE_DEBUG_FAILED_LOG_FILE and os.path.exists(config.REMOTE_DEBUG_FAILED_LOG_FILE):
                with open(config.REMOTE_DEBUG_FAILED_LOG_FILE, "r+", encoding="utf-8") as f:
                    logs = f.readlines()
                    if logs and is_db_healthy():
                        remaining_logs = []
                        for log in logs:
                            try:
                                payload = json.loads(log.strip())
                                # Defensive: extract required fields for save_debug_log_to_oracle
                                save_debug_log_to_oracle(
                                    payload=payload.get('payload', {}),
                                    request_body=payload.get('request_body', ""),
                                    response_body=payload.get('response_body', ""),
                                    debug_logs=payload.get('debug_logs', ""),
                                    debug_active_by=payload.get('debug_active_by', None),
                                    debug_end_by=payload.get('debug_end_by', None),
                                    start_time=payload.get('start_time', None),
                                    end_time=payload.get('end_time', None),
                                    flag=payload.get('flag', None)
                                )
                                # If no DB error, log is flushed
                            except Exception as e:
                                logging.warning(f"Failed to flush debug log to DB: {e}")
                                remaining_logs.append(log)
                        # Rewrite the file with remaining logs
                        f.seek(0)
                        f.truncate()
            ...
            logging.error(f"Error in periodic debug log flushing: {e}")
        interval = get_flush_interval_sec_from_db()
        time.sleep(interval)

# Start the background thread for debug log flushing
threading.Thread(target=flush_failed_debug_logs, daemon=True).start()
...
def save_activity_log_locally(payload: dict):
    if not payload:
        return
    try:
        _append_json_daily(config.LOCAL_ACTIVITY_LOG_FILE, payload)
    except Exception:
        logging.exception("save_activity_log_locally: failed")

def send_log_to_global_service(payload: dict):
    # Save activity log locally if enabled
    if get_global_service_local_storage_enabled():
        save_activity_log_locally(payload)
    try:
        ...
        try:
            with open(config.REMOTE_ACTIVITY_FAILED_LOG_FILE, "a", encoding="utf-8") as f:
...
def save_full_debug_log_locally(
    app_code,
    uuid_val,
    payload_rrn,
    endpoint,
    ...
    """
    Save full debug log details locally in the same structure as Oracle DB.
    """
    ...
    try:
        ...
        _append_json_daily(config.LOCAL_DEBUG_LOG_FILE, log_entry)
    except Exception:
        logging.exception("save_full_debug_log_locally: failed")
...
def save_debug_log_to_oracle(
    ...
    try:
        ...
        connection.commit()
        # Save full debug log locally if enabled
        if get_global_service_local_storage_enabled():
            save_full_debug_log_locally(
                app_code=app_code,
                uuid_val=uuid_val,
                payload_rrn=payload_rrn,
                endpoint=endpoint,
        ...
        # Save failed debug log to local remote file for later flush
        try:
            failed_payload = {
                "payload": payload,
                "request_body": request_body,
            ...
            with open(config.REMOTE_DEBUG_FAILED_LOG_FILE, "a", encoding="utf-8") as f:
                f.write(json.dumps(failed_payload) + "\n")
        except Exception as e2:
            logging.error(f"Could not save failed debug log to file: {e2}")
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'connection' in locals():
