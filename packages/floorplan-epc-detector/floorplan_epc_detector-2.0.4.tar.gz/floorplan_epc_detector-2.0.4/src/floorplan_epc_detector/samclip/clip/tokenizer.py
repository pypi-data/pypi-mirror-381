...
class Tokenizer(object):
    ...
    def __init__(self, bpe_path: str = default_bpe(), device='cuda'):
        ...
        data_dir = os.path.dirname(bpe_path)
        os.makedirs(data_dir, exist_ok=True)

        # Download vocabulary file if it doesn't exist
        if not os.path.exists(bpe_path):
            logging.info(f"Downloading vocabulary file to {bpe_path}")
            vocab_url = 'https://drive.google.com/uc?id=1zPQqH85z5zS8zBSKXI6VqpR9JdG8hz5s'
            success = gdown.download(url=vocab_url, output=bpe_path, fuzzy=True)
            
            if not success or not os.path.exists(bpe_path):
                raise RuntimeError(f"Failed to download vocabulary file to {bpe_path}")
            
            if os.path.getsize(bpe_path) == 0:
                os.remove(bpe_path)
                raise RuntimeError(f"Downloaded vocabulary file is empty: {bpe_path}")
            
            logging.info(f"Vocabulary file downloaded successfully to {bpe_path}")
        ...
        try:
            ...
            logging.error(f"Vocabulary path: {bpe_path}")
            logging.error(f"Directory exists: {os.path.exists(data_dir)}")
            if os.path.exists(bpe_path):
                logging.error(f"Vocabulary file size: {os.path.getsize(bpe_path)} bytes")
