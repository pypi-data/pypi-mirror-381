...
class Range:
    ...
    @deprecated(details="Use __and__ operator instead")
    ...
    def __and__(self, other: "Range") -> "Range | None":
        ...
        if other is None:
            return None
        if self.min_val < other.max_val and other.min_val < self.max_val:
            return Range(
                max(self.min_val, other.min_val), min(self.max_val, other.max_val)
            )
        return None
...
class Rect:
    ...
    def __and__(self, other: "Rect") -> "Rect | None":
        ...
        """
        x_intersect = self.x_range & other.x_range
        y_intersect = self.y_range & other.y_range
        if x_intersect is not None and y_intersect is not None:
            return Rect(x_intersect, y_intersect)
        return None
    ...
    def __or__(self, other: "Rect") -> "Rect | None":
        ...
        """
        x_union = self.x_range | other.x_range
        y_union = self.y_range | other.y_range
        return Rect(
            x_range=x_union,
            y_range=y_union,
        )
    ...
    def trim(self, image_shape: tuple[int, int]):
        ...
        画像の範囲を超える領域をtrimする。
        """
        image_rect = Rect.from_bounds(0, image_shape[1], 0, image_shape[0])
...
class Hyperbox:
    ...
    def __and__(self, other: "Hyperbox") -> "Hyperbox | None":
        if len(self.ranges) != len(other.ranges):
            raise ValueError("Hyperbox dimensions do not match")
        return Hyperbox([r1 & r2 for r1, r2 in zip(self.ranges, other.ranges)])

    def __or__(self, other: "Hyperbox") -> "Hyperbox":
        if len(self.ranges) != len(other.ranges):
            raise ValueError("Hyperbox dimensions do not match")
        return Hyperbox([r1 | r2 for r1, r2 in zip(self.ranges, other.ranges)])

    def validate(self, min_sizes: list[int]):
        for i, r in enumerate(self.ranges):
            r.validate(min_sizes[i])
