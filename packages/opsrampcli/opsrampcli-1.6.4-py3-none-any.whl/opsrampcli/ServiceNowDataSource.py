...
class ServiceNowDataSource(DataSource):
    ...
    def get_resources_df(self):
        ...
        if auth_type == 'oauth2':
            ...
            grant_type = os.getenv("SERVICENOW_GRANT_TYPE") or job['source']['servicenow']['auth']['grant_type'] or "password"
            ...
            scope = os.getenv("SERVICENOW_SCOPE") or job['source']['servicenow']['auth']['scope']
            token_url = os.getenv("SERVICENOW_TOKEN_URL") or job['source']['servicenow']['auth']['token_url'] or f"{instance_url}/oauth_token.do"
            oauth_payload = {
