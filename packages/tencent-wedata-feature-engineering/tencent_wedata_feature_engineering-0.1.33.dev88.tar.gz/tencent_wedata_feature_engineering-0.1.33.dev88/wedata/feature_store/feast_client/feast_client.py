...
class FeastClient:
    ...
    def remove_offline_table(self, table_name: str, schema: StructType):
        ...
        #     entity_name = _get_entity_name(feast_table_name, field.name)
    ...
    def remove_online_table(self, table_name: str):
        ...
        if self._client.config.online_store.type == "redis":
            RedisOnlineStore().delete_table(self._client.config, table_view)
            table_view.online = False
            self._client.registry.apply_feature_view(table_view, self._client.project)
        else:
