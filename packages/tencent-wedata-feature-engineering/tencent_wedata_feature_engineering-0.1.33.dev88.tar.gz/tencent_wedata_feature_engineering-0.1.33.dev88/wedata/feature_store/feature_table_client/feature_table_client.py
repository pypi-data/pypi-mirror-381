...
from wedata.feature_store.cloud_sdk_client.models import (
    ...
    CreateOnlineFeatureTableRequest, DescribeNormalSchedulerExecutorGroupsRequest)
...
class FeatureTableClient:
    ...
    def write_table(
        ...
        try:
            if is_streaming:
                ...
                #     # 写入Feast
                #     self._feast_client.client.write_to_offline_store(
                #         feature_view_name=table_name,
    ...
    def register_table(self, name, database_name, timestamp_key: str, engine_type: EngineTypes,
                       primary_keys: Union[str, List[str]]):
        """注册表 为特征表
                Args:
                    name: 表名（格式：<table>）
    ...
    def drop_table(self, name: str, database_name: Optional[str] = None, is_auto_delete_online: bool = False) -> None:
        ...
        Args:
            name: 特征表名称（格式：<table>）
            database_name: 特征库名称
            is_auto_delete_online: 是否自动删除在线表
        Raises:
            ValueError: 当表不存在时抛出
        ...
        try:
            ...
            try:
                ...
                if feature_view.online:
                    if not is_auto_delete_online:
                        raise ValueError(f"Table '{name}' has a online table, please set is_auto_delete_online=True")

            # 执行删除
            self._spark.sql(f"DROP TABLE {table_name}")
            print(f"Table '{name}' dropped")
            try:
                self._feast_client.remove_offline_table(table_name=table_name, schema=df.schema)
            except Exception as e:
                raise
    ...
    def _read_online_table(self,
        ...
        if not entity_row:
            ...
            if result_row:
                ...
                return self._spark.createDataFrame(online_view, schema=schema)
            else:
    ...
    def publish_table(self, table_name: str, data_source_name: str, cloud_secret_id: str, cloud_secret_key: str,
        ...
        # 检查表是否存在
        if not self._spark.catalog.tableExists(full_table_name):
        ...
        if is_cycle:
            ...
            if isinstance(cycle_obj, TaskSchedulerConfiguration):
                ...
                cycle_obj.CycleType = "ONEOFF_CYCLE"
                # 设置默认当前时间延后1分钟
                cycle_obj.CrontabExpression = (datetime.datetime.now() + datetime.timedelta(minutes=1)).strftime(
                    "%M %H %d %m %w ? %y")
        ...
        res_group_item = self._get_default_resource_group(
    ...
    def _get_default_resource_group(self, project_id: str, secret_id: str, secret_key: str, region: str):
