...
class BHA_Iterator(Iterator,metaclass=ResurrectMeta):
    ...
    __rand__,__ror__,__rxor__ = __and__,__or__,__xor__
class ProtectedBuiltinsDict(dict):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.protected_names = ["T", "F", "BHA_Bool", "BHA_List", "BoolHybridArray", "BoolHybridArr", "TruesArray", "FalsesArray"]
    def __setitem__(self, name, value):
        if name in ["T", "F"]:
            ...
            if isinstance(current_T, BHA_bool) and isinstance(current_F, BHA_bool):
                ...
                if is_swap:
                    print(f"警告：禁止交换内置常量 builtins.{name} 和 builtins.{'' if name == 'T' else 'F'}！")
                    raise AttributeError(f"禁止交换内置常量 builtins.{name} 和 builtins.{'' if name == 'T' else 'F'}")
...
builtins.id = fake_id
original_builtins_dict = builtins.__dict__
builtins.__dict__ = ProtectedBuiltinsDict(original_builtins_dict)