...
class BoolHybridArray(MutableSequence,Exception,metaclass=ResurrectMeta):
    ...
    def __len__(self) -> int:
        return self.size
    def __iter__(self):
        return BHA_Iterator(self[i] for i in range(self.size))
    def __next__(self):
...
class BHA_bool(int,metaclass=ResurrectMeta):
    ...
    def __del__(self):
        global T, F
        print(f'你删除了1个常变量！可回收变量数：{gc.collect()}')
        if self:builtins.T = self
...
def module_setattr(name, value):
    if name in current_module.__dict__:
        raise AttributeError(f"禁止修改常量 {name}")
    current_module.__dict__[name] = value
def module_delattr(name):
    if name in current_module.__dict__:
        raise AttributeError(f"禁止删除常量 {name}")
    raise AttributeError(f"常量 {name} 不存在")
current_module.__setattr__ = module_setattr
current_module.__delattr__ = module_delattr
original_builtins_setattr = builtins.__setattr__
def protected_builtins_setattr(name, value):
    protected_names = ["T", "F", "BHA_Bool", "BHA_List", "BoolHybridArray", "BoolHybridArr", "TruesArray", "FalsesArray"]
    if name in protected_names:
        print(f"警告：禁止修改内置常量 builtins.{name}！")
        raise AttributeError(f"禁止修改内置常量 builtins.{name}")
    original_builtins_setattr(name, value)
def protected_builtins_delattr(name):
    protected_names = ["T", "F", "BHA_Bool", "BHA_List", "BoolHybridArray", "BoolHybridArr", "TruesArray", "FalsesArray"]
    if name in protected_names:
        print(f"警告：禁止删除内置常量 builtins.{name}！")
        raise AttributeError(f"禁止删除内置常量 builtins.{name}")
    original_builtins_delattr(name)
builtins.__setattr__ = protected_builtins_setattr
builtins.__delattr__ = protected_builtins_delattr
builtins.np = np
