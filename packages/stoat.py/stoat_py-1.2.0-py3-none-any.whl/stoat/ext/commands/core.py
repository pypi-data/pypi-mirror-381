...
import stoat
from stoat.utils import MISSING
...
def get_signature_parameters(
    ...
    *,
    ...
    eval_annotation = stoat.utils.evaluate_annotation
    required_params = stoat.utils.is_inside_class(function) + 1 if skip_parameters is None else skip_parameters
...
def extract_descriptions_from_docstring(
    ...
    divide = stoat.utils.PARAMETER_HEADING_REGEX.split(docstring, 1)
    if len(divide) == 1:
        return docstring

    description, param_docstring = divide
    for match in stoat.utils.NUMPY_DOCSTRING_ARG_REGEX.finditer(param_docstring):
...
class _StatelessAssetIterator:
    def __init__(self, data: list[stoat.StatelessAsset], /):
        self.data: list[stoat.StatelessAsset] = data
        self.index: int = 0

    def __iter__(self) -> Self:
        return self

    def __next__(self) -> stoat.StatelessAsset:
        try:
...
class _AssetIterator:
    ...
    def __init__(self, data: list[stoat.Asset], /):
        self.data: list[stoat.Asset] = data
        self.index: int = 0

    def __iter__(self) -> Self:
        return self

    def __next__(self) -> stoat.Asset:
        try:
            value = self.data[self.index]
        except IndexError:
            raise StopIteration
...
class Command(_BaseCommand, typing.Generic[GearT, P, T]):
    ...
    def callback(
        ...
        unwrap = stoat.utils.unwrap_function(function)
    ...
    def add_check(self, func: UserCheck[Context[typing.Any]], /) -> None:
        ...
        .. seealso:: The :func:`~stoat.ext.commands.check` decorator
    ...
    def update(self, /, **kwargs: typing.Any) -> None:
        ...
        This works similarly to the :func:`~stoat.ext.commands.command` decorator in terms
    ...
    async def transform(self, ctx: Context[BotT], param: Parameter, attachments: _AssetIterator, /) -> typing.Any:
        ...
        if isinstance(converter, Greedy):
            # Special case for Greedy[stoat.Asset] to consume the attachments iterator
            if issubclass(converter.converter, stoat.StatelessAsset):
        ...
        # Try to detect Optional[stoat.Asset] or stoat.Asset special converter
        if issubclass(converter, stoat.StatelessAsset):
            try:
        ...
        if self._is_typing_optional(param.annotation) and issubclass(
            param.annotation.__args__[0], stoat.StatelessAsset
        ):
            if attachments.is_empty():
                # I have no idea who would be doing Optional[stoat.Asset] = 1
    ...
    def cooldown(self) -> typing.Optional[Cooldown]:
        """Optional[:class:`~stoat.ext.commands.Cooldown`]: The cooldown of a command when invoked
    ...
    def signature(self) -> str:
        ...
        for param in params.values():
            ...
            if issubclass(annotation, stoat.Asset):
                # For stoat.Asset we need to signal to the user that it's an attachment
        ...
        try:
            ...
            if gear is not None:
                ...
                if local_check is not None:
                    ret = await stoat.utils.maybe_coroutine(local_check, ctx)
                    if not ret:
                        return False
...
class GroupMixin(typing.Generic[GearT]):
    ...
    def command(
        ...
        **kwargs: typing.Any,
        ...
        """A shortcut decorator that invokes :func:`~stoat.ext.commands.command` and adds it to
...
def group(
    ...
    **attrs: typing.Any,
    ...
    This is similar to the :func:`~stoat.ext.commands.command` decorator but the ``cls``
...
def has_role(item: str, /) -> Check[typing.Any]:
    ...
    def predicate(ctx: Context[BotT], /) -> bool:
        ...
        if not isinstance(author, stoat.Member):
...
def bot_has_role(item: str, /) -> Callable[[T], T]:
    ...
    def predicate(ctx: Context[BotT], /):
        ...
        if not isinstance(me, stoat.Member):
            raise NoPrivateMessage()
...
def is_nsfw() -> Check[typing.Any]:
    ...
    def predicate(ctx: Context[BotT], /) -> bool:
        ch = ctx.channel
        if isinstance(ch, stoat.PartialMessageable) or getattr(ch, 'nsfw', True):
            return True
...
def dynamic_cooldown(
    ...
    return a :class:`~stoat.ext.commands.Cooldown` or ``None``.
    ...
    Parameters
    ----------
    cooldown: Callable[[:class:`.Context`], Optional[:class:`~stoat.ext.commands.Cooldown`]]
