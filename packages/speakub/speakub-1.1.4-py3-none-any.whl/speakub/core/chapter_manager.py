...
class ChapterManager:
    ...
    def _build_chapters_from_spine_and_toc(self) -> List[Dict]:
        ...
        # --- START OF FIX ---
        # 2. Find a sensible initial title instead of hardcoding "Introduction".
        #    Iterate through the spine to find the first chapter that actually has a title in the TOC.
        initial_title = "Untitled Chapter"  # A more neutral fallback
        for src in self.spine_order:
            normalized_spine_src = normalize_src_for_matching(src)
            if normalized_spine_src in title_map:
                initial_title = title_map[normalized_spine_src]
                break  # Found the first valid title, so we can stop searching.
        ...
        for src in self.spine_order:
            ...
            # Create the final chapter object, always use current_title
            chapters.append(
                {
                    "type": "chapter",
