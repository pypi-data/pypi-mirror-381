...
def is_running_in_terminal(debug: bool = False) -> bool:
    ...
    if debug:
        print(f"DEBUG: stdout.isatty()={stdout_is_tty}, "
              f"stderr.isatty()={stderr_is_tty}",
              file=sys.stderr)

    if not (stdout_is_tty and stderr_is_tty):
        if debug:
            print("DEBUG: Not running in tty, need to relaunch in terminal",
                  file=sys.stderr)
        return False
    ...
    if not term or term == "dumb":
        if debug:
            print("DEBUG: TERM is not set or dumb, need to relaunch",
                  file=sys.stderr)
        return False
    ...
    if debug:
        print("DEBUG: TERM not recognized as proper terminal, need to relaunch",
              file=sys.stderr)
    return False
...
def relaunch_in_terminal(original_args: List[str], debug: bool = False) -> None:
    ...
    if debug:
        print(
            f"DEBUG: Relaunching in terminal with args: {original_args}", file=sys.stderr)
    ...
    if term_name == "xfce4-terminal":
        # xfce4-terminal: -H (hold) must come before -e (execute)
        full_cmd = ["xfce4-terminal", "-H", "-e", cmd_string]
    elif term_name == "xterm":
        # xterm: use -hold -e
        full_cmd = ["xterm", "-hold", "-e", cmd_string]
    elif term_name in ("gnome-terminal", "konsole"):
        # These terminals work better with bash -c
        full_cmd = term_cmd + ["bash", "-c", cmd_string]
    else:
        # For other terminals, try the standard -e approach
        full_cmd = term_cmd + [cmd_string]
    if debug:
    ...
    try:
        ...
        if debug:
            print("DEBUG: Subprocess launched, exiting current process", file=sys.stderr)
