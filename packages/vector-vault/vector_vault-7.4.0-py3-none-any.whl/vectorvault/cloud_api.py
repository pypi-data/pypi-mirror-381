...
def login_with_api(email, api_key):
    ...
    try:
        ...
        print("Fresh login successful - new access token obtained")
...
def refresh_access_token(user, api_key, max_retries=1):
    ...
    if not refresh_token:
        print("No refresh_token available; performing fresh login...")
    ...
    try:
        ...
        if response.status_code == 404:
            print(f"Refresh endpoint not available (404) at {url} - performing fresh login instead...")
        ...
        if response.status_code != 200:
            print(f"Refresh failed with status {response.status_code} - performing fresh login instead...")
            access_token = None
            refresh_token = None
            return login_with_api(user, api_key)
        ...
        print(f"Error refreshing token: {e} - performing fresh login instead...")
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                if not refresh_access_token(user, api_key):
                    print("Failed to refresh token, cannot continue.")
                    return None
