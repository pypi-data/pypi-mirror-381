...
def __bootstrap__():
   ...
   if os_name == 'windows':
      pyd_file = os.path.join(os.path.dirname(__file__),f"gehseeker.2.0.0.cpython-{py_version}-{os_name}.pyd")
      spec = importlib.util.spec_from_file_location("gehseeker", pyd_file)      
   else:
      so_file = os.path.join(os.path.dirname(__file__),f"gehseeker.2.0.0.cpython-{py_version}-{os_name}.so")
      spec = importlib.util.spec_from_file_location("gehseeker", so_file)
