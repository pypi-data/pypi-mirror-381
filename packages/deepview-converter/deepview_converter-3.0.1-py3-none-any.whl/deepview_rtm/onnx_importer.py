...
class ONNXImporter:
    def __init__(self, input_model, input_format='none', batch=1, default_shape=None, subgraph_names=None,
        ...
        self.import_nxgraph = nx.OrderedDiGraph()
    ...
    def import_nodes(self):
        for i in range(len(self.onnx_graph.node)):
            ...
            if self.trim_input_names:
                if onnx_node.output[0] in self.trim_input_names and onnx_node.output[0] not in self.nxgraph.nodes:
                    datatype = np.dtype('float32')
                    if self.model_input_type == np.float32:
                        datatype = np.dtype('float32')
                    elif self.model_input_type == np.int8:
                        datatype = np.dtype('int8')
                    elif self.model_input_type == np.uint8:
                        datatype = np.dtype('uint8')
                    self.nxgraph.add_node(onnx_node.output[0],
                                          op='external',
                                          datatype=datatype,
                                          format=self.input_format,
                                          output_shape=self.default_shape)
                    continue
    ...
    def set_batch(self):
        for node_name, data in self.nxgraph.nodes(data=True):
            if -1 in data['output_shape']:
                data['output_shape'][data['output_shape'].index(-1)] = self.batch
            if 'begin' in data and -1 in data['begin']:
                data['begin'][data['begin'].index(-1)] = self.batch
            if 'end' in data and -1 in data['end']:
                data['end'][data['end'].index(-1)] = self.batch
    ...
    def import_unary(self, node, op):
        ...
        if 'base_shape' in x_node:
            output_shape = x_node['base_shape'][:]
        else:
    ...
    def import_binary(self, node, op):
        ...
        datatype = self.nxgraph.nodes[x_name]['datatype']
        ...
        if 'np_tensor' in x_node and 'np_tensor' in y_node and op == 'add':
                self.nxgraph.nodes[name]['np_tensor'] = x_node['np_tensor'] + y_node['np_tensor']
    ...
    def import_Concat(self, node):
        ...
        for i in range(len(node.input)):
            ...
            value_nodes.append(self.nxgraph.nodes[value_names[i]])
    ...
    def import_Conv(self, node):
        ...
        if len(input_shape) == 3:
            ...
            new_shape.append(1)
            self.nxgraph.add_node(name + '_dv_reshape_conv2d',
                                  op='reshape',
                                  input=input_name,
                                  datatype=datatype,
                                  format='nchw',
                                  output_shape=new_shape)
            self.nxgraph.add_edge(input_name, name + '_dv_reshape_conv2d')
            input_name = name + '_dv_reshape_conv2d'
            input_shape = new_shape[:]
            input_node = self.nxgraph.nodes[input_name]
            filt_node = self.nxgraph.nodes[filt_name]
            filt_shape = filt_shape[:]
            filt_shape.append(1)
            filt_node['output_shape'] = filt_shape
            name = name + '_dv_conv1d'
            filt_node['np_tensor'] = filt_node['np_tensor'].reshape(filt_shape)
        ...
        if conv1d:
            ...
            self.nxgraph.add_edge(name, orig_name)
    ...
    def import_DequantizeLinear(self, node):
        ...
        input_node = self.nxgraph.nodes[input_name]
        scale_name = node.input[1]
        scale_node = self.nxgraph.nodes[scale_name]
        ...
        if len(node.input) == 3:
            ...
            zero_node = self.nxgraph.nodes[zero_name]
    ...
    def import_Reshape(self, node):
        ...
        if 'base_shape' in input_node:
            ...
            input_shape = input_node['output_shape']
        datatype = input_node['datatype']
        out_shape = list(self.nxgraph.nodes[node.input[1]]['np_tensor'])
    ...
    def clean_graph(self):
        ...
        self.clean_pad()
        self.clean_clip_to_relu6()
        self.clean_expand_mul_to_resize_quant()
        self.qdq_resolver()
        self.clean_nchw_to_nhwc()
        self.clean_transpose()
        self.clean_filters()
        self.clean_reduce_ops()
        self.clean_params()
    ...
    def clean_constants(self):
        for node_name, data in self.nxgraph.nodes(data=True):
            ...
            for key in data.keys():
                if key not in ['datatype', 'format', 'np_tensor', 'output_shape', 'op',
                    'quant_scale', 'zero_point', 'quant_axis']:
    ...
    def clean_clip_to_relu6(self):
        for node_name, data in self.nxgraph.nodes(data=True):
            if data['op'] != 'clip':
                continue
            min_node = self.nxgraph.nodes[data['min']]
            max_node = self.nxgraph.nodes[data['max']]
            if min_node['op'] != 'constant' or max_node['op'] != 'constant':
                continue
            if min_node['output_shape'] != [1] or max_node['output_shape'] != [1]:
                continue
            if min_node['np_tensor'][0] != 0 or max_node['np_tensor'][0] != 6:
                continue

            data['op'] = 'relu6'
            data['x'] = data['input']
            self.nxgraph.remove_edge(data['min'], node_name)
            self.nxgraph.remove_edge(data['max'], node_name)
            del data['input']
            del data['min']
            del data['max']
    ...
    def clean_transpose(self):
        removed_transpose = True
        relabel_nodes = {}
        while removed_transpose:
            removed_transpose = False
            for node_name in nx.topological_sort(self.nxgraph):
                data = self.nxgraph.nodes[node_name]
                if data['op'] != 'transpose':
                    continue
                ...
                for edge in out_edges:
                    edge_name = edge[1]
                    edge_node = self.nxgraph.nodes[edge_name]
                    if edge_node['op'] != 'transpose':
                        continue
                    init_axes = data['axes'][:]
                    out_axes = []
                    for axis in edge_node['axes']:
                        out_axes.append(init_axes[axis])
                    if out_axes != list(range(len(init_axes))):
                        continue

                    removed_transpose = True
                    if edge_name in self.output_names:
                        if len(self.nxgraph.out_edges(input_name)) == 1:
                            relabel_nodes[input_name] = edge_name
                        else:
                            self.output_names[self.output_names.index(edge_name)] = input_name
                    else:
                        trans_out_edges = list(self.nxgraph.out_edges(edge_name))
                        for out_edge in trans_out_edges:
                            trans_out_name = out_edge[1]
                            trans_out_node = self.nxgraph.nodes[trans_out_name]
                            for input_val in ['input', 'x', 'y']:
                                if input_val in trans_out_node and trans_out_node[input_val] == edge_name:
                                    trans_out_node[input_val] = input_name
                                    self.nxgraph.add_edge(input_name, trans_out_name)
                            if 'values' in trans_out_node:
                                for i in range(len(trans_out_node['values'])):
                                    if trans_out_node['values'][i] == edge_name:
                                        trans_out_node['values'][i] = input_name
                                        self.nxgraph.add_edge(input_name, trans_out_name)
                    self.nxgraph.remove_node(edge_name)
                    if len(list(self.nxgraph.out_edges(node_name))) == 0:
                        self.nxgraph.remove_node(node_name)
                if removed_transpose:
                    break
        ...
        for node_name, data in self.nxgraph.nodes(data=True):
            ...
            for edge in out_edges:
                out_name = edge[1]
                out_node = self.nxgraph.nodes[out_name]
                if out_node['op'] != 'transpose':
                    valid_mod = False
                    break
                if transpose_axis == None:
                    transpose_axis = out_node['axes']
                elif transpose_axis != out_node['axes']:
                    valid_mod = False
                    break
            if not valid_mod:
                continue
            ...
            new_shape = []
            for axis in transpose_axis:
                new_shape.append(data['output_shape'][axis])
            data['output_shape'] = new_shape
            for edge in out_edges:
                out_name = edge[1]
                next_edges = self.nxgraph.out_edges(out_name)
                for out_edge in next_edges:
                    next_name = out_edge[1]
                    ...
                    self.nxgraph.add_edge(node_name, next_name)
                remove_nodes.append(out_name)
        self.nxgraph.remove_nodes_from(remove_nodes)

        for key, val in relabel_nodes.items():
            if key not in self.nxgraph.nodes:
                del relabel_nodes[key]
        if relabel_nodes:
            nx.relabel_nodes(self.nxgraph, relabel_nodes, copy=False)
    ...
    def clean_filters(self):
        ...
        for node_name, data in self.nxgraph.nodes(data=True):
            ...
            if data['groups'] != 1:
                t_axis = [2, 3, 0, 1]
            if filt_node['op'] == 'dequant':
                quant_name = filt_node['x']
                quant_node = self.nxgraph.nodes[quant_name]
                if quant_node['op'] != 'constant':
                    raise ValueError("Unable to transpose quantized filter.")
                new_tensor = quant_node['np_tensor']
                new_tensor = np.transpose(new_tensor, t_axis)
                quant_node['np_tensor'] = new_tensor
                quant_node['output_shape'] = list(new_tensor.shape)
                filt_node['output_shape'] = list(new_tensor.shape)
                filt_node['quant_axis'] = t_axis.index(filt_node['quant_axis'])
            elif filt_node['op'] == 'constant':
                new_tensor = filt_node['np_tensor']
                new_tensor = np.transpose(new_tensor, t_axis)
    ...
    def clean_reduce_ops(self):
        modified_layer = True
        while modified_layer:
            modified_layer = False
            for node_name, data in self.nxgraph.nodes(data=True):
                if data['op'] not in ['mean_reduce']:
                    continue
                if 'keep_dims' not in data:
                    continue
                if not data['keep_dims']:
                    del data['keep_dims']
                    continue
                new_name = node_name + '_mean_reduction_dv'
                relabel_dict = {node_name: new_name}
                nx.relabel_nodes(self.nxgraph, relabel_dict, False)
                data = self.nxgraph.nodes[new_name]
                new_out_shape = data['output_shape'][:]
                pop_axes = data['axes'][:]
                pop_axes.sort()
                for axis in reversed(pop_axes):
                    new_out_shape.pop(axis)
                ...
                data['output_shape'] = new_out_shape
                del data['keep_dims']
                data['format'] = 'none'

                out_edges = list(self.nxgraph.out_edges(new_name))
                for edge in out_edges:
                    out_name = edge[1]
                    out_node = self.nxgraph.nodes[out_name]
                    for val in ['x', 'y', 'input']:
                        if val in out_node and out_node[val] == new_name:
                            out_node[val] = node_name
                        if 'values' in out_node:
                            for i in range(len(out_node['values'])):
                                if out_node['values'][i] == new_name:
                                    out_node['values'][i] = node_name
                    self.nxgraph.remove_edge(new_name, out_name)
                    self.nxgraph.add_edge(node_name, out_name)
                self.nxgraph.add_edge(new_name, node_name)
                modified_layer = True
                break
        ...
        for node_name, data in self.nxgraph.nodes(data=True):
            if data['op'] in ['conv', 'transpose_conv']:
                if data['format'] == 'nhwc':
                    ...
                    data['head'] = [0, 0] + data['head']
                    data['tail'] = [0, 0] + data['tail']
            elif data['op'] in ['avg_pool', 'max_pool']:
                if data['format'] not in ['nchw', 'nhwc']:
                    continue
                if data['format'] == 'nhwc':
                    data['size'] = [1] + data['size'] + [1]
                    data['stride'] = [1] + data['stride'] + [1]
                    if 'head' in data and 'tail' in data:
                        data['head'] = [0] + data['head'] + [0]
                        data['tail'] = [0] + data['tail'] + [0]
                elif data['format'] == 'nchw':
                    data['size'] = [1, 1] + data['size']
                    data['stride'] = [1, 1] + data['stride']
                    if 'head' in data and 'tail' in data:
                        data['head'] = [0, 0] + data['head']
                        data['tail'] = [0, 0] + data['tail']
                    ...
                    for val in ['x', 'y', 'input']:
                        ...
                        if 'values' in next_node:
                            for i in range(len(next_node['values'])):
                                if next_node['values'][i] == out_name:
                                    next_node['values'][i] = node_name
                    self.nxgraph.remove_edge(out_name, next_name)
                    self.nxgraph.add_edge(node_name, next_name)
