...
if getattr(sys, 'frozen', False):
   # Running as compiled executable - data files are in temp directory root
   SCRIPT_DIR = os.path.dirname(sys.executable)
else:
   # Running as script - data files are in the same directory as this file
...
class OsmosePresetsApp(App):
   ...
   if getattr(sys, 'frozen', False):
      ...
      CSS_PATH = os.path.join(os.path.dirname(sys.executable), "osmose_presets.tcss")
   else:
      # Running as script
      CSS_PATH = "osmose_presets.tcss"
   ...
   def on_mount(self) -> None:
      ...
      # Debug logs here - early but after initialization
      log(f"SCRIPT_DIR: {SCRIPT_DIR}")
      log(f"PRESET_DATA: {PRESET_DATA}")
      log(f"File exists: {os.path.exists(PRESET_DATA)}")
