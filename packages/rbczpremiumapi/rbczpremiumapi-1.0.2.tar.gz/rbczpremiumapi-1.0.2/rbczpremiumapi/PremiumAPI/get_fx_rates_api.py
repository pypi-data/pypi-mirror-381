...
class GetFxRatesApi:
    ...
    def get_fx_rates(self, x_ibm_client_id, x_request_id, currency_code, psu_ip_address=None, var_date=None, **kwargs) -> "CurrencyListSimple":
        ...
        :param currency_code: The foreign currency code in ISO-4217 format. (required)
        ...
        :param var_date: The effective date for which the FX rates are requested. Will default to **now** when not specified.
        :type var_date: date or None
        ...
        :rtype: CurrencyListSimple
        """
        if x_ibm_client_id is None:
            raise ValueError("Missing the required parameter `x_ibm_client_id` when calling `get_fx_rates`")
        if x_request_id is None:
            raise ValueError("Missing the required parameter `x_request_id` when calling `get_fx_rates`")
        if currency_code is None:
            raise ValueError("Missing the required parameter `currency_code` when calling `get_fx_rates`")

        # Construct URL
        url = self.api_client.configuration.host + "/rbcz/premium/api/fxrates/{currencyCode}"
        ...
        response = self.api_client.call_api(
            resource_path="/rbcz/premium/api/fxrates/{currencyCode}",
            method="GET",
            headers=headers,
            query_params=query_params,
            body=body,
            response_type="CurrencyListSimple",
            **kwargs
        )
