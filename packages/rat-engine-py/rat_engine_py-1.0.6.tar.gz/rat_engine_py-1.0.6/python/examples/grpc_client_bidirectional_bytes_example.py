...
class ClientBidirectionalHandler:
    ...
    def delegate_send_message(self, message: str, stream_id: str) -> str:
        """委托的消息发送预处理逻辑 - 字节模式版本"""
        self.message_count += 1
        ...
        # 注意：这里不再创建 ChatMessage 对象，而是直接返回处理后的消息字符串
    ...
    def _preprocess_message(self, message: str) -> str:
        ...
        return f"[{self.user_name}][{timestamp}] {message}"
...
class BidirectionalStreamHandler:
    ...
    def prepare_message(self, message: str, stream_id: str) -> str:
        """准备发送消息 - 委托给专门的处理器 - 字节模式版本"""
...
class AutoTestClient:
    ...
    def send_message(self, message: str) -> bool:
        ...
        try:
            # 委托给处理器进行消息预处理
            processed_message = self.handler.prepare_message(message, str(self.stream_id))
            
            # 直接发送字节，不进行 JSON 序列化
            data = processed_message.encode('utf-8')
            self.client.grpc_bidirectional_send(self.stream_id, data)
            print(f"📤 [{self.user_name}] 委托发送字节消息 #{self.handler.message_count}: {processed_message}")
            return True
