...
impl PortConfigBuilder {
    ...
    /// 设置分端口模式（绑定到 localhost）
    pub fn separated_ports_localhost(mut self, http_port: u16, grpc_port: u16) -> Self {
        self.mode = Some(PortMode::Separated {
            http_port,
            grpc_port,
            bind_addr: IpAddr::V4(Ipv4Addr::LOCALHOST),
    ...
    /// 设置分端口模式（绑定到所有地址）
    pub fn separated_ports_any(mut self, http_port: u16, grpc_port: u16) -> Self {
        self.mode = Some(PortMode::Separated {
            http_port,
            grpc_port,
            bind_addr: IpAddr::V4(Ipv4Addr::UNSPECIFIED),
...
pub enum PortConfigError {
    ...
    #[error("分端口模式下不支持单一端口设置，请使用 separated_ports 方法分别设置 HTTP 和 gRPC 端口")]
    UnsupportedSinglePortInSeparatedMode,
}
...
impl PortConfig {
    ...
    /// 修改端口（仅适用于统一端口模式）
    pub fn with_port(mut self, port: u16) -> Result<Self, PortConfigError> {
        match &mut self.mode {
            PortMode::Unified { port: p, .. } => {
                *p = port;
                Ok(self)
            }
            PortMode::Separated { .. } => {
                // 分端口模式下不支持单一端口设置，抛出错误
                Err(PortConfigError::UnsupportedSinglePortInSeparatedMode)
            }
        }
    }

    /// 从 SocketAddr 创建统一端口配置
    ...
    /// 创建绑定到 localhost 的分端口配置
    pub fn separated_localhost(http_port: u16, grpc_port: u16) -> Result<Self, PortConfigError> {
        PortConfigBuilder::new()
            .separated_ports(http_port, grpc_port, IpAddr::V4(Ipv4Addr::LOCALHOST))
            .build()
    }
    ...
    /// 创建绑定到所有地址的分端口配置
    pub fn separated_any(http_port: u16, grpc_port: u16) -> Result<Self, PortConfigError> {
        PortConfigBuilder::new()
            .separated_ports(http_port, grpc_port, IpAddr::V4(Ipv4Addr::UNSPECIFIED))
            .build()
    }

    /// 创建绑定到指定地址的分端口配置
    pub fn separated_with_addr(http_port: u16, grpc_port: u16, bind_addr: IpAddr) -> Result<Self, PortConfigError> {
        PortConfigBuilder::new()
            .separated_ports(http_port, grpc_port, bind_addr)
            .build()
    }
}
