...
async fn main() -> Result<()> {
    ...
    // 2. 大数据路由 (测试多版本缓存的编码协商优势)
    router.add_route(Method::GET, "/large-data", |_req| {
        Box::pin(async move {
            // 模拟复杂计算延迟
            sleep(Duration::from_millis(500)).await;
            
            // 生成大量数据，确保超过压缩阈值(500字节)
            let mut items = Vec::new();
            for i in 0..200 {
                items.push(serde_json::json!({
                    "id": i,
                    "name": format!("Item {}", i),
                    "description": format!("这是第{}个项目的详细描述，包含了很多信息，用于测试缓存和压缩功能的有效性。这个描述故意写得很长，以确保整个响应体足够大。", i),
                    "metadata": {
                        "created_at": "2024-01-01T00:00:00Z",
                        "updated_at": "2024-01-01T00:00:00Z",
                        "tags": ["tag1", "tag2", "tag3", "performance", "test", "cache", "compression"],
    ...
    // 3. 动态数据路由 (测试不缓存场景，对比多版本缓存的处理)
    router.add_route(Method::GET, "/dynamic-data", |_req| {
        Box::pin(async move {
            // 模拟实时数据生成
            ...
            // 生成足够大的数据以触发压缩（超过500字节）
            let mut large_details = String::new();
            for i in 0..20 {
                large_details.push_str(&format!("这是第{}行动态生成的详细信息，包含大量文本数据以确保响应体足够大来触发压缩功能。", i));
            }
            
            let data = serde_json::json!({
                "message": "动态数据",
                "timestamp": timestamp,
                "random_data": format!("随机数据: {}", random_number),
    ...
    // 4. 原始数据路由 (测试快速响应，无缓存无压缩场景)
    router.add_route(Method::GET, "/raw-data", |_req| {
        Box::pin(async move {
            // 模拟快速响应
...
async fn run_performance_tests() -> Result<()> {
    ...
    // 测试场景
    let test_cases = vec![
        ("小数据 (多版本缓存基础功能)", "/small-data", 3),
        ("大数据 (多版本缓存编码协商)", "/large-data", 3),
        ("动态数据 (无缓存对比)", "/dynamic-data", 3),
        ("原始数据 (无处理基准)", "/raw-data", 3),
    ];
