...
from flockparsecli import (
    ...
    get_similar_chunks,
    list_documents,
    load_balancer,  # Use the global load balancer instance
    EMBEDDING_MODEL,
    CHAT_MODEL,
    CHAT_KEEP_ALIVE
)
...
async def list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            name="process_pdf",
            description="Process a PDF file and add it to the knowledge base. Extracts text, creates embeddings, and converts to multiple formats (TXT, MD, DOCX).",
            inputSchema={
                "type": "object",
                "properties": {
                    "file_path": {
        ...
        Tool(
            ...
            description="Query the document knowledge base using semantic search. Returns relevant text chunks from processed documents.",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {
        ...
        Tool(
            ...
            description="Ask questions about your processed documents using AI. The system will find relevant context and generate an answer.",
            inputSchema={
                "type": "object",
                "properties": {
                    "question": {
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "node_url": {
                        "type": "string",
                        "description": "URL of the Ollama node to remove"
                    }
                },
                "required": ["node_url"]
            }
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    if name == "process_pdf":
        ...
        try:
            ...
            print(f"[MCP] Got event loop, calling run_in_executor", file=sys.stderr)
            await asyncio.wait_for(
                loop.run_in_executor(executor, process_pdf, file_path),
                timeout=300.0  # 5 minute timeout
            )
            print(f"[MCP] process_pdf completed successfully", file=sys.stderr)
            return [TextContent(
                type="text",
            ...
            return [TextContent(
                type="text",
                text=f"❌ Timeout processing PDF: Operation took longer than 5 minutes"
            )]
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            print(f"[MCP] Calling get_similar_chunks", file=sys.stderr)
            chunks = await asyncio.wait_for(
                loop.run_in_executor(executor, get_similar_chunks, query, top_k),
            ...
            return [TextContent(
                type="text",
                text=f"❌ Timeout querying documents: Operation took longer than 1 minute"
            )]
        except Exception as e:
            import traceback
            error_details = traceback.format_exc()
        ...
        try:
            ...
            print(f"[MCP] Getting relevant chunks", file=sys.stderr)
            chunks = await asyncio.wait_for(
            ...
            system_prompt = "You are FlockParser AI, a helpful assistant that answers questions based on the provided document context. Only use information from the context. If you don't know or the answer isn't in the context, say so."

            def generate_response():
                print(f"[MCP] Calling load_balancer.chat_distributed", file=sys.stderr)
                ...
                print(f"[MCP] Got response from LLM", file=sys.stderr)
                return response['message']['content']

            print(f"[MCP] Starting LLM generation", file=sys.stderr)
            answer = await asyncio.wait_for(
                loop.run_in_executor(executor, generate_response),
                timeout=120.0  # 2 minute timeout for chat
            )
            print(f"[MCP] LLM generation completed", file=sys.stderr)
            ...
            return [TextContent(
                type="text",
                text=f"❌ Timeout generating answer: Operation took too long"
            )]
        except Exception as e:
            import traceback
            error_details = traceback.format_exc()
        ...
        try:
            ...
            return [TextContent(
                type="text",
                text=f"❌ Error discovering nodes: {str(e)}"
            )]
        ...
        try:
            ...
            if success:
                return [TextContent(
                    type="text",
                    text=f"✅ Successfully added node: {node_url}"
                )]
            else:
                return [TextContent(
                    type="text",
                    text=f"⚠️ Node {node_url} could not be added (already exists or unreachable)"
                )]
        except Exception as e:
            return [TextContent(
                type="text",
                text=f"❌ Error adding node: {str(e)}"
            )]
        ...
        try:
            ...
            if success:
                return [TextContent(
                    type="text",
                    text=f"✅ Successfully removed node: {node_url}"
                )]
            else:
                return [TextContent(
                    type="text",
                    text=f"⚠️ Node {node_url} could not be removed"
                )]
        except Exception as e:
            return [TextContent(
                type="text",
                text=f"❌ Error removing node: {str(e)}"
            )]
