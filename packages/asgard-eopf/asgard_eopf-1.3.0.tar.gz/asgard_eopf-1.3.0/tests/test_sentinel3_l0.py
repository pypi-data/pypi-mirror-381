...
def get_start_stop_time(path: str) -> tuple:
    ...
    start_time = root.find(
        "metadataSection/metadataObject/metadataWrap/xmlData/sentinel-safe:acquisitionPeriod/sentinel-safe:startTime",
        namespace,
    ).text
    stop_time = root.find(
        "metadataSection/metadataObject/metadataWrap/xmlData/sentinel-safe:acquisitionPeriod/sentinel-safe:stopTime",
        namespace,
    ).text
...
def test_sentinel3_footprint_l0(orbits_file, manifest):
    """
    ...
    footprint = root.find(
        ...
        "sentinel-safe:frameSet/sentinel-safe:footPrint/gml:posList",
        namespace,
    ).text.split()
    ...
    # Saving as .csv
    # np.savetxt("gp_ref.csv", gp_ref, delimiter=',', fmt='%.8f')
    # np.savetxt("gp_calc.csv", gp_calc, delimiter=',', fmt='%.8f')
    ...
    # # The coverage is close. But as footprint is small, the ratio is not above 0.9
    # np.testing.assert_(poly_diff[0] > 0.85)
    # # We look at the maximal distance to verify it also (<20m)
    # np.testing.assert_(poly_diff[1] < 20)
