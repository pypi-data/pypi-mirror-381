...
class TrCIDBQ05300(TRAccnoAbstract):
    ...
    def _build_response(self, resp: Optional[object], resp_json: Optional[Dict[str, Any]], resp_headers: Optional[Dict[str, Any]], exc: Optional[Exception]) -> CIDBQ05300Response:
        if exc is not None:
            return CIDBQ05300Response(header=None, block1=None, block2=[], block3=None, rsp_cd="", rsp_msg="", error_msg=str(exc))
        ...
        result = CIDBQ05300Response(
            ...
            block1=CIDBQ05300OutBlock1.model_validate(block1) if block1 is not None else None,
            block2=[CIDBQ05300OutBlock2.model_validate(item) for item in block2],
            block3=CIDBQ05300OutBlock3.model_validate(block3) if block3 is not None else None,
