...
class O3126InBlock(BaseModel):
    ...
    Attributes:
        mktgb (Literal["F", "O"]): 시장구분 (예: F(선물), O(옵션))
        symbol (str): 종목심볼
    """
    mktgb: Literal["F", "O"] = Field(..., title="시장구분", description="시장구분, 예: F(선물), O(옵션)")
    symbol: str = Field(..., title="종목심볼", description="종목심볼")
...
class O3126Request(BaseModel):
    ...
    o3126 API 요청 전체 구조
    """
    header: O3126RequestHeader = O3126RequestHeader(
    ...
    body: dict[Literal["o3126InBlock"], O3126InBlock] = Field(
        ...,
        title="입력 데이터 블록",
        description="입력 데이터 블록 (키: 'o3126InBlock')"
    )
