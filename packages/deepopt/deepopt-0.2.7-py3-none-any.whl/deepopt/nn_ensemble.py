"""
This module contains the NN Ensemble model used for single-fidelity and multi-fidelity
neural networks.
"""
import os
import warnings
...
class NNEnsemble(Model):
    """
    The `NNEnsemble` class represents an ensemble model for neural
    ...
    def __init__(
        ...
        :param networks: A list of neural network modules
        :param config: Configuration settings provided by the user
        :param optimizers: A list of optimizer objects from torch to help train the neural networks (1 per NN in the ensemble)
        :param X_train: The full input dataset
        :param y_train: The full output array
        :param mutli_fidelity: Whether the model is mult-fidelity. If it is, the different fidelities have outputs independently scaled to [0,1]
        :param fantasy_dict: Used during fantasizing to track y_max, y_min, and number of fantasy training points (set to `None` when not a fantasy model)
        """
        super().__init__()
        ...
        if fantasy_dict is None:
            self.is_fantasy_model = False
        else:
            self.is_fantasy_model = True
            self.y_max = fantasy_dict['y_max']
            self.y_min = fantasy_dict['y_min']
            self.fantasy_dict = fantasy_dict
    ...
    def fit(self):
        ...
        for ind, n_bin in enumerate(hist):
            if ind==0:
                ...
                loc = self.y_train_nn.mean(axis=1)>=bins[-2]
            else:
                loc = (self.y_train_nn.mean(axis=1)>=bins[ind])&(self.y_train_nn.mean(axis=1)<=bins[ind+1])
            weights[loc] = weight_progression[ind]/n_bin if n_bin>0 else weight_progression[ind]
        if self.is_fantasy_model:
            weights[-self.fantasy_dict['n_fantasy_training_pts']:] = weights[-self.fantasy_dict['n_fantasy_training_pts']:
                ]*(len(weights)-self.fantasy_dict['n_fantasy_training_pts'])/self.fantasy_dict['n_fantasy_training_pts']
        weights = weights/weights.mean()
        for estimator_num,(f_pred,opt) in enumerate(zip(self.f_predictor,self.f_optimizer)):
            selection = np.random.choice(len(self.X_train_nn),int(0.8*len(self.X_train_nn)),replace=False)
    ...
    def get_prediction_with_uncertainty(
        ...
        **kwargs: Any,
        ...
        if get_cov:
            ...
            cov = torch.einsum("i...A,i...B->...AB", del_pred, del_pred) / (self.n_estimators - 1)
    ...
    def fantasize(
        ...
        **kwargs,
        ...
        :returns: A new `NNEnsemble` model with the augmented dataset
        """
        ...
        config_fantasy = deepcopy(self.config)
        ...
        with torch.enable_grad():
            ...
            for f_pred,f_pred_fantasy in zip(self.f_predictor,fantasy_model.f_predictor):
                
                
                state_dict_prev = f_pred.state_dict()
                state_dict_new = f_pred_fantasy.state_dict()
                if f_pred.B is None:
                    f_pred_fantasy.B = None
                    for ind,(key_new,key_prev) in enumerate(zip(state_dict_new,state_dict_prev)):
                        if ind==0:
                            M_in_new = state_dict_new[key_new]
                            M_in_prev = state_dict_prev[key_prev]
                            size_mult = M_in_new.shape[1]//M_in_prev.shape[1]
                            state_dict_new[key_new] = M_in_prev.tile((1,size_mult))/size_mult
                        else:
                            state_dict_new[key_new] = state_dict_prev[key_prev].expand(state_dict_new[key_new].shape).detach().clone()
                else:
                    size_mult = f_pred_fantasy.B.shape[1]//f_pred.B.shape[1]
