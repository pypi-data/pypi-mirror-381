...
class DualClusterBomb: 
    ...
    def check_payload(self,payload_sets,insertion_points,url_encoded,checks=0):
        payloads3 = [''.join(random.choices(string.ascii_uppercase + string.digits, k=random.randint(3,50))) for _ in range(len(insertion_points))]
    ...
    def scan(self, insertion_points):
        ...
        for word in wordlist:
            ...
            payload_sets = []
            for c, i in enumerate(payloads):
                if c % 2 == 0:
                    payload = i
                else:
                    payload_sets.append(payload+ "§§§§" + i)

            payloads1 = payloads[:int(len(payloads)/2)]
            payloads2 = payloads[int(len(payloads)/2):]

            if self.stop is True:
                break

            for i in permutations(payload_sets):
                self.queue.put((i,insertion_points,url_encoded))
