...
def create_flowlens_mcp(token=None):
    # You can store the token in the FastMCP instance for later use (e.g., in middleware)
    mcp = FastMCP("Flowlens Map", stateless_http=True)
    mcp.token = token
    return mcp
...
def main():
    ...
    global flowlens_mcp
    flowlens_mcp = create_flowlens_mcp(token=args.token)
