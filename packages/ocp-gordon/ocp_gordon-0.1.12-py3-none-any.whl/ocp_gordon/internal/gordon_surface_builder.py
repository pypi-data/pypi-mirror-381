...
class GordonSurfaceBuilder:
    ...
    def _create_gordon_surface(self, profiles: list[Geom_BSplineCurve], guides: list[Geom_BSplineCurve], intersection_params_spline_u: list[float], intersection_params_spline_v: list[float]):
        ...
        if len(profiles) < 2:
            raise error("There must be at least two profiles for the gordon surface.", ErrorCode.MATH_ERROR)
        
        if len(guides) < 2:
            raise error("There must be at least two guides for the gordon surface.", ErrorCode.MATH_ERROR)
        ...
        self._check_curve_network_compatibility(profiles, guides, intersection_params_spline_u, intersection_params_spline_v, self.tolerance)
        ...
        for spline_idx in range(len(profiles)):
            for intersection_idx in range(n_u_params):
                ...
                intersection_points.SetValue(
                    intersection_idx + 1,
                    spline_idx + 1,
                    spline_u.Value(parameter)
                )
        
        # Check whether to build a closed continuous surface
        curve_u_tolerance = BSplineAlgorithms.REL_TOL_CLOSED * BSplineAlgorithms.scale(guides)
        curve_v_tolerance = BSplineAlgorithms.REL_TOL_CLOSED * BSplineAlgorithms.scale(profiles)
        tp_tolerance = BSplineAlgorithms.REL_TOL_CLOSED * BSplineAlgorithms.scale(intersection_points)
        
        make_u_closed = BSplineAlgorithms.is_u_dir_closed(intersection_points, tp_tolerance) and guides[0].IsEqual(guides[-1], curve_u_tolerance)
        make_v_closed = BSplineAlgorithms.is_v_dir_closed(intersection_points, tp_tolerance) and profiles[0].IsEqual(profiles[-1], curve_v_tolerance)
        
        # Skinning in v-direction with u directional B-Splines; u, v is normal
        surf_profiles_skinner = CurvesToSurface(list(profiles), intersection_params_spline_v, make_v_closed)
        surf_profiles = surf_profiles_skinner.surface()
        
        # Skinning in u-direction with v directional B-Splines; u, v is swapped
        surf_guides_skinner = CurvesToSurface(list(guides), intersection_params_spline_u, make_u_closed)
        ...
        tensor_prod_surf = GordonSurfaceBuilder._points_to_surface_internal(
            intersection_points, intersection_params_spline_u, intersection_params_spline_v, make_u_closed, make_v_closed
        )
        
        # Match degree of all three surfaces
        degree_u = max(
            surf_guides.UDegree(),
            surf_profiles.UDegree(),
            tensor_prod_surf.UDegree()
        )
        
        degree_v = max(
            surf_guides.VDegree(),
            surf_profiles.VDegree(),
            tensor_prod_surf.VDegree()
        )
        ...
        surfaces_vector = GordonSurfaceBuilder._create_common_knots_vector_surface_internal(
        ...
        assert (self._surface_guides.NbUPoles() == self._surface_profiles.NbUPoles() and 
                self._surface_profiles.NbUPoles() == self._surface_intersections.NbUPoles())
        assert (self._surface_guides.NbVPoles() == self._surface_profiles.NbVPoles() and 
                self._surface_profiles.NbVPoles() == self._surface_intersections.NbVPoles())
        ...
        for cp_u_idx in range(1, self._surface_gordon.NbUPoles() + 1):
            for cp_v_idx in range(1, self._surface_gordon.NbVPoles() + 1):
                ...
                self._surface_gordon.SetPole(cp_u_idx, cp_v_idx, gp_Pnt(new_pole_xyz.X(), new_pole_xyz.Y(), new_pole_xyz.Z()))
    
    def _assert_range(self, curve, umin, umax, tol=1e-7):
        """Check if curve parameters are within expected range."""
        if (abs(curve.FirstParameter() - umin) > tol or 
    ...
    def _check_curve_network_compatibility(self, profiles: list[Geom_BSplineCurve], guides: list[Geom_BSplineCurve], intersection_params_spline_u, intersection_params_spline_v, tol):
        ...
        if (abs(intersection_params_spline_u[0]) > splines_scale * tol or 
            abs(intersection_params_spline_u[-1] - 1.0) > splines_scale * tol):
            raise error("WARNING: B-splines in u-direction mustn't stick out, spline network must be 'closed'!", ErrorCode.MATH_ERROR)
        
        if (abs(intersection_params_spline_v[0]) > splines_scale * tol or 
            abs(intersection_params_spline_v[-1] - 1.0) > splines_scale * tol):
            raise error("WARNING: B-splines in v-direction mustn't stick out, spline network must be 'closed'!", ErrorCode.MATH_ERROR)
