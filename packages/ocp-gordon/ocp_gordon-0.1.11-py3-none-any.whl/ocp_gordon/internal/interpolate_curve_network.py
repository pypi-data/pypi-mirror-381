...
import numpy as np
from OCP.Geom import Geom_BSplineSurface, Geom_BSplineCurve, Geom_Curve
...
from .error import error, ErrorCode # Ensure error and ErrorCode are imported for internal use
...
class InterpolateCurveNetwork:
    ...
    def __init__(
        ...
        if len(profiles) < 2:
            raise InvalidInputError("There must be at least two profiles for the curve network interpolation.")
        if len(guides) < 2:
        ...
        if len(unique_profiles) < 2:
            raise InvalidInputError("There must be at least two unique profiles for the curve network interpolation.")
        if len(unique_guides) < 2:
            raise InvalidInputError("There must be at least two unique guides for the curve network interpolation.")
    ...
    def _compute_intersections_matrix(self, initial_intersection_params_u: np.ndarray, initial_intersection_params_v: np.ndarray):
        """
        ...
        for spline_u_idx in range(n_profiles):
            for spline_v_idx in range(n_guides):
                ...
                if not intersections:
                    ...
                    initial_intersection_params_u[spline_u_idx, spline_v_idx] = intersections[0]["parmOnCurve1"]
                    initial_intersection_params_v[spline_u_idx, spline_v_idx] = intersections[0]["parmOnCurve2"]
                elif len(intersections) == 2:
                    # For closed curves, take the smaller parameter values
                    initial_intersection_params_u[spline_u_idx, spline_v_idx] = min(intersections[0]["parmOnCurve1"], intersections[1]["parmOnCurve1"])
                    initial_intersection_params_v[spline_u_idx, spline_v_idx] = min(intersections[0]["parmOnCurve2"], intersections[1]["parmOnCurve2"])
    ...
    def _make_curves_compatible(self):
        ...
        self._compute_intersections_matrix(initial_intersection_params_u, initial_intersection_params_v)
        
        # 3. Sort curves and update intersection parameters
        sorter = CurveNetworkSorter(
            list(self.profiles), list(self.guides), initial_intersection_params_u, initial_intersection_params_v
        )
        sorter.perform()
        ...
        is_closed_profile = self.profiles[0].IsClosed() or self.profiles[0].IsPeriodic()
        is_closed_guides = self.guides[0].IsClosed() or self.guides[0].IsPeriodic()
        ...
        if is_closed_profile and is_closed_guides:
             raise CompatibilityError("Both profiles and guides cannot be closed simultaneously.")
        ...
        if is_closed_profile:
            ...
            original_n_profiles = current_n_profiles # Number of profiles before potential duplication
            original_n_guides = current_n_guides - 1 # Number of guides before duplication
            
            final_intersection_params_u[:original_n_profiles, :original_n_guides] = sorted_intersection_params_u
            final_intersection_params_v[:original_n_profiles, :original_n_guides] = sorted_intersection_params_v
            ...
            for i in range(original_n_profiles):
                ...
                final_intersection_params_u[i, original_n_guides] = 1.0 if abs(val_u) < BSplineAlgorithms.PAR_CHECK_TOL else val_u
                final_intersection_params_v[i, original_n_guides] = sorted_intersection_params_v[i, 0]
            ...
            # Copy existing data from sorted matrices
            original_n_profiles = current_n_profiles - 1 # Number of profiles before duplication
            original_n_guides = current_n_guides # Number of guides before potential duplication
            ...
            for j in range(original_n_guides):
                ...
                final_intersection_params_u[original_n_profiles, j] = sorted_intersection_params_u[0, j]
                final_intersection_params_v[original_n_profiles, j] = 1.0 if abs(val_v) < BSplineAlgorithms.PAR_CHECK_TOL else val_v
        else:
        ...
        self._eliminate_inaccuracies_network_intersections(
            self.profiles, self.guides, final_intersection_params_u, final_intersection_params_v
        )
        ...
        for j in range(current_n_guides): # Iterate over guides (columns)
            ...
            new_parameters_profiles.append(sum_u / current_n_profiles)
        ...
        for i in range(current_n_profiles): # Iterate over profiles (rows)
            ...
            new_parameters_guides.append(sum_v / current_n_guides)
        
        # Validate that intersections are at the beginning
        if (new_parameters_profiles[0] > BSplineAlgorithms.PAR_CHECK_TOL or 
            new_parameters_guides[0] > BSplineAlgorithms.PAR_CHECK_TOL):
            raise CompatibilityError("At least one B-spline has no intersection at the beginning.")
        ...
        for i in range(current_n_profiles):
            old_parameters = [final_intersection_params_u[i, j] for j in range(current_n_guides)]
            
            # Eliminate small inaccuracies at the first and last knots
            if abs(old_parameters[0]) < BSplineAlgorithms.PAR_CHECK_TOL: old_parameters[0] = 0.0
            if abs(new_parameters_profiles[0]) < BSplineAlgorithms.PAR_CHECK_TOL: new_parameters_profiles[0] = 0.0
            if abs(old_parameters[-1] - 1.0) < BSplineAlgorithms.PAR_CHECK_TOL: old_parameters[-1] = 1.0
            if abs(new_parameters_profiles[-1] - 1.0) < BSplineAlgorithms.PAR_CHECK_TOL: new_parameters_profiles[-1] = 1.0
            
            # C++ uses reparametrizeBSplineContinuouslyApprox. Python has a TODO.
            # For now, use the existing approx method.
            result = BSplineAlgorithms.reparametrize_bspline_continuously_approx(
                self.profiles[i], old_parameters, new_parameters_profiles, final_max_cp_u
            )
        ...
        for j in range(current_n_guides):
            old_parameters = [final_intersection_params_v[i, j] for i in range(current_n_profiles)]
            
            # Eliminate small inaccuracies
            if abs(old_parameters[0]) < BSplineAlgorithms.PAR_CHECK_TOL: old_parameters[0] = 0.0
            if abs(new_parameters_guides[0]) < BSplineAlgorithms.PAR_CHECK_TOL: new_parameters_guides[0] = 0.0
            if abs(old_parameters[-1] - 1.0) < BSplineAlgorithms.PAR_CHECK_TOL: old_parameters[-1] = 1.0
            if abs(new_parameters_guides[-1] - 1.0) < BSplineAlgorithms.PAR_CHECK_TOL: new_parameters_guides[-1] = 1.0
    ...
    def _eliminate_inaccuracies_network_intersections(self, sorted_splines_u, sorted_splines_v, 
        ...
        # first intersection
        for spline_u_idx in range(n_profiles):
            if abs(intersection_params_u[spline_u_idx, 0] - sorted_splines_u[0].Knot(1)) < 0.001:
        ...
        for spline_v_idx in range(n_guides):
            if abs(intersection_params_v[0, spline_v_idx] - sorted_splines_v[0].Knot(1)) < 0.001:
        ...
        for spline_u_idx in range(n_profiles):
            ...
            if abs(intersection_params_u[spline_u_idx, n_guides - 1] - last_knot_u) < 0.001:
                intersection_params_u[spline_u_idx, n_guides - 1] = last_knot_u
        
        for spline_v_idx in range(n_guides):
            last_knot_v = sorted_splines_v[0].Knot(sorted_splines_v[0].NbKnots())
            if abs(intersection_params_v[n_profiles - 1, spline_v_idx] - last_knot_v) < 0.001:
