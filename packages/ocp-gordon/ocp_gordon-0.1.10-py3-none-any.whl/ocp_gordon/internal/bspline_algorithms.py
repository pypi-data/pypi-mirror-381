...
from OCP.TColStd import TColStd_Array1OfReal, TColStd_HArray1OfReal, TColStd_Array1OfInteger, TColStd_HArray1OfInteger
...
class BSplineAlgorithms:
    ...
    def is_v_dir_closed(points: TColgp_Array2OfPnt, tolerance: float) -> bool:
        ...
        for u_idx in range(points.LowerRow(), points.UpperRow() + 1):
            if not points(u_idx, vlo).IsEqual(points(u_idx, vhi), tolerance): # Use __call__ for TColgp_Array2OfPnt
                v_dir_closed = False
                break
        return v_dir_closed
    ...
    def get_kink_parameters(curve: Geom_BSplineCurve) -> List[float]:
        ...
        for knot_index in range(2, curve.NbKnots()):
            if curve.Multiplicity(knot_index) == curve.Degree():
                ...
                if not (abs(angle) < angle_tolerance or abs(angle - math.pi) < angle_tolerance):
                    kinks.append(knot)
        return kinks
    ...
    @staticmethod
    def trim_curve(curve: Geom_BSplineCurve, umin: float, umax: float) -> Geom_BSplineCurve:
    ...
    def create_common_knots_vector_curve(splines_vector: List[Geom_BSplineCurve],
        ...
        result = []
        for spline in splines_vector:
            new_spline = BSplineAlgorithms._insert_knots(spline, common_knots, multiplicities, tol)
            result.append(new_spline)
        
        return result
    
    @staticmethod
    def reparametrize_bspline(spline: Geom_BSplineCurve, 
                            umin: float, umax: float, 
                            tol: float = 1e-15) -> None:
        """
    ...
    def _scale_array1_of_pnt(points: TColgp_Array1OfPnt) -> float:
        ...
        for i in range(points.Lower(), points.Upper() + 1):
            for j in range(i + 1, points.Upper() + 1): # C++ iterates from i+1 to Upper()-1, but Upper() is inclusive
    ...
    @staticmethod
    def _get_knot_multiplicity(spline: Geom_BSplineCurve, knot: float, tol: float) -> int:
    ...
    def reparametrize_bspline_continuously_approx(spline: Geom_BSplineCurve,
        ...
        interpolationObject = Geom2dAPI_Interpolate(old_parameters_pnts, new_parameters_array, False, 1e-6)
        ...
        reparametrizing_spline = interpolationObject.Curve() # This returns Geom2d_BSplineCurve
        ...
        for ikink in range(len(kinks)):
            projected_param = Geom2dAPI_ProjectPointOnCurve(gp_Pnt2d(kinks[ikink], 0.), reparametrizing_spline).LowerDistanceParameter()
        ...
        parameters = BSplineAlgorithms.linspace_with_breaks(new_parameters[0], new_parameters[-1], max(101, n_control_pnts * 2), breaks)
        ...
        makeContinuous = spline.IsClosed() and (spline.DN(spline.FirstParameter(), 1).Angle(spline.DN(spline.LastParameter(), 1)) < 6. / 180. * math.pi)

        from .bspline_approx_interp import BSplineApproxInterp, ApproxResult # Assuming ApproxResult is exported from here
        approximationObj = BSplineApproxInterp(points, int(n_control_pnts), 3, makeContinuous)
    ...
    @staticmethod
    def reparametrize_bspline_simple(spline: Geom_BSplineCurve, umin: float, umax: float) -> Geom_BSplineCurve:
        """
        ...
        return Geom_BSplineCurve(
            poles, weights, knots_array, mult_array,
    ...
    def knots_from_curve_parameters(params: List[float], degree: int, closed_curve: bool = False) -> List[float]:
        ...
        if closed_curve and degree % 2 == 0:
            ...
            for iparm in range(1, m):
                inner_knots[iparm + 1] = inner_knots[iparm] + 0.5 * (dparm[iparm - 1] + dparm[iparm])
            ...
            if len(inner_knots) != len(params):
                raise ValueError("Inner knots size must match parameters size for closed curves")
    ...
    @staticmethod
    def bspline_basis_mat(degree: int, knots: TColStd_Array1OfReal, 
                         params: TColStd_Array1OfReal, deriv_order: int = 0) -> np.ndarray:
        """
        ...
        for i_param in range(1, n_params + 1):
            ...
            basis_functions = BSplineAlgorithms._basis_functions(knots, span_idx, param_val, degree)
    ...
    def is_closed(points: TColgp_HArray1OfPnt, c2_continuous: bool = False) -> bool:
        ...
        return (points(points.Lower()).IsEqual(points(points.Upper()), error) 
                and c2_continuous)
    ...
    @staticmethod
    def _pnt_array2_get_column(matrix: TColgp_Array2OfPnt, col_index: int) -> TColgp_HArray1OfPnt:
        """
    ...
    @staticmethod
    def _pnt_array2_get_row(matrix: TColgp_Array2OfPnt, row_index: int) -> TColgp_HArray1OfPnt:
        """
    ...
    def compute_params_bspline_surf(points: TColgp_Array2OfPnt, alpha: float = 0.5) -> Tuple[List[float], List[float]]:
        ...
        for v_idx in range(points.LowerCol(), points.UpperCol() + 1):
            ...
            parameters_u_line = BSplineAlgorithms.compute_params_bspline_curve(points_u_line, alpha)
        ...
        for u_idx in range(points.LowerRow(), points.UpperRow() + 1):
            ...
            # Compute parameters for this 1D array of points
            parameters_v_line = BSplineAlgorithms.compute_params_bspline_curve(points_v_line, alpha)
    ...
    def _insert_knot_with_multiplicity(knot: float, count: int, degree: int, knots: List[float], mults: List[int], tol: float = 1e-5):
        ...
        if found_pos != -1:
            ...
            mults.insert(insert_idx, min(count, degree)) # Multiplicity should not exceed degree
    ...
    def have_same_range(splines_vector: List[Union['BSplineAlgorithms.SurfAdapterView', 'BSplineAlgorithms.CurveAdapterView']], par_tolerance: float) -> bool:
        ...
        if not isinstance(first_adapter, (BSplineAlgorithms.SurfAdapterView, BSplineAlgorithms.CurveAdapterView)):
            raise TypeError("Unsupported spline type in have_same_range: first element is not an adapter.")
        ...
        for spline_idx in range(1, len(splines_vector)):
            ...
            if not isinstance(current_adapter, (BSplineAlgorithms.SurfAdapterView, BSplineAlgorithms.CurveAdapterView)):
                raise TypeError("Mixed types in splines_vector for have_same_range: element is not an adapter.")
            
            begin_param_current = current_adapter.get_knot(1)
            end_param_current = current_adapter.get_knot(current_adapter.get_n_knots())

            if abs(begin_param_current - begin_param_ref) > par_tolerance or \
               abs(end_param_current - end_param_ref) > par_tolerance:
                return False
        return True
