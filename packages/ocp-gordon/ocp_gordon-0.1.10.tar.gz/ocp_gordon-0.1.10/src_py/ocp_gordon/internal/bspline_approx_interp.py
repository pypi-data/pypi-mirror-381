...
from OCP.TColStd import TColStd_Array1OfReal, TColStd_Array1OfInteger, TColStd_HArray1OfReal, TColStd_HArray1OfInteger
from OCP.gp import gp_Pnt, gp_Vec
from OCP.GeomAPI import GeomAPI_ProjectPointOnCurve # Equivalent to Geom2dAPI_ProjectPointOnCurve for 3D
...
class BSplineApproxInterp:
    ...
    """
    def __init__(self, points: TColgp_Array1OfPnt, n_control_points: int, degree: int = 3, continuous_if_closed: bool = False):
    ...
    def interpolate_point(self, point_index: int, with_kink: bool = False):
        if point_index not in self.m_index_of_approximated:
            raise error("Invalid index in BSplineApproxInterp::interpolate_point", ErrorCode.INDEX_ERROR)
    ...
    def is_closed(self) -> bool:
        ...
        return self.m_pnts(self.m_pnts.Lower()).IsEqual(self.m_pnts(self.m_pnts.Upper()), error_val)
    ...
    def _compute_knots(self, ncp: int, params: List[float]) -> Tuple[List[float], List[int]]:
        ...
        for kink_idx in self.m_index_of_kinks:
            ...
            BSplineAlgorithms._insert_knot_with_multiplicity(knot_val, self.m_degree, self.m_degree, knots, mults, 1e-4)
    ...
    def fit_curve_optimal(self, initial_params: Optional[List[float]] = None, max_iter: int = 10) -> ApproxResult:
        ...
        while result.error > 0 and (old_error - result.error) / max(result.error, 1e-6) > 1e-3 and iteration < max_iter and result.curve is not None:
    ...
    def _project_on_curve(self, pnt: gp_Pnt, curve: Geom_Curve, initial_param: float) -> ProjectResult:
        max_iter = 10
        eps = 1.0E-6
    ...
    def _get_continuity_matrix(self, n_ctr_pnts: int, contin_cons: int, params: List[float], flat_knots: TColStd_Array1OfReal) -> np.ndarray:
        ...
        diff1_1 = BSplineAlgorithms.bspline_basis_mat(self.m_degree, flat_knots, continuity_params1, 1)
        diff1_2 = BSplineAlgorithms.bspline_basis_mat(self.m_degree, flat_knots, continuity_params2, 1)
        
        diff2_1 = BSplineAlgorithms.bspline_basis_mat(self.m_degree, flat_knots, continuity_params1, 2)
        diff2_2 = BSplineAlgorithms.bspline_basis_mat(self.m_degree, flat_knots, continuity_params2, 2)
        ...
        if not self._first_and_last_interpolated():
            diff0_1 = BSplineAlgorithms.bspline_basis_mat(self.m_degree, flat_knots, continuity_params1, 0)
            diff0_2 = BSplineAlgorithms.bspline_basis_mat(self.m_degree, flat_knots, continuity_params2, 0)
    ...
    def _solve(self, params: List[float], knots: TColStd_Array1OfReal, mults: TColStd_Array1OfInteger) -> ApproxResult:
        ...
        if n_ctr_pnts < n_interpolated + n_continuity_conditions or n_ctr_pnts < self.m_degree + 1 + n_continuity_conditions:
            raise error("Too few control points for curve interpolation!")

        if n_approximated == 0 and n_ctr_pnts != n_interpolated + n_continuity_conditions:
        ...
        if n_approximated > 0:
            ...
            A = BSplineAlgorithms.bspline_basis_mat(self.m_degree, flat_knots_array, app_params_array)
            At = A.T
        ...
        if n_interpolated + n_continuity_conditions > 0:
            ...
            if n_interpolated > 0:
                interp_params_list = [params[idx] for idx in self.m_index_of_interpolated]
                ...
                C = BSplineAlgorithms.bspline_basis_mat(self.m_degree, flat_knots_array, interp_params_array)
                
                lhs[:n_ctr_pnts, n_ctr_pnts : n_ctr_pnts + n_interpolated] = C.T # C++ uses Ct, but here C is already (n_interpolated, n_ctr_pnts)
            ...
            if make_closed:
                continuity_entries = self._get_continuity_matrix(n_ctr_pnts, n_continuity_conditions, params, flat_knots_array)
            ...
            rhs_x[n_ctr_pnts : n_ctr_pnts + n_interpolated + n_continuity_conditions] = dx
            rhs_y[n_ctr_pnts : n_ctr_pnts + n_interpolated + n_continuity_conditions] = dy
            rhs_z[n_ctr_pnts : n_ctr_pnts + n_interpolated + n_continuity_conditions] = dz
