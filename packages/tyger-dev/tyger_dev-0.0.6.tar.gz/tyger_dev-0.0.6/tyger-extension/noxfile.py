...
def _install_bundle(session: nox.Session) -> None:
    ...
    # Install local tyger package if it exists
    tyger_path = os.environ.get("TYGER_PATH", None)
    if tyger_path and os.path.exists(tyger_path):
        try:
            session.install(
                "-t",
                "./bundled/libs",
            ...
            session.log(f"Successfully installed local tyger package from {tyger_path}")
        except nox.command.CommandFailed as e:
            session.log(f"Warning: Failed to install local tyger package: {e}")
    else:
        session.log("No local tyger package found. Using the version from requirements.txt if available.")
...
def _setup_template_environment(session: nox.Session) -> None:
    session.install("pip==25.0", "wheel", "pip-tools")
...
def lint(session: nox.Session) -> None:
    ...
    # Using session.run with success_codes to continue despite warnings
    # Exit code 28 is for convention, refactor and warning issue
    session.run(
        "pylint",
        "-d",
        "W0511",
        "./bundled/tool",
        success_codes=[0, 4, 8, 12, 16, 20, 24, 28],
    )
    session.run(
        "pylint",
        "-d",
