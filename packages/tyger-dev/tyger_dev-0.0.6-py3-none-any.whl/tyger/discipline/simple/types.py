...
from tyger.discipline.base.types import BaseTypeSystem, unk, tslice
from tyger.discipline.types import Context, TypeException
...
class SimpleTypeSystem(BaseTypeSystem):
    ...
    def is_class(self, ty) -> bool:
        # Use builtins registered for this type system to avoid hard imports
        builtins = self.get_builtin_types()
        class_type = builtins.get("ClassType")
        class_instance = builtins.get("ObjectType")
        try:
            return (
                (class_type is not None and isinstance(ty, class_type))
                or (class_instance is not None and isinstance(ty, class_instance))
            )
        except Exception:
            return False

    def get_builtin_types(self):
        base = super().get_builtin_types().copy()
        try:
            ...
            # If class modules are unavailable, skip adding them
            pass
        return base
    ...
    def dom_cod(self, ty):
        # Custom class handling (duck-typed to avoid imports here)
        if self.is_class(ty):
            # ClassType provides constructor_types and instance_type
            if hasattr(ty, "constructor_types") and hasattr(ty, "instance_type"):
                return ty.constructor_types, ty.instance_type
            # ClassInstance should carry a callable signature in env under "__call__"
            try:
                return self.dom_cod(ty["__call__"])
            except Exception:
                pass
        
        # For built-in types, use __qualname__
    ...
    def consistent(self, a: type, b: type) -> bool:
        ...
        # Handle custom types first (ClassType, ClassInstance)
        a_is_custom = self.is_class(a)
        b_is_custom = self.is_class(b)
        
        # If both are custom types, check if they're the same class
        if a_is_custom and b_is_custom:
            return a.__class__ == b.__class__ and a.name == b.name if hasattr(a, 'name') and hasattr(b, 'name') else a.__class__ == b.__class__
        
        # If one is custom and the other isn't, they're not consistent
        ...
        try:
            ...
            # If __qualname__ is not available, fall back to direct comparison
            return a == b
