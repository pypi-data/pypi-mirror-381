...
def main():
    ...
    # product two splines (explicit transformation)
    basisProd = bs.Basis()
    trf = basis0.prod(basis1, basisProd)
    coeffProd = kron(splines[0].getCoefficients(), splines[1].getCoefficients())
    splines[2] = bs.Spline(basisProd, trf @ coeffProd)
