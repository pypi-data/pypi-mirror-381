...
def main():
    ...
    # add two splines (explicit transformation)
    basisSum = bs.Basis(np.array([]), 0)
    trfLeft, trfRight = basis0.add(basis1, basisSum)
    coeffSum = (
        trfLeft @ splines[0].getCoefficients() + trfRight @ splines[1].getCoefficients()
    )
    splines[2] = bs.Spline(basisSum, coeffSum)
