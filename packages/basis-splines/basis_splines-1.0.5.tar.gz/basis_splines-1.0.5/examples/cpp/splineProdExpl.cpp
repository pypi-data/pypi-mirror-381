...
int main(int argc, char *argv[]) {
  ...
  // product two splines
  Bs::Basis basisProd{};
  const auto trf{splines[0].basis()->prod(*splines[1].basis(), basisProd)};
  splines[2] = Bs::Spline{std::make_shared<Bs::Basis>(basisProd),
                          trf * Bs::kron(splines[0].getCoefficients(),
                                              splines[1].getCoefficients())};
