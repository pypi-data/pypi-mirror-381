...
class NotebookSession:
    ...
    Example:
        ```python
        # Local file notebook
        from jupyter_agent_toolkit.kernel import create_session, SessionConfig
        from jupyter_agent_toolkit.notebook import make_document_transport, NotebookSession
    ...
    async def _execute_with_streaming(
        ...
        def _on_exec_count(n: Optional[int]) -> None:
            nonlocal exec_count
            exec_count = n
            asyncio.create_task(_flush())
        ...
        try:
            ...
            try:
                if res is not None:
                    ...
                    logger.debug(f"[_execute_with_streaming] Kernel execution result: {res.__dict__}")
                    logger.debug(f"[_execute_with_streaming] Set outputs for cell {cell_index}")
                else:
                    # Execution failed - use accumulated outputs
                    await self.doc.update_cell_outputs(cell_index, list(accum), exec_count)
                    logger.debug(f"[_execute_with_streaming] Using accumulated outputs for cell {cell_index} (execution failed)")
            except Exception as e:
                logger.warning(f"[_execute_with_streaming] Failed to update cell outputs for {cell_index}: {e}")

        # Return result or create error result for failed execution
        return res if res is not None else ExecutionResult(status="error", stderr="Execution failed or timed out")
    ...
    async def run_markdown(self, text: str, *, index: Optional[int] = None) -> int:
        """
