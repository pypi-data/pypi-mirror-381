...
def _parse_headers(headers_json: Optional[str]) -> Optional[Dict[str, str]]:
    if not headers_json:
        return None
    try:
...
def make_document_transport(
    ...
    *,
    ...
    if mode == "local" and local_path:
        return LocalFileDocumentTransport(local_path)
    ...
    if mode == "server" and remote_path:
        ...
        if prefer_collab and remote_base:
            ...
            return CollabYjsDocumentTransport(
                remote_base, remote_path, token=token, headers=headers, create_if_missing=create_if_missing
            )
