...
def register_handler(module: str, cls: str, mimetype: str, serializer: Callable, deserializer: Callable):
    """
...
def register_pandas_handlers():
    ...
    try:
        ...
        register_handler("pandas.core.frame", "DataFrame", "application/vnd.apache.arrow.stream", serialize_df_arrow, deserialize_df_arrow)
        register_handler("pandas.core.frame", "DataFrame", "application/json", lambda df: df.to_json(), lambda s, _: pd.read_json(s))
    except ImportError:
        pass
...
def register_numpy_handlers():
    """Register NumPy ndarray handler for JSON serialization."""
    try:
        import numpy as np
...
def serialize_object(obj: Any) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        ...
        return {"application/python-pickle": pickled.hex()}, {"application/python-pickle": {"type": key}}


def deserialize_object(data: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None) -> Any:
    """
...
def register_image_handlers():
    ...
    try:
        ...
        register_handler("PIL.Image", "Image", "image/png", serialize_pil_image, deserialize_pil_image)
    except ImportError:
        pass
