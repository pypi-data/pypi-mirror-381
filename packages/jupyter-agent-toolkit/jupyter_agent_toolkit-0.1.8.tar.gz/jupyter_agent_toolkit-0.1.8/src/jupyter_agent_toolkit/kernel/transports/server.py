"""
Remote kernel transport for jupyter-agent-toolkit.
...
class ServerTransport(KernelTransport):
    ...
    async def start(self) -> None:
        ...
        if self._kernel_id:
            ...
            self._start_pump()
            logger.info("WS reconnected (kernel_id=%s, session_id=%s)", self._kernel_id, self._client_session_id)
            return

        logger.info(
            "Connecting to Jupyter base=%s (notebook_path=%s)",
            self._base, self.cfg.notebook_path or "<standalone kernel>"
        )
        ...
        if self.cfg.notebook_path:
            ...
            if not sess:
                ...
                sess = await self._create_session_for_path(self.cfg.notebook_path, self.cfg.kernel_name)
    ...
    async def execute(
        ...
        *,
        ...
        async with self._exec_lock:
            ...
            logger.info(
                "execute(request_id=%s, timeout=%s) code=%r ...",
                request_id, timeout, preview
            )
            ...
            try:
                ...
                async for msg in self._collect_for_request_stream(request_id, timeout=timeout):
                    ...
                    elif msg_type in ("stream", "display_data", "update_display_data", "execute_result", "error"):
                ...
                logger.info(
                    ...
                    request_id, res.status, res.execution_count,
                    len(res.outputs or []), len(res.stdout or "")
                )
                return res
    ...
    async def _collect_for_request_stream(self, request_id: str, *, timeout: Optional[float]):
        """
        ...
        while True:
            if deadline is not None and loop.time() > deadline:
                raise asyncio.TimeoutError("Timeout waiting for kernel events")
            ...
            if msg_type == "status" and content.get("execution_state") == "idle" and parent_id == request_id:
            ...
            if ws_logger.isEnabledFor(logging.DEBUG):
                ws_logger.debug(
                    ...
                    msg_type, parent_id, seen_idle_for_req, seen_reply_for_req
                )
    ...
    async def _collect_for_request(self, request_id: str, *, timeout: Optional[float]) -> List[dict]:
        """
        ...
        while True:
            ...
            if msg_type in (
                "stream", "display_data", "update_display_data", "execute_result",
                "error", "execute_input", "execute_reply", "status",
            ):
                ...
                if got_idle and got_reply:
                    logger.debug(
                        "collect_for_request(%s) done: events=%d (idle & reply seen)",
                        request_id, n
                    )
                    break
    ...
    def _auth_headers(self) -> Dict[str, str]:
        ...
        logger.debug(
            ...
            "yes" if self.cfg.token else "no", bool(self.cfg.headers)
        )
        return h
    ...
    async def _get_session_for_path(self, path: str) -> Optional[dict]:
    ...
    async def _create_session_for_path(self, path: str, kernel_name: str) -> dict:
        ...
        logger.info(
            ...
            path, kernel_name, sess["kernel"]["id"]
        )
        return sess
