...
async def check_package_availability(session, packages: List[str]) -> Dict[str, bool]:
    """
...
"""
    return await _run_json(session, code, timeout=10.0)


async def ensure_packages_with_report(session, packages: List[str], timeout: float = 120.0) -> Dict[str, Any]:
    """
...
for pip_name in PKGS:
    ...
    try:
        res = subprocess.run(
            [sys.executable, "-m", "pip", "install", pip_name, "--no-warn-script-location", "--quiet"],
...
async def update_dependencies(session, packages: List[str], timeout: float = 120.0) -> bool:
    ...
    """
    availability = await check_package_availability(session, packages)
