...
def is_yarray(v: Any) -> bool:
    ...
    if v is None:
        return False
    looks_legacy = callable(getattr(v, "length", None)) and callable(
        getattr(v, "push", None)
    )
...
def make_code_cell_dict(
    source: str, metadata: Optional[Dict[str, Any]], tags: Optional[List[str]]
) -> Dict[str, Any]:
    """
...
def make_md_cell_dict(source: str, tags: Optional[List[str]] = None) -> Dict[str, Any]:
    """
