...
Example:
    ```python
    from jupyter_agent_toolkit.notebook.utils import (
...
def _normalize_outputs(outs: List[Dict[str, Any]], *, exec_count: int | None) -> List[Dict[str, Any]]:
    """
        ...
        if check_exists:
            async with session.get(url, timeout=timeout) as resp:
                if resp.status == 200:
                    ...
                    raise RuntimeError(f"Failed to check notebook existence ({resp.status}): {text}")
