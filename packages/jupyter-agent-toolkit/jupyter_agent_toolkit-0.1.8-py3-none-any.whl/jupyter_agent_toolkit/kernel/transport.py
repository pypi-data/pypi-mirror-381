    ...
    async def execute(
        ...
        *,
        ...
        - Shape: `outputs` is nbformat-like (dicts with `output_type`, `data`, `metadata`, etc.).
            It represents the *current* state (e.g., after a `clear_output`, the list may become empty).
        - Count: `execution_count` may be `None` until the kernel emits `execute_input`.
        - Final snapshot: The callback is typically invoked again with the final set once the request is complete.
        """
        ...