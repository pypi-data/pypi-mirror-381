...
BASE_LOGGER = "jupyter_agent_toolkit.notebook.transports.collab"
...
class CollabYjsDocumentTransport(NotebookDocumentTransport):
    ...
    def __init__(
        ...
        *,
        ...
        self._doc: Optional["YDoc"] = None
        self._awareness: Optional["Awareness"] = None
        self._root: Optional["YMap"] = None
        self._cells: Optional["YArray"] = None

        # YNotebook (Lab-compatible schema)
        self._ynb: Optional[YNotebook] = None
    ...
    async def start(self) -> None:
        ...
        if self._create_if_missing:
            ...
            try:
                ...
                if created:
                    wslog.info(
                        "Notebook created successfully via Contents API: %s", self._path
                    )
            except Exception as e:
                wslog.error("Failed to ensure notebook exists: %s", e)
                raise RuntimeError(
                    f"Could not create/verify notebook {self._path}: {e}"
                )
        ...
        async with self._http.put(
            url, json={"format": "json", "type": "notebook"}
        ) as r:
            r.raise_for_status()
            info = await r.json()
        # Parse response
    ...
    async def append_code_cell(
        ...
        async with self._op_lock:
            ...
            with doc.transaction():
                ...
                ycell = self._ynb.create_ycell(
                    make_code_cell_dict("", metadata, tags=tags)
                )
    ...
    async def append_markdown_cell(
        self, source: str, tags: Optional[List[str]] = None
    ) -> int:
        ...
        async with self._op_lock:
            ...
            with doc.transaction():
                ...
                try:
                    ...
                    wslog.warning(
                        "append_markdown_cell: failed to set YText source: %s", e
                    )
            await self._broadcast_update()
            await self._wait_for_sync_completion()
    ...
    async def insert_markdown_cell(
        self, index: int, source: str, tags: Optional[List[str]] = None
    ) -> None:
        ...
        async with self._op_lock:
            ...
            with doc.transaction():
                ...
                try:
                    ...
                    wslog.warning(
                        "insert_markdown_cell: failed to set YText source: %s", e
                    )
            await self._broadcast_update()
            await self._wait_for_sync_completion()
    ...
    async def delete_cell(self, index: int) -> None:
        ...
        async with self._op_lock:
            ...
            for _ in range(3):
                ...
                raise IndexError(
                    f"delete_cell: index {index} out of range 0..{current_len - 1}"
                )
            await self._broadcast_update()
            await self._wait_for_sync_completion()
    ...
    async def _validate_cell_index(self, index: int, operation: str) -> bool:
        ...
        try:
            ...
            wslog.error(
                "Error validating cell index %d for %s: %s", index, operation, e
            )
            return False
    ...
    async def update_cell_outputs(
        ...
        if outputs is not None and (
            ...
            or not all(isinstance(o, dict) for o in outputs)
        ):
        ...
        try:
            ...
            wslog.warning(
                "update_cell_outputs: Failed to validate index %d: %s", index, e
            )
            return
        ...
        async with self._op_lock:
            ...
            try:
                ...
                if self._pending_updates:
                    await asyncio.wait_for(
                        self._wait_for_sync_completion(), timeout=1.0
                    )

            except asyncio.TimeoutError:
                wslog.warning(
                    "Output update sync timeout for cell %d, continuing", index
                )
            except Exception as e:
            ...
            try:
                ...
                for i in range(cell_count):
                    ...
                    get = (
                        cell.get
                        if hasattr(cell, "get")
                        else (lambda k: cell.get(k) if isinstance(cell, dict) else None)
                    )
        ...
        with doc.transaction():
            ...
            try:
                ...
                if cell_count > 0:
                    ...
                    wslog.debug(
                        "_ensure_root: cells array is empty after initialization"
                    )
            except Exception as e:
    ...
    async def _wait_for_sync_completion(self) -> None:
        ...
        try:
            await asyncio.wait_for(
                asyncio.gather(*current, return_exceptions=True), timeout=2.0
            )
        except asyncio.TimeoutError:
            wslog.debug(
                "Sync completion timeout, cancelling %d pending updates", len(current)
            )
            for fut in current:
                if not fut.done():
                    fut.cancel()
    ...
    async def _send_awareness(self) -> None:
        ...
        if not (self._ws and not self._ws.closed) or not self._awareness:
            return
        try:
            payload = self._awareness.encode_awareness_update(
                [self._awareness.client_id]
            )
    ...
    async def _request_resync(self, reason: str, backoff: float) -> None:
        ...
        try:
            ...
            if sync_msg:
                ...
                wslog.debug(
                    "Requested Yjs resync after %s (backoff=%.2fs)", reason, backoff
                )
        except Exception as e:
        ...
        try:
            while True:
                ...
                if msg.type == aiohttp.WSMsgType.BINARY:
                    data: bytes = msg.data
                    wslog.debug(
                        "WS BINARY len=%d head=%s", len(data), hex_preview(data)
                    )
                    ...
                    # inbound awareness is non-structural for the doc; ignore to avoid format mismatches
                    if kind == "awareness":
                        continue
                    ...
                    if applied_ok:
                        ...
                        if (
                            not self._cells_bootstrapped.is_set()
                            and self._ynb is not None
                        ):
                            try:
                                with doc.transaction():
                                    cells = self._ynb.ycells
            ...
            with doc.transaction():
                ...
                for i in range(min(n, 4)):
                    try:
                        ...
                        wslog.debug(
                            "%s: snapshot aborted (array changed at i=%d)", where, i
                        )
                        break
                    get = (
                        c.get
                        if hasattr(c, "get")
                        else (lambda k: c.get(k) if isinstance(c, dict) else None)
                    )
                    ct = get("cell_type")
                    cid = get("id")
                    src = get("source")
