...
class ContentsApiDocumentTransport(NotebookDocumentTransport):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            path: Notebook path relative to server root (e.g. "analysis.ipynb" or "notebooks/analysis.ipynb")
    ...
    async def start(self) -> None:
        ...
        if self._create_if_missing:
            ...
            try:
                ...
                raise RuntimeError(f"Could not create/verify notebook {self._path}: {e}")
    ...
    async def update_cell_outputs(
        ...
        def m(cells):
            if index < 0 or index >= len(cells):
                raise IndexError(f"update_cell_outputs: index {index} out of range 0..{len(cells)-1}")
            cell = cells[index]
    ...
    async def append_markdown_cell(self, source: str, tags: Optional[List[str]] = None) -> int:
    ...
    async def insert_markdown_cell(self, index: int, source: str, tags: Optional[List[str]] = None) -> None:
        """Insert a markdown cell at a given index (0..len)."""
        def m(cells):
            if index < 0 or index > len(cells):
                raise IndexError(f"insert_markdown_cell: index {index} out of range 0..{len(cells)}")
            cell = {
                "id": uuid.uuid4().hex,
                "cell_type": "markdown",
                "metadata": {},
    ...
    async def _json_request(self, method: str, url: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"{method} {url} timed out after {self._timeout}s")
