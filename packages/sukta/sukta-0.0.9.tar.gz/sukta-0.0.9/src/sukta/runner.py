...
from sukta.utils import sem

T = TypeVar("T")


def cache(*cache_dir, cache_name=None):
...
class Runner:
    ...
    def __init__(
        ...
        run_hash = str(uuid.uuid5(uuid.NAMESPACE_URL, str(self.run_dir)))
    ...
    def default_parser(
        ...
        parser.add_argument(
            "--log_flush_interval", type=float, default=log_flush_interval
        )
        return parser
    ...
    def from_args(
        ...
        runner = cls(
            ...
            flush_interval=args.log_flush_interval,
            debug=args.debug,
    ...
    def parse_cstr(cstr):
        ...
        E.g.:
        - r'n4_h512_p0.0_act$relu_d-1e3_D-108\_000' -> dict({'n': 4, 'h': 512, 'p': 0.0, 'act': 'relu', 'd': -1000.0, 'D': -108000})
        - r'model$aloha\_v2_p0.1_d-1e-4' -> dict({'model': 'aloha_v2', 'p': 0.1, 'd': -0.0001})
        - r'layer\_s\1ze$H100_block\_\0_n4' -> dict({'layer_s1ze': 'H100', 'block_0': True, 'n': 4})
        """
        replaces, restores = {}, {}
