...
from argmatch._compat import OrderedDict as _OrderedDict
from argmatch.base import BaseMatcher
from argmatch.general import Any
from argmatch.types import InstanceOf
...
class Generator(BaseMatcher):
    ...
    .. note::
        ...
        :class:`~argmatch.functions.GeneratorFunction` matcher instead.
    """
...
class MappingMatcher(CollectionMatcher):
    ...
    def _initialize(self, *args, **kwargs):
        ...
        if args:
            ...
            if not (has_kv or has_of):
                raise TypeError(
                    "expected keys/values or items matchers, "
    ...
    def match(self, value):
        ...
        if self.keys is not None and self.values is not None:
            return all(self.keys == k and self.values == v for k, v in value.items())
