...
class SecurityScanner:
    """Main security scanner that orchestrates the multi-agent analysis"""
    
    def __init__(self, api_key: Optional[str] = None, model: str = "claude-3-5-sonnet-20241022"):
        """
        Initialize scanner with multi-agent orchestrator
    ...
    async def scan(self, repository_path: str, save_results: bool = True) -> ScanResult:
        """
        Scan a repository for security vulnerabilities using multi-agent approach
        ...
        # Run multi-agent workflow (this already saves scan_results.json)
        scan_result = await self.orchestrator.run_full_scan(str(repo_path))
        
        # Calculate scan time
        scan_time = time.time() - start_time
        
        # Update the scan result with additional metadata
        scan_result.files_scanned = files_scanned
        scan_result.scan_time_seconds = round(scan_time, 2)
        
        # Re-save with updated metadata
        if save_results:
            output_dir = repo_path / ".vulnguard"
            output_file = output_dir / "scan_results.json"
            JSONReporter.save(scan_result, output_file)
