...
class TestCLIBasics:
    ...
    def test_cli_help(self, runner):
        ...
        assert 'VulnGuard' in result.output
    ...
    def test_cli_version(self, runner):
        ...
        assert result.exit_code == 0
        assert 'vulnguard' in result.output.lower()
...
class TestThreatModelCommand:
    ...
    def test_threat_model_with_security_md(self, runner, test_repo):
        ...
        vulnguard_dir.mkdir()
        (vulnguard_dir / "SECURITY.md").write_text("# Security Architecture\n\n## Overview\nTest")
...
class TestInitCommand:
    ...
    def test_init_creates_config(self, runner, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            assert result.exit_code == 0
            assert Path('.vulnguard.yaml').exists()
            
            # Check config content
            config = Path('.vulnguard.yaml').read_text()
            assert 'scan:' in config
            assert 'severity_threshold:' in config
    
    def test_init_overwrites_with_confirmation(self, runner, tmp_path):
        """Test init asks before overwriting"""
        with runner.isolated_filesystem(temp_dir=tmp_path):
            # Create existing config
            Path('.vulnguard.yaml').write_text("existing: config")
            
            # Try to init again (should ask for confirmation)
