...
from vulnguard import SecurityScanner, __version__
...
def scan(path: str, api_key: Optional[str], model: str, output: Optional[str], format: str, 
    ...
    try:
        ...
        if not quiet:
            console.print("[bold cyan]🛡️ VulnGuard Security Scanner[/bold cyan]")
...
async def _run_scan(path: str, api_key: Optional[str], model: str, save_results: bool, quiet: bool):
    ...
    if not quiet:
        with Progress(
            ...
            task = progress.add_task("[cyan]Analysis in progress...", total=None)
...
def assess(path: str, api_key: Optional[str], model: str):
    """
    Run only the architecture assessment phase.
    
    Creates SECURITY.md in .vulnguard/ directory.
    
    Examples:
    
        vulnguard assess .
        
        vulnguard assess /path/to/project
    """
    try:
        console.print(Panel.fit(
            "[bold cyan]📐 VulnGuard Assessment Agent[/bold cyan]\n"
            "[dim]Architecture Documentation[/dim]",
            border_style="cyan"
        ))
        
        orchestrator = OrchestratorAgent(api_key=api_key, model=model, debug=True)
        result = asyncio.run(orchestrator.run_assessment_only(path))
        
        console.print(f"\n✅ [green]{result['summary']}[/green]")
        console.print(f"📄 Output: [cyan]{result['file']}[/cyan]")
        console.print(f"📊 Sections: {result['sections']}, Size: {result['size_bytes']} bytes\n")
...
@click.option('--api-key', envvar='CLAUDE_API_KEY', help='Claude API key')
@click.option('--model', '-m', default='claude-3-5-haiku-20241022', help='Claude model to use')
def threat_model(path: str, api_key: Optional[str], model: str):
    """
    ...
    Examples:
        ...
        vulnguard threat-model .
    """
    try:
        console.print(Panel.fit(
            "[bold cyan]🎯 VulnGuard Threat Modeling Agent[/bold cyan]\n"
            "[dim]STRIDE Analysis[/dim]",
            border_style="cyan"
        ))
        
        orchestrator = OrchestratorAgent(api_key=api_key, model=model, debug=True)
        result = asyncio.run(orchestrator.run_threat_modeling_only(path))
        
        console.print(f"\n✅ [green]{result['summary']}[/green]")
        console.print(f"📄 Output: [cyan]{result['file']}[/cyan]")
        console.print(f"📊 By Severity: {result['by_severity']}\n")
...
@click.option('--api-key', envvar='CLAUDE_API_KEY', help='Claude API key')
@click.option('--model', '-m', default='claude-3-5-haiku-20241022', help='Claude model to use')
def review(path: str, api_key: Optional[str], model: str):
    """
    ...
    try:
        console.print(Panel.fit(
            "[bold cyan]🔍 VulnGuard Code Review Agent[/bold cyan]\n"
            "[dim]Vulnerability Validation[/dim]",
            border_style="cyan"
        ))
        
        orchestrator = OrchestratorAgent(api_key=api_key, model=model, debug=True)
        result = asyncio.run(orchestrator.run_code_review_only(path))
