...
pub fn sc_fluctanal_2_dfa_50_2_logi_r2_se2(y: &[f64], normalize: bool) -> f64 {
    ...
    for i in min_points..(n_tau - min_points + 1) {
        ...
        // Use euclidean_norm_f from common.rs
        sserr[i - min_points] = euclidean_norm_f(&buffer1);
        
        let buffer2: Vec<f64> = (0..(n_tau - i + 1))
            .map(|j| log_tt[j + i - 1] * m2 + b2 - log_ff[j + i - 1])
            .collect();
    ...
    let first_min_ind = sserr.iter().position(|&v| v == minimum).unwrap() + min_points - 1;
