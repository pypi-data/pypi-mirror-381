...
class PNA(torch.nn.Module):
    def __init__(
        self, in_features: int, features: int, out_features: int, layers: int, deg = None
    ) -> None:
        super().__init__()
        ...
        self.layers = torch.nn.ModuleList(
            [PNAConv(features, features, aggregators=aggregators, scalers=scalers, deg=deg, towers=1) for i in range(layers - 1)]
        )
    ...
    def forward(
        ...
        assert getattr(data, "edge_index", None) is not None, (
            "PNA.forward expects 'data.edge_index' to be present"
        )
        assert getattr(data, "batch", None) is not None, (
            "PNA.forward expects 'data.batch' to be present"
        )
        if data.x is None:
            x = torch.ones(
