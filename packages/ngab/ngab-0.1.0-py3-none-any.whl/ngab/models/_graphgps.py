...
class GraphGPS(torch.nn.Module):
    def __init__(
        self, in_features: int, features: int, out_features: int, layers: int, heads: int
    ) -> None:
        super().__init__()
        ...
        self.layers = torch.nn.ModuleList(
            [GPSConv(features, conv=GatedGraphConv(out_channels=features, num_layers=2), heads=heads) for i in range(layers - 1)]
        )
    ...
    def forward(
        ...
        assert getattr(data, "edge_index", None) is not None, (
            "SGC.forward expects 'data.edge_index' to be present"
        )
        assert getattr(data, "batch", None) is not None, (
            "GCN.forward expects 'data.batch' to be present"
        )
        if data.x is None:
            x = torch.ones(
