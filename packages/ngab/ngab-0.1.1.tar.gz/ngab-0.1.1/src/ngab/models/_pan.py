...
class PAN(torch.nn.Module):
    def __init__(
        self, in_features: int, features: int, out_features: int, layers: int, filters: int
    ) -> None:
        super().__init__()
        ...
        self.layers = torch.nn.ModuleList(
            [PANConv(features, features, filters) for i in range(layers - 1)]
        )
        self.gns = torch.nn.ModuleList([GraphNorm(features) for _ in range(layers - 1)])
    ...
    def forward(
        ...
        assert getattr(data, "edge_index", None) is not None, (
            "PAN.forward expects 'data.edge_index' to be present"
        )
        assert getattr(data, "batch", None) is not None, (
            "PAN.forward expects 'data.batch' to be present"
        )
        if data.x is None:
            x = torch.ones(
        ...
        for i in range(len(self.layers)):
            x, M = self.layers[i](x, edge_index)
            x = F.gelu(x)
            x = x + self.gns[i](x, data.batch)
        x = self.linear(x)
