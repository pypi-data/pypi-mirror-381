...
class TAGCN(torch.nn.Module):
    def __init__(
        self, in_features: int, features: int, out_features: int, layers: int, k: int
    ) -> None:
        super().__init__()
        ...
        self.layers = torch.nn.ModuleList(
            [TAGConv(features, features, K=k) for i in range(layers - 1)]
        )
    ...
    def forward(
        ...
        assert getattr(data, "edge_index", None) is not None, (
            "TAGCN.forward expects 'data.edge_index' to be present"
        )
