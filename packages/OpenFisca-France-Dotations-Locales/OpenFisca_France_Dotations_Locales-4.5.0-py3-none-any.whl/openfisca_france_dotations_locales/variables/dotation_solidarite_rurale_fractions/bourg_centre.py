...
class dsr_eligible_fraction_bourg_centre(Variable):
    ...
    entity = Commune
    definition_period = YEAR
    label = "Éligibilité à la fraction bourg-centre de la DSR"
    reference = [
        'Code général des collectivités territoriales - Article L2334-21',
        'https://www.legifrance.gouv.fr/affichCodeArticle.do?idArticle=LEGIARTI000036433099&cidTexte=LEGITEXT000006070633',
        "http://www.dotations-dgcl.interieur.gouv.fr/consultation/documentAffichage.php?id=94"
...
class dsr_bourg_centre_accroissement_metropole(Variable):
    value_type = float
    entity = Etat
    definition_period = YEAR
    label = "En métropole, pourcentage d'accroissement du montant de la DSR d'une année à l'autre"
    ...
    def formula_2019(etat, period, parameters):
        dsr_bc_effective_annee_precedente = parameters(period.last_year).dotation_solidarite_rurale.bourg_centre.montant
        dsr_bc_annee_courante = parameters(period).dotation_solidarite_rurale.bourg_centre.montant

        pourcentage_accroissement_dsr_bc_annee_courante = (dsr_bc_annee_courante - dsr_bc_effective_annee_precedente) / dsr_bc_effective_annee_precedente
        return pourcentage_accroissement_dsr_bc_annee_courante
...
class dsr_montant_total_fraction_bourg_centre(Variable):
    ...
    def formula_2013_01(etat, period, parameters):
        # TODO prendre en compte dotation_solidarite_rurale.majoration_montant après ajout de ses valeurs 2013>2018
        ...
        dsr_bourg_centre_accroissement_metropole = etat('dsr_bourg_centre_accroissement_metropole', period)
        return montants_an_prochain - accroissement * dsr_bourg_centre_accroissement_metropole  # an prochain => soustraction

    def formula_2019_01(etat, period, parameters):
        return parameters(2019).dotation_solidarite_rurale.bourg_centre.montant
    ...
    def formula_2020_01(etat, period, parameters):
        ...
        dsr_bourg_centre_accroissement_metropole = etat('dsr_bourg_centre_accroissement_metropole', period)
        return montants_an_precedent + (montants_an_precedent * dsr_bourg_centre_accroissement_metropole)
...
class dsr_fraction_bourg_centre(Variable):
    ...
    reference = "https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000048862119"
    # reference initiale datée : https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000048862119/2020-08-13/
    documentation = '''Fraction bourg-centre également nommée première fraction de la DSR.'''
