...
class StateNetwork:
    ...
    def get_states(self) -> StateEnsemble:
    ...
    def micro_pKa(self, PE: np.ndarray, beta: float=1.0) -> dict[int,np.ndarray]:
        """Calculate micro-pKa with provided potential energies.

        Args:
            PE (np.ndarray): potential energies of states.
            beta (float, optional): 
                :math:`\\beta = \\frac{1}{kT}` in which :math:`k` is Boltzmann constant 
                and :math:`T` is Temperature in Kelvin unit. 
                Defaults to 1.0.

        Returns:
            dict[int,list[float]]: micro-pKa values for each ionizable site.
        """
        assert PE.shape[0] == len(self.visited_states), "PE should be given for all states"
    ...
    def macro_pKa(self, PE: np.ndarray, beta: float = 1.0) -> np.ndarray:
    ...
    def population(self, PE: np.ndarray, pH: np.ndarray, C: float, beta: float = 1.0) -> np.ndarray:
        ...
        .. math::
            \\Delta G_{i, ref} = PE_{i} - PE_{ref} \\\\
            \\Delta m_{i, ref} = charge_{i} - charge _{ref} \\
            \\Delta G_{i, pH} = \\Delta G_{i, ref} + \\Delta m_{i, ref} C pH \\
            p_{i, pH} = \\frac {exp(-\\beta \\Delta G_{i, pH})}{\\sum_{i} exp(-\\beta \\Delta G_{i, pH})}

        Args:
            PE (np.ndarray): array of potential energies of states.
            pH (np.ndarray): array of pH values.
            C (float): constant for pH-dependent dG calculation.
            beta (float, optional): 
                :math:`\\beta = \\frac{1}{kT}` in which :math:`k` is Boltzmann constant 
                and :math:`T` is Temperature in Kelvin unit. 
                Defaults to 1.0.
        ...
        assert p.shape == (len(self.visited_states), pH.shape[0])
        
        return p


    def population_chart(self, PE: np.ndarray, pH: np.ndarray, C: float, beta: float = 1.0, 
                        pH_label: np.ndarray | None = None,
                        width: int = 600,
                        height: int = 300,
                        palette: str = 'tableau10') -> alt.vegalite.v5.api.LayerChart:
        ...
        p = self.population(PE, pH, C, beta)
        ...
        for i, state in enumerate(p):
            for j, p in enumerate(state):
                data['Microstate'].append(i)
                data['pH'].append(pH_label[j])
                data['p'].append(p)
