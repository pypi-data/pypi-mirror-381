...
def test_unipka_workflow():
    ...
    assert sn.get_num_nodes() == sn.get_states().size()
    ...
    PE = np.array([-6.025253772735596, -2.9201512336730957, -2.7405877113342285, 
    ...
    p = sn.population(PE, pH, C=LN10, beta=1.0)
    expected_p = [
    ...
    for k, st in enumerate(sn.visited_states):
        print(f"{k:2} {st.smiles:50} {PE[k]:8.3f} {p[k]}")

    # population chart
    pH = np.linspace(0, 14, 60)
    p = sn.population(PE, pH=pH-TRANSLATE_PH, C=LN10, beta=1.0)

    chart = sn.population_chart(PE, 
                               pH=pH-TRANSLATE_PH, 
                               pH_label=pH, 
                               C=LN10, 
                               beta=1.0,
    ...
    chart.save(workdir / 'microstate_population.svg')
    # note: saving to a .png requires another package
