...
from localstack.pro.core.bootstrap.pods.api_types import MergeStrategy
...
def cmd_pod_save(name=_A,remote=_A,services=_A,message=_A,visibility=_A,secret=_A,format_=_A):
	F=visibility;E=remote;B=name;A=services;G=CloudPodsClient(_C);_assert_host_reachable()
	if A and not is_comma_delimited_list(A):raise CLIError('Input the services as a comma-delimited list')
	H=RemoteConfigParams(remote_name=E)if E else _A
	if F:
		try:G.set_remote_attributes(pod_name=B,attributes=CloudPodRemoteAttributes(is_public=_C),remote=H)
		except Exception as C:raise CLIError(str(C))
		console.print(f"Cloud Pod {B} made {F}")
	I=[A.strip()for A in A.split(',')]if A else _A
	try:D=G.save(pod_name=B,attributes=CloudPodRemoteAttributes(is_public=_B,description=message,services=I),remote=H,secret=secret)
	except Exception as C:raise CLIError(f"Failed to create Cloud Pod {B} ❌ - {C}")from C
	if format_==_D:console.print_json(json.dumps(D))
	else:A=','.join(D[_L])or'none';console.print(f"Cloud Pod `{B}` successfully created ✅\nVersion: {D[_M]}\nRemote: {D[_F]}\nServices: {A}")
...
@click.argument(_F,required=_B)
@click.option('-s',_N,_O,help=_P)
@click.option('--strategy',type=click.Choice([A.value for A in MergeStrategy]),default=pro_config.MERGE_STRATEGY,help='The merge strategy to adopt when loading the Cloud Pod.')
...
def cmd_pod_versions(name,format_):
	...
	if format_==_D:console.print_json(json.dumps(A));return
	print_table(column_headers=['Version','Creation Date','LocalStack Version','Services','Description'],columns=[[str(A[_M])for A in A],[timestamp(A['created_at'],format=DATE_FORMAT)for A in A],[A['localstack_version']for A in A],[','.join(A[_L]or[])for A in A],[A['description']for A in A]])