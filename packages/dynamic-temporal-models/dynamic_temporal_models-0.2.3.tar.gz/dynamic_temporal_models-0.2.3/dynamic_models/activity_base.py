...
def create_activity_type(name: str):
    class ActivityBase:
        ...
        async def run(self, data: ActivityInput, *args, **kwargs):
            if not self.is_heartbeat:
                event_cancel = asyncio.Event()
                return await self.executor(event_cancel, data, *args, **kwargs)
            else:
                return await self.heartbeat_runner(data, *args, **kwargs)
