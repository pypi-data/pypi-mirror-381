...
async def start_workflow(wflow: input_module.Workflow, input_data: input_module.WorkflowInput, wait_execution: bool, workflow_executor=None):
...
async def process_workflow(wflow: input_module.Workflow, input_data: dict, wait_execution: bool, workflow_executor=None):
    workflow_executor = workflow_executor or (workflow.execute_child_workflow if wait_execution else workflow.start_child_workflow)
    child_workflow = await workflow_executor(
        wflow.name,
        input_module.ActivityInput(input=input_data, app_config=wflow.app_config),
        id=wflow.id or None,
        task_queue=wflow.queue,
        retry_policy=to_temporal_retry_policy(wflow.retry_policy),
        execution_timeout=to_timedelta(wflow.execution_timeout),
...
async def process_activity(activity: input_module.Activity, input_data: dict, wait_execution: bool):
    activity_executor = workflow.execute_activity if wait_execution else workflow.start_activity
    result = await activity_executor(
        activity.name,
        input_module.ActivityInput(input=input_data, app_config=activity.app_config),
...
async def process_executor(executor: input_module.Executor, input_data: dict):
    if executor.is_workflow and executor.workflow:
        return await process_workflow(executor.workflow, input_data, executor.wait_execution)
    elif not executor.is_workflow and executor.activity:
        return await process_activity(executor.activity, input_data, executor.wait_execution)
    else:
