...
class LazyTableHandle:
    ...
    async def get_records(
        ...
        handle = await self._ensure_resolved()
        filter_dict = await _normalize_filter(handle, filter)
        return await handle.get_records(filter_dict, limit, skip)
...
class TableHandle:
    ...
    async def get_records(
        ...
        filter_dict = await _normalize_filter(self, filter)
        return await self._client.get_records(self.base_id, self.table_id, filter_dict, limit, skip)
...
async def _normalize_filter(table_handle, simple_filter: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Преобразует {fieldName: value} или {fieldId: value} в правильный filterSet.

    Поддерживает два режима:
    - Если filter уже содержит filterSet -> возвращаем как есть
    - Если в get_info() есть field_id -> используем fieldId
    - Иначе используем fieldName
    """
    if not simple_filter:
        return None

    # уже нормализованный фильтр
    ...
    for f in fields:
        ...
        if name:
            field_map_by_name[name] = fid
        if fid:
            field_map_by_id[fid] = name

    filter_set = []
    for key, value in simple_filter.items():
        field_id = field_map_by_name.get(key)
        if field_id:
            filter_set.append({
                "fieldId": field_id,
                "operator": "is",
            ...
            # fallback: используем имя, если field_id отсутствует в схеме
            if key in field_map_by_name or not field_map_by_name:
                filter_set.append({
                    "fieldName": key,
                    "operator": "is",
                    "value": value
