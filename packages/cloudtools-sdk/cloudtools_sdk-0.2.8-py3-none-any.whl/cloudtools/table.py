...
async def _normalize_filter(table_handle, simple_filter: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    print(">>> _normalize_filter input =", simple_filter)
    info = await table_handle.get_info()
    print(">>> _normalize_filter info =", info)
    fields = info.get("fields", []) or []
    field_map = {f["name"]: f.get("field_id") for f in fields if f.get("field_id")}
    ...
    print(">>> _normalize_filter output =", {"filterSet": filter_set, "conjunction": "and"})
