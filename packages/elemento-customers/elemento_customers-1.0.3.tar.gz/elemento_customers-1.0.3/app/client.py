...
from .types import Customer, CustomerCreate, CustomerId, CustomerUpdate, SearchFilter, Card, CardId
...
class ElementoCustomersClient(RPCClientService):
    ...
    async def create_card(self, customer_id: CustomerId | int, _max_timeout: int = None, _nowait: bool = False) -> Card:
        """Find a customer by phone number or return `None` if not exists."""
        result = await self.call(
            method="Customers.cards.create", params=dict(customer_id=customer_id), max_timeout=_max_timeout, nowait=_nowait
        )
        return Card(**result)
