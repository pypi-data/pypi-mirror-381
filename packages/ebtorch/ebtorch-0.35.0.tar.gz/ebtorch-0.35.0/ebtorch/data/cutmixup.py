...
#           HuggingFace
#           Emanuele Ballarin <emanuele@ballarin.cc> [minor edits]
# All Rights Reserved.
#
...
def _mixup_target(target, num_classes, lam=1.0, smoothing=0.0):
    ...
    y1 = _one_hot(target, num_classes, on_value=on_value, off_value=off_value)
    y2 = _one_hot(target.flip(0), num_classes, on_value=on_value, off_value=off_value)
    return y1 * lam + y2 * (1.0 - lam)


def _rand_bbox(img_shape, lam, margin=0.0, count=None):
...
def _rand_bbox_minmax(img_shape, minmax, count=None):
...
def _cutmix_bbox_and_lam(img_shape, lam, ratio_minmax=None, correct_lam=True, count=None):
    ...
    if ratio_minmax is not None:
        ...
        yl, yu, xl, xu = _rand_bbox(img_shape, lam, count=count)
...
class Mixup:
    ...
    def _mix_elem(self, x):
        ...
        for i in range(batch_size):
            ...
            if lam != 1.0:
                if use_cutmix[i]:
                    (yl, yh, xl, xh), lam = _cutmix_bbox_and_lam(
                        x[i].shape,
                        lam,
    ...
    def __call__(self, x, target):
        ...
        target = _mixup_target(target, self.num_classes, lam, self.label_smoothing)
        return x, target
