...
def patchify_2d(img: torch.Tensor, patch_size: int, stride: int = 1) -> torch.Tensor:
    ...
    """
    patches: torch.Tensor = img.unfold(1, patch_size, stride).unfold(2, patch_size, stride)
    patches: torch.Tensor = patches.permute(1, 2, 0, 3, 4).contiguous()
    return patches.view(-1, *patches.shape[-3:])


def patchify_batch(imgbatch: torch.Tensor, patch_size: int, stride: int = 1) -> torch.Tensor:
    """
    ...
    """
    patches: torch.Tensor = imgbatch.unfold(2, patch_size, stride).unfold(3, patch_size, stride)
    patches: torch.Tensor = patches.permute(0, 2, 3, 1, 4, 5).contiguous()
    return patches.view(imgbatch.shape[0], -1, *patches.shape[-3:])


def patchify_dataset(imgdata: torch.Tensor, patch_size: int, stride: int = 1) -> torch.Tensor:
    """
