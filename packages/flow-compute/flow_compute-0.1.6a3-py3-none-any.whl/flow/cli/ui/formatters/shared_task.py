...
class TaskFormatter:
    ...
    def get_display_status(task: Task) -> str:
        ...
        - PENDING tasks are refined based on instance status:
          • "starting" - instances are booting (initializing, starting, provisioning)
          • "pending" - task is queued or scheduled, no instances yet
        - RUNNING tasks are shown as "running" (instance status already reflected in task.status).

        Since task.status now correctly uses instance status when available, the display
        logic is simplified to just refine PENDING → "starting" for better UX during boot.
        """
        ...
        # Distinguish "starting" (instances booting) from "pending" (queued/scheduled)
        if status_value == "pending":
            ...
            if instance_status_from_api:
                ...
                # Show "starting" for instance boot states (more specific than "pending")
            ...
            # Otherwise, keep as "pending" (queued, scheduled, not yet allocated)
            return "pending"
