...
def build_empty_state_next_steps(has_history: bool) -> list[str]:
    ...
    # Create a <noun> (quick)
    steps.append(
        f"[accent][bold]flow[/bold][/accent] run -- 'nvidia-smi' [muted]— Create a {noun} (quick)[/muted]"
    )
    steps.append(
    ...
    if has_history:
        steps.append(
            f"[accent][bold]flow[/bold][/accent] status 1 [muted]— View {labels.empty_plural[:-1] if labels.empty_plural.endswith('s') else labels.empty_plural} details (by index)[/muted]"
        )
        noun_name = f"{labels.header.lower()}-name"
        steps.append(
            f"[accent][bold]flow[/bold][/accent] status [task.name]<{noun_name}>[/task.name] [muted]— View {labels.empty_plural[:-1] if labels.empty_plural.endswith('s') else labels.empty_plural} details (by name)[/muted]"
        )
    else:
...
def build_generic_recommendations(*, index_help: str, active_tasks: int) -> list[str]:
    ...
    noun = labels.empty_plural[:-1] if labels.empty_plural.endswith("s") else labels.empty_plural
    recs.append(
        f"[accent][bold]flow[/bold][/accent] status {index_help} [muted]— View {noun} details (by index)[/muted]"
    )
    noun_name = f"{labels.header.lower()}-name"
    recs.append(
        f"[accent][bold]flow[/bold][/accent] status [task.name]<{noun_name}>[/task.name] [muted]— View {labels.empty_plural[:-1] if labels.empty_plural.endswith('s') else labels.empty_plural} details (by name)[/muted]"
...
def build_status_recommendations(
    ...
    *,
    ...
    labels = _get_labels()
    noun_lower = labels.header.lower()
    noun_name = f"{noun_lower}-name"
    if has_pending:
        recs.append(
            f"[accent][bold]flow[/bold][/accent] status {index_example_single} [muted]— View {noun_lower} details (readiness)[/muted]"
        )
        recs.append(
    ...
    if has_active:
        ...
        recs.append(
            "[accent][bold]flow[/bold][/accent] status --watch [muted]— Watch updates[/muted]"
        )
