...
class SSHCommand(BaseCommand, TaskOperationCommand):
    ...
    def handle_error(self, error: Exception | str, exit_code: int = 1) -> None:
        ...
        if isinstance(error, SshAuthenticationError):
            ...
            # We'll catch the exit and handle the prompt before actually exiting
            try:
            ...
            key_path = str(error.key_path)
            chmod_successful = False

            # Show next steps before prompting
            next_steps = []
            if key_path:
                next_steps.append(
                    f"[accent]chmod 600 {key_path}[/accent] [muted]— Fix key permissions[/muted]"
                )
            next_steps.extend(
                [
            ...
            # Then prompt for permission to fix chmod automatically
            if key_path:
                try:
                    console.print("Auto fix found:")
                    console.print(f"  [accent]chmod 600 {key_path}[/accent]")
                    if click.confirm("Proceed?", default=True):
                        console.print(f"[dim]Running: chmod 600 {key_path}[/dim]")
                        try:
                            os.chmod(key_path, 0o600)
                            chmod_successful = True
                        except OSError as e:
            ...
            # If chmod was successful, automatically retry the SSH connection
            if chmod_successful:
                console.print("[green]✓ Updated key permissions[/green]")

                # Try to auto-retry if we have task information
            ...
            # Now exit after showing error and handling the prompt
...
Remote commands:
  flow ssh task -- nvidia-smi      # Run command on all nodes (multi-node)
  flow ssh task -- htop            # Monitor system resources
  flow ssh task --node 0 -- nvidia-smi  # Run on a specific node (0-based)
...
Tip: Use verbose help for container mode and advanced examples (flow ssh --verbose)."""
    ...
    def _resolve_endpoint(self, client, task: Task, node: int) -> Task:
        ...
        try:
            ...
            task.ssh_host = host
            task.ssh_port = self._get_ssh_port(task, {"ssh_port": port})
        ...
        with adapter:
            ...
            while True:
                ...
                if ssh_key_path and _S.is_ssh_ready(
                    ...
                    host=task.ssh_host,
                    port=self._get_ssh_port(task),
                    key_path=ssh_key_path,
                    prefix_args=pfx,
                ):
                    break
                if not ssh_key_path and _S.tcp_port_open(task.ssh_host, self._get_ssh_port(task)):
                    try:
                        # Banner check avoids false-positive TCP listeners
                        from flow.sdk.ssh import SshStack as _CoreS

                        if _CoreS.has_ssh_banner(task.ssh_host, self._get_ssh_port(task)):
                            break
                    except Exception:
                        pass
    ...
    def _get_ssh_port(self, task: Task, cached: dict | None = None, default: int = 22) -> int:
    ...
    def _try_direct_exec_from_cache(
        ...
        def get_host_and_port():
            ...
            ssh_port = self._get_ssh_port(task, cached)
        ...
        try:
            ...
            logger.debug("oht _try_direct_exec_from_cache exception")
    ...
    def get_command(self) -> click.Command:
        ...
        def ssh(
            ...
            Examples:
                ...
                flow ssh task --all-nodes -- nvidia-smi  # Run command on all nodes
