...
def run_setup_wizard(provider: str | None = None, environment: str = "production") -> bool:
    ...
    try:
        ...
        if (resolved_provider or "").strip().lower() == "mock":
            demo_enabled = str(_os.environ.get("FLOW_ENABLE_DEMO_ADAPTER", "")).strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
        ...
        if len(providers) == 1:
            ...
            try:
                ...
                selector = InteractiveSelector(
                    ...
                    breadcrumbs=["Mithril Flow Setup", "Provider"],
        ...
        if not adapter:
            ...
            try:
                ...
                if providers:
                    ...
                    if alt_adapter:
                        console.print(
                            f"[warning]Hint:[/warning] Falling back to provider '{alt}'. Run 'flow init --provider mithril' once the Mithril adapter is available."
                        )
                        adapter = alt_adapter
                    else:
                        return False
...
class InitCommand(BaseCommand):
    ...
    def get_command(self) -> click.Command:
        ...
        def init(
            ...
            \b
            Examples:
                flow init                              # Interactive setup wizard (production)
                flow init --environment staging       # Configure for staging environment
                flow init --dry-run                   # Preview configuration
                flow init --show                      # Show current configuration
                flow init --provider mithril --api-key xxx  # Direct setup

            Environment switching:
                flow init --environment staging       # All future commands use staging
                flow init --environment production    # All future commands use production
    ...
    def _print_verbose_help(self) -> None:
        ...
        # Header panel
        header = _Panel(
            f"[bold]{_flow_icon()} Mithril Flow — Detailed Guide[/bold]\n[muted]Init configures your environment; it never provisions resources.[/muted]",
            border_style=border,
            expand=False,
        )
        ...
        body1 = "\n".join(
            [
                ...
                f"  {bullet} Default project and region",
        ...
        body3 = "\n".join(
            [
                ...
                f"  {bullet} Configure once in YAML (provider, api_key, project, region, ssh_keys)",
            ]
        )
        console.print(
            _Panel(
        ...
        body5 = "\n".join(
            [
                f"  {bullet} Discover/sync keys: [accent]flow ssh-key list --sync[/accent]",
                f"  {bullet} Upload a local key: [accent]flow ssh-keys upload ~/.ssh/id_ed25519.pub[/accent]",
                f"  {bullet} Inspect a key:      [accent]flow ssh-keys info <sshkey_id>[/accent]",
    ...
    async def _init_async(
        ...
        if explicit_non_interactive:
            ...
            success = await self._configure_with_options(
                provider, environment, api_key, project, region, api_url, dry_run, output_path
            )
        else:
            # Interactive mode
            if assume_yes:
            ...
            if success:
                ...
                self.show_next_actions(
                    [
                        ...
                        "Submit a job now: [accent]flow run 'nvidia-smi'[/accent]",
    ...
    async def _configure_with_options(
        ...
        if api_key:
            # Skip validation during dry-run mode
            if not dry_run:
        ...
        if region:
            ...
            if not dry_run:
                ...
                if not vr.is_valid:
                    console.print(f"[error]Invalid region:[/error] {escape(str(vr.message))}")
                    return False
        ...
        try:
            ...
            # If required fields missing, fail fast in non-interactive mode (but allow dry-run)
            if not dry_run:
                required = [f.name for f in adapter.get_configuration_fields() if f.required]
                missing = [name for name in required if name not in config]
                if missing:
                    console.print(
                        f"[error]Missing required fields for non-interactive init:[/error] {', '.join(missing)}"
                    )
                    return False
            ...
            if environment == "staging":
                ...
                console.print(
                    f"[{success_color}]✓[/{success_color}] Environment set to production - all future flow commands will use production"
                )
            self.show_next_actions(
                [
                    ...
                    "Submit your first task: [accent]flow run <config.yaml>[/accent]",
