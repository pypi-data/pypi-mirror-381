...
class ConfigSources:
    ...
    def get_upload_config(self) -> dict[str, Any]:
        ...
        default_dest_env = self.env_vars.get("FLOW_UPLOAD_DEFAULT_DEST")
        if default_dest_env:
            cfg["default_dest"] = default_dest_env
        else:
            try:
                file_default = self.config_file.get("upload", {}).get("default_dest")
            except Exception:
                file_default = None
            cfg["default_dest"] = file_default or "~/{project}"

        # Sudo policy for absolute path preparation (non-interactive only)
        allow_sudo_env = self.env_vars.get("FLOW_UPLOAD_ALLOW_SUDO")
        if allow_sudo_env is not None:
            allow = str(allow_sudo_env).strip().lower() in {"1", "true", "yes", "on"}
        else:
            try:
                allow = bool(self.config_file.get("upload", {}).get("allow_sudo_absolute", False))
            except Exception:
                allow = False
        cfg["allow_sudo_absolute"] = allow
        return cfg
