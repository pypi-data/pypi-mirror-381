    ...
    def get_command(self) -> click.Group:
        ...
        def volumes(ctx):
            ...
            if ctx.invoked_subcommand is None:
                ...
                console.print("  flow run <config.yaml>  # References volume by name")
                console.print("  ")
        ...
        def volumes_describe(
            ...
            try:
                ...
                if bool(getattr(volume, "attachments_supported", False)):
                    ...
                    add_left_aligned_column(atable, "Task", width=24)
                    add_left_aligned_column(atable, "Task ID", width=14)
                ...
                if details and referenced_tasks:
                    ...
                    add_left_aligned_column(rtable, "Task", width=24)
                    add_left_aligned_column(rtable, "Task ID", width=14)
                ...
                tips: list[str] = [
                    "Attach to a task: [accent]flow mount <volume> <task>[/accent]",
        ...
        def volumes_list(ctx, details: bool, region: str | None, output_json: bool):
            ...
            try:
                ...
                if details or output_json:
                    with AnimatedEllipsisProgress(
                        console, "Fetching task associations", start_immediately=True
                    ):
                        instance_task_map = self._build_instance_task_map(flow_client)
    ...
    def _create_command(self) -> click.Command:
        ...
        def volumes_create(
            ...
            try:
                ...
                # Interactive prompts for missing parameters (unless JSON output or --yes)
                if not output_json and not yes:
                    # Prompt for storage type FIRST if not provided
                    if interface is None:
                        console.print("\n[bold]Storage Type Selection[/bold]")
                        console.print("Choose the type of storage you need:")
                        console.print(
                            "• [accent]block[/accent] - High-performance exclusive storage"
                        )
                        console.print(
                            "• [accent]file[/accent] - Shared storage that can be mounted to multiple tasks"
                        )
                    ...
                    # Prompt for region AFTER storage type, filtered by available regions
                    if region is None:
                        console.print("\n[bold]Region Selection[/bold]")
                        console.print(
                            f"Choose a region that supports [accent]{interface}[/accent] storage:"
                        )

                        available_regions = self._get_available_regions(flow_client, interface)
                        if not available_regions:
                            console.print(
                                f"[error]No regions available for {interface} storage[/error]"
                            )
                            return
                    ...
                    if size is None:
                        ...
                        size = click.prompt(
                            "\nEnter volume size in GB (max 15TB, contact support for larger volumes)",
                            type=int,
                            default=100,
                        )
                    ...
                    if name is None:
                        ...
                        console.print(
                            "Names help identify volumes and can be used in task configs."
                        )
                    ...
                    console.print(
                        f"Monthly cost: [accent]${monthly_cost:.2f}[/accent] (${size} GB × $0.08/GB)"
                    )
                ...
                if size is None:
                    raise click.ClickException("--size is required when not running interactively")
                if interface is None:
                    interface = "block"
                if region is None:
                ...
                try:
                    ...
                    # Invalidate volume cache after successful creation
                    try:
                ...
                if output_json:
                    ...
                    console.print(
                        f"[success]✓[/success] Volume created: [accent]{volume.volume_id}[/accent]"
                    )
                    if name:
                        console.print(f"Name: [accent]{name}[/accent]")
                    try:
                        if getattr(volume, "region", None):
                            console.print(f"Region: {volume.region}")
                    except Exception:
                        pass
