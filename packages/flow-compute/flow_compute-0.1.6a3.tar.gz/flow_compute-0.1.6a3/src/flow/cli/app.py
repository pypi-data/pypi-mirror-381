...
from flow.cli.utils.mode_config import Mode, get_current_mode, set_mode
...
def switch_mode(ctx: click.Context, param: click.Option | None, value: str | None) -> None:
    ...
    if value == "__show_current__":
        ...
        console.print(f"[bold]Current mode:[/bold] {mode_display}")
...
def main() -> int:
    ...
    if (
        ...
        try:
            ...
            console = theme_manager.create_console()
            console.print("[warning]⚠ Flow SDK is not configured[/warning]\n")
            ...
            try:
                ...
                if _hs.is_supported():
                    ...
                    console.print(f"Documentation: {docs_url}  [dim](or run 'flow docs')[/dim]")
            except Exception:
                pass
