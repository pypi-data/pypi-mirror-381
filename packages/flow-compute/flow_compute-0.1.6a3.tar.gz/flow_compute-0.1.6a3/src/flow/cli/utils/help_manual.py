...
from flow.cli.utils.mode_config import Mode, get_current_mode, get_mode_config, get_other_mode
...
def render_mode_indicator(mode: Mode) -> list[str]:
    ...
    lines.append(f"Current mode: [bold]{current_mode_cfg.display_name}[/bold]")
    lines.append(
        f"[muted]Switch to {other_mode_cfg.display_name} mode: {get_switch_command()}[/muted]"
    )
    lines.append("")
...
def render_mode_commands(mode: Mode, show_more: bool = False) -> list[str]:
    ...
    if show_more:
        ...
        if more_for_mode:
            ...
            lines.append("[muted]  Use flow --help --all to show the full command set.[/muted]")
            lines.append("")
