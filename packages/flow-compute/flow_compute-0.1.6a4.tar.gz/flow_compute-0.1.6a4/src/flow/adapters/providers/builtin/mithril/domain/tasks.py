...
class TaskService:
    ...
    def build_task(
        ...
        *,
        ...
        # Note: "terminated" maps to COMPLETED per MITHRIL_STATUS_MAPPINGS
        # Instance status will override this if available for accurate terminal state
        ...
        # Prefer instance status over bid status when available (more accurate)
        # Bid "Allocated" is coarse; instance "running" vs "initializing" is precise
        if provider_metadata.get("instance_status"):
            ...
            if debug:
                logger.debug(
                    f"[build_task] bid={task_id} overriding bid_status={status.value} with instance_status={instance_status_from_api} → {refined_status.value}"
                )
