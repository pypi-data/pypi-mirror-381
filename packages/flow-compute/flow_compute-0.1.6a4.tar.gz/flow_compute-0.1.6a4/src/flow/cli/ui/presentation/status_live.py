    ...
    *,
    ...
    def get_display():
        try:
            ...
            return Text(f"Error fetching tasks: {escape(str(e))}", style=_tm.get_color("error"))

        if not tasks:
            try:
                from flow.cli.ui.presentation.nomenclature import get_entity_labels as _labels

                return Text(f"No {_labels().empty_plural} found", style="dim")
            except Exception:
        ...
        if panel is None:
            ...
            return Text("Error: Could not render tasks", style=_tm.get_color("error"))
