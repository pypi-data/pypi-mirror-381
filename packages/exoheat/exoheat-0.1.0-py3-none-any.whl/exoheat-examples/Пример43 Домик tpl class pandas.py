...
#Пример выполнения задания на КМ-4, расчет тепловых потерь за счет теплопроводности, инфильтрации и удельные показатели
import pandas as pd
...
class Home:
    ...
    def __init__(self, r_walls, r_okna):
        ...
        def tpl_WL(r_mat,ориентир_потерь,F_c,ограждение='наружное'):
            return (loc,r_mat,ограждение,ориентир_потерь,F_c,H_зд)
    ...
    def get_data_Q(self):
        """Данный метод мне будет выводить теплоэнергетические показатели"""
        ...
        data_dict['Имя'] = id_name
        data_dict['Q, кВт'] = Q_val
        data_dict['q, %'] = [it / sum(Q_val) * 100 for it in Q_val]
        data_dict['F, м²'] = F_val
        data_dict['Q/F, кВт*м⁻²'] = [Q_val[i] / F_val[i] for i in range(len(Q_val))]
