...
def convert_to_json_schema(xml_schema):
    ...
    xml_to_json_schema = {
        ...
        # "gYear": {"type": "string", "format": "lixi-year"},
         "gYear":{
            "description": "Enforces that a value entered for the Year to be a XSD compliant",
            "label": "gYear",
            "path": "gYear",
            "pattern": "^(-?(?![0]{4,4})(([1-9][0-9]{3,}|0[0-9]{3})(Z|(\\+|-)((0[0-9]|1[0-3]):[0-5][0-9]|14:00))?))$",
            "type": "string"
            ...
            if "use" in attribute.attrib:
                if attribute.attrib["use"] != "optional":
                    if "required" not in parent_schema:
                        ...
                        if "@" + attribute.attrib["name"] not in parent_schema["required"]:
                            parent_schema["required"].append("@" + attribute.attrib["name"])
        ...
        if max_occurs == "1":
            ...
            if min_occurs != "0" and not isComplex:
                if "required" not in parent_schema:
                    ...
                    if element_name not in parent_schema["required"]:
                        parent_schema["required"].append(element_name)
                ...
                if (
                    ...
                    if (
                        ...
                        if temp_schema_properties:
                            ...
                            # temp_schema["properties"] = copy.deepcopy(temp_schema_properties)                            
                            # temp_schema["properties"]["oneOf"] = []
                            temp_schema["oneOf"] = []
                        ...
                        for sub_element in complex_type.findall(
                            ...
                            if "required" not in new_temp_dict:
                                new_temp_dict["required"] = []
                                new_temp_dict["required"].append(sub_element.attrib.get("name"))

                            new_temp_dict["properties"].update(copy.deepcopy(temp_schema_properties))  
                            ...
                            if temp_schema_properties:
                                # temp_schema["properties"]["oneOf"].append(new_temp_dict)
    ...
    if len(schemacustomisation) > 0:
        # The schemacustomisation element now wraps the original Customisations element.
        # We need to parse the embedded CustomisationJSON instead of the XML.
        customisation_json_text = root.xpath(
            "./xs:annotation/xs:appinfo/lx:CustomisationJSON/text()", namespaces=ns
        )
        if len(customisation_json_text) > 0:
            try:
                # The text is inside a CDATA, so we get it as a string and parse it as JSON
                json_data = json.loads(customisation_json_text[0])
                if "schemacustomisation" in json_data:
                    schema["schemacustomisation"] = json_data["schemacustomisation"]
            except json.JSONDecodeError:
                # Fallback or error handling if JSON is malformed
                schema["schemacustomisation"] = {"error": "Could not parse CustomisationJSON"}
        else:
            # Fallback if CustomisationJSON is not present for some reason
            schema["schemacustomisation"] = {"error": "CustomisationJSON not found"}
        ...
        if isAnnotated:
            ...
            if "references" in json_element:
                ...
                for target in json_element["references"]:
                    lx_target = ns_element(lx, "target")
    ...
    def create_complexType(parent, json_element, name=None):
        ...
        if "properties" in json_element:  ##type is object
            ...
            for key, item in json_element["properties"].items():
                if key[:1] != "@" and key != "$" and key != "oneOf":
            ...
            for key, item in json_element["properties"].items():
                ...
                if key == "oneOf":
                    ...
                    # for choice_item in json_element["properties"]["oneOf"]:
