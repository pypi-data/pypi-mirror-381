...
import os, io, glob, copy, xml.etree.ElementTree, xml.dom.minidom as dome, json
...
def CustomiseSingleSchema(
    ...
    # Create a schemacustomisation wrapper
    xmlSchemaCustomisation = etree.Element(
        "{lixi.org.au/schema/appinfo_instructions}schemacustomisation"
    )
    # Append a deep copy of the original instructions into the wrapper
    xmlSchemaCustomisation.append(copy.deepcopy(customisation_instruction_file))
    xmlSchemaAppinfo.insert(1, xmlSchemaCustomisation)

    # Create and insert JSON version of customisation instructions
    json_customisation = {}
    # Add namespace attributes
    for prefix, uri in customisation_instruction_file.nsmap.items():
        if prefix: # Handle prefixes like 'xsi'
            json_customisation[f"xmlns:{prefix}"] = uri

    # Add attributes from the root element
    root_attribs = customisation_instruction_file.attrib
    for key, value in root_attribs.items():
        if key != "{http://www.w3.org/2001/XMLSchema-instance}noNamespaceSchemaLocation":
            json_customisation[key] = value

    # Add LIXITransactionType if not present, from schema
    if "LIXITransactionType" not in json_customisation:
        try:
            schema_transaction_type = xmlParsedSchemaRoot.xpath(
            ...
            json_customisation["LIXITransactionType"] = schema_transaction_type.attrib["fixed"]
        except (IndexError, KeyError):
            pass  # Ignore if not found

    items = []
    for item in customisation_instruction_file.xpath("./CustomiseItem"):
        json_item = {}
        ...
        for child in item:
            if not isinstance(json_item.get(child.tag), list):
                json_item[child.tag] = []
            json_item[child.tag].append(child.text)
        
        items.append(json_item)

    # If a list has only one item, convert it to a single value, except for 'Element', 'Attribute', 'Enumeration'
    for item in items:
        for key, value in item.items():
            if isinstance(value, list) and len(value) == 1 and key not in ['Element', 'Attribute', 'Enumeration']:
                item[key] = value[0]
    json_customisation["CustomiseItems"] = items

    xmlCustomisationJSON = etree.Element("{lixi.org.au/schema/appinfo_elements}CustomisationJSON")
    xmlCustomisationJSON.text = etree.CDATA(json.dumps({"schemacustomisation": json_customisation}, indent=2))
    xmlSchemaAppinfo.insert(2, xmlCustomisationJSON)
...
def validate_instructions_set(instructions):
    ...
    with io.open(
        ...
        importlib_resources.files(__name__).joinpath("customization.xsd"),
        "r",
        encoding="utf-8",
    ) as xsd_file_data:
