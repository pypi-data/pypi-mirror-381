...
class OpenAIResponse:
    ...
    def send_response(self, request: ResponseRequest) -> ResponseResult:
        ...
        try:
            response = httpx.post(OPENAI_API_URL, headers=self.headers, json=payload)
    ...
    def send_streaming_response(self, request: ResponseRequest) -> Generator[Dict[str, Any], None, None]:
        ...
        with httpx.stream("POST", OPENAI_API_URL, headers=self.headers, json=payload) as response:
            response.raise_for_status()
