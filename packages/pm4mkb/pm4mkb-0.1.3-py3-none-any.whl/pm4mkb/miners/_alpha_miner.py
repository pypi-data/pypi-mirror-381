...
from pm4mkb.visual._graph import Graph, create_petri_net
from pm4mkb.visual._types import NodeType
...
class AlphaMiner(AbstractMiner):
    ...
    def _calculate(causal_pairs, independence_matrix, act2label, label2act, n_jobs):
        ...
        while True:
            ...
            new_pairs, indexes_of_parents_to_remove = AlphaMiner._calc_new_pairs(
                causal_pairs_encoded, prev_gen, independence_matrix, n_jobs
            )
            if len(new_pairs) != 0:
        ...
        for gen in generations:
            all_pairs += [
                ({label2act[act] for act in pair[0]}, {label2act[act] for act in pair[1]}) for pair in gen
            ]
        return all_pairs

    @staticmethod
    def _calc_new_pairs(
        causal_pairs_encoded, list_of_pairs, independence_matrix, n_jobs, iterations_per_batch=10000
    ):
        """
        ...
        if total_iterations < iterations_per_batch or n_jobs == 1:
            new_pairs, indexes_of_parents_to_remove = AlphaMiner.make_new_pairs(
                causal_pairs_encoded, list_of_pairs, 0, independence_matrix
            )
        else:
            ...
            result_objects = [
                pool.apply_async(AlphaMiner.make_new_pairs, args=arg)
                for arg in AlphaMiner._generate_args(
                    causal_pairs_encoded, list_of_pairs, columns_per_batch, independence_matrix
                )
            ]
            new_pairs = set()
