...
class entry_point(object):
    ...
    def set_covariates(self, sample_cov, covariates_names=None):
        ...
        ), "the data must be specified with set_data before specifying covariates"
        ...
        if isinstance(sample_cov, list) and all(
            ...
            assert all(
                [c in self.data_opts["samples_metadata"][0].columns for c in sample_cov]
            ), "sample_cov provided as string but not found in samples_metadata"
            sample_cov_list = []
        ...
        assert (
            len(sample_cov) == self.dimensionalities["G"]
        ), "sample_cov needs to be a list of same length as data (same number of groups)"
        ...
        if covariates_names is None:
            ...
            ), "covariates_names must be a string or a list"
            assert (
                len(covariates_names) == self.dimensionalities["C"]
            ), "covariates_names must be of length equivalent to the number of covariates"
    ...
    def set_data_df(self, data, likelihoods=None):
        ...
        data["feature"] = data["feature"].astype(str) + data["view"].astype(
            str
        ...
        self.data_opts["samples_groups"] = (
            ...
            .drop_duplicates()
            .set_index("sample")
            .loc[np.concatenate(self.data_opts["samples_names"])]
            .group.tolist()
        )
    ...
    def set_data_from_anndata(
        ...
        if groups_label is None:
            ...
            self.data_opts["samples_names"] = (
                ...
                .rename(columns={adata.obs.index.name: "index"})
    ...
    def set_data_from_loom(self, loom, groups_label=None, layer=None, cell_id="CellID"):
        ...
        n_grouped = (
            [loom.shape[1]]
            if n_groups == 1
            else pd.DataFrame({"label": loom.ca[groups_label]})
            .groupby("label")
            .size()
            .values
        )
        ...
        self.data_opts["views_names"] = ["rna"]
        self.data_opts["features_names"] = (
            [loom.ra.Accession] if "Accession" in loom.ra.keys() else [loom.ra.Gene]
        )
        ...
        if groups_label is None:
            ...
            # List of names of groups, i.e. [group1, group2, ...]
            self.data_opts["groups_names"] = (
                loom_metadata.groupby(groups_label)[groups_label]
                .apply(list)
                .index.values
            )
            # Nested list of names of samples, one inner list per group, i.e. [[group1_sample1, group1_sample2, ...], ...]
        ...
        self.intercepts[0] = [
            np.nanmean(x, axis=0)
            for x in [
                data[0][np.where(np.array(self.data_opts["samples_groups"]) == g)[0], :]
                for g in self.data_opts["groups_names"]
            ]
        ]
    ...
    def set_train_options(
        ...
        if freqELBO is None or freqELBO == 0:
            freqELBO = iter + 1
        ...
        if schedule is None:
            ...
            if self.model_opts["ard_factors"]:
                assert "AlphaZ" in schedule
            if self.model_opts["ard_weights"]:
                assert "AlphaW" in schedule
            if self.model_opts["spikeslab_factors"]:
                assert "ThetaZ" in schedule
            if self.model_opts["spikeslab_weights"]:
                assert "ThetaW" in schedule
    ...
    def set_smooth_options(
        ...
        if self.smooth_opts["warping"] is True:
            assert (
                self.dimensionalities["G"] > 1
            ), "The warping functionality is only relevant when having multi-group data"
            assert (
        ...
        # Sparse GPs
        if sparseGP is True:
            assert not self.smooth_opts[
                "warping"
            ], "The warping functionality cannot be used in conjunction with the sparseGP option."
            assert np.all(
                [l == "gaussian" for l in self.model_opts["likelihoods"]]
            ), "Non-gaussian likelihoods cannot be used in conjunction with the sparseGP option."
            ...
            idx_inducing = set_inducing_points(
                self.data,
                self.sample_cov,
                self.data_opts["samples_groups"],
                self.dimensionalities,
    ...
    def run(self):
        ...
        if "schedule" in self.train_opts:
            assert set(self.train_opts["schedule"]) == set(
                list(self.model.getNodes().keys())
            ), "Some nodes defined in the training schedule are not present in the model, or viceversa"
        else:
    ...
    def predict_factor(self, new_covariates=None, uncertainty=True, groups="all"):
        ...
        assert hasattr(self, "smooth_opts"), (
            "Using predict_factors requires the use of GP_factors,"
        ...
        all = np.hstack(
            [
                np.repeat(groups, all_covariates.shape[0])[:, None],
                np.tile(all_covariates.transpose(), G).transpose(),
            ]
        )
        new = np.hstack(
            [
                np.repeat(groups, new_covariates.shape[0])[:, None],
                np.tile(new_covariates.transpose(), G).transpose(),
            ]
        )
        oldix = np.concatenate(
            [
                np.where(np.all(np.equal(old[j, :], all), axis=1))[0]
                for j in range(old.shape[0])
            ]
        )
        newidx = np.concatenate(
            [
                np.where(np.all(np.equal(new[j, :], all), axis=1))[0]
        ...
        if uncertainty:
            assert np.all(
                Z_new_var >= 0
            ), "Something went wrong in the prediction: variances of the predictive distribution are negative"
            self.Zpredictions = {
                "mean": Z_new_mean,
                "variance": Z_new_var,
                "values": new_covariates,
                "groups": groups,
            }
        else:
            self.Zpredictions = {
    ...
    def save(
        self, outfile=None, save_data=True, save_parameters=False, expectations=None
    ):
        ...
        # Save predictions
        if self.Zcompleted:
            tmp.saveZpredictions(
                self.Zpredictions["mean"],
                self.Zpredictions["variance"],
                self.Zpredictions["values"],
                self.Zpredictions["groups"],
            )
