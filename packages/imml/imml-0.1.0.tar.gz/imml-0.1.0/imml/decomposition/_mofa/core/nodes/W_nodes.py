...
class W_Node(UnivariateGaussian_Unobserved_Variational_Node):
    ...
    def calculateELBO(self):
        ...
        if "MuW" in self.markov_blanket:
            PE, PE2 = (
                self.markov_blanket["MuW"].getExpectations()["E"],
...
class SW_Node(BernoulliGaussian_Unobserved_Variational_Node):
    ...
    def _updateParameters(
        ...
        self.Q.setParameters(
            mean_B0=np.zeros((self.dim[0], self.dim[1])),
            var_B0=Qvar_S0,
            mean_B1=Qmean_S1,
            var_B1=Qvar_S1,
            theta=Qtheta,
    ...
    def calculateELBO(self):
        ...
        lb_ps = S * theta["lnE"] + (1.0 - S) * theta["lnEInv"]
        lb_qs = S * np.log(S) + (1.0 - S) * np.log(1.0 - S)

        # Replace NAs (due to theta=1) with zeros
        lb_ps[np.isnan(lb_ps)] = 0.0
        lb_qs[np.isnan(lb_qs)] = 0.0
