...
class ZgU_node(UnivariateGaussian_Unobserved_Variational_Node):
    ...
    def _updateParameters(self, U, Sigma, GPparam, Qmean, Qvar, Y, W, tau, mask):
        ...
        for k in range(K):
            unstructured = (
                Sigma["cov"][k] == np.eye(N)
            ).all()  # TODO: Are there better ways to choose between sparse and non-sparse inference depending on factor smoothness?
            if unstructured:  # updates according to q(z) without sparse inference
                ...
                for n in range(N):
                    exp_var = SigmaZZ[n, n] - gpu_utils.dot(
                        gpu_utils.dot(SigmaZZ[n, self.idx_inducing], p_cov_inv),
