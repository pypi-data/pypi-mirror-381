...
class Gamma(Distribution):
    ...
    def density(self, x):
        ...
        return np.prod(
            (1 / special.gamma(self.params["a"]))
            * self.params["b"] ** self.params["a"]
            * x ** (self.params["a"] - 1)
            * np.exp(-self.params["b"] * x)
        )

    def loglik(self, x):
        assert x.shape == self.dim, "Problem with the dimensionalities"
        return np.sum(
            -np.log(special.gamma(self.params["a"]))
            + self.params["a"] * np.log(self.params["b"])
            + (self.params["a"] - 1) * np.log(x)
            - self.params["b"] * x
        )

    def sample(self, n=1):
        k = self.params["a"]
        theta = 1.0 / self.params["b"]  # using shape/scale parametrisation
