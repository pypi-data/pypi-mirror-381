...
class saveModel:
    def __init__(
        ...
        assert (
            len(samples_groups) == data[0].shape[0]
        ...
        if not sample_cov is None:
            assert (
                sample_cov.shape[0] == data[0].shape[0]
    ...
    def saveNames(self):
        ...
        for g in range(len(self.groups_names)):
            samples_grp.create_dataset(
                self.groups_names[g], data=np.array(self.samples_names[g], dtype=dt)
            )
        ...
        for m in range(len(self.data)):
            features_grp.create_dataset(
                self.views_names[m], data=np.array(self.features_names[m], dtype=dt)
            )

        # Save covariate names
        if not self.covariates_names is None:
            covariates_grp = self.hdf5.create_group("covariates")
            covariates_grp.create_dataset(
                "covariates", data=np.array(self.covariates_names, dtype=dt)
            )
    ...
    def saveMetaData(self):
        ...
        if self.samples_metadata:
            ...
            for g in range(len(self.groups_names)):
                ...
                if len(set(cols)) != len(cols):
                    warn(
                        "There are duplicated columns in samples metadata, some will be renamed"
                    )
                ...
                for col in cols:
                    ...
                    if col in cols_cat:
                        orig_type = self.samples_metadata[g][
                            col
                        ].cat.categories.values.dtype
                        self.samples_metadata[g][col] = self.samples_metadata[g][
                            col
                        ].astype(orig_type)
                    ...
                    if ctype == "object":
                        try:
                            ...
                            group_meta.create_dataset(
                                col,
                                data=np.array(
                                    self.samples_metadata[g][col], dtype="|S"
                                ),
                            )
                        ...
                        group_meta.create_dataset(
                            col,
                            data=np.array(
                                self.samples_metadata[g][col], dtype=ctype.type
                            ),
                        )
        ...
        if self.features_metadata:
            ...
            for m in range(len(self.views_names)):
                ...
                if len(set(cols)) != len(cols):
                    warn(
                        "There are duplicated columns in features metadata, some will be renamed"
                    )
                    cols = _make_unique(cols)
                ...
                for col in cols:
                    # Convert categorical columns
                    if col in cols_cat:
                        orig_type = self.features_metadata[m][
                            col
                        ].cat.categories.values.dtype
                        self.features_metadata[m][col] = self.features_metadata[m][
                            col
                        ].astype(orig_type)
    ...
    def saveData(self):
        ...
        for m in range(len(self.data)):
            ...
            for g in range(len(self.groups_names)):
                ...
                intercept_subgrp.create_dataset(
                    self.groups_names[g], data=self.intercepts[m][g]
                )

        # Save sample covariates for GP prior
        if not self.sample_cov is None:
            cov_samples_grp = self.hdf5.create_group("cov_samples")
            cov_samples_transformed_grp = self.hdf5.create_group(
                "cov_samples_transformed"
            )
            if "Sigma" in self.model.getNodes():
                sample_cov_transformed = self.model.getNodes()[
                    "Sigma"
            ...
            for g in range(len(self.groups_names)):
                ...
                # Create hdf5 data set for data
                cov_samples_grp.create_dataset(
                    self.groups_names[g],
                    data=tmp,
                    compression="gzip",
                    compression_opts=self.compression_level,
                ...
                if not sample_cov_transformed is None:
                    ...
                    cov_samples_transformed_grp.create_dataset(
                        self.groups_names[g],
                        data=tmp_transformed,
                        compression="gzip",
                        compression_opts=self.compression_level,
                    )
    ...
    def saveImputedData(self, mean, variance):
        ...
        for m in range(len(mean)):
            ...
            for g in range(len(self.groups_names)):
                ...
                group_subgrp.create_dataset(
                    "mean",
                    data=mean[m][samples_idx, :],
                    compression="gzip",
                    compression_opts=self.compression_level,
                )
    ...
    def saveZpredictions(self, mean, variance, values, groups):
        ...
        for g in groups:
            ...
            group_subgrp.create_dataset(
                "mean",
                data=mean[sampleidx, :][:, self.order_factors],
                compression="gzip",
                compression_opts=self.compression_level,
            )
    ...
    def saveParameters(self, nodes="all"):
        ...
        if type(nodes) is str:
            ...
            assert set(nodes).issubset(
                ["Z", "W", "Tau", "AlphaW", "AlphaZ", "ThetaZ", "ThetaW", "Sigma", "U"]
            ), "Unrecognised nodes"
        nodes_dic = {x: nodes_dic[x] for x in nodes if x in nodes_dic}
    ...
    def saveModelOptions(self):
        ...
        opts = dict(
            (k, np.asarray(self.model_opts[k]).astype("S")) for k in options_to_save
        )
    ...
    def saveSmoothOptions(self, smooth_opts):
        ...
        opts = dict(
            (k, np.asarray(smooth_opts[k]).astype("S")) for k in options_to_save
        )
    ...
    def saveTrainingStats(self):
        ...
        if "length_scales" in stats.keys():
            stats_grp.create_dataset(
                "length_scales", data=stats["length_scales"][self.order_factors]
            )
