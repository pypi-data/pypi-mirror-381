...
class Distribution(object):
    ...
    def to_float32(self):
        ...
        for i in self.params.keys():
            self.params[i] = self.params[i].astype(np.float32)
        for i in self.expectations.keys():
            self.expectations[i] = self.expectations[i].astype(np.float32)
    ...
    def removeDimensions(self, axis, idx):
        ...
        for k in self.params.keys():
            self.params[k] = np.delete(self.params[k], idx, axis)
        for k in self.expectations.keys():
            self.expectations[k] = np.delete(self.expectations[k], idx, axis)
