...
class Unobserved_Variational_Node(Variational_Node):
    ...
    def getParameters(self, dist="Q"):
        ...
        if dist == "Q":
            params = self.Q.getParameters()
        elif dist == "P":
            params = self.P.getParameters()
        return params
    ...
    def removeFactors(self, idx, axis=None):
        ...
        if hasattr(self, "covariates"):
            self.covariates = self.covariates[np.arange(len(self.covariates)) != idx]
        if axis is not None:
