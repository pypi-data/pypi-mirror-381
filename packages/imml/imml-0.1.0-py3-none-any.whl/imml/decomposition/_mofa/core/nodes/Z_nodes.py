...
class Z_Node(UnivariateGaussian_Unobserved_Variational_Node):
    ...
    def calculateELBO(self):
        ...
        if "MuZ" in self.markov_blanket:
            PE, PE2 = (
                self.markov_blanket["MuZ"].getExpectations()["E"],
        ...
        if "AlphaZ" in self.markov_blanket:
            ...
            Alpha = dict()
            Alpha["E"] = 1.0 / self.P.params["var"]
            Alpha["lnE"] = np.log(1.0 / self.P.params["var"])
        ...
        lb_p = tmp1 + tmp2
        lb_q = -(np.log(Qvar).sum() + self.dim[0] * self.dim[1]) / 2.0
...
class SZ_Node(BernoulliGaussian_Unobserved_Variational_Node):
    ...
    def updateParameters(self, ix=None, ro=1.0):
        ...
        if "AlphaZ" in self.markov_blanket:
            ...
            Alpha = 1.0 / self.P.params["var_B1"]
            if ix is not None:
                Alpha = Alpha[ix, :]
        ...
        for k in range(K):
            ...
            for m in range(M):
                ...
                term4_tmp2_tmp = (
                    tau_gpu
                    * gpu_utils.dot(
                        gpu_utils.array(SZ[:, np.arange(K) != k]),
                        (Wk_gpu * gpu_utils.array(W[m]["E"][:, np.arange(K) != k].T)),
                    )
                ).sum(axis=1)
            ...
            Qtheta[:, k] = 1.0 / (1.0 + np.exp(-(term1 + term2 - term3 + term4)))
    ...
    def calculateELBO(self):
        ...
        if "AlphaZ" in self.markov_blanket:
            ...
            alpha = dict()
            alpha["E"] = 1.0 / self.P.params["var_B1"]
            alpha["lnE"] = np.log(1.0 / self.P.params["var_B1"])
        ...
        lb_pt = T * theta["lnE"] + (1.0 - T) * theta["lnEInv"]
        lb_qt = T * np.log(T) + (1.0 - T) * np.log(1.0 - T)

        # Replace NAs (due to theta=1) with zeros
        lb_pt[np.isnan(lb_pt)] = 0.0
        lb_qt[np.isnan(lb_qt)] = 0.0
