...
# -------------------------------
# Make imports compatible with both
# 1) direct script run (python main.py)
# 2) installed package (python -m hardware_ml_benchmark.main)
# -------------------------------
if __name__ == "__main__" and __package__ is None:
    # when use main.py，take 'parent dir' to 'sys.path' (for PyPI)
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

# Try absolute imports first (installed package)
try:
    from hardware_ml_benchmark.core.utils import setup_logging, check_dependencies, print_dependency_status
    ...
    from hardware_ml_benchmark.benchmark.monitoring import ResourceMonitor, StatisticsCalculator
    from hardware_ml_benchmark.output.output import ResultExporter, Visualizer
except ImportError:
    # Fallback for direct script run
    from core.utils import setup_logging, check_dependencies, print_dependency_status
    from core.cli import CommandLineInterface
    from data.datasets import DatasetLoader
    from models.models import ModelLoader
    from benchmark.rendering import RenderingEngine
    from benchmark.benchmarks import BenchmarkRunner
    from benchmark.monitoring import ResourceMonitor, StatisticsCalculator
    from output.output import ResultExporter, Visualizer
