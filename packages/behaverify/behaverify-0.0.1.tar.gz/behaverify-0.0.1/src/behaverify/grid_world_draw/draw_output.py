...
def create_grid_from_states(states, x_size, y_size):
    ...
    grid[states['x_g']][states['y_g']] = 'T'
    grid[states['x_d']][states['y_d']] = 'D'
    return grid
...
def draw_grid_line(list_of_states, output_name, x_size, y_size):
    ...
    drone_line = [(list_of_states[0]['x_d'] * tile_side + int(tile_side/2), list_of_states[0]['y_d'] * tile_side + int(tile_side/2))]
    target_count = 0
    target = (-1, -1)
    for index in range(1, len(list_of_states)):
        current_states = list_of_states[index]
        if 'x_d' in current_states or 'y_d' in current_states:
            drone_line.append(
                ((current_states['x_d'] * tile_side + int(tile_side/2)) if 'x_d' in current_states else drone_line[-1][0],
                 (current_states['y_d'] * tile_side + int(tile_side/2)) if 'y_d' in current_states else drone_line[-1][1])
            )
        if current_states['x_g'] != target[0] or current_states['y_g'] != target[1]:
            target_count = target_count + 1
            target = (current_states['x_g'], current_states['y_g'])
