...
def create_nodes(nodes, root_node_name, node_name_to_number, tick_condition):
    ...
    def create_composite_with_memory(node):
        success_on_all = 'TRUE' if 'success_on_all' == node['policy'] else 'FALSE'
        parallel = 'parallel' == node['type']
        node_module_type = format_node_type(node, False).replace('success_on_all_', '').replace('success_on_one_', '')
        return (
            ''.join(
                [
                    (
