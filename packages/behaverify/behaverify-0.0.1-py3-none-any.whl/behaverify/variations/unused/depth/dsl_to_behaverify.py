...
def resolve_statements(statements, nodes):
    ...
    def handle_return_statement(statement, nodes, misc_args):
        ...
        node['internal_status_module_code'] = (
            None if (len(statement.case_results) == 0
                     or len(statuses) == 1) else
            (
                [
                    ((format_code(case_result.condition, create_misc_args(node_name, False, False, None, None)), case_result.status))
                    for case_result in statement.case_results
                ]
                +
                [
                    ('TRUE', statement.default_result.status)
                ]
            )
        )
        return
    ...
    def handle_condition(condition, nodes, misc_args):
        ...
        node['internal_status_module_code'] = (
            [
                (format_code(condition, create_misc_args(node_name, False, False, None, None)), 'success'),
                ('TRUE', 'failure')
            ]
        )
        return
