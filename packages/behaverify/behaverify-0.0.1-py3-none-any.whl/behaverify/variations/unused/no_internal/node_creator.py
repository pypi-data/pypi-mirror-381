    ...
    return (
        ...
        + (
            (
                ...
                + tab_indent(3) + 'case' + os.linesep
                + tab_indent(4) + '!(active) : invalid;' + os.linesep
                + tab_indent(4) + 'TRUE : {' + statuses.replace('_', ', ') + '};' + os.linesep
                + tab_indent(3) + 'esac;' + os.linesep
            )
            if '_' in statuses else
            (
                tab_indent(1) + 'DEFINE' + os.linesep
                + tab_indent(2) + 'status := active ? ' + statuses + ' : invalid;' + os.linesep
            )
        )
    )
# -----------------------------------------------------------------
...
def create_composite_selector_with_partial_memory(number_of_children):
    ...
    for child in range(number_of_children):
        ...
        return_string += ("\t\t\t\t(" + str(child) + " >= resume_point) & !(child_" + str(child) + ".status = failure) : child_" + str(child) + ".status;" + os.linesep
...
def create_composite_parallel_success_on_all_with_partial_memory(number_of_children):
    ...
    for child in range(number_of_children):
        return_string += "\t\t\t\t(child_" + str(child) + ".status = failure) & !(skip_child[" + str(child) + "] = -2) : failure;" + os.linesep
    ...
    for child in range(number_of_children):
        ...
        return_string += ("\t\t\t\t!(child_" + str(child) + ".status = success) & !(skip_child[" + str(child) + "] = -2) : child_" + str(child) + ".status;" + os.linesep
