...
PARALLEL_STATUS_STRING = 'parallel_status__'
...
def create_parallel_status(nodes):
    define_string = ''
    var_string = ''.join(
        [
            (tab_indent(2) + PARALLEL_STATUS_STRING + node['name'] + ' : {success, running, failure};' + os.linesep)
            for node in nodes.values() if 'parallel' in node['type']
        ]
    )
    init_string = ''.join(
        [
            (tab_indent(2) + 'init(' + PARALLEL_STATUS_STRING + node['name'] + ') := '
             + ('success' if 'success_on_all' in node['type'] else 'running')
             + ';' + os.linesep)
            for node in nodes.values() if 'parallel' in node['type']
        ]
    )
...
def write_smv(nodes, variables, tick_condition, specifications, output_file = None, do_not_trim = False):
    ...
    (new_define, new_var, new_init, new_next) = create_parallel_status(nodes)
    define_string += new_define
    var_string += new_var
    init_string += new_init
    next_string += new_next
