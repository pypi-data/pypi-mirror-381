...
from torque.views import explore, search, search_data
...
def test_explore_with_search_template_and_num_per_page(wiki_test_setup):
    ...
    assert response_data["num_results"] == 1 # just the count of results returned, not the total in this context
    assert "mwiki_text" in response_data
