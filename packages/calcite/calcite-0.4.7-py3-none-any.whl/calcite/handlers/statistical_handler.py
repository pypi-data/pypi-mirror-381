    ...
    def perform_t_test(self):
        ...
        if dialog.exec():
            ...
            try:
                ...
                results_summary = []
                g1_display_name_base = f"{group_col}={g1_cond['x']}" + (f", {hue_col}={g1_cond['hue']}" if hue_col and g1_cond.get('hue') else "")
                g2_display_name_base = f"{group_col}={g2_cond['x']}" + (f", {hue_col}={g2_cond['hue']}" if hue_col and g2_cond.get('hue') else "")
                ...
                if facet_col and facet_col in df.columns:
                    ...
                    for category in facet_categories:
                        ...
                        summary = (
                            ...
                            f"- Group 1: {g1_display_name_base} (n={len(group1_values)}, Mean: {group1_values.mean():.3f})\n"
                            f"- Group 2: {g2_display_name_base} (n={len(group2_values)}, Mean: {group2_values.mean():.3f})\n\n"
                    ...
                    if p_value < 0.05:
                        ...
                        summary += "Conclusion: The difference is not statistically significant (p >= 0.05)."
                    results_summary.append(summary)
                ...
                if facet_col and facet_col in df.columns:
                    for category in df[facet_col].dropna().unique():
                        ...
                        summary = (
                            f"--- Facet: {facet_col} = {category} ---\n"
                            f"Comparing '{value_col}' between:\n"
                            f"- Group 1: {g1_display_name_base} (n={len(group1_values)})\n"
                            f"- Group 2: {g2_display_name_base} (n={len(group2_values)})\n\n"
                            f"U-statistic: {u_stat:.4f}, p-value: {p_value:.4f}\n"
                        )
                        results_summary.append(summary)
