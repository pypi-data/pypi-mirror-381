...
class HWModuleExternOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, module_type, parameters, *, per_port_attrs=None, port_locs=None, verilogName=None, files=None, loc=None, ip=None):
    operands = []
    attributes = {}
    regions = None
    _ods_context = _ods_get_default_loc_context(loc)
    ...
    if files is not None: attributes["files"] = (files if (
  ...
  @builtins.property
  def files(self) -> _Optional[_ods_ir.ArrayAttr]:
    if "files" not in self.operation.attributes:
      return None
    return self.operation.attributes["files"]

  @files.setter
  def files(self, value: _Optional[_ods_ir.ArrayAttr]):
    if value is not None:
      self.operation.attributes["files"] = value
    elif "files" in self.operation.attributes:
      del self.operation.attributes["files"]
...
def module_extern(sym_name, module_type, parameters, *, per_port_attrs=None, port_locs=None, verilog_name=None, files=None, loc=None, ip=None) -> HWModuleExternOp:
  return HWModuleExternOp(sym_name=sym_name, module_type=module_type, parameters=parameters, per_port_attrs=per_port_attrs, port_locs=port_locs, verilogName=verilog_name, files=files, loc=loc, ip=ip)
