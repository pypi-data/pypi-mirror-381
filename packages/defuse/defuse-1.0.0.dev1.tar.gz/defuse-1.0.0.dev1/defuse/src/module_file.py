...
class ProjectAnalyzer:
    def __init__(self):
        ...
        self.assigned_vars = defaultdict(dict)  # file → {var_name: lineno}
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                for alias in node.names:
                    ...
                    self.assigned_vars[filepath][name] = getattr(node, 'lineno', '?')
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        self.assigned_vars[filepath][target.id] = getattr(node, 'lineno', '?')
                ...
                self.used_vars[filepath].add(node.id)
        ...
        if isinstance(obj, ast.Name):
            ...
            if class_name:
                ...
                if resolved_class:
                    ...
                    self.calls[key].add((filepath, lineno))
                    self.calls[class_name].add((filepath, lineno))
                    return
    ...
    def get_unused_variables(self):
        ...
        for file in self.assigned_vars:
            assigned = self.assigned_vars[file]
            used = self.used_vars.get(file, set())
            for var in assigned.keys():
                if var in self.func_defs or var in self.class_defs:
                    continue
                if var not in used:
                    unused[file].append((var, file, assigned[var]))
        return unused
    ...
    def report_unused(self):
        ...
        if all([not unused_classes, not unused_methods, not unused_functions, not unused_variables]):
        ...
        if unused_variables:
            print("\n🔸 Unused Variables:")
            for filename, values in unused_variables.items():
                print(f"  - {filename}")
                for value in values:
                    print(f"    - {value[0]} at line {value[2]}")