...
class DataProduct:
    ...
    def _fetch_zonal_statistics(
        ...
        """Internal method to fetch existing zonal statistics (GET only, no job submission).
        ...
        Returns:
            Optional[FeatureCollection]: Existing zonal statistics, or None if not found.
        """
    ...
    def get_zonal_statistics(
        ...
        Args:
            zonal_layer_id (str): ID of zonal layer.
            wait (bool): If True and statistics don't exist, submit job and poll for results.
                If False, submit job but return immediately. Defaults to True.
            timeout (int): Maximum seconds to wait for results (only used if wait=True).
                Defaults to 300 seconds (5 minutes).
            poll_interval (int): Seconds between polling attempts (only used if wait=True).
        ...
        # Poll for results
        logger.info(
            f"Waiting for zonal statistics (timeout: {timeout}s, checking every {poll_interval}s)..."
        )
        elapsed = 0
