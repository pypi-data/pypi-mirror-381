...
class VCGenerator:
    ...
    def __init__(self, brain_server_url: str):
        """
        Initialize VC Generator.
        
        Args:
            brain_server_url: URL of the Brain Server
        """
        self.brain_server_url = brain_server_url.rstrip('/')
        self.enabled = False
    ...
    def generate_execution_vc(self, execution_context: Any, input_data: Any,
                            output_data: Any, status: str, error_message: Optional[str] = None,
                            duration_ms: int = 0) -> Optional[ExecutionVC]:
        """
        ...
        try:
            logger.debug(f"Generating VC for execution: {execution_context.execution_id}")
            ...
            vc_data = {
                "execution_context": {
                    ...
                    "timestamp": execution_context.timestamp.isoformat() + "Z" if execution_context.timestamp.tzinfo is None else execution_context.timestamp.isoformat()
                },
            ...
            # Send VC generation request to Brain Server
            response = requests.post(
                f"{self.brain_server_url}/api/v1/execution/vc",
                json=vc_data,
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to generate execution VC: {response.status_code} - {response.text}")
                return None
    ...
    def verify_vc(self, vc_document: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.brain_server_url}/api/v1/did/verify",
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to verify VC: {response.status_code} - {response.text}")
                return None
    ...
    def get_workflow_vc_chain(self, workflow_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = requests.get(
                f"{self.brain_server_url}/api/v1/did/workflow/{workflow_id}/vc-chain",
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to get workflow VC chain: {response.status_code} - {response.text}")
                return None
    ...
    def create_workflow_vc(self, workflow_id: str, session_id: str, 
                          execution_vc_ids: List[str]) -> Optional[WorkflowVC]:
        """
        ...
        try:
            ...
            response = requests.post(
                f"{self.brain_server_url}/api/v1/did/workflow/{workflow_id}/vc",
                json=workflow_data,
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to create workflow VC: {response.status_code} - {response.text}")
                return None
    ...
    def export_vcs(self, filters: Optional[Dict[str, Any]] = None) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            response = requests.get(
                f"{self.brain_server_url}/api/v1/did/export/vcs",
                params=params,
                timeout=30
            )
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to export VCs: {response.status_code} - {response.text}")
                return None
    ...
    def _parse_execution_vc(self, vc_data: Dict[str, Any]) -> ExecutionVC:
        ...
        return ExecutionVC(
            ...
            created_at=datetime.fromisoformat(vc_data["created_at"].replace('Z', '+00:00'))
        )
    ...
    def _parse_workflow_vc(self, vc_data: Dict[str, Any]) -> WorkflowVC:
        ...
        if vc_data.get("end_time"):
            end_time = datetime.fromisoformat(vc_data["end_time"].replace('Z', '+00:00'))
        ...
        return WorkflowVC(
            ...
            status=vc_data["status"],
            start_time=datetime.fromisoformat(vc_data["start_time"].replace('Z', '+00:00')),
            end_time=end_time,
...
class VCContext:
    ...
    def __init__(self, vc_generator: VCGenerator, execution_context: Any, 
                 function_name: str):
        """
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            ...
            if vc:
                logger.debug(f"Generated VC {vc.vc_id} for execution {self.execution_context.execution_id}")
            else:
                logger.warning(f"Failed to generate VC for execution {self.execution_context.execution_id}")
