    ...
    async def _start_stdio_server(self, config: MCPServerConfig) -> bool:
        ...
        try:
            ...
            if success:
                ...
                if self.dev_mode:
                    logger.info(
                        f"Stdio MCP server {config.alias} started successfully"
                    )
                return True
            else:
                if self.dev_mode:
                    logger.error(
                        f"Stdio MCP server {config.alias} failed to start"
                    )
                return False
    ...
    async def _start_http_server(self, config: MCPServerConfig) -> bool:
        ...
        try:
            ...
            if process.poll() is None:
                ...
                if self.dev_mode:
                    logger.info(
                        f"HTTP MCP server {config.alias} started successfully"
                    )
                return True
            else:
