...
class JobClient:
    ...
    def __init__(
        ...
        self.base_uri = "https://make-next-plans-dev.dev.api.platform.futurehouse.org" # base_uri
        self.agent = agent
    ...
    async def update_world_model_with_trajectory(
        self, world_model_id: str | UUID, trajectory_id: str | UUID
    ) -> str:
        """Update world model with trajectory data.

        Args:
            world_model_id: The ID of the world model to update
            trajectory_id: The ID of the trajectory to update the world model with

        Returns:
            The updated world model ID
        """
        try:
            async with httpx_aiohttp.HttpxAiohttpClient(
                timeout=self.REQUEST_TIMEOUT
                ...
                url = (
                    f"{self.base_uri}/v0.1/assistant/update-world-model-with-trajectory"
                )
                headers = {
                    "Authorization": f"Bearer {self.oauth_jwt}",
                ...
                with external_trace(
                    ...
                    custom_params={
                        ...
                        "agent": self.agent,
                        "environment": self.environment,
                        "operation": "update_world_model_with_trajectory",
                    },
                ):
                    headers = insert_distributed_trace_headers(headers)
                ...
                logger.info(f"Successfully updated world model {world_model_id}")

                return result["updated_world_model_id"]

        except httpx.HTTPStatusError as e:
            logger.exception(
                f"HTTP error while updating world model with trajectory. "
            ...
            logger.exception(
                f"Timeout while updating world model with trajectory after {self.REQUEST_TIMEOUT}s",
            )
            raise
        except httpx.NetworkError:
            logger.exception("Network error while updating world model with trajectory")
            raise
        except Exception:
            logger.exception("Failed to update world model with trajectory")
            raise
    ...
    async def generate_plans_for_world_model(self, world_model_id: str | UUID) -> dict:
        """Generate follow-up plans for a world model.

        Args:
            world_model_id: The ID of the world model to generate plans for

        Returns:
            Dictionary containing generated plans
        """
        try:
            async with httpx_aiohttp.HttpxAiohttpClient(
                timeout=self.REQUEST_TIMEOUT
                ...
                with external_trace(
                    ...
                    custom_params={
                        ...
                        "environment": self.environment,
                        "world_model_id": str(world_model_id),
                        "operation": "generate_plans_for_world_model",
                    },
                ):
                    headers = insert_distributed_trace_headers(headers)
                ...
                logger.info(
                    f"Successfully generated {len(result.get('generated_plans', []))} plans for world model {world_model_id}"
                )
            ...
            logger.exception(
                f"HTTP error while generating plans for world model. "
                f"Status code: {e.response.status_code}, "
                f"Response: {e.response.text}",
            )
            raise
            ...
            logger.exception(
                f"Timeout while generating plans for world model after {self.REQUEST_TIMEOUT}s",
            )
            raise
        except httpx.NetworkError:
            logger.exception("Network error while generating plans for world model")
            raise
        except Exception:
            logger.exception("Failed to generate plans for world model")
            raise
