...
def ensure_newrelic() -> bool:  # noqa: PLR0911
    ...
    try:
        ...
        logger.debug(f"Failed to fetch NewRelic application: {exc}")
        app = None
...
def monitored_transaction(
    ...
    try:
        ...
        txn_context = (
            newrelic.agent.WebTransaction(app, name, group=group)
            if created_new_txn
        ...
        with txn_context:
            ...
            if parsed_headers:
                accepted = newrelic.agent.accept_distributed_trace_headers(parsed_headers)
                if not accepted:
