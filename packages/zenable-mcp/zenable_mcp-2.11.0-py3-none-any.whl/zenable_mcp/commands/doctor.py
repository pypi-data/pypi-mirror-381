...
def check_dependencies() -> dict[str, str]:
    ...
    if git_path:
        ...
        if not git_path_obj.is_absolute():
            raise ValueError(f"git path is not absolute: {git_path}")
        ...
        try:
            ...
            dependencies["git"] = f"Available but error: {e}"
    else:
        dependencies["git"] = "NOT FOUND"

    # Check for uv
    uv_path = shutil.which("uv")
    if uv_path:
        uv_path_obj = Path(uv_path)
        if not uv_path_obj.is_absolute():
            raise ValueError(f"uv path is not absolute: {uv_path}")
        ...
        try:
            ...
            dependencies["uv"] = f"Available but error: {e}"
    else:
        dependencies["uv"] = "NOT FOUND"

    return dependencies


def check_config_files() -> dict[str, str]:
    """Check for the presence of important configuration files."""
    config_files = {}
    ...
    for config_name in config_names:
        ...
        config_files[config_name] = "Found" if config_path.exists() else "Not found"
...
@log_command
def doctor():
    """Diagnose and troubleshoot zenable-mcp issues"""
    ...
    if all(status == "Not found" for status in config_files.values()):
        echo(
            "  ⚠️  No configuration files found in current directory",
            persona=Persona.USER,
        )
        echo(
