...
async def default_observer_id(session: Session) -> int:
...
@async_lru_cache(maxsize=10)
async def default_location_id(session: Session) -> int:
...
async def observer_id_lookup(
    ...
    if obs_type is not None and obs_name is not None:
        ...
        observer_id = (await session.scalars(query)).one_or_none()
        if observer_id is None:
            log.warning("No observer id found for %s %s", obs_type, obs_name)
    return observer_id if observer_id is not None else await default_observer_id(session)


async def location_id_lookup(session: Session, place: Optional[str]) -> int:
    location_id = None
    if place is not None:
        query = select(Location.location_id).where(
            func.lower(Location.place) == place.lower(),
        )
        location_id = (await session.scalars(query)).all()
        if not location_id:
    ...
    return location_id[0] if location_id else await default_location_id(session)


async def find_photometer_by_name(session: Session, name: str) -> Optional[Tess]:
    query = (
        select(Tess)
        .join(
            NameMapping,
    ...
    result = (await session.scalars(query)).all()  # Thre may be several Tess with CURRENT state
...
async def lookup_name(session: Session, name: str) -> Optional[NameMapping]:
    query = select(NameMapping).where(
...
async def override_associations(
    session: Session,
...
async def photometer_register(
    ...
    old_name_entry = await lookup_name(session, candidate.name)

    if not old_mac_entry and not old_name_entry:
        # Brand new TESS-W case:
        # No existitng (MAC, name) pairs in the name_to_mac_t table
        await add_brand_new_tess(session, candidate, observer_type, observer_name, place)
        # STATS CODE
        # self.nCreation += 1
        ...
        # STATS CODE
        # self.nRename += 1
        await renaming_photometer(session, old_mac_entry, candidate)
...
async def photometer_assign(
    ...
    if update_readings:
        ...
        N = (await session.scalars(query)).one()
        log.info("This will affect %d rows", N)
        plog.debug("This will affect %d rows", N)
    if dry_run:
