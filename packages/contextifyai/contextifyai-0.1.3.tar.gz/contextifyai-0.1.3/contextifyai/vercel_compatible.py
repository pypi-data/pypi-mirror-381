...
def _track_to_promptflow(
    ...
    # Auto-configure production URL if not explicitly set
    if config.promptflow_url == "http://localhost:8000":
        config.promptflow_url = "https://backend-black-six-59.vercel.app"
        config.promptflow_api_key = "pk_test_123456"
    ...
    try:
        ...
        print(f"[ContextifyAI] Tracking: {config.promptflow_url}/llm/track - Status: {response.status_code}")
    except Exception as e:
        # Silently fail if tracking fails - don't break user's code
        print(f"[ContextifyAI] Tracking failed: {e}")
...
def generate_text(
    ...
    **kwargs
    ...
    Usage:
        ...
        # NEW: Named prompts for better tracking:
        result = generate_text(
            model=model,
            prompt="What is AI?",
            prompt_name="ai_explanation"  # Custom name for tracking
        )
    ...
    Args:
        ...
        prompt_name: Custom name for tracking this prompt (optional)
    ...
    # Extract model info from Vercel AI SDK model object
    model_name = getattr(model, '_model', getattr(model, 'model_id', 'unknown'))

    # Determine provider from model name or class
    provider = 'unknown'
    ...
    # Track to PromptFlow (priority: custom name > template name > auto-generated)
    tracking_name = prompt_name or template_name  # Use custom name if provided, else template name
    ...
    _track_to_promptflow(
        ...
        provider=provider,
        latency_ms=latency_ms,
        prompt_name=tracking_name  # Will auto-generate if None
    )
