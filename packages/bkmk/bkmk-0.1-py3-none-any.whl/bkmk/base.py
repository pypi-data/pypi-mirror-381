...
def _maybe_fake_separator(fake_separators, node):
    if fake_separators and isinstance(node, Separator):
        return Bookmark(node.id, node.date_added, "--------", FAKE_SEPARATOR_ICON, None, FAKE_SEPARATOR_URLS[0], None, None)
    else:
        return node
...
__all__ = [
    ...
    "Separator", "Bookmark", "SpecialFolder", "Folder", "_keep_child", "_maybe_fake_separator",
    "_d", "_o", "_oe", "_on",
    "log",
]
