...
def to_ast(node, cull_special, fake_separators):
    node = _maybe_fake_separator(fake_separators, node)
    if isinstance(node, Separator):
        ...
        return _d({
            ...
            "date_modified": _on(to_fmt_time(node.date_modified)),
            "children": [
                to_ast(c, cull_special, fake_separators) for c in node.children
...
def write(root, fp_out, cull_special, _cull_attr, fake_separators):
    w = {
    ...
    for r in special_children:
        w["roots"][SPECIAL_FOLDERS_BY_ENUM[r.special]] = to_ast(r, cull_special, fake_separators)
    ...
    w["roots"][SPECIAL_FOLDERS_BY_ENUM[None]] = to_ast(root, cull_special, fake_separators)
...
def _roundtrip_acceptable_diff(_cull_attr, depth, ty, attr, v_a, v_b):
    if attr == "icon":
        if v_a != "" and v_b == "":
