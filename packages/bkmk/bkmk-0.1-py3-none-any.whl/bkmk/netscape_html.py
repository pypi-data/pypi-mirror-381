...
def write_node(node, fp_out, cull_special, cull_attr, fake_separators, depth=0):
    ...
    if isinstance(node, Separator):
        ...
        for c in node.children:
            if _keep_child(cull_special, SPECIAL_FOLDERS_BY_ENUM.keys(), c):
                write_node(c, fp_out, cull_special, cull_attr, fake_separators, depth+1)
        print("%s</DL><p>" % indent, file=fp_out)
    else:
        assert False

def write(root, fp_out, cull_special, cull_attr, fake_separators):
    print("<!DOCTYPE %s>" % DOCTYPE, file=fp_out)
    print('<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=UTF-8">', file=fp_out)
    write_node(root, fp_out, cull_special, cull_attr, fake_separators, 0)
