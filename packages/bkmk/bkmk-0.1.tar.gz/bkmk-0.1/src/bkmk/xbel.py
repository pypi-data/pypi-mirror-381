...
def to_ast(node, cull_special, cull_attr, fake_separators, depth=0):
    ...
    if isinstance(node, Separator):
        ...
        w.extend([
            to_ast(c, cull_special, cull_attr, fake_separators, depth+1) for c in node.children
...
def write(root, fp_out, cull_special, cull_attr, fake_separators):
    w = to_ast(root, cull_special, cull_attr, fake_separators)
    ET.indent(w)
