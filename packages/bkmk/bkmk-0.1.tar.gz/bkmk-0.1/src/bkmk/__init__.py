...
class Bookmarks:
    ...
    def write(self, fp_out, fmt_out, cull_special=False, cull_attr=False, fake_separators=False):
        if fmt_out not in FORMATS:
            raise ValueError("not a valid format: %s" % fmt_out)
        globals()[fmt_out.replace("-", "_")].write(self.root, fp_out, cull_special, cull_attr, fake_separators)
