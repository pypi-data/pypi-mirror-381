...
class TextOverlay(GameObject):
    def __init__(self, text, pos, color=Color.white(), visible=True, name="text", font_size=40, font_family="Arial", bold=True, italic=False, antialiasing=True):
        super().__init__(name)
        self.text = text
        self.color = color
        self.visible = visible
    ...
    def draw(self, screen, camera: Camera):
        ...
        midpoint = ((vector2d.fromtuple(tuple=screen_size) / 2) + self.pos).totuple()
