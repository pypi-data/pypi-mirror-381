...
class GPUBackendAdapter(GPURepository):
    ...
    def _initialize_backends(self) -> None:
        ...
        self._logger.info(f"Factory returned backends: {list(available_backends.keys())}")

        for backend_name, backend in available_backends.items():
            self._logger.info(f"Checking {backend_name} backend availability...")
            is_avail = backend.is_available()
            self._logger.info(f"{backend_name} backend is_available() = {is_avail}")
    ...
    def _discover_devices_for_backend(self, backend_name: str, backend) -> None:
        ...
        try:
            ...
            for device_id in range(device_count):
                try:
                    self._logger.info(f"Getting device info for {backend_name} device {device_id}...")
                    device_info = backend.get_device_info(device_id)
                    self._logger.info(f"Device info for {backend_name} device {device_id}: {device_info}")

                    if "error" in device_info:
                        self._logger.error(f"Device {device_id} returned error: {device_info['error']}")
                        continue
                    ...
                    self._logger.info(f"Discovered device {device_id}: {gpu_device.info.name}")

                except Exception as e:
                    self._logger.error(f"Failed to get info for device {device_id}: {e}")
                    import traceback
