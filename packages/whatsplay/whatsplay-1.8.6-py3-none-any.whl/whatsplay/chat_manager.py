...
class ChatManager:
    ...
    async def collect_messages(self) -> list[Union[Message, FileMessage]]:
        """
        Recorre todos los contenedores de mensaje visibles y devuelve la lista de Message o FileMessage
        combinando multilinea y mensajes consecutivos del mismo remitente.
        """
        resultados = []
        ...
        # merge de mensajes consecutivos del mismo remitente en <=5s
        mensajes_combinados = []
        ultimo = None
        for m in resultados:
            if ultimo and m.sender == ultimo.sender and (m.timestamp - ultimo.timestamp).total_seconds() <= 5:
                ultimo.text += "\n" + m.text
            else:
                mensajes_combinados.append(m)
                ultimo = m
