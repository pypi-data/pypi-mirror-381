...
class P:
    ...
    def Hash(self, CA_Cert):
        ...
        if f.startswith(b'-----'): f = C.base64.b64decode(''.join(f.decode().splitlines()[1:-1]))
        CERT = x509.Certificate.load(f)['tbs_certificate']['subject_public_key_info'].dump()

        sha1 = C.base64.b64encode(C.hashlib.sha1(CERT).digest()).decode()
        sha256 = C.base64.b64encode(C.hashlib.sha256(CERT).digest()).decode()
