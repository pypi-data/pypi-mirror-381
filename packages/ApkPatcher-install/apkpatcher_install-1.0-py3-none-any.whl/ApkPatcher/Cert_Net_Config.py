from .C_M import CM; C = CM()


# ---------------- Write Cert ----------------
def Write_Cert(decompile_dir, isAPKEditor, CA_Cert):

    if isAPKEditor:
        decompile_dir = C.os.path.join(decompile_dir, 'resources', 'package_1')

    default_certificate = """\
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"""

    directory_path = C.os.path.join(decompile_dir, 'res', 'raw')
    C.os.makedirs(directory_path, exist_ok=True)

    created_certs = []

    if CA_Cert:
        for i, cert_path in enumerate(CA_Cert, start=1):
            file_name = f'Techno_India_{i}.pem' if i > 1 else 'Techno_India.pem'
            C.shutil.copy(cert_path, C.os.path.join(directory_path, file_name))
            created_certs.append(file_name)

        print(f"\n{C.lb}[ {C.pr}Certificate {C.lb}] {C.c} Your Certificate Path...\n{C.g}       |\n       |\n       └──── {C.r}Input ~{C.g}#{C.y} {', '.join(CA_Cert)} {C.g}✔\n")
        print(f"\n{C.lb}[ {C.c}Write Certificate {C.lb}] {C.rkj}➸❥ {C.y}raw/{', '.join(created_certs)}{C.g} ✔\n")

    else:
        file_path = C.os.path.join(directory_path, 'Techno_India.pem')
        open(file_path, 'w', encoding='utf-8', errors='ignore').write(default_certificate)

        print(f"\n{C.lb}[ {C.pr}Certificate {C.lb}] {C.g} The default certificate is from TechnoIndia's Modded HttpCanary... ✔\n")
        print(f"\n{C.lb}[ {C.c}Write Default Certificate {C.lb}] {C.rkj}➸❥ {C.y}{C.os.path.basename(directory_path)}/Techno_India.pem{C.g} ✔\n")


# ---------------- Write Net Config ----------------
def Write_Net_Config(decompile_dir, isAPKEditor, CA_Cert, file_names='network_security_config.xml'):

    Write_Cert(decompile_dir, isAPKEditor, CA_Cert)

    if isAPKEditor:
        decompile_dir = C.os.path.join(decompile_dir, 'resources', 'package_1')
            
    network_config = """\
<?xml version="1.0" encoding="utf-8"?>
<network-security-config xmlns:android="http://schemas.android.com/apk/res/android">
    <domain-override host="*" domain-override-cleartext-traffic-permitted="true" />
    <domain-config cleartextTrafficPermitted="true">
        <domain includeSubdomains="true">*</domain>
        <domain includeSubdomains="true">0.0.0.0</domain>
        <domain includeSubdomains="true">127.0.0.1</domain>
        <trust-anchors>
            {certificates_placeholder}
            <certificates src="system" overridePins="true" />
            <certificates src="user" overridePins="true" />
        </trust-anchors>
    </domain-config>
    <base-config cleartextTrafficPermitted="true">
        <trust-anchors>
            {certificates_placeholder}
            <certificates src="system" overridePins="true" />
            <certificates src="user" overridePins="true" />
        </trust-anchors>
    </base-config>
    <debug-overrides cleartextTrafficPermitted="true">
        <trust-anchors>
            {certificates_placeholder}
            <certificates src="system" overridePins="true" />
            <certificates src="user" overridePins="true" />
        </trust-anchors>
    </debug-overrides>
</network-security-config>"""

    certificates_entries = ""

    if CA_Cert:
        for i, _ in enumerate(CA_Cert, start=1):
            file_name = f'Techno_India_{i}' if i > 1 else 'Techno_India'
            certificates_entries += f'            <certificates src="@raw/{file_name}" overridePins="true" />\n'

    else:
        certificates_entries += '            <certificates src="@raw/Techno_India" overridePins="true" />\n'

    final_config = network_config.format(certificates_placeholder=certificates_entries.strip())
    directory_path = C.os.path.join(decompile_dir, 'res', 'xml')
    C.os.makedirs(directory_path, exist_ok=True)

    config_path = C.os.path.join(directory_path, file_names)
    open(config_path, 'w', encoding='utf-8', errors='ignore').write(final_config)

    print(f"\n{C.lb}[ {C.c}Write Network Config {C.lb}] {C.rkj}➸❥ {C.y}{C.os.path.basename(directory_path)}/{file_names}{C.g} ✔\n")

    if isAPKEditor:
        update_public_xml(decompile_dir, CA_Cert)


# ---------------- For isAPKEditor ----------------
def update_public_xml(decompile_dir, CA_Cert):

    public_xml_path = C.os.path.join(decompile_dir, 'res', 'values', 'public.xml')  

    public_xml = open(public_xml_path, 'r', encoding='utf-8', errors='ignore').readlines()

    raw_ids = []; xml_ids = []

    raw_xml_regex = C.re.compile(r'<public id="(0x[0-9a-fA-F]+)" type="(raw|xml)" name="([^"]*)" />')

    raw_found = False; xml_found = False

    for line in public_xml:
        for match in raw_xml_regex.findall(line):
            res_id = int(match[0], 16); res_type = match[1]; res_name = match[2]

            if res_type == "raw" and res_name == "Techno_India": raw_found = True
            elif res_type == "xml" and res_name == "network_security_config": xml_found = True

            if res_type == "raw": raw_ids.append(res_id)
            elif res_type == "xml": xml_ids.append(res_id)

    if raw_found and xml_found:
        return

    new_raw_id = max(raw_ids) + 1 if raw_ids else 0x7fffff01
    new_xml_id = max(xml_ids) + 1 if xml_ids else 0x7ffffff1

    new_entries = []

    if not raw_found:
        if CA_Cert:
            for i, _ in enumerate(CA_Cert, start=1):
                file_name = f'Techno_India_{i}' if i > 1 else 'Techno_India'
                new_entries.append(f'  <public id="0x{new_raw_id:08X}" type="raw" name="{file_name}" />\n')
                new_raw_id += 1

        else:
            new_entries.append(f'  <public id="0x{new_raw_id:08X}" type="raw" name="Techno_India" />\n')

    if not xml_found:
        new_entries.append(f'  <public id="0x{new_xml_id:08X}" type="xml" name="network_security_config" />\n')

    for i in range(len(public_xml)):
        if "</resources>" in public_xml[i]:
            public_xml[i:i] = new_entries
            break

    open(public_xml_path, 'w', encoding='utf-8', errors='ignore').writelines(public_xml)

    print(f"\n{C.lb}[ {C.c}Write New Entries {C.lb}] {C.rkj}➸❥ {C.g}public.xml\n\n{C.rkj}{''.join(new_entries).strip()}\n")