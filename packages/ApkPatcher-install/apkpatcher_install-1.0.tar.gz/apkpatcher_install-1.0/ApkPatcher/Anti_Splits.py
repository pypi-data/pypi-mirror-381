...
def Anti_Split(apk_path, isMerge, isCoreX):
    ...
    if apk_path and isCoreX and C.os.path.splitext(apk_path)[-1].lower() not in Merge_Ext:
    ...
    if Ext in Merge_Ext:
        ...
        print(f"{C.g}  |\n  └──── {C.r}Decompiling ~{C.g}$ java -jar {C.os.path.basename(F.APKEditor_Path)} m -i {apk_path} -f -o {output_path}" + (" -extractNativeLibs true" if isCoreX else "") + f"\n\n{C_Line}{C.g}\n")
