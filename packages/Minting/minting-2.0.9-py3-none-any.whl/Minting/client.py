...
class Database:
    def __init__(self, mongo_uri, db_name):
        ...
        self.access_tokens = self.db["AccessToken"]
        self.user_plans = self.db["userPlan"]
        self.plans = self.db["plan"]
...
class TokenManager:
    def __init__(self, db: Database, raw_token: str):
        self.db = db
        # Use raw token directly (DB already stores raw tokens)
        self.token = raw_token  

    def check_token_and_credits(self):
        print("🔎 Checking token:", repr(self.token))  # DEBUG
        token_doc = self.db.access_tokens.find_one({"token": self.token, "isActive": True})
        print("🔎 Token doc found:", token_doc)  # DEBUG
        if not token_doc:
            return {"valid": False, "error": "Invalid or inactive access token"}
    ...
    def deduct_credit(self):
        ...
        result = self.db.user_plans.update_one(
            {"_id": plan_doc["_id"], "creditsRemaining": {"$gt": 0}},
            {"$inc": {"creditsRemaining": -1}, "$set": {"updatedAt": datetime.utcnow()}}
        )
        return result.modified_count > 0
