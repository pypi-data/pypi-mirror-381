...
class Component(OpenApiObject):
    ...
    @property
    def choice(self):
        # type: () -> Union[Literal["cpu"], Literal["custom"], Literal["device"], Literal["memory"], Literal["nic"], Literal["npu"], Literal["port"]]
        """choice getter

        Type of component.. - `custom` If the type of component is not listed as an enum it can be defined using the custom type.. `device` This enum allows device to be composed of other devices. When this enum is selected the name of the component MUST be the name of device that exists in the Infrastructure.devices array.. `cpu` high level definition for cpu. `npu` high level definition for neural processing unit. `nic` high level definition for network interface card, for more detailed breakdowns create device representing specific type network interface card. `memory` high level definition for memory. `port` high level definitiion for an IO port.

        Returns: Union[Literal["cpu"], Literal["custom"], Literal["device"], Literal["memory"], Literal["nic"], Literal["npu"], Literal["port"]]
        """
    ...
    def choice(self, value):
        ...
        value: Union[Literal["cpu"], Literal["custom"], Literal["device"], Literal["memory"], Literal["nic"], Literal["npu"], Literal["port"]]
        """
        if value is None:
...
class ComponentIter(OpenApiIter):
    ...
    def __getitem__(self, key):
        # type: (str) -> Union[Component, ComponentCpu, ComponentCustom, ComponentDevice, ComponentMemory, ComponentNic, ComponentNpu, ComponentPort]
...
class QueryShortestPathItem(OpenApiObject):
    ...
    _TYPES = {
        "name": {"type": str},
        "edges": {"type": "QueryEdgeIter"},
    }  # type: Dict[str, str]

    _REQUIRED = ("name", "edges")  # type: tuple(str)
    ...
    def edges(self):
        ...
        Returns: QueryEdgeIter
        """
        return self._get_property("edges", QueryEdgeIter, self._parent, self._choice)


class QueryEdge(OpenApiObject):
    __slots__ = "_parent"
    ...
    def __init__(self, parent=None, ep1=None, ep2=None):
        super(QueryEdge, self).__init__()
        self._parent = parent
        self._set_property("ep1", ep1)
        self._set_property("ep2", ep2)

    def set(self, ep1=None, ep2=None):
        for property_name, property_value in locals().items():
            if property_name != "self" and property_value is not None:
                self._set_property(property_name, property_value)
...
class QueryEdgeIter(OpenApiIter):
    __slots__ = ("_parent", "_choice")

    _GETITEM_RETURNS_CHOICE_OBJECT = False

    def __init__(self, parent=None, choice=None):
        super(QueryEdgeIter, self).__init__()
        self._parent = parent
        self._choice = choice

    def __getitem__(self, key):
        # type: (str) -> Union[QueryEdge]
        return self._getitem(key)
    ...
    def _instanceOf(self, item):
        if not isinstance(item, QueryEdge):
            raise Exception("Item is not an instance of QueryEdge")

    def queryedge(self, ep1=None, ep2=None):
        # type: (str,str) -> QueryEdgeIter
        """Factory method that creates an instance of the QueryEdge class

        TBD

        Returns: QueryEdgeIter
        """
        item = QueryEdge(parent=self._parent, ep1=ep1, ep2=ep2)
        self._add(item)
        return self

    def add(self, ep1=None, ep2=None):
        # type: (str,str) -> QueryEdge
        """Add method that creates and returns an instance of the QueryEdge class
...
class QueryShortestPathItemIter(OpenApiIter):
    ...
    def queryshortestpathitem(self, name=None):
        # type: (str) -> QueryShortestPathItemIter
        ...
        Returns: QueryShortestPathItemIter
        """
        item = QueryShortestPathItem(parent=self._parent, name=name)
        self._add(item)
        return self

    def add(self, name=None):
        # type: (str) -> QueryShortestPathItem
...
class Api(object):
    ...
    def __init__(self, **kwargs):
        ...
        self._version_meta.api_spec_version = "0.3.0"
