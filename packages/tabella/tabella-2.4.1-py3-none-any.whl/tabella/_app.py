...
from tabella._monitor import monitor, monitor_ws
from tabella._routes import (
    add_item,
    api_pass_through,
    discover,
    download_client,
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        routes: list[Route | WebSocketRoute] = [
            Route("/", index),
            Route("/docs/discover-{trigger}", discover),
            Route("/docs/try-it-modal/{method_idx}", try_it),
            Route(
                "/docs"
                "/add-{item_type}-item"
                "/{method_id}"
            ...
            Route("/openrpc.json", openrpc_doc),
            Route("/rpc-api", api_pass_through, methods=["POST"]),
            Route("/download-client/{version}", download_client),
        ]
        # Add monitoring routes if in debug.
        if debug:
            routes.append(Route("/monitor", monitor))
            routes.append(WebSocketRoute("/monitor-ws", monitor_ws))
    ...
    def _get_ws_function(self) -> Callable[..., Any]:
        ...
        async def ws_process_rpc_debug(websocket: WebSocket) -> None:
            ...
            try:
                ...
                while True:
                    data = await websocket.receive_text()
                    task = loop.create_task(_process_rpc(data))
                    tasks = [t for t in tasks if not t.done()] + [task]
            except WebSocketDisconnect:
                pass
