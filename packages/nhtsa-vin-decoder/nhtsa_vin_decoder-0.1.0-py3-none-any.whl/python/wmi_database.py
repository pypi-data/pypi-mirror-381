...
Offline fallback for VIN manufacturer identification
...
class WMIDatabase:
    ...
    @classmethod
    def get_manufacturer(cls, vin: str) -> str:
        """Get manufacturer from VIN using WMI mapping"""
        if not vin or len(vin) < 3:
            return None

        wmi = vin[:3].upper()
        return cls.WMI_MAP.get(wmi, 'Unknown')
    ...
    def get_country(cls, vin: str) -> str:
        ...
        return cls.REGION_MAP.get(vin[0].upper(), 'Unknown')
    ...
    def is_supported(cls, vin: str) -> bool:
        """Check if WMI is in our database"""
        if not vin or len(vin) < 3:
            return False

        return vin[:3].upper() in cls.WMI_MAP
