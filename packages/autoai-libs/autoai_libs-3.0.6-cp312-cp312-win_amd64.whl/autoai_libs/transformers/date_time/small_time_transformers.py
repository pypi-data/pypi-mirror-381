...
# TODO: Revisit this transformer, it doesn't seem to work
# class TimeIntervalSecondsTransformer(SmallDataColumnTransformer):
#     def __init__(self, *args, **kwargs):
#         super().__init__(lambda x, y: (pd.to_datetime(x) - pd.to_datetime(y)).total_seconds(), *args, **kwargs)


class DateToSecondsTransformer(SmallDataColumnTransformer):
    def __init__(self, *args, **kwargs):
        super().__init__(
            lambda x: (
                pd.to_datetime(x) - pd.to_datetime(pd.DataFrame({"year": [1970], "month": [1], "day": [1]}))
            ).dt.total_seconds(),
            *args,
            **kwargs,
        )
