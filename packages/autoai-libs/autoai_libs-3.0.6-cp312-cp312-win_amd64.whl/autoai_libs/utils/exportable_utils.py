    ...
    if columns_indices_list is not None and isinstance(columns_indices_list, list) and columns_indices_list:
        ...
        if x_ndim == 1:
            ...
            for i, index in enumerate(columns_indices_list):
                if index < 0 and index >= x_numelts:
                    logger.warning(
                        ...
                        + "th index is "
                        + index
                        + ": this is out of dataset column range [0,"
                        + x_numelts
                        + "-1]"
                    )
...
def numpy_replace_values(X, filling_value, reference_values_list, invert_flag=False):
    """
    :param X: Input matrix: ndarray
    :param filling_value: special value assigned array entries that match the reference values
    :param reference_values_list: reference list of lists of known values for each column. if it is a 1-dimensional list, then
           it is applicable to each column
    :param invert_flag: If True replace the values NOT in the reference values list. Default False
    """
    if reference_values_list is not None and reference_values_list:
        if isinstance(reference_values_list[0], list):
            list2_flag = True  # list of lists
        else:
            list2_flag = False  # single dimensional list
        ...
        for j in range(num_columns):
            ...
            # set_reference_values_list_col = set(reference_values_list_col)
            # if invert_flag:
            #     replace_values_mask = np.array([value not in set_reference_values_list_col for value in Xcol])
            # else:
            #     replace_values_mask = np.array([value in set_reference_values_list_col for value in Xcol])

            if invert_flag:
                replace_values_mask = ~numpy_isin(Xcol, reference_values_list_col)
            else:
                replace_values_mask = numpy_isin(Xcol, reference_values_list_col)

            if np.any(replace_values_mask):
                if X.ndim == 1:
                    Y = numpy_putmask(Xcol, replace_values_mask, filling_value)
                else:
                    Y[:, j] = numpy_putmask(Xcol, replace_values_mask, filling_value)
            else:
                if X.ndim == 1:
                    Y = Xcol
                else:
                    Y[:, j] = Xcol
                # np.putmask(Xcol.astype(object),replace_values_mask,filling_value)
    else:
        Y = X
    return Y
    ...
    #
    # if X.ndim == 1:
    #     known_val_list = known_values_list2[0]
    #
    #     for i in range(X.shape[0]):
    #         if X[i] is not np.nan and X[i] not in known_val_list:
    #             Y[i] = filling_value
    #         else:
    #             Y[i] = X[i]
    # else:
    #     for i in range(X.shape[1]):
    #         known_val_list = known_values_list2[i]
    #
    #         for j in range(X.shape[0]):
    #             if X[j, i] is not np.nan and X[j, i] not in known_val_list:
    #                 Y[j, i] = filling_value
    #             else:
    #                 Y[j, i] = X[j, i]
    #
    # for j in range(num_columns):
    #     uniq_values, indices, inv, counts = np.unique(X[:,j], return_index=True, return_inverse=True, return_counts=True)
    #     if np.nan in uniq_values:
    #         nan_indices=np.argwhere(np.isnan(X[:,j]))
    #         X[:,j][nan_indices]=filling_value
    #     uniq_values_list = list(uniq_values)
    #     cleaned_uniq_values_list = [x for x in uniq_values_list if pd.isnull(x)]
    #     l_intersection=list_intersection(cleaned_uniq_values_list,known_values_list2[j])
    #     if l_intersection:
    #         np.where(X[:,j] in l_intersection,X[:,j],filling_value)
    # # for each col:
    # # uniq_values = np.unique(X[:, i]
    # # if nan in uniq_values:
    # #     nan_indices = np.argwhere(np.isnan(x))
    # # X[:, i][nan_indices] = self.filling_values
    # # if intersection of uniq_values and known_val_list is not null:
    # # then
    # # do np.where and replace those
    # # with filling values


def numpy_permute_array(X, permutation_indices=None, axis=None):
    """
    Performs a permultation on rows or columns of an nparray based on a list of permuted
    indices.
    :param X:
    ...
    if not l_permutation_indices:  # if permutation list is empty  or was not defined return original matrix
        ...
        # i = np.asarray(self.permutation_indices)
...
def numpy_boolean2float(X, missing_values_reference_list=None):
    """
    Converts a 1-D numpy array of strings that represent floats to floats
    and replaces missing values with np.nan. Also changes type of array from 'object' to 'float'
    :param X: 1-d numpy array of booleans
    :param missing_values_reference_list: list of values that should be considered as missing
    :return: 1-D numpy array of floats and np.nan as missing values
    """
...
def WML_raise_exception(
    ...
    if exception_obj is not None and exception_trace_flag:
        trace_error_message = "".join(
            traceback.format_exception(etype=type(exception_obj), value=exception_obj, tb=exception_obj.__traceback__)
        )
...
def numpy_putmask(X, replace_values_mask, filling_value):
    Y = X.astype(object)
    np.putmask(Y, replace_values_mask, filling_value)

    # #slower version
    # Y=numpy_flatten_column(X)
    # for i in range(X.shape[0]):
    #     if replace_values_mask[i]:
    #         Y[i]=filling_value
