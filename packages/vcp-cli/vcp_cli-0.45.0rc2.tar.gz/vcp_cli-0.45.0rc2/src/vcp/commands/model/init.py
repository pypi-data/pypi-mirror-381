...
def init_command(
    ...
    try:
        ...
        check_url = f"{model_hub_url}/api/models/check"
        ...
        if repo_path:
            ...
            # Call model hub API to create new repository
            create_url = f"{model_hub_url}/api/github/contribution/create-repo"
            create_data = {
                "model_name": model_name,
                "model_version": model_version,
                "license_type": license_type,
            }
            _verbose_print(f"Create repository URL: {create_url}", verbose)
            _verbose_print(f"Create data: {create_data}", verbose)

            # Get authentication headers for API calls
            _verbose_print(
                "Getting authentication headers for repository creation", verbose
            )
            token_manager = TokenManager()
            headers = token_manager.get_auth_headers()
            ...
            _verbose_print("Making API request to create repository", verbose)
            ...
            _verbose_print(f"Create result: {create_result}", verbose)
