...
for _ in range(100):
    ...
    next_state, reward, done, effective, info =  env.step(action) # 4. --- 执行动作

    if not effective:
        print(f"动作 {action} 无效，位置未改变")
    else:
        print(f"前一状态: {info['state']}, 执行动作: {action_names[action]}, 当前状态: {next_state}, 当前奖励: {reward:.2f}")
    if done:
        break
env.close()
