...
class GridWorldEnv:
    ...
    def __init__(self, grid_size=(5, 5), enable_keep=False, start_state=(0, 0), render_mode="human", 
                 keyboard_control=False, target_state=None, max_steps=100000000,
    ...
    def _draw_cell_text(self, i, j, center_x, center_y):
        ...
        if state_value is not None:
            # 显示两行：奖励和状态价值
            num_width = max(len(f"{reward_val:.2f}"), len(f"{state_value:.2f}"))
