...
"""
BRS-XSS Report Templates

Template classes for different report formats.

Company: EasyProTech LLC (www.easypro.tech)
Dev: Brabus
Created: Mon 04 Aug 2025 12:49:01 MSK
...
"""
        ...
        <div class="vulnerability severity-{severity}">
            <div class="vuln-header">
                Vulnerability #{i} - {vuln.get('parameter', 'Unknown Parameter')} ({severity.upper()})
            </div>
            <div class="vuln-content">
                ...
                <p><strong>Context:</strong> {vuln.get('context', 'N/A')}</p>
                 <p><strong>Detection Score:</strong> {vuln.get('detection_score', 'N/A')}</p>
                 <p><strong>Exploitation Likelihood:</strong> {float(vuln.get('exploitation_likelihood', 0)):.1%}</p>
                 <p><strong>Confidence:</strong> {float(vuln.get('confidence', 0)):.1%}</p>
...
"""
            ...
            html_content += "<h2>No Vulnerabilities Found</h2><p>The target appears to be secure against XSS attacks.</p>"
        
        policy = data.get('policy', {})
        min_score = policy.get('min_vulnerability_score', 'N/A')
        severity_bands = policy.get('severity_bands', {
            'critical': '>= 9.0', 'high': '>=7.0', 'medium': '>=4.0', 'low': '>=1.0', 'info': '>0'
        })
        html_content += f"""
        <div class="footer">
            <p>Generated by BRS-XSS - Professional XSS Scanner</p>
            <p>Policy: detection threshold = {min_score}; severity bands: {severity_bands}</p>
            <p>EasyProTech LLC - https://t.me/EasyProTech</p>
        </div>
    </div>
</body>
</html>"""
