...
class ManualRunInputV1(object):
    ...
    def table_batch_action(self, table_batch_action):
        ...
        :type: str
        """
        allowed_values = ["MAKE_EXTINCT", "CLEANUP", "NO_ACTION"]
        if table_batch_action not in allowed_values:
            raise ValueError(
                "Invalid value for `table_batch_action` ({0}), must be one of {1}"
                .format(table_batch_action, allowed_values)
            )
