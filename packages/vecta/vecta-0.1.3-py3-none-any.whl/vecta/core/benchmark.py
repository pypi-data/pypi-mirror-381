...
from vecta.connectors.base import BaseConnector
...
class VectaClient:
    ...
    def __init__(
        self,
        vector_db_connector: Optional[BaseConnector] = None,
        ...
        Args:
            vector_db_connector: Connector to the data source (optional for API client usage)
            openai_api_key: OpenAI API key for question generation (not needed for API client)
            openai_base_url: OpenAI base URL
            model: OpenAI model to use
        """
        self.connector = vector_db_connector
        ...
        if openai_api_key:
            ...
            self.semantic_searcher = (
                EmbeddingSimilaritySearcher(vector_db_connector)
                if vector_db_connector
                else None
            )
        else:
    ...
    def load_knowledge_base(self) -> List[ChunkData]:
        ...
        self.all_chunks = self.connector.get_all_chunks()
    ...
    def generate_benchmark(
        ...
        # All chunks can be used for similarity search since we no longer require embeddings
        ...
        # This filters candidates BEFORE making expensive LLM judgment calls
        print(
        ...
        for chunk, synthetic_q in tqdm(
            ...
            # This uses fast embedding similarity (dot product) to filter candidates
            ...
            # This minimizes expensive LLM judgment calls
        ...
        # We only make LLM calls for chunks that are semantically similar
        print(
