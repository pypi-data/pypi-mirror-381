...
class Packages:
    """
    A class to manage package operations on a remote system using `pip`.
    ...
    **Examples:**
    ...
    def execute(self):
        ...
        r1 = yield Operation(f"pip list --format=json",guard=self.guard, sudo=self.sudo, su=self.su)

        # Add or remove packages based on the `present` flag
        r2 = yield Operation(f"pip install -y {self.op}",guard=self.guard and self.present, sudo=self.sudo, su=self.su)

        r3 = yield Operation(f"pip uninstall -y {self.op}",guard=self.guard and not self.present, sudo=self.sudo, su=self.su)

        # Retrieve the updated list of installed packages
        r4 = yield Operation(f"pip list --format=json",guard=self.guard, sudo=self.sudo, su=self.su)
