...
from mielto.db.postgres.schemas import get_table_schema_definition
from mielto.db.postgres.utils import (
    apply_sorting,
    bulk_upsert_metrics,
...
from mielto.db.schemas.evals import EvalFilterType, EvalRunRecord, EvalType
from mielto.db.schemas.knowledge import KnowledgeRow
from mielto.db.schemas.memory import UserMemory
from mielto.session import AgentSession, Session, TeamSession, WorkflowSession
...
class PostgresDb(BaseDb):
    ...
    def get_user_memory(
        self, memory_id: str, user_id: Optional[str] = None, deserialize: Optional[bool] = True
    ...
    def get_user_memories_by_ids(
        self, 
        memory_ids: List[str], 
        user_id: Optional[str] = None, 
        deserialize: Optional[bool] = True
    ) -> List[Union[UserMemory, Dict[str, Any]]]:
        """Get multiple memories from the database by their IDs.
        ...
        Returns:
            List[Union[UserMemory, Dict[str, Any]]]:
                ...
                - When deserialize=False: List of UserMemory dictionaries
        ...
        try:
            ...
            with self.Session() as sess, sess.begin():
                stmt = select(table).where(table.c.memory_id.in_(memory_ids))
            ...
            log_error(f"Exception reading memories by IDs from memory table: {e}")
            return []
    ...
    def get_user_memories(
        ...
        Args:
            ...
            workspace_id (Optional[str]): The ID of the workspace to filter by.
            memory_id (Optional[str]): The ID of the memory to filter by.
            memory_type (Optional[str]): The type of the memory to filter by.
            topics (Optional[List[str]]): The topics to filter by.
            facts (Optional[List[str]]): The facts to filter by.
            search_content (Optional[str]): The content to search for.
            limit (Optional[int]): The maximum number of memories to return.
            page (Optional[int]): The page number.
            sort_by (Optional[str]): The column to sort by.
            sort_order (Optional[str]): The order to sort by.
            after_memory_id (Optional[str]): The ID of the memory to filter by.
            deserialize (Optional[bool]): Whether to serialize the memories. Defaults to True.
        ...
        try:
            ...
            with self.Session() as sess, sess.begin():
                ...
                if memory_id is not None:
                    stmt = stmt.where(table.c.memory_id == memory_id)
                if memory_type is not None:
                    stmt = stmt.where(table.c.memory_type == memory_type)
                ...
                if facts is not None:
                    for fact in facts:
                        stmt = stmt.where(func.cast(table.c.facts, String).like(f'%"{fact}"%'))
                if after_memory_id is not None:
                    stmt = stmt.where(table.c.memory_id > after_memory_id)
        ...
        try:
            ...
            with self.Session() as sess, sess.begin():
                if memory.memory_id is None:
                    memory.memory_id = generate_prefix_ulid("mem")

                print(memory.memory, memory.topics, memory.facts, memory.metadata)

                stmt = postgresql.insert(table).values(
                    memory_id=memory.memory_id,
                    memory=memory.memory,
                    memory_type=memory.memory_type or "user",
                    input=memory.input,
    ...
    def migrate_table_from_v1_to_v2(self, v1_db_schema: str, v1_table_name: str, v1_table_type: str):
        ...
        from mielto.db.migrations.v1_to_v2 import (
