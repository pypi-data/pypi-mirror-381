...
from mielto.db.schemas.knowledge import KnowledgeRow
from mielto.knowledge.content import Content, ContentAuth, ContentStatus, FileData
from mielto.knowledge.document import Document
from mielto.knowledge.reader import Reader, ReaderFactory
from mielto.knowledge.remote_content.remote_content import GCSContent, RemoteContent, S3Content
from mielto.utils.common import generate_prefix_ulid
from mielto.utils.http import async_fetch_with_retry
from mielto.utils.log import log_debug, log_error, log_info, log_warning
from mielto.vectordb import VectorDb
...
class Knowledge:
        ...
        if args and isinstance(args[0], list):
            ...
            for argument in arguments:
                await self.add_content_async(
                    ...
                    collection_id=argument.get("collection_id"),
    ...
    @overload
    async def add_content_async(self, *args, **kwargs) -> Any: ...
    ...
    async def _load_from_url(
        ...
        if reader is None:
            ...
            elif file_extension == ".markdown" or file_extension == ".md":
                ...
                if isinstance(content.file_data.content, bytes):
                    ...
                    if self._is_text_mime_type(content.file_data.type):
                        ...
                        try:
                            ...
                            log_debug(f"latin-1 encoding also failed, using UTF-8 with error replacement")
                            content_bytes = content.file_data.content.encode("utf-8")
                    content_io = io.BytesIO(content_bytes)
                else:
    ...
    async def _handle_vector_db_insert(self, content, read_documents, upsert):
        ...
        if self.vector_db.upsert_available() and upsert:
            try:
                await self.vector_db.async_upsert(content.content_hash, read_documents, content.metadata, collection_id=content.collection_id, workspace_id=content.workspace_id)
            except Exception as e:
    ...
    def _update_content(self, content: Content) -> Optional[Dict[str, Any]]:
        if self.contents_db:
            ...
            if content.collection_id is not None:
                content_row.collection_id = content.collection_id
            if content.workspace_id is not None:
                content_row.workspace_id = content.workspace_id
    ...
    def search(
        self, query: str, max_results: Optional[int] = None, filters: Optional[Dict[str, Any]] = None, collection_id: Optional[str] = None, workspace_id: Optional[str] = None
    ) -> List[Document]:
        ...
        try:
            ...
            return self.vector_db.search(query=query, limit=_max_results, filters=filters, collection_id=collection_id, workspace_id=workspace_id)
        except Exception as e:
    ...
    async def async_search(
        ...
        try:
            ...
            log_debug(f"Getting {_max_results} relevant documents for query: {query}")
            try:
                return await self.vector_db.async_search(query=query, limit=_max_results, filters=filters, collection_id=collection_id, workspace_id=workspace_id)
    ...
    def get_content_by_id(self, content_id: str) -> Optional[Content]:
        ...
        content = Content(
            ...
            collection_id=content_row.collection_id,
            workspace_id=content_row.workspace_id,
