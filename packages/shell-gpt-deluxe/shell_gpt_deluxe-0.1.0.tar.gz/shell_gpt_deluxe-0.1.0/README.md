...
```text
--plan-execute / -planexec        Enable plan-execute mode for multi-step tasks
--auto-approve                    Skip step-by-step approval (execute entire plan automatically)
--hide-costs                      Hide cost estimates in plan display
```

See [DEMO_SCRIPT_TESTING.md](DEMO_SCRIPT_TESTING.md) for a comprehensive walkthrough and [DESIGN_WRITEUP.txt](DESIGN_WRITEUP.txt) for design decisions and tradeoffs.
...
│ --plan-execute    -pe                     Enable multi-step autonomous task execution.                   │
