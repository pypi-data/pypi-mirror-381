...
def test_location(log_detector):
    ...
    assert log_detector.loc(time) != log_detector.loc(time + 3 * u.hour)


def test_save_dict(log_detector):
    """Test the save dictionary method. By asserting the values of
    the dictionary are the same as the expected values."""
    ret_dict = log_detector.save_dict()
    assert str("tests/test_data/Trajectories/test.log") in str(ret_dict["flight_log"])


def test_limb_angle(log_detector):
    ...
    assert log_detector.limb_angle(start_time).to(u.deg).value == approx(-5.8, 0.1)


def test_location_out_of_bounds(log_detector):
    """Test the location of the balloon is set correctly but changes over time"""
    time = Time("2023-05-13T03:00:00", format="isot", scale="utc")
    with pytest.raises(ValueError):
        log_detector.loc(time)
    time = Time("2023-05-15T03:00:00", format="isot", scale="utc")
    with pytest.raises(ValueError):
        log_detector.loc(time)
