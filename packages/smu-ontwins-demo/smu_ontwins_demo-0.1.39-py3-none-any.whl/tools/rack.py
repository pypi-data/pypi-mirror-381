...
class Racks:
    ...
    def _get_cell_position(self, rack: int, level: int, cell: int):
        ...
        position = [
            ...
            rack_pos[2] + max(5.33, (level - 1) * PER_LEVEL),
        ]
    ...
    def state_current(self, *, at=None) -> dict:
        ...
        def hue_for(product: str, products: set[str]) -> float:
        ...
        for _, r in self.state_df.iterrows():
            ...
            all_cell_states.append({
                ...
                "color": f"hsl({hue_for(product, self.products)}, 100%, {60+30*(1-qty/self.cfg.cell_capacity)}%)" if product else None
            })
