...
class DeviceFlowAuth:
    ...
    def _show_login_button(self, url: str, user_code: str | None = None) -> None:
        btn_html = f"""
        <div style="padding:14px;border:1px solid #ddd;border-radius:8px;margin:8px 0;">
          ...
          {'<div style="margin-bottom:6px;">User Code: <code>'+user_code+'</code></div>' if user_code else ''}
    ...
    def login(self, open_browser: bool = True) -> Dict[str, Any]:
        ...
        if open_browser:
            try:
                ...
                pass
        self._show_login_button(verify_url, user_code)
