...
class Racks:
    ...
    def _get_cell_position(self, rack: int, level: int, cell: int):
        props = self.cfg.rack_entities[rack].get("properties") if rack < len(self.cfg.rack_entities) else {}
        rack_pos = props.get("worldPosition", props.get("position", [0, 0, 0]))

        PER_LEVEL = 1.778
        CELL_HALF = 1

        # self.cfg.cells_per_level # N
        # cell # 1 ~ N
        # (cell - 1) # 0 ~ N-1
        # (cell - 1) / (self.cfg.cells_per_level - 1) # 0 ~ 1
        # ((cell - 1) / (self.cfg.cells_per_level - 1) * 2 - 1) # -1 ~ +1

        position = [
            rack_pos[0],
            rack_pos[1] + ((cell - 1) / (self.cfg.cells_per_level - 1) * 2 - 1) * CELL_HALF,
            rack_pos[2] + (level - 1) * PER_LEVEL,
        ]
    ...
    def state_current(self, *, at=None) -> list[dict]:
        ...
        for _, r in self.state_df.iterrows():
            events.append({
                ...
                "co_product": None, "co_quantity": None,
                "version": 1,
            })
        return events
