...
def load_sim_settings(path: str, title: str, year: int, month: int = None):
    ...
    df_orders = _load_worksheet(co, f"{year}", ["datetime", "product", "quantity"], pd.DataFrame(columns=["datetime", "product", "quantity"]))
    return df_goods, df_init_stocks, df_orders, month
...
def _load_init_stock(sh, month) -> pd.DataFrame:
    ...
    return _load_worksheet(sh, f"{month:02d}월", HEADERS, df)
