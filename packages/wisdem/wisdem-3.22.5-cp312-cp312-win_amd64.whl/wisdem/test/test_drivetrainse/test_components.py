...
class TestComponents(unittest.TestCase):
    def testBearing(self):
        ...
        npt.assert_almost_equal(outputs["mb_mass"], (1 + 80 / 27) * 1561.4 * 3**2.6007)
        npt.assert_almost_equal(outputs["mb_max_defl_ang"], 0.5 * np.pi / 180)
    ...
    def testBrake(self):
        ...
        npt.assert_equal(outputs["brake_mass"], 12200)
        npt.assert_equal(outputs["brake_cm"], 3)
        npt.assert_equal(outputs["brake_I"], 12200 * np.r_[0.5, 0.25, 0.25])

        discrete_inputs["direct_drive"] = False
        inputs["s_gearbox"] = np.array([ 5.0 ])
        myobj.compute(inputs, outputs)
        npt.assert_equal(outputs["brake_mass"], 12200)
        npt.assert_equal(outputs["brake_cm"], 3)
        npt.assert_equal(outputs["brake_I"], 12200 * np.r_[0.5, 0.25, 0.25])

        inputs["brake_mass_user"] = np.array([ 42.0 ])
        myobj.compute(inputs, outputs)
        npt.assert_equal(outputs["brake_mass"], 42.0)
        npt.assert_equal(outputs["brake_cm"], 3)
    ...
    def testGeneratorSimple(self):
        ...
        myobj.compute(inputs, outputs)
        npt.assert_equal(outputs["R_generator"], 1.5)
        m = 37.68 * 10e3
        npt.assert_equal(outputs["generator_mass"], m)
        npt.assert_equal(outputs["generator_rotor_mass"], 0.5 * m)
        npt.assert_equal(outputs["generator_stator_mass"], 0.5 * m)
        npt.assert_equal(
        ...
        inputs["generator_mass_user"] = np.array([ 2.0 ])
        inputs["generator_radius_user"] = np.array([ 3.0 ])
        myobj.compute(inputs, outputs)
        npt.assert_equal(outputs["R_generator"], 3.0)
        npt.assert_equal(outputs["generator_mass"], 2.0)
        npt.assert_equal(outputs["generator_rotor_mass"], 1.0)
        npt.assert_equal(outputs["generator_stator_mass"], 1.0)
        npt.assert_equal(outputs["generator_rotor_I"], 0.5 * outputs["generator_I"])
        npt.assert_equal(outputs["generator_stator_I"], 0.5 * outputs["generator_I"])
    ...
    def testElectronics(self):
        ...
        inputs["converter_mass_user"] = np.array([ 42.0 ])
        inputs["transformer_mass_user"] = np.array([ 420.0 ])
        myobj.compute(inputs, outputs)
        npt.assert_almost_equal(outputs["converter_mass"], 42.0)
        npt.assert_equal(outputs["converter_cm"], np.r_[0.0, -(2.5 + 0.5 * s), 0.5 * s])
        npt.assert_almost_equal(outputs["converter_I"], (1.0 / 6.0) * 42 * s**2)
        npt.assert_equal(outputs["transformer_mass"], 420.0)
        npt.assert_equal(outputs["transformer_cm"], np.r_[0.0, -(2.5 + 0.5 * s), 0.5 * s])
    ...
    def testYaw(self):
        ...
        for k in inputs:
            inputs[k] = np.array( [inputs[k]] )
        myobj.compute(inputs, outputs)
        npt.assert_equal(outputs["yaw_mass"], 5e3 * np.pi * 0.1 * 5**2 * 0.2 + 190 * 12)
    ...
    def testMiscDirect(self):
        ...
        W = 1.1 * 2 * 2
        H = 1.1 * (2 + 4)
        npt.assert_equal(outputs["cover_mass"], 0.02 * 2e3 * 2 * (L * W + L * H + W * H))
        npt.assert_equal(outputs["cover_cm"], np.array([0.5 * (L - 5), 0.0, 0.5 * H]))

        npt.assert_equal(outputs["hvac_mass"], 0.1 * 10e3 * 2 * np.pi * 0.75 * 2)
        npt.assert_equal(outputs["hvac_cm"], 6.0)
        npt.assert_equal(outputs["hvac_I"], outputs["hvac_mass"] * 1.5**2 * np.r_[1.0, 0.5, 0.5])

        t = 0.04
        npt.assert_equal(outputs["platform_mass"], t * 3e3 * 12**2)
    ...
    def testMiscGeared(self):
        ...
        t = 0.04
        npt.assert_equal(outputs["platform_mass"], t * 3e3 * L * W)
        npt.assert_equal(outputs["platform_cm"], 0.0)
        npt.assert_equal(
            outputs["platform_I"],
    ...
    def testNacelle_noTilt(self):
        ...
        for k in components:
            inputs[k + "_mass"] = np.array([ 1e3 ])
            inputs[k + "_I"] = 1e3 * np.array([1, 2, 3])
            if k in cm3:
                inputs[k + "_cm"] = np.array([-5.0, 0.0, 2.0])
            else:
                inputs[k + "_cm"] = np.array([ 3.0 ])

        myobj.compute(inputs, outputs, discrete_inputs, discrete_outputs)
        npt.assert_equal(outputs["other_mass"], 1e3 * 6)
        npt.assert_equal(outputs["nacelle_mass"], 1e3 * (len(components) - 2))  # gen stator / rotor duplication
        ...
        myobj.compute(inputs, outputs, discrete_inputs, discrete_outputs)
        npt.assert_equal(outputs["other_mass"], 1e3 * 6)
        npt.assert_equal(outputs["nacelle_mass"], 1e3 * (len(components) - 2))
        npt.assert_equal(outputs["nacelle_cm"], np.r_[5.0, 0.0, 2.0])
        npt.assert_equal(outputs["nacelle_I"], 1e3 * (len(components) - 2) * np.r_[1.0, 2.0, 3.0, np.zeros(3)])
        ...
        myobj.compute(inputs, outputs, discrete_inputs, discrete_outputs)
        npt.assert_equal(outputs["other_mass"], 1e3 * 6)
        npt.assert_equal(outputs["nacelle_mass"], 1e3 * (len(components) - 4))
        npt.assert_equal(outputs["nacelle_cm"], np.r_[5.0, 0.0, 2.0])
        npt.assert_equal(outputs["nacelle_I"], 1e3 * (len(components) - 4) * np.r_[1.0, 2.0, 3.0, np.zeros(3)])

        inputs["above_yaw_mass_user"] = np.array([1e5])
        myobj.compute(inputs, outputs, discrete_inputs, discrete_outputs)
        npt.assert_equal(outputs["nacelle_mass"], 1e5+1e3)
    ...
    def testRNA(self):
        ...
        myobj.compute(inputs, outputs, discrete_inputs, discrete_outputs)
        npt.assert_equal(outputs["rotor_mass"], 125e3)
        npt.assert_equal(outputs["rna_mass"], 325e3)
    ...
    def testDynamics(self):
        ...
        npt.assert_equal(outputs["drivetrain_damping_coefficient"], 10)
