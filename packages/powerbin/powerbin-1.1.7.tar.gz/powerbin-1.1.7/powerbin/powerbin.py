    ...
    def __init__(
        ...
        if callable(capacity_spec):
            ...
            if capacity_spec.ndim != 1 or capacity_spec.shape[0] != npix:
                raise ValueError(f"If 'capacity_spec' is an array, it must have shape ({npix},), but got {capacity_spec.shape}")
...
#----------------------------------------------------------------------------
    ...
    def plot(
        ...
        ax1.plot(r, pixel_capacity, '.k', alpha=0.2, markeredgewidth=0, label='Input')
        if single.sum() > 0:
            ax1.plot(rad[single], bin_capacity[single], 'xb', markersize=3, label='Single')
        ax1.plot(rad[~single], bin_capacity[~single], 'or', markersize=4.2, markeredgewidth=0, label='Bins')
        ax1.plot(rad[~single], bin_capacity[~single], 'ok', markersize=1, markeredgewidth=0)
        ax1.axhline(target_capacity, linestyle='--', linewidth=1, color='gray')
        ax1.axis([-0.5, np.max(r), np.min(pixel_capacity), np.max(bin_capacity) * 1.5])
