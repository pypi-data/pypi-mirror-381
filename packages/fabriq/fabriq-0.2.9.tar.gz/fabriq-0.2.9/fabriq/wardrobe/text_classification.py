    ...
    def run(
        ...
        if text and len(categories) > 0:
            if multiclass:
                ...
                categories = self.llm.generate(prompt,response_schema=ResponseFormat_MultiClass.model_json_schema())
            else:
                prompt = self._singleclass_prompt.format(
                    text=text, categories=categories
                )
                categories = self.llm.generate(prompt,response_schema=ResponseFormat.model_json_schema())
