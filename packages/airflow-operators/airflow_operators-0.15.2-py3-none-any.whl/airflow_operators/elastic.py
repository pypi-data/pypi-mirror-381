...
class ElasticSaveOperator(ElasticsearchBulkOperator):
    ...
    def create_bulk(self, ti: TaskInstance) -> list[dict]:
        bulk = []
        item_jsons = ti.xcom_pull(key=self.xcom_key, map_index=ti.map_index)
...
class ElasticSaveS3Operator(ElasticsearchBulkOperator):
    ...
    def get_data(self, ti: TaskInstance) -> dict | list[dict]:
        if self.xcom_key:
            s3_key = ti.xcom_pull(key=self.xcom_key, map_index=ti.map_index)
        else:
            s3_key = ti.xcom_pull(key=self.xcom_template_key, map_index=ti.map_index).format(**self.xcom_params)
