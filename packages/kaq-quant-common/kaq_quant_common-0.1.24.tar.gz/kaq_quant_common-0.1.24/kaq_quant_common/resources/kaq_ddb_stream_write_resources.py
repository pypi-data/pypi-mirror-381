...
class KaqQuantDdbStreamWriteRepository:
    ...
    def __init__(self, host, port, user, passwd):
        ...
        try:
            ...
            self.session.connect(host, port, user, passwd, tryReconnectNums=10, reconnect=True, keepAliveTime=120, readTimeout=10, writeTimeout=10)
            ...
            logger.error(f'KaqQuantDdbStreamWriteRepository.__init__ is occured error: {str(e)} - {str(traceback.format_exc())}')
        finally:
            mutex.release()
    ...
    def save2stream(self, ddb_table_name: str, df : pd.DataFrame):
        ...
        for index, row in df.iterrows():
            ...
            try:
                ...
                logger.error(f'KaqQuantDdbStreamWriteRepository.save2stream is occured error: tableName is {ddb_table_name} - {str(e)} - {str(traceback.format_exc())}')
