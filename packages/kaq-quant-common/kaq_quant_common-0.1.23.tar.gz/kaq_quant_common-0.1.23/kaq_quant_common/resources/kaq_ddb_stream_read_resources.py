...
class KaqQuantDdbStreamReadRepository:
    ...
    def __init__(self, host, port, user, passwd):
        ...
        try:
            ...
            logger.error(f'KaqQuantDdbStreamReadRepository.__init__ is occured error: {str(e)} - {str(traceback.format_exc())}')
        finally:
            mutex.release()

    def getStreamQueryCount(self, query: str) -> int:
        '''
        获取数量, 再去获取总数据
        ...
        try:
            ...
            logger.error(f'KaqQuantDdbStreamReadRepository.getStreamQueryCount is occured error: {str(e)} - {str(traceback.format_exc())}')
        return -1
    ...
    def getStreamQuery(self, query: str) -> pd.DataFrame:
        '''
        从流数据表中获取数据
        '''
        try:
            ...
            logger.error(f'KaqQuantDdbStreamReadRepository.getStreamQuery is occured error: {str(e)} - {str(traceback.format_exc())}')
        return pd.DataFrame()
...
if __name__ == '__main__':
    ...
    # kaq.drop_streaming()
    count = kaq.getStreamQueryCount(f"select count(*) from kaq_binance_force_order_streaming where symbol == 'FIOUSDT'")
