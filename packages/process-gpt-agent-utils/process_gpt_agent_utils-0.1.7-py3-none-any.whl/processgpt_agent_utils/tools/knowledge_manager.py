...
def _apply_vecs_drop_if_exists_patch():
    ...
    def Patched_create_index(self, *args, **kwargs):
        # 항상 replace=False로 강제 (드롭 방지)
        kwargs["replace"] = False

        # 컬렉션명 기반 고유 advisory lock 키 (int32)
        # 스키마 분리 고려 시 schema.name 조합 권장
        lock_key = abs(zlib.crc32(f"vecs:{self.table.name}".encode()))

        with self.client.Session() as sess:
            # 선택: 과도 대기 방지 (필요 시 값 조정)
            sess.execute(sql_text("SET LOCAL lock_timeout = '10s'"))
            # 1) advisory lock 획득 (같은 키는 직렬화)
            sess.execute(sql_text("SELECT pg_advisory_lock(:k)"), {"k": lock_key})
            try:
                # 인덱스 캐시 무효화 후 최신 상태 확인
                try:
                    setattr(self, "_index", None)
                except Exception:
                    pass

                current_index = self.index  # DB에서 조회하여 채움
                # 2) 이미 인덱스가 있으면 조용히 스킵 (에러/드롭 없음)
                if current_index is not None:
                    return None

                # 3) 없을 때만 생성 (replace=False로 강제되어 있어 드롭 없음)
