...
def load_icon(name, size=(20, 20)):
    try:
        with res.files("chaeruldesktop.icons").joinpath(name).open("rb") as f:
            img = Image.open(f)
...
def inputView(
    ...
    if eyeIcon:
        ...
        try:
            eye_open = load_icon("eye.png")
            eye_closed = load_icon("crossed-eye.png")
        except Exception as e:
...
def appDesktop(title="App", width=400, height=600, body=[], on_ready=None, align='start'):
    input_ = {}
