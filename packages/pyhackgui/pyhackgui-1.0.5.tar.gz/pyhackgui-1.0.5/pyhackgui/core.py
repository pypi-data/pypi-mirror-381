...
class PyHackGUI:
    ...
    def toggle(self, name: str, pos: Tuple[int, int], checked: bool, text: str = "") -> bool:
        ...
        if checked:
            knob_x = x + w - radius  # 右侧位置
        else:
            knob_x = x + radius      # 左侧位置
        knob_center = (knob_x, y + radius)
        pygame.draw.circle(self.buffer_surface, self.style['toggle']['knob_color'], knob_center, knob_r)
    ...
    def radio_group(self, name: str, options: List[str], selected: str, pos: Tuple[int, int], item_spacing: int = 26) -> str:
        ...
        for i, opt in enumerate(options):
            ...
            # 外圈
            pygame.draw.circle(self.buffer_surface, self.style['radio']['outer_color'], (cx, cy), circle_r, self.scale_value(2))
            # 选中内点
            item_rect = pygame.Rect(cx - circle_r, cy - circle_r, circle_r*2, circle_r*2)
            hover = item_rect.collidepoint(self.mouse_pos)
            if selected == opt:
                pygame.draw.circle(self.buffer_surface, self.style['radio']['inner_color'], (cx, cy), circle_r-3)
            # 文本
    ...
    def draw_circle(self, center: Tuple[int, int], radius: int, color: Tuple[int, int, int, int], thickness: int = 1):
        ...
        if thickness <= 0:
            # 填充圆形，使用更好的抗锯齿
            pygame.draw.circle(self.buffer_surface, color, c, r)
        else:
            # 空心圆形，使用pygame.draw.circle获得更好的抗锯齿效果
