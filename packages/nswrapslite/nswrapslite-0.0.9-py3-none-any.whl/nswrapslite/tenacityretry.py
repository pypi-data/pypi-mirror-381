...
本模块提供以下核心功能：
- retry_wraps_tenacity：函数重试装饰器，同时支持同步和异步函数
...
aiohttpClientResponseError = aiohttp.ClientResponseError
aiohttpClientConnectorError = aiohttp.ClientConnectorError
aiohttpClientTimeout = aiohttp.ClientTimeout
...
# 重试相关常量
RETRYABLE_EXCEPTIONS = (gaierror, SSLError, SSLZeroReturnError, ConnectionError, Timeout, urllib3.exceptions.HTTPError)
...
class RetryHandler:
    ...
    def configure(self, default_return: Any | None = None, custom_message: str | None = None) -> None:
    ...
    def err_back(self, retry_state: RetryCallState) -> Any:
        ...
        if original_exception is None:
            original_exception = RuntimeError('重试失败但没有捕获到具体异常')

        # 确保传递给handle_exception的是Exception类型
        if isinstance(original_exception, Exception):
            return _handle_exception(exc=original_exception, default_return=self._default_return, custom_message=self._custom_message)
        # 对于非Exception类型的异常（如KeyboardInterrupt），直接抛出
...
def retry_wraps_tenacity(
    fn: Callable | None = None,
    max_attempts: int = DEFAULT_RETRY_ATTEMPTS,
    min_wait: float = DEFAULT_MIN_WAIT_TIME,
    max_wait: float = DEFAULT_MAX_WAIT_TIME,
    retry_exceptions: tuple[type[Exception], ...] = RetryHandler.RETRYABLE_EXCEPTIONS,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        custom_message = _get_function_location(func)
        retry_handler = RetryHandler()
        retry_handler.configure(default_return=default_return, custom_message=custom_message)
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            try:
                ...
                return _handle_exception(exc=exc, default_return=default_return, custom_message=custom_message)
