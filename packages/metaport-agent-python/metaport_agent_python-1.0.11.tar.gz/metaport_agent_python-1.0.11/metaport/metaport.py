...
class Metaport(object):
    ...
    def _determine_lock_status_from_constraint(self, constraint):
        ...
        # RANGE: Version ranges with operators (check first to avoid false positives)
        if re.match(r'^(\^|\~=|\~|>=|<=|>|<|!=|==.*\*|\*)', constraint):
            return 'RANGE'
        
        # LOCKED: Exact version starting with digits (e.g., "1.2.3", "2.0.1")
        # This should come after operator checks to avoid false positives
