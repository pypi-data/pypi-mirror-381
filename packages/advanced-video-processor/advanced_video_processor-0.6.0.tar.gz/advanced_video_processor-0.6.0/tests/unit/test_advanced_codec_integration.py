...
class TestAdvancedCodecIntegration:
    ...
    def test_av1_format_recognition(self):
        ...
        config = ProcessorConfig(output_containers=["mp4", "webm"], video_codecs=["av1"])
    ...
    def test_hevc_format_recognition(self):
        ...
        config = ProcessorConfig(output_containers=["mp4"], video_codecs=["hevc"])
        encoder = VideoEncoder(config)
    ...
    def test_config_validation_with_advanced_codecs(self):
        ...
        assert config.output_containers == ["mp4", "webm"]
        assert config.video_codecs == ["h264", "hevc", "av1"]
