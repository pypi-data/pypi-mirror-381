...
class TestAdaptiveStreamProcessor:
    ...
    def test_get_output_format_mapping(self):
        ...
        assert processor._get_output_format("hevc") == "mp4"  # Updated to match new implementation
        assert processor._get_output_format("av1") == "mp4"   # Updated to match new implementation
    ...
    def test_get_quality_preset_for_bitrate(self):
        ...
        assert processor._get_quality_preset_for_bitrate(400) == "low"       # < 500, so low
        assert processor._get_quality_preset_for_bitrate(500) == "medium"    # 500 >= 500, so medium
        assert processor._get_quality_preset_for_bitrate(1500) == "medium"   # < 2000, so medium
        assert processor._get_quality_preset_for_bitrate(2000) == "high"     # 2000 >= 2000, so high
        assert processor._get_quality_preset_for_bitrate(4999) == "high"     # < 5000, so high
        assert processor._get_quality_preset_for_bitrate(5000) == "ultra"    # >= 5000, so ultra
        assert processor._get_quality_preset_for_bitrate(10000) == "ultra"   # >= 5000, so ultra
    ...
    def test_get_ffmpeg_options_for_level(self):
        ...
        # Updated to match the new BitrateManager implementation
        assert "preset" in options
        assert "profile" in options
        assert "level" in options
        assert "pix_fmt" in options
        assert options["preset"] == "slow"  # Expected for 3000 bitrate
        assert options["profile"] == "high"  # Expected for h264
        assert options["level"] == "4.0"
        assert options["pix_fmt"] == "yuv420p"
    ...
    @pytest.mark.asyncio
    @patch("video_processor.streaming.adaptive_orchestrator.VideoContentAnalyzer")
    ...
    @pytest.mark.asyncio
    @patch("video_processor.streaming.adaptive_orchestrator.VideoProcessor")
    @patch("video_processor.streaming.adaptive_orchestrator.asyncio.to_thread")
    ...
    async def test_generate_thumbnail_track(self, mock_to_thread):
        ...
        with patch("video_processor.streaming.adaptive_orchestrator.VideoProcessor"):
