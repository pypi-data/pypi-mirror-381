...
from ..constants import ENCODING, VIDEO_360
...
class Video360StreamProcessor:
    ...
    async def _generate_360_bitrate_ladder(
        ...
        for level in base_levels:
            ...
            if hasattr(analysis.quality, "motion_intensity"):
                motion_multiplier = ENCODING["motion_multipliers"]["base"] + (
                    analysis.quality.motion_intensity * ENCODING["motion_multipliers"]["motion_factor"]
                )
    ...
    def _get_projection_bitrate_multiplier(self, projection: ProjectionType) -> float:
        ...
        return VIDEO_360["bitrate_multipliers"].get(
            projection.value, VIDEO_360["bitrate_multipliers"]["default"]
        )
    ...
    async def _generate_tiled_manifests(
        ...
        for level_name, _rendition_file in rendition_files.items():
