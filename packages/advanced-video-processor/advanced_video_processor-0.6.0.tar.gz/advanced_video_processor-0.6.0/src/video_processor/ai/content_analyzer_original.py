"""Computer vision-enhanced video content analysis using existing infrastructure."""
...
class VideoContentAnalyzer:
    """Computer vision-enhanced video content analysis leveraging existing infrastructure."""
    ...
    async def analyze_content(self, video_path: Path) -> ContentAnalysis:
        ...
        Builds on existing metadata extraction and adds computer vision-enhanced insights.
        """
    ...
    async def _analyze_scenes(self, video_path: Path, duration: float) -> SceneAnalysis:
        ...
        try:
            ...
            scene_threshold = SCENE_DETECTION["scene_threshold"]
            scene_filter = f"select='gt(scene,{scene_threshold})'"

            # Run scene detection
            process = (
                ffmpeg.input(str(video_path))
                .filter("select", f"gt(scene,{SCENE_DETECTION['scene_threshold']})")
                .filter("showinfo")
                .output("-", format="null")
            ...
            # Select key moments (offset into each scene)
            key_offset = SCENE_DETECTION["key_moment_offset"]
            max_moments = SCENE_DETECTION["max_key_moments"]
            key_moments = [
                boundary + (avg_length * key_offset)
                for boundary in scene_boundaries[:max_moments]
            ]

            # Add start if no boundaries
            if not key_moments:
                min_timestamp = THUMBNAILS["minimum_timestamp"]
                key_moments = [min(min_timestamp, duration * 0.2)]
    ...
    def _generate_fallback_scenes(self, duration: float) -> list[float]:
        ...
        short_threshold = SCENE_DETECTION["short_video_threshold"]
        medium_threshold = SCENE_DETECTION["medium_video_threshold"]
        ...
        if duration <= short_threshold:
            ...
            num_scenes = min(int(duration / short_threshold), max_scenes)
    ...
    def _fallback_scene_analysis(self, duration: float) -> SceneAnalysis:
        ...
        return SceneAnalysis(
            ...
            confidence_scores=[SCENE_DETECTION["fallback_confidence"]] * len(boundaries),
        )
    ...
    async def _detect_motion(self, video_path: Path, duration: float) -> dict[str, Any]:
        ...
        try:
            ...
            max_sample = MOTION_DETECTION["sample_duration"]
            sample_duration = min(max_sample, duration)
            ...
            motion_threshold = MOTION_DETECTION["threshold"]
            return {
                "has_motion": motion_data["intensity"] > motion_threshold,
                "intensity": motion_data["intensity"],
            }

        except Exception as e:
            logger.warning(f"Motion detection failed: {e}")
            # Conservative fallback - assume no motion when detection fails
            return {"has_motion": False, "intensity": 0.0}
    ...
    def _parse_motion_data(self, ffmpeg_output: str) -> dict[str, float]:
        ...
        batch_size = MOTION_DETECTION["frame_batch_size"]
        intensity = min(processed_frames / batch_size, 1.0)
    ...
    def _detect_360_video(self, probe_info: dict[str, Any]) -> bool:
        ...
        try:
            ...
            min_aspect = VIDEO_360["equirectangular_aspect_min"]
            max_aspect = VIDEO_360["equirectangular_aspect_max"]
    ...
    def _recommend_thumbnails(
        ...
        max_key_moments = 3  # Limit key moments for thumbnails
        recommendations.extend(scenes.key_moments[:max_key_moments])

        # Add beginning if video is long enough and quality is good
        min_duration = SCENE_DETECTION["short_video_threshold"]
        quality_threshold = QUALITY_THRESHOLDS["sharpness_good"]
        min_timestamp = THUMBNAILS["minimum_timestamp"]

        if duration > min_duration and quality.overall_quality > quality_threshold:
            recommendations.append(min(min_timestamp, duration * 0.1))

        # Add middle timestamp for longer videos
        long_video_threshold = THUMBNAILS["long_video_threshold"]
        ...
        max_recommendations = THUMBNAILS["max_recommendations"]
    ...
    async def _analyze_regional_motion(
        ...
        try:
            ...
            # Apply regional motion patterns based on typical content
            multipliers = VIDEO_360["regional_multipliers"]
            regional_motion = {
                region: base_intensity * multiplier
                for region, multiplier in multipliers.items()
            }

            # Apply reasonable regional variation based on typical content patterns
            # Front regions typically have more action than background regions
            regional_motion["front"] = min(1.0, regional_motion["front"] * 1.1)
            regional_motion["back"] = max(0.0, regional_motion["back"] * 0.9)

            # Vertical regions (up/down) typically have less motion
            regional_motion["up"] = max(0.0, regional_motion["up"] * 0.8)
            regional_motion["down"] = max(0.0, regional_motion["down"] * 0.8)
    ...
    def _identify_dominant_regions(
        ...
        motion_threshold = MOTION_DETECTION["threshold"] * 3  # Higher threshold for dominance
        dominant = [region for region, intensity in sorted_regions if intensity > motion_threshold][
            :3
        ]
