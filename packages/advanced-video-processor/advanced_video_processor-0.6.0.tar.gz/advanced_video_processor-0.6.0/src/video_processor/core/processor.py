...
try:
    from ..ai.video_transcriber import VideoTranscriber

    HAS_TRANSCRIPTION_SUPPORT = True
except ImportError:
    HAS_TRANSCRIPTION_SUPPORT = False
...
class VideoProcessingResult:
    ...
    def __init__(
        ...
        transcription_text: Path | None = None,
    ) -> None:
...
class VideoProcessor:
    ...
    def __init__(self, config: ProcessorConfig) -> None:
        ...
        # Initialize transcriber if available and enabled
        if HAS_TRANSCRIPTION_SUPPORT and config.enable_transcription:
    ...
    def process_video(
        ...
        try:
            ...
            encoded_files = {}
            for format_name in self.config.output_containers:
            ...
            if self.transcriber:
                try:
                    ...
                    if asyncio.iscoroutinefunction(self.transcriber.transcribe_video):
                        # We're in a sync context, so we need to run the async function
                        loop = None
                        try:
                        ...
                        transcription_result = loop.run_until_complete(
                            self.transcriber.transcribe_video(
                                ...
                                self.config.enable_ollama_enhancement,
                                self.config.transcription_domain_context,
                            )
                        )
                    else:
                    ...
                    if transcription_result:
                        transcription_json, transcription_text = self.transcriber.save_transcription_results(
                            transcription_result, output_dir
                        )
                except Exception as e:
                    # Log transcription error but don't fail the whole process
