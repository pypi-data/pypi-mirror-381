...
class TestL10nEsAeatMod182(TestL10nEsAeatModBase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        # Create model
        cls.model182 = cls.env["l10n.es.aeat.mod182.report"].create(
            {
                "name": "9990000000182",
        ...
        cls.invoice_3 = cls._invoice_sale_create(
            "2019-01-01", {"partner_id": cls.customer_3.id, "not_in_mod182": True}
        )
    ...
    def test_model_182(self):
        ...
        self.assertFalse(self.invoice_1.not_in_mod182)
        self.assertTrue(self.invoice_3.not_in_mod182)
        # Check model
        self.model182.button_calculate()
        partner_record_vals = [
            # key, partner, amount, 1T, 2T, 3T, 4T
            ("A", self.supplier, 3300, 3300, 0, 0, 0),
            ("A", self.supplier_2, -4400, -4400, 0, 0, 0),
            ("B", self.customer_2, 4400, 0, 4400, 0, 0),
            ("B", self.customer_4, 6600, 0, 0, 6600, 0),
            ("B", self.customer_6, -5500, -5500, 0, 0, 0),
            ("B", self.customer_5, 0, 0, 0, 0, 0),
        ]
        self.assertEqual(self.model182.total_partner_records, len(partner_record_vals))
        self.assertAlmostEqual(
            self.model182.total_amount, sum(x[2] for x in partner_record_vals)
        )
        for vals in partner_record_vals:
            partner_record = self.model182.partner_record_ids.filtered(
        ...
        export_config_xml_ids = [
            "l10n_es_aeat_mod182.aeat_mod182_main_export_config",
        ]
        for xml_id in export_config_xml_ids:
            export_config = self.env.ref(xml_id)
            self.assertTrue(export_to_boe._export_config(self.model182, export_config))
