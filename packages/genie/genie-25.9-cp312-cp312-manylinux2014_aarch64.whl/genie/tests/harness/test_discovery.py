...
class MockReportCall:
    def __init__(self, reporter, name):
        self.reporter = reporter
        self.name = name

    def __call__(self, *args, **kwargs):
        info = {'id': self.reporter.section.uid,
                'result': getattr(self.reporter.section, 'result', None)}
        self.reporter.calls.append(info)
...
class MockReporter:
    ...
    def __init__(self, section, parent=None):
        ...
        if not parent:
            self.calls = multiprocessing.Manager().list()
        
    def get_section(self):
        return getattr(self, 'dummy_dict', {})

    def __getattr__(self, name):
        if name.startswith('start') or name.startswith('stop'):
            return MockReportCall(self, name)
        if self.parent:
            return getattr(self.parent, name)

    def child(self, section):
        return MockReporter(section, self)
...
class TestTestScriptDiscovery(unittest.TestCase):
    ...
    triggers9 = {'TriggerShutNoShutBgp':
                    {'source': {'class': 'genie.tests.harness.script.triggers.TriggerShutNoShutBgp'},
                    'name': 'trigger', 'banana': 'yellow',
                    'groups': ['bgp'],
                    'devices': ['PE1'],
                ...
                'TriggerDummy':
                    {'source': {'class': 'genie.tests.harness.script.triggers.TriggerDummy'},
                    'groups': ['ospf'],
                    'name': 'test', 'var1': 'value',
                    'devices': ['PE1'],
                    },
                'order': ['TriggerShutNoShutBgp', 'TriggerDummy'],
                }
    ...
    triggers_order_dict_python_object = {
        'TriggerShutNoShutBgp': {
            'source': {
                'class': 'genie.tests.harness.script.triggers.TriggerShutNoShutBgp'
            },
            'name': 'trigger',
            'banana': sys,  # non pickable object
            'groups': ['L2'],
            'devices': ['PE1'],
        },
        'order': [{
    ...
    verf_fail = {
        ...
        'Verify_Fail': {
            'source': {'class': 'genie.tests.harness.script.verifications.a_failing_callable'},
            'name': 'trigger', 
            'var1': 'yellow',
            'devices': ['PE1'],
            'parallel_verifications': False
    ...
    parallel_verf_fail = {
        'Verify_1': {
            'source': {'class': 'genie.tests.harness.script.verifications.a_error_callable'},
            'name': 'trigger', 
            'var1': 'yellow',
            'devices': ['PE1', 'PE2'],
            'parallel_verifications': True
    ...
    alias_triggers = {'TriggerShutNoShutBgp':
                      {
                          'alias': 'T01',
                          'source': {'class': 'genie.tests.harness.script.triggers.TriggerShutNoShutBgp'},
                          'name': 'trigger', 'banana': 'yellow',
    ...
    @patch('genie.harness.base.runtime')
    @patch('genie.harness.discovery.runtime')
    def test_execution_results_fail_verification(self, mock_runtime_discovery, mock_runtime_base):
        self.script.verification_uids = Or('')
        self.script.trigger_uids = Or('')
        self.script.reporter = MockReporter(self.script)
        self.script.reporter.dummy_dict = {'result': results.Failed}
        ...
        # Testing synchro dict was not called for sequential runs
        self.assertFalse(mock_runtime_discovery.synchro.dict.called)
        self.assertFalse(mock_runtime_base.synchro.dict.called)
    ...
    def test_verification_parameters(self):
        ...
        veri1 = {
            'callable_verification': {
                'source': {
                    'class': 'genie.tests.harness.script.verifications.a_missing_arg_callable'},
                'name': 'trigger', 'var1': 'yellow',
                'groups': ['L2'],
                'devices': ['PE1'],
                'parameters': {'missing': 'None'}
            ...
            'VerifyOpsParameters':{
                'source': {
                    'class': 'genie.tests.harness.script.verifications.VerifyOpsWithParameters'},
                'some_args': 'trigger',
                'var1': 'yellow',
                'groups': ['L2'],
                'devices': ['PE1'],
        ...
        self.script.trigger_uids = Or('')
        self.script.reporter = MockReporter(self.script)
        self.script.reporter.dummy_dict = {'result': results.Passed}
        items = list(self.discoverer)
    ...
    def test_trigger_with_aetest_retry(self):
        # Get the instance of TriggerAetestRetry
        trigger = TriggerAetestRetry()
        uids = [trig.uid for trig in trigger.__retry__]
        expected_uids = ['TriggerAetestRetry [Retry 1]', 'TriggerAetestRetry [Retry 2]', 'TriggerAetestRetry [Retry 3]']
        assert uids == expected_uids, 'uids do not equal expected_uids'
    ...
    def test_extended_trigger_entrypoint(self, mock_entry_points):
        # Add triggers information to the script
        self.script.triggers = self.triggers_external_override
    ...
    @patch('genie.harness.base.runtime')
    @patch('genie.harness.discovery.runtime')
    def test_parallel_verifications(self, mock_runtime_discovery, mock_runtime_base):
        # Add triggers information to the script
        self.script.verification_uids = Or('')
        self.script.trigger_uids = Or('')
        ...
        self.script.subsection_data = self.subsections1
        self.script.parallel_verifications = True
        self.script.reporter = MockReporter(self.script)
        self.script.reporter.dummy_dict = {'result': results.Passed}
        ...
        self.assertTrue(mock_runtime_discovery.synchro.dict.called)
        self.assertTrue(mock_runtime_base.synchro.dict.called)
        
        # testing pre verifiers
        items[1]()
        verf1_pe1_result = sum([d['result']
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE1.1'])
        self.assertEqual(verf1_pe1_result, results.Passed)
        verf1_pe2_result = sum([d['result']
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE2.1'])
        self.assertEqual(verf1_pe2_result, results.Passed)
        
        verf2_pe1_result = sum([d['result']
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_2.PE1.1'])
        self.assertEqual(verf2_pe1_result, results.Passed)
        verf2_pe2_result = sum([d['result']
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_2.PE2.1'])
        self.assertEqual(verf2_pe2_result, results.Passed)
        self.assertEqual(items[1].result, results.Passed)

        # clear list for post verifiers
        self.script.reporter.calls[:] = []

        # testing post verifiers        
        items[3]()
        verf1_pe1_result = sum([d['result']
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE1.2'])
        self.assertEqual(verf1_pe1_result, results.Passed)
        verf1_pe2_result = sum([d['result']
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE2.2'])
        self.assertEqual(verf1_pe2_result, results.Passed)
        
        verf2_pe1_result = sum([d['result']
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_2.PE1.2'])
        self.assertEqual(verf2_pe1_result, results.Passed)
        verf2_pe2_result = sum([d['result']
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_2.PE2.2'])
        self.assertEqual(verf2_pe2_result, results.Passed)
    ...
    def test_triggers_order_dict_python_object(self):
        self.script.triggers = self.triggers_order_dict_python_object
        items = list(self.discoverer)

        self.assertEqual(items[1].uid, 'TriggerShutNoShutBgp.PE2')

    def test_verifications_fail(self):
        # Add triggers information to the script
        self.script.verification_uids = Or('')
        self.script.trigger_uids = Or('')
        self.script.triggers = self.trigger_sleep
        self.script.verifications = self.verf_fail
        self.script.subsection_data = self.subsections1
        self.script.reporter = MockReporter(self.script)
        self.script.reporter.dummy_dict = {'result': results.Failed}
        ...
        self.assertEqual(items[1].verifiers[0].uid, 'Verify_1.PE1.1')
        self.assertEqual(sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE1.1']), results.Passed)
        self.assertEqual(items[1].verifiers[1].uid, 'Verify_1.PE2.1')
        self.assertEqual(sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE2.1']), results.Passed)
        self.assertEqual(items[1].verifiers[2].uid, 'Verify_Fail.PE1.1')
        self.assertEqual(sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_Fail.PE1.1']), results.Failed)
        self.assertEqual(items[1].result, results.Failed)

    @patch('genie.harness.base.runtime')
    @patch('genie.harness.discovery.runtime')
    def test_parallel_verifications_fail(self, mock_runtime_discovery, mock_runtime_base):
        # Add triggers information to the script
        self.script.verification_uids = Or('')
        self.script.trigger_uids = Or('')
        self.script.triggers = self.trigger_sleep
    ...
    def test_verifications_error(self):
        # Add triggers information to the script
        self.script.verification_uids = Or('')
        self.script.trigger_uids = Or('')
        self.script.triggers = self.trigger_sleep
        self.script.verifications = self.parallel_verf_fail
        self.script.subsection_data = self.subsections1
        self.script.reporter = MockReporter(self.script)
        self.script.reporter.dummy_dict = {'result': results.Errored}
        ...
        self.assertEqual(sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE1.1']), results.Errored)
        self.assertEqual(items[1].verifiers[1].uid, 'Verify_1.PE2.1')
        self.assertEqual(sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE2.1']), results.Errored)
        self.assertEqual(items[1].verifiers[2].uid, 'Verify_Fail.PE1.1')
        self.assertEqual(sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_Fail.PE1.1']), results.Failed)
    ...
    @patch('genie.harness.base.runtime')
    @patch('genie.harness.discovery.runtime')
    def test_parallel_verifications_error(self, mock_runtime_discovery, mock_runtime_base):
        # Add triggers information to the script
        self.script.verification_uids = Or('')
        self.script.trigger_uids = Or('')
        self.script.triggers = self.trigger_sleep
        ...
        self.assertTrue(mock_runtime_discovery.synchro.dict.called)
        self.assertTrue(mock_runtime_base.synchro.dict.called)
        verf1_result = sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE1.1'])
        self.assertEqual(verf1_result, results.Errored)
        verf2_result = sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_1.PE2.1'])
        self.assertEqual(verf2_result, results.Errored)
        verf3_result = sum([d['result'] 
                            for d in self.script.reporter.calls 
                            if d['id'] == 'Verify_Fail.PE1.1'])
        self.assertEqual(verf3_result, results.Failed)
        self.assertEqual(items[1].result, results.Errored)

    def test_unloaded_triggers_exception(self):
        """
        Test that the discoverer raises a TriggerLoadException for triggers that fail to load.
        Verifies the trigger name, root cause, and relevant error message.
        """
        self.script.trigger_uids = And('TriggerDummy')
        # Add triggers information to the script
        self.script.triggers = self.triggers9
        self.script.subsection_data = self.subsections1
        # Verify that the discoverer raises the correct exception
        with self.assertRaises(TriggerLoadException) as cm:
            list(self.discoverer)  # Run the discoverer

        # Capture the exception instance
        exception = cm.exception

        # Assert that the exception contains the correct trigger name
        self.assertEqual(exception.name, 'TriggerDummy')

        # Assert that the root cause of the exception is an AttributeError
        self.assertIsInstance(exception.__cause__, AttributeError)

        # Optionally, verify the exception message (if relevant)
        self.assertIn("Couldn't find 'TriggerDummy'", str(exception.__cause__))

    def test_alias_triggers(self):
        self.script.trigger_uids = Or('T01')
        # Add triggers information to the script
        self.script.triggers = self.alias_triggers
        items = list(self.discoverer)
        for expected, item in zip([CommonSetup, TriggerShutNoShutBgp, CommonCleanup],
                                  items):
            self.assertTrue(isinstance(item, expected)) 
