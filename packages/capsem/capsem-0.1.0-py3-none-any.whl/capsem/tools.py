...
class ToolSet:
    """
    A collection of tools with utility methods for retrieval and filtering.
    """

    def __init__(self):
        self._tools: List[Tool] = []
        self._name2tool: Dict[str, Tool] = {}
        self._agent2tools: Dict[str, List[Tool]] = defaultdict(list)

    def add_tool(self, tool: Tool, agent_name: str):
        """
        Adds a tool to the ToolSet.

        Args:
            tool (Tool): The tool to add.
            agent_name (str): The name of the agent associated with the tool.
        """
        if tool.name in self._name2tool:
            warnings.warn(f"Tool with name {tool.name} already exists. Overwriting.")
        self._tools.append(tool)
        self._name2tool[tool.name] = tool
        self._agent2tools[agent_name].append(tool)

    def get_tools(self) -> List[Tool]:
        """
        Retrieves all tools in the ToolSet.
    ...
    def get_agent_tools(self, agent_name: str) -> List[Tool]:
        ...
        Returns:
            List[Tool]: A list of tools associated with the agent.
        """
        return self._agent2tools.get(agent_name, [])
    ...
    def get_tool_by_name(self, name: str) -> Optional[Tool]:
        ...
        """
        return self._name2tool.get(name)
    ...
    def get_tools_by_tag(self, tag: str) -> List[Tool]:
        ...
        Args:
            tag (str): The tag to filter tools by.

        Returns:
            List[Tool]: A list of tools that have the specified tag.
        """
        return [tool for tool in self._tools if tool.tags and tag in tool.tags]
