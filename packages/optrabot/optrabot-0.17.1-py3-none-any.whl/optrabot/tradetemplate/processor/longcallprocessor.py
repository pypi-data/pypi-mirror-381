		...
		if signalData and signalData.strike > 0:
			...
			if longStrikeData.delta:
				...
				try:
					...
					return None
			if longStrikeData.price:
				logger.debug(f'Selecting Strike based on price: {longStrikeData.price}')
				try:
					longStrike = self.get_strike_by_price("SPX", OptionRight.CALL, longStrikeData.price)
				except ValueError as value_error:
			...
			legs: List[Leg] = []
			legs.append(Leg(action=OrderAction.BUY, strike=longStrike, symbol=self._template.symbol, right=OptionRight.CALL, expiration=datetime.today().date(), quantity=1))
