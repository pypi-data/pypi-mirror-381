...
class IBTWSTConnector(BrokerConnector):
	...
	async def placeOrder(self, managed_trade: ManagedTrade, order: GenericOrder, parent_order: GenericOrder = None) -> None:
		...
		async with self._lock:
			...
			if not managed_trade.long_legs_removed and (order.price_effect == PriceEffect.CREDIT and order.price > 0) or (order.price_effect == PriceEffect.DEBIT and order.price < 0) or (managed_trade.template.is_credit_trade() and order.price > 0):
	...
	async def adjustOrder(self, managed_trade: ManagedTrade, order: GenericOrder, price: float) -> bool:
		...
		# For credit trades, the price must be negative
		assert order.price_effect is not None
		if not managed_trade.long_legs_removed and (order.price_effect == PriceEffect.CREDIT and order.price > 0) or (not managed_trade.template.is_credit_trade() and order.price_effect == PriceEffect.DEBIT and order.price < 0) or (managed_trade.template.is_credit_trade() and order.price > 0):
			price = -price
