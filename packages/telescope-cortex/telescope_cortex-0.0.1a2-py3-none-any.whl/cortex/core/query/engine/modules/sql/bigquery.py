...
class BigQueryGenerator(SQLQueryGenerator):
    ...
    def _apply_database_formatting(self, column_expression: str, object_name: str, formatting_map: FormattingMap) -> str:
        """Apply BigQuery-specific formatting to a column expression"""
        ...
        for format_def in formats:
            if format_def.mode == OutputFormatMode.IN_QUERY:
                current_expression = self._apply_bigquery_format(current_expression, format_def)
            
        return current_expression
    
    def _apply_bigquery_format(self, column_expression: str, format_def: OutputFormat) -> str:
        """Apply a single BigQuery format to a column expression"""
        if format_def.type == OutputFormatType.FORMAT:
            return self._apply_bigquery_string_format(column_expression, format_def)
        elif format_def.type == OutputFormatType.CAST:
            return self._apply_bigquery_cast(column_expression, format_def)
        elif format_def.type == OutputFormatType.CALCULATE:
            return self._apply_bigquery_calculate(column_expression, format_def)
        elif format_def.type == OutputFormatType.COMBINE:
            return self._apply_bigquery_combine(column_expression, format_def)
        else:
            return column_expression
    
    def _apply_bigquery_string_format(self, column_expression: str, format_def: OutputFormat) -> str:
        """Apply BigQuery string formatting using FORMAT_DATE, FORMAT_DATETIME, etc."""
        ...
        if format_type == FormatType.DATETIME:
            # Handle date/time formatting using FORMAT_DATETIME
            # Cast to DATETIME to ensure proper type handling, but use user's format string
            return f"FORMAT_DATETIME('{format_def.format_string}', CAST({column_expression} AS DATETIME))"
                
        elif format_type == FormatType.NUMBER:
            # Handle number formatting using FORMAT - cast to FLOAT64 for proper handling
            return f"FORMAT('{format_def.format_string}', CAST({column_expression} AS FLOAT64))"
                
        elif format_type == FormatType.CURRENCY:
            # Handle currency formatting - cast to FLOAT64 for proper handling
            return f"FORMAT('{format_def.format_string}', CAST({column_expression} AS FLOAT64))"
            
        elif format_type == FormatType.PERCENTAGE:
            # Handle percentage formatting - cast to FLOAT64 for proper handling
            return f"FORMAT('{format_def.format_string}', CAST({column_expression} AS FLOAT64))"
            ...
            if any(word in format_def.format_string.upper() for word in ['YYYY', 'MM', 'DD', 'HH', 'MI', 'SS']):
                ...
                return f"FORMAT('{format_def.format_string}', CAST({column_expression} AS STRING))"
            
        else:
            # Fallback to FORMAT with the format string
    ...
    def _apply_bigquery_cast(self, column_expression: str, format_def: OutputFormat) -> str:
        """Apply BigQuery CAST formatting"""
        if not format_def.target_type:
            return column_expression
            
        # Map target types to BigQuery CAST types
        sql_type_mapping = {
            "string": "STRING",
            "integer": "INT64", 
            "float": "FLOAT64",
        ...
        sql_type = sql_type_mapping.get(format_def.target_type, "STRING")
        return f"CAST({column_expression} AS {sql_type})"
    
    def _apply_bigquery_calculate(self, column_expression: str, format_def: OutputFormat) -> str:
        """Apply BigQuery calculation formatting"""
    ...
    def _apply_bigquery_combine(self, column_expression: str, format_def: OutputFormat) -> str:
        """Apply BigQuery combine formatting using CONCAT"""
