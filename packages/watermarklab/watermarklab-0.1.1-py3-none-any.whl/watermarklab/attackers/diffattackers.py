...
class Hue(BaseDiffAttackModel):
    ...
    def forward(self, marked_img: torch.Tensor, cover_img: torch.Tensor = None, now_step: int = 0) -> torch.Tensor:
        ...
        return self.test(marked_img, cover_img, hue_factor=factor)  # Call test with computed factor

    def test(self, marked_img: torch.Tensor, cover_img: torch.Tensor = None, hue_factor=None) -> torch.Tensor:
        """
        ...
        noised_img = Fs.adjust_hue(marked_img, hue_factor)
        return noised_img.clamp(0, 1)


class Crop(BaseDiffAttackModel):
    def __init__(self, crop_ratio_max=0.3, mode="constant_pad", constant: float = 0.0,
    ...
    def forward(self, marked_img: Tensor, cover_img: Tensor = None, now_step: int = 0) -> Tensor:
        ...
        crop_ratio = 1 - self.crop_ratio_max * (min(now_step, self.max_step) / self.max_step)
        return self.test(marked_img, cover_img, crop_ratio)

    def test(self, marked_img: Tensor, cover_img: Tensor = None, crop_ratio: float = None) -> Tensor:
        """
        ...
        if crop_ratio is None:
            crop_ratio = self.crop_ratio_max

        # Generate a random rectangle mask
        crop_mask = self.random_cropout_mask(marked_img, crop_ratio)
    ...
    def random_cropout_mask(self, marked_img: Tensor, crop_ratio: float) -> Tensor:
        ...
        Args:
            ...
            crop_ratio (float): The ratio of the image area that should remain visible.
        ...
        crop_ratio = 1 - crop_ratio  # Convert to ratio to be cropped out

        for i in range(N):
            num_pixels = int(H * W * crop_ratio)

            # Ensure we don't try to crop more than the image area
            num_pixels = min(num_pixels, H * W)

            if num_pixels == 0:
                continue  # No cropping needed

            # Calculate maximum possible width and height
            max_width = min(W, num_pixels)
            max_height = min(H, num_pixels)

            # Calculate minimum width and height to ensure valid range for random.randint
            min_width = min(1, max_width)
            min_height = min(1, max_height)

            # Ensure min_width <= max_width and min_height <= max_height
            if min_width > max_width:
                width = max_width
            else:
                width = random.randint(min_width, max_width)

            # Calculate height based on width and required area
            height = min(num_pixels // width, max_height)
            height = max(height, min_height)

            # Ensure width * height <= num_pixels
            if width * height > num_pixels:
                width = min(num_pixels, max_width)
                height = min(num_pixels // width, max_height)
...
class Cropout(BaseDiffAttackModel):
    ...
    def forward(self, marked_img: Tensor, cover_img: Tensor = None, now_step: int = 0) -> Tensor:
        ...
        remain_ratio = 1.0 - self.crop_ratio_max * min(now_step, self.max_step) / self.max_step
        noised_img = self.test(marked_img, cover_img, remain_ratio)
        return noised_img

    def test(self, marked_img: Tensor, cover_img: Tensor = None, remain_ratio: float = None) -> Tensor:
        """
        ...
        if remain_ratio is None:
            remain_ratio = self.crop_ratio_max

        crop_out_mask = self.random_cropout_mask(marked_img, remain_ratio)

        # Apply different filling methods based on mode
        if self.mode == "cover_pad" and cover_img is not None:
            noised_img = marked_img * crop_out_mask + (1 - crop_out_mask) * cover_img  # Use cover_img for padding
        else:
            noised_img = marked_img * crop_out_mask + (1 - crop_out_mask) * self.constant_value  # Use constant value for padding
