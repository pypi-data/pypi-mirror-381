...
import fbnconfig
from fbnconfig import Deployment, fund_accounting, property
...
def configure(env):
    ...
    perp_prop = fund_accounting.PerpetualProperty(
    ...
    perp_prop_two = fund_accounting.PerpetualProperty(
        property_key=property.DefinitionRef(
    ...
    return Deployment(deployment_name, [account, chart_of_account])
...
def test_teardown(setup_deployment):
    ...
    client = fbnconfig.create_client(lusid_env, token)
    # setup deployment
    fbnconfig.deploy(configure(setup_deployment), lusid_env, token)
...
def test_update_nochange(setup_deployment):
    fbnconfig.deploy(configure(setup_deployment), lusid_env, token)
    update = fbnconfig.deploy(configure(setup_deployment), lusid_env, token)
