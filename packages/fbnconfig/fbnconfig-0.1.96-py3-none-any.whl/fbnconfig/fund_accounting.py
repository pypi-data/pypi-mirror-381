...
def serialize_properties_as_dict(values: List[PerpetualProperty], info):
    style = info.context.get("style", "api") if info.context else "api"
    if style == "dump":
        return values
    property_dict = {}
...
def des_properties_dict(data, info) -> Sequence[PerpetualProperty] | Dict | None:
...
PerpetualPropertyDict = Annotated[
    Sequence[PerpetualProperty],
...
class ChartOfAccountsResource(CamelAlias, BaseModel, Resource):
    ...
    properties: PerpetualPropertyDict | None = None
