...
class Generator:
    ...
    EMOJIS = [e for e in list(emoji.EMOJI_DATA.keys()) if len(e) == 1]

    def __init__(self, adjectives: list = None, nouns: list = None,
                 use_number: bool = True, number_range: tuple = (1, 99),
    ...
    def generate_k_nickname(self) -> str:
        adj = random.choice(self.adjectives)
        noun = random.choice(self.nouns)
        nickname = f"{adj}{noun}"

        if self.use_number:
            picked_number = random.randint(*self.number_range)
            nickname = f"{nickname}{picked_number}"
        if self.use_emoji:
            picked_emoji = random.choice(self.EMOJIS)
            nickname = f"{nickname}{picked_emoji}"
        return nickname
