...
def add_parameters(func: Callable[..., T], udfParams: list[inspect.Parameter]):
    @wraps(func)
        ...
        if content_type == None or 'application/json' in content_type:
                ...
                if file == "parameters":
                    ...
                    for param in udfParams:
                        if param.name in json_data:
                            ...
                            kwargs[param.name] = UserDataFunctionInvalidInputError(properties={'parameter_type': param.annotation.__name__})
                else:
            ...
            for arg in udfParams:
                if arg.name not in kwargs:
                    kwargs[arg.name] = UserDataFunctionInvalidInputError(properties={'parameter_type': arg.name, 'parameter_value': "Parameter not found in request"})
