...
from typing import Any, Callable, get_type_hints, TypeVar, get_args, get_origin, Any
...
def prettify_arg_metadata(arg_metadata: ArgMetadata) -> str:
    ...
    if default == "":
        default = "''"
    return f"{name}: {arg_type}={default}"
...
class FuncMetadata:
    ...
    def as_pretty_str(self):
        ...
        pretty_res = self.result_type.__name__
...
def extract_and_validate_obj_methods_metadatas(obj):
    ...
    if metadatas != metadatas_i:
        ...
        metadatas_pretty = [md.as_pretty_str() for md in metadatas.values()]
