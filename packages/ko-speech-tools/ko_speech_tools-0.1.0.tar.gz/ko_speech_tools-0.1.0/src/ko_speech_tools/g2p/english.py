...
def convert_eng(string: str, cmu: CMUDict) -> str:
    ...
    for eng_word in eng_words:
        ...
        for i in range(len(phonemes)):
            ...
            # 2. 짧은 모음과 유음·비음([l], [r], [m], [n]) 이외의 자음 사이에
            #    오는 무성 파열음([p], [t], [k])은 받침으로 적는다.
            # 3. 위 경우 이외의 어말과 자음 앞의 [p], [t], [k]는 '으'를 붙여 적는다.

            if p in "PTK":
                if (p_prev[:2] in short_vowels and p_next == "$") or (
                    p_prev[:2] in short_vowels and p_next[0] not in "AEIOULRMN"
                ):  # 1
                    ret += to_jongseong(p)
            ...
            # 3. 어말 또는 자음 앞의 [ʒ]는 '지'로 적고, 모음 앞의 [ʒ]는 'ㅈ'으로 적는다.
                ...
                if p in ("S", "Z", "F", "V", "TH", "DH"):  # 1
                    ...
                    if p_next[0] in "$":
                        ...
                        ret += "Y"
                elif p == "ZH" and p_next[0] in syllable_final_or_consonants:  # 3
                    ret += "ᅵ"
            ...
            # 2. 어중의 [l]이 모음 앞에 오거나, 모음이 따르지 않는 비음([m], [n])
            #    앞에 올 때에는 'ㄹㄹ'로 적는다.
            ...
            # 중모음은 각 단모음의 음가를 살려서 적되, [ou]는 '오'로, [auə]는
            # '아워'로 적는다.
            elif p[0] in "AEIOU":
                ret += to_jungseong(p)
