...
class Transition:
    ...
    def __init__(
        ...
        clip_a: _VideoMedia,
        clip_b: _VideoMedia,
        # TODO: Maybe TransitionConfig (?)
        transition_clip: _VideoMedia,
        duration: float,
    ):
        ...
        """
        self._transition_clip: _VideoMedia = transition_clip
        """
        The transition clip to use in between the two
        clips we are joining.
        """
    ...
    def get_video_frame_at(
        ...
        def apply_blending(
            ...
            if mode == 'crossfade':
                return (1 - t_progress) * numpy_frame_a + t_progress * numpy_frame_b
            elif mode == 'wipe':
                # wipe horizontal
                w = numpy_frame_a.shape[1]
                mask = np.linspace(0, 1, w)[None, :, None]
                mask = (mask < t_progress).astype(np.float32)

                return numpy_frame_a * (1 - mask) + numpy_frame_b * mask
            elif mode == 'circle':
                # máscara circular simple
                h, w, _ = numpy_frame_a.shape
                Y, X = np.ogrid[:h, :w]
                cx, cy = w // 2, h // 2
