...
Synthesize news into actionable insights. Identify sentiment shifts and key catalysts.""",
        ...
        model="haiku",  # Use SDK model alias for speed and cost efficiency
    ),
}


# Agent tool access mapping for validation
# DISABLED: Causes 'method' object is not iterable error in v1.5.9
# AGENT_TOOL_MAPPINGS: Dict[str, List[str]] = {
#     agent_name: config.tools if config.tools else []
#     for agent_name, config in INVESTMENT_RESEARCH_AGENTS.items()
# }
AGENT_TOOL_MAPPINGS: Dict[str, List[str]] = {}  # Disabled - using Markdown agents instead


def get_agent_config(agent_name: str) -> AgentDefinition:
    """
