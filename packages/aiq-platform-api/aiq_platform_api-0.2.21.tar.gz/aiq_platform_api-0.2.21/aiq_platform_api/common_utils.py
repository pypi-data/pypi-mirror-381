...
class AssetUtils:
    ...
    @staticmethod
    def get_active_assets_with_details(client: AttackIQRestClient, limit: Optional[int] = 10) -> list:
        """Get active assets with OS and agent details."""
        params = {"status": AssetStatus.ACTIVE.value}
        assets = []

        for asset in AssetUtils.get_assets(client, params=params, limit=limit):
            assets.append(
                {
                    "id": asset.get("id"),
...
class ScenarioUtils:
    ...
    @staticmethod
    def search_scenarios(client: AttackIQRestClient, query: str, limit: Optional[int] = 20) -> list:
        """Search scenarios by keyword, MITRE technique ID, or tag."""
        logger.info(f"Searching scenarios with query: '{query}', limit: {limit}")
        params = {"search": query}
        scenarios = list(ScenarioUtils.list_scenarios(client, params=params, limit=limit))
        logger.info(f"Found {len(scenarios)} scenarios matching '{query}'")
        return scenarios
