# coding: utf-8

"""
    Hostinger API Python SDK

    API Version: 0.1.16

    NOTE: This file is auto-generated, DO NOT EDIT THIS FILE MANUALLY!
    If you want to contribute or request a new feature, please create an issue or pull request on https://github.com/hostinger/api
"""  # noqa: E501


import unittest

from hostinger_api.models.billing_v1_order_store_request import BillingV1OrderStoreRequest

class TestBillingV1OrderStoreRequest(unittest.TestCase):
    """BillingV1OrderStoreRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> BillingV1OrderStoreRequest:
        """Test BillingV1OrderStoreRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `BillingV1OrderStoreRequest`
        """
        model = BillingV1OrderStoreRequest()
        if include_optional:
            return BillingV1OrderStoreRequest(
                payment_method_id = 517244,
                items = [
                    hostinger_api.models.billing_v1_order_store_request_items_inner.Billing_V1_Order_StoreRequest_items_inner(
                        item_id = 'hostingercom-vps-kvm2-usd-1m', 
                        quantity = 1, )
                    ],
                coupons = [
                    ["Coupon 3"]
                    ]
            )
        else:
            return BillingV1OrderStoreRequest(
                payment_method_id = 517244,
                items = [
                    hostinger_api.models.billing_v1_order_store_request_items_inner.Billing_V1_Order_StoreRequest_items_inner(
                        item_id = 'hostingercom-vps-kvm2-usd-1m', 
                        quantity = 1, )
                    ],
        )
        """

    def testBillingV1OrderStoreRequest(self):
        """Test BillingV1OrderStoreRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
