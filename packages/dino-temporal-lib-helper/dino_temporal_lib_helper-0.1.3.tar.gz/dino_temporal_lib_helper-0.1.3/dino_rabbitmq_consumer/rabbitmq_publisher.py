...
class RabbitMQPublisher:
    ...
    async def publish_message(self, message_type: str, message_data: Dict[str, Any], source='api', id_message: str = None, priority: int = 1) -> bool:
        """
    ...
    async def publish_batch(self, messages: list, message_ids: list = [], source='api') -> Dict[str, Any]:
        ...
        for i, (msg, message_id) in enumerate(zip_longest(messages, message_ids, fillvalue=None)):
            try:
                ...
                success = await self.publish_message(message_type, message_data, priority=priority, id_message=message_id, source=source)
                if success:
                    stats["successful"] += 1
                else:
                    stats["failed"] += 1
