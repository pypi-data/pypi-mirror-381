...
class ETLDuckDBHandler:
    ...
    def __init__(
        ...
        self.logger: logging.Logger = _init_logger()
    ...
    def _resolve_jsonl_path(
        ...
        """
        base = _output_path(self.config, adapter, step, substep, entity, filename, "")
    ...
    def ingest_jsonl(
        ...
        """
        obj_label = (entity.value if isinstance(entity, Enum) else entity) or label or "<unknown>"
        src = self._resolve_jsonl_path(adapter, step, entity, filename, substep)

        if src is None:
            msg = f"No JSONL file found for entity {obj_label} (adapter={adapter}, step={step})."
        ...
        self.logger.info(f"Ingesting JSONL into DuckDB: entity={obj_label} -> table={tname} (file={src})")
        ...
        if cast_map:
            ...
            cast_projection = ", ".join(cast_exprs + ["sub.*"]) if cast_exprs else "sub.*"
        ...
        if create_mode == "replace":
            ...
            rowcount = self._count_rows(tname)
    ...
    def export_table(
        ...
        *,
        ...
        """
        if not self._table_exists(_safe_table_name(table_name)) and not self._table_exists(table_name):
            raise ValueError(f'DuckDB table does not exist: "{table_name}"')

        # Build SELECT
        ident = table_name if self._table_exists(table_name) else _safe_table_name(table_name)
        ...
        res = self.con.execute(sql)
        col_names = [d[0] for d in res.description]  # tuples like (name, type_code, ...)
        ...
        self.logger.info(f'Exported table "{table_name}" to JSONL at {out_path} with {total:,} records.')

        # If you want the exact total without recounting during the loop (for speed),
        # you can compute it via DuckDB afterwards:
        try:
            total = int(self.con.execute(f'SELECT COUNT(*) FROM {_quote_ident(ident)};').fetchone()[0])
        except Exception:
    ...
    def to_parquet(self, table: str, path: str | Path, overwrite: bool = True) -> None:
        dest = str(path)
        if overwrite and Path(dest).exists():
            Path(dest).unlink()
        self.con.execute(f"COPY {_quote_ident(table)} TO ? (FORMAT PARQUET);", [dest])

    def to_csv(self, table: str, path: str | Path, overwrite: bool = True) -> None:
        dest = str(path)
        if overwrite and Path(dest).exists():
            Path(dest).unlink()
        self.con.execute(f"COPY {_quote_ident(table)} TO ? (HEADER, DELIMITER ',');", [dest])