"""
HubSpot ETL Configuration Models.

This module defines Pydantic models for configuring the HubSpot ETL pipeline.
...
class TableHubSpot(BaseModel):
    """
    Configuration model for individual HubSpot tables.
    
    This model defines the configuration structure for each table in the HubSpot
    system that will be processed by the ETL pipeline. Each table configuration
    includes identification and control parameters.
    
    Attributes:
        GUID (UUID): Unique identifier for the table in the HubSpot system.
...
class ExtractHubSpot(BaseModel):
    """
    Configuration model for the extraction phase of HubSpot ETL.
    ...
    Attributes:
        tables (Dict[str, TableHubSpot]): Dictionary mapping table names 
...
class PipelineHubSpot(BaseModel):
    """
    Comprehensive configuration model for the entire HubSpot ETL pipeline.
    ...
    Attributes:
        ...
        extract (ExtractHubSpot): Configuration for the extraction phase.
