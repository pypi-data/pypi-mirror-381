"""
RepPL ETL Configuration Models.

This module defines Pydantic models for configuring the RepPL ETL pipeline.
...
class TableRepPL(BaseModel):
    """
    Configuration model for individual RepPL tables.
    
    This model defines the configuration structure for each table in the RepPL
    system that will be processed by the ETL pipeline. Each table configuration
    includes identification and control parameters.
    
    Attributes:
        GUID (str): Unique identifier for the table in the RepPL system.
...
class ExtractRepPL(BaseModel):
    """
    Configuration model for the extraction phase of RepPL ETL.
    ...
    Attributes:
        tables (Dict[str, TableRepPL]): Dictionary mapping table names 
                                                                to their configuration models.
                                                                Defaults to empty dict.
    """
    tables: Dict[str, TableRepPL] = Field(default_factory=dict)


class PipelineRepPL(BaseModel):
    """
    Comprehensive configuration model for the entire RepPL ETL pipeline.
    ...
    Attributes:
        ...
        extract (ExtractRepPL): Configuration for the extraction phase.
    ...
    extract: ExtractRepPL = Field(default_factory=ExtractRepPL)
...
TABLE_MODEL = TableRepPL
PIPELINE_MODEL = PipelineRepPL
