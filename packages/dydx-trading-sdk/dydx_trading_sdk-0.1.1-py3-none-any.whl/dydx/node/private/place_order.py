...
class PlaceOrder(PrivateNodeMixin):
  ...
  def build_order(
    ...
    return mkt.order(
      ...
      size=order['size'], # type: ignore
      price=order.get('price', 0), # type: ignore
      time_in_force=parse_tif(order.get('time_in_force')),
      reduce_only=order.get('reduce_only', False),
      post_only=order.get('post_only', False),
      good_til_block=good_til_block, # type: ignore
      good_til_block_time=good_til_block_time, # type: ignore
    )

  async def build_order_now(self, market: PerpetualMarket, order: Order):
    if (gtb := order.get('good_til_block')) is None and order.get('flags') == 'SHORT_TERM':
  ...
  async def place_order(self, market: PerpetualMarket, order: Order, *, unsafe: bool = False) -> OrderResponse:
    order_proto = await self.build_order_now(market, order)
    tx = cast(BroadcastTxResponse, await self.node_client.place_order(self.wallet, order_proto))
