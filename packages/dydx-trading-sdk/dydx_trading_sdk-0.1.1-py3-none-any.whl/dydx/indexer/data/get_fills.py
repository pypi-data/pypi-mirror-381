...
from dydx.core.types import OrderSide, FillType, Liquidity
...
class GetFills(IndexerMixin):
  ...
  async def get_fills_paged(
    ...
    page = 1
    while True:
      r = await self.get_fills(address, subaccount=subaccount, ticker=ticker, end_height=end_height, end=end, limit=limit, page=page, unsafe=True, validate=validate)
      if not r['fills']:
        break
      yield r['fills']
      page += 1