...
class CancelOrder(PrivateNodeMixin):
  async def cancel_order(
    ...
    if good_til_block_time is None and order_id.order_flags == OrderFlags.LONG_TERM:
      good_til_block_time = int(time.time()) + 3600*24
    
    if good_til_block is None and order_id.order_flags == OrderFlags.SHORT_TERM:
      latest_block = await self.node_client.latest_block()
      good_til_block = latest_block.block.header.height + 20
    
    r: BroadcastTxResponse = await self.node_client.cancel_order(self.wallet, order_id, good_til_block=good_til_block, good_til_block_time=good_til_block_time) # type: ignore
