...
class ExampleApi(Controller):
    ...
    async def get_example(self, req: Request, example_id: int) -> Response[ResponseModel]:
        ...
        example: Example = await Example.query().filter_by(id=example_id).first()
        if example is None:
    ...
    @produces(MediaType.APPLICATION_JSON)
    async def create_example(self, req: Request, data: CreateExample) -> Response[CreateExample]:
