...
class CerilloBasePlateReader(ABC):
    ...
    def __init__(self, port: str = '/dev/ttyUSB0', baudrate: int = 250000,
                 timeout: float = 30.0, simulate: bool = False):
        self.port = port
        ...
        try:
            ...
            while (self.serial_number == None):
                ...
                if elapsed > self.timeout:
                    raise TimeoutError(
                        f"Device startup timed out after {self.timeout} seconds waiting for response with ID {command_id}")
    ...
    def send_command(self, command: CDCLCommand, timeout: int = None) -> CDCLResponse:
        ...
        try:
            ...
            if not CDCLParser.is_cdcl_response(response):
                raise TypeError("response is not a valid CDCL response")
    ...
    def read_absorbance(self, wavelength: int, **kwargs) -> Dict[str, Any]:
        """Read absorbance at specified wavelength"""
        if self.simulate:
            print("[SIMULATION] Read absorbance")

        # TODO get wavelength index from shadow
