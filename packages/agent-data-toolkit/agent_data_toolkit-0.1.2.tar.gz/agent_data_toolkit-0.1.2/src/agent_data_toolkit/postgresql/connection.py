...
def _import_psycopg_pool():
    """
    Return psycopg_pool.ConnectionPool if available; otherwise None.
...
def _is_localhost_host(dsn: str) -> bool:
    ...
    We intentionally do not parse unix-socket paths here; this is TCP host only.
    """
    try:
        u = urlparse(dsn)
...
def dsn_from_env(*, require_ssl: bool = False, default_sslmode: str = "require") -> str:
    ...
    if not dsn:
        raise RuntimeError(
            "Set PG_DSN / POSTGRES_DSN / DATABASE_URL in the kernel environment."
        )
    return (
        _ensure_sslmode(dsn, default_sslmode, enforce_minimum=require_ssl)
        if require_ssl
        else dsn
    )
...
class ConnectionInfo:
    ...
    session_settings: dict[str, str | int] | None = None
...
class ConnectionManager:
    ...
    - Applies session settings (statement_timeout, search_path,
      application_name, extra session_settings).
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            connect_backoff_base: Initial sleep in seconds; doubles each retry.
                0 disables sleeping.
            ssl_fallback_on_error: If True and a connect attempt fails,
                try again once with sslmode=disable. This is **insecure**
                and should be used for local dev only.
            ssl_fallback_allow_any_host: By default, fallback is allowed only
                for localhost/127.0.0.1. Set True to allow fallback for
        ...
        if use_pool:
            ConnectionPool = _import_psycopg_pool()
            ...
            try:
                ...
                self.logger.info(
                    "Initialized connection pool (min=%d, max=%d)",
                ...
                if self._should_attempt_ssl_fallback(info.dsn):
                    try:
                        ...
                        self.logger.warning(
                            "Initialized connection pool WITHOUT SSL after "
                            "failure. Host is local and "
    ...
    def from_env(
        ...
        **kwargs,
        ...
        Args:
            ...
            min_sslmode: Minimum sslmode to enforce when force_ssl=True (default: 'require').
    ...
    def healthcheck(self, *, timeout: int = 5) -> dict:
        """
        Run a simple 'SELECT 1' to verify connectivity.

        Args:
            timeout: Temporary per-connection statement timeout (seconds).
        ...
        try:
            with self.connection() as conn, conn.cursor() as cur:
                # Apply a short-lived timeout just for this check
    ...
    def __repr__(self) -> str:
        # Redact DSN details in logs
        return (
            f"{self.__class__.__name__}("
            f"dsn={_redact_dsn(self.info.dsn)}, "
            f"use_pool={'yes' if self._pool else 'no'}, "
