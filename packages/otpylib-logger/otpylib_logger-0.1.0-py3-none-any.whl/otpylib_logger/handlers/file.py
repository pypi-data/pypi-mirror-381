...
async def init(config: Dict[str, Any]):
    ...
    print(f"[file_handler] Initialized (path={path})")
    
    # Register with logger manager
    my_pid = process.self()
    await process.send(LOGGER, ("add_handler", my_pid))
    print(f"[file_handler] Registered with logger manager")
...
async def terminate(reason, state):
    ...
    if state.get("file"):
        state["file"].close()
        print(f"[file_handler] Closed file {state['path']}")
...
async def start_link(config: Dict[str, Any]):
    ...
    print(f"[file_handler] start_link called with config: {config}")
    pid = await gen_server.start(
        callbacks,
        config,
        name=atom.ensure("handler_file")
    )
    print(f"[file_handler] GenServer started with PID: {pid}")
    return pid
