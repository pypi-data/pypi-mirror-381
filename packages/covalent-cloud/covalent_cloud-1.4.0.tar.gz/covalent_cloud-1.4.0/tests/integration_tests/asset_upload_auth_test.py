...
class TestAssetUploadAuthIntegration:
    ...
    def test_function_serve_upload_includes_auth_headers_integration(self, mock_asset_api_client_class):
    ...
    def test_auth_headers_consistent_between_modules(self):
        ...
        headers1 = client.get_upload_headers(test_url)
        headers2 = client.get_upload_headers(test_url)
        ...
        has_bearer_auth = "Authorization" in headers1 and headers1["Authorization"].startswith("Bearer")
        has_api_key_auth = "x-api-key" in headers1
        ...
        s3_headers = client.get_upload_headers(s3_url)
    ...
    def test_auth_header_types_with_different_settings(self):
        ...
        dr_headers = dr_client.get_upload_headers(test_url)
        ...
        api_key_headers = api_key_client.get_upload_headers(test_url)

        # Should contain either Bearer token or API key auth
        has_bearer_auth = "Authorization" in api_key_headers and api_key_headers["Authorization"].startswith("Bearer")
        has_api_key_auth = "x-api-key" in api_key_headers

        assert has_bearer_auth or has_api_key_auth, f"API key settings should produce auth headers but got: {api_key_headers}"
    ...
    def test_s3_url_detection_patterns(self):
        ...
        for url in s3_presigned_urls:
            assert client.is_s3_presigned_url(url), f"Expected {url} to be detected as S3 presigned URL"
            assert client.get_upload_headers(url) == {}, f"Expected no auth headers for S3 URL: {url}"

        # Test non-S3 URL detection
        for url in non_s3_urls:
            assert not client.is_s3_presigned_url(url), f"Expected {url} to NOT be detected as S3 presigned URL"
            headers = client.get_upload_headers(url)
    ...
    def test_non_s3_url_still_gets_auth_headers(self):
        ...
        headers = client.get_upload_headers(non_s3_url)
        ...
        assert has_bearer_auth or has_api_key_auth, f"Non-S3 URL should have auth headers but got: {headers}"

        # Verify it's not detected as S3
        assert not client.is_s3_presigned_url(non_s3_url), "Non-S3 URL was incorrectly detected as S3"