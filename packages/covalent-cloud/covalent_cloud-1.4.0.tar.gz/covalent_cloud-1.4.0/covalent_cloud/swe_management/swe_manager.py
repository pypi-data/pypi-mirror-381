...
from ..shared.classes.exceptions import CovalentAPIKeyError, CovalentGenericAPIError, CovalentSDKError
            ...
            with open(definition_file.name, "rb") as def_file_reader:
                ...
                if base_image:
                    image_config = EnvironmentRuntimeConfig(
                        image=BaseImageConfig(base_image=base_image, nvidia=nvidia)
                        if base_image
                        else None
                    )
...
def _get_envs_filtered(settings: Settings = None, **filters) -> Dict[str, Environment]:
    """
