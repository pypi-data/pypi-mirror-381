...
class AssetAPIClient:
    ...
    def is_s3_presigned_url(self, url: str) -> bool:
        ...
        try:
            ...
            s3_presigned_params = ['X-Amz-Signature', 'X-Amz-Algorithm', 'X-Amz-Credential']
            has_presigned_params = any(param in query_params for param in s3_presigned_params)

            return has_presigned_params

        except Exception:
            # If URL parsing fails, assume it's not S3
            return False

    def get_upload_headers(self, url: str) -> dict:
        """
        Get appropriate headers for uploading to the given URL.
    ...
    def upload_asset(self, url: str, data: bytes, additional_headers: dict = None) -> requests.Response:
        """
        ...
        """
        headers = self.get_upload_headers(url)
