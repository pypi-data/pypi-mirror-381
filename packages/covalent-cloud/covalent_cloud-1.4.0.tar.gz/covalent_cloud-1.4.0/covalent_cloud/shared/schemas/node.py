...
class NodeResult(BaseModel):
    ...
    def __str__(self) -> str:
        result_preview = str(self.result)[:30] + "..." if self.result and len(str(self.result)) > 30 else self.result
...
class NodeFailure(BaseModel):
    ...
    def __str__(self) -> str:
        # Format timestamps
        started_str = self.started_at.strftime("%Y-%m-%d %H:%M:%S") if self.started_at else "Unknown"
