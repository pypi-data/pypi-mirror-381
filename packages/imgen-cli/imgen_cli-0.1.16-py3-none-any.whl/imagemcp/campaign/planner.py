...
def normalize_plan_campaign_routes_payload(
    ...
    for raw in route_payloads:
        if not isinstance(raw, dict):
            raise TypeError("route entries must be mappings")
    ...
    for raw in placement_payloads:
        if not isinstance(raw, dict):
            raise TypeError("placement entries must be mappings")
...
def plan_campaign_routes(payload: PlanCampaignRoutesPayload) -> Dict[str, object]:
    ...
    notes = [
        "Ensure the CLI is installed (pipx install imgen-cli) before executing the plan.",
