...
class StockAnalyzer:
    ...
    def save_chart_to_file(self, df: pd.DataFrame, symbol: str, interval: str, patterns: Dict[str, List[Dict]] = None) -> str:
        """保存K线图到临时文件并返回文件路径"""
        try:
            # 创建临时目录
    ...
    async def analyze_stock(self, symbol: str, interval: str = "1d") -> Dict:
        ...
        # 生成图表文件路径
        chart_path = self.save_chart_to_file(df, symbol, interval, candle_patterns)
...
def _format_analysis(analysis: Dict) -> str:
    ...
    if analysis.get('chart_path'):
        result.append(f"📊 **K线图已保存到:** {analysis['chart_path']}")
        result.append("💡 您可以在文件管理器中查看此图片")
        result.append("")
    elif analysis.get('chart_image'):
...
@mcp.tool()
async def get_chart_as_base64(symbol: str, interval: str = "1d") -> str:
    """获取股票K线图的base64编码
