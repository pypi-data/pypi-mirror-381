...
if True:
    ...
    def legend(label=None ,ax=None, fontsize=None, color="fg", frameon=True, framealpha=1, facecolor="bg", edgecolor="fg", edgewidth=None, title=None, fontsize_title=None, **kwargs):
        if edgewidth is None: edgewidth=Linewidth().get()
        if fontsize is None: fontsize=_GetFontSize_("legend")
        if fontsize_title is None: fontsize_title=_GetFontSize_("legend") * 1.2
        global _ax_
        if ax is None: ax = _ax_
        if label is None:
            if 'zorder' in kwargs:
        ...
        if title is not None:
            ...
            LG.get_title().set_fontproperties(fm.FontProperties(fname=FontFamily().get("default"), size=fontsize_title))
        return ax
