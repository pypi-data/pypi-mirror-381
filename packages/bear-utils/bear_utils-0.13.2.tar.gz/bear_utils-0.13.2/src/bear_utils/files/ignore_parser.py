...
class IgnoreContainer:
    ...
    def add(self, path: Path, ignored: bool = True) -> None:
        """Add a path to the appropriate list based on whether it's ignored.
...
class IgnoreHandler:
    ...
    def __init__(self, ignore_file: Path | None = None, combine: bool = False, verbose: bool = False) -> None:
        ...
        self.logger: BearLogger = BearLogger(level=LogLevel.VERBOSE if verbose else LogLevel.INFO)
...
class IgnoreDirectoryHandler(IgnoreHandler):
    ...
    def _scan_directory(directory: Path | str, spec: PathSpec) -> tuple[list[Path], list[Path]]:
        ...
        non_ignored: list[Path] = [Path(f) for f, ig in zip(all_paths, ignored_status, strict=False) if not ig]
        ignored_paths: list[Path] = [Path(f) for f, ig in zip(all_paths, ignored_status, strict=False) if ig]
        return non_ignored, ignored_paths
    ...
    def ignore_report_full_codebase(self) -> None:
        ...
        """
        paths: tuple[list[Path], list[Path]] = self._scan_directory(self.directory_to_search, self.spec)
        self.files.non_ignored = paths[0]
        self.files.ignored = paths[1]
