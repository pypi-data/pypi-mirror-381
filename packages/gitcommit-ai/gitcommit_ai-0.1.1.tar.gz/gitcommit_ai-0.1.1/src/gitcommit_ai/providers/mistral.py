...
class MistralProvider(AIProvider):
    ...
    def __init__(self, api_key: str | None = None, model: str = "mistral-small"):
        """Initialize Mistral provider.

        Args:
            api_key: Mistral API key (or reads from MISTRAL_API_KEY env).
            model: Model to use (mistral-tiny, mistral-small, mistral-medium).
        """
    ...
    def validate_config(self) -> list[str]:
        ...
        errors = []
        if not self.api_key:
            errors.append("Mistral API key not found. Set MISTRAL_API_KEY environment variable.")
        return errors
    ...
    async def generate_commit_message(self, diff: GitDiff) -> CommitMessage:
        ...
        if validation_errors:
            raise RuntimeError(f"Mistral validation failed: {'; '.join(validation_errors)}")
        ...
        try:
            ...
            # Extract message (OpenAI-compatible)
            ...
            if e.response.status_code == 401:
                ...
                raise RuntimeError("Mistral API rate limit exceeded")
            else:
