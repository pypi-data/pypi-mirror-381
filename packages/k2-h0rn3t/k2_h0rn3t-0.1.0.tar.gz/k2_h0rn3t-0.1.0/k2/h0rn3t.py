...
class h0rn3t:
    ...
    def __init__(self, sites_file, paths, check_texts, threads=10, output_file="BADS_OK.txt", timeout=6, verify_ssl=False):
        ...
        try:
            ...
            for keyword in self.check_texts:
                if keyword and keyword in low:
                    ...
                    print(f"{fw}BADS : {url} {fg}--> VULN")
                    return True
            # not vulnerable (or keyword not found)
            print(f"{fw}BADS : {url} {fr}--> NO")
            return False
            ...
            try:
                for fut in as_completed(future_to_target):
                    try:
                        ...
                        print(f"{fr}Error checking {site}/{path}: {e}")
