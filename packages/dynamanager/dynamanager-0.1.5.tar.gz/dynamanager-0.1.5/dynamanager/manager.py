...
class Dynamanager:
    ...
    def _resolve_path(self, file_path: Union[str, Path]) -> Path:
        ...
        logger.trace("Resolved file path", arg=file_path, resolved=path)
    ...
    def _add_config(
        ...
        **dynaconf_kwargs: Union[bool, list[str], str, dict[str, Any], Path, None],
        ...
        try:
            logger.trace("Instatiating Dynaconf object", dynaconf_args=load_kwargs)
            ...
            if not Path(config_file).exists():
                logger.debug("Creating settings TOML file", domain_name=named_attribute)
    ...
    def _save_config(self, data: dict[str, Any], path: Path) -> None:
        logger.trace(
            "Assuring configuration path accessibility in filesystem", filepath=path
        )

        path.parent.mkdir(parents=True, exist_ok=True)

        backup_extenstion = ".toml.bak"
        backup_file = path.with_suffix(backup_extenstion)

        for file_path in [path, backup_file]:
            if file_path.suffix == ".bak":
                operation_msg = "Saving configuration to backup file"
                success_msg = "Saved configuration backup"

            else:
                operation_msg = "Saving configuration to file"
                success_msg = "Saved configuration file"

            with file_path.open("w") as toml_file:
                logger.debug(operation_msg, filepath=file_path)

                toml_dump(data, toml_file)

                logger.success(success_msg, filepath=file_path)
    ...
    def save(self, config_domain: str) -> None:
        ...
        """
        logger.info("Saving configuration domain", domain_name=config_domain)
    ...
    def reload(self, config_domain: str) -> None:
        ...
        """
        logger.info("Reloading configuration object", domain_name=config_domain)
    ...
    def reload_all(self) -> None:
        ...
        logger.info("Reloading all configuration objects")
