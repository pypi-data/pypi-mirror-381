import numpy as np
import pytest
from pytrade.risk.models.factor.pca import _numpy_compute_single_period_pca_cov
...
@pytest.mark.parametrize(
    ["returns", "num_factors", "expected_loadings",
     "expected_specific_var"],
    [
        # this test checks implementation against an example from "Applied
        # Multivariate Statistical Analysis" by Johnson and Wichern
        pytest.param(
            johnson_stock_data(),
            2,
            str_to_pandas("""    
    factor  jpmorgan  citibank  wellsfargo  royaldutchshell  exxonmobil
    0       0.732322  0.831180    0.726203         0.604713    0.563090
    1       0.436520  0.280485    0.373859        -0.693960   -0.718639
            """, index_col="factor"),
            str_to_pandas("""
    asset              specific_var
    jpmorgan           0.273154
    citibank           0.230468
    wellsfargo         0.332858
    royaldutchshell    0.152742
    exxonmobil         0.166487
            """, index_col="asset", squeeze=True)
        ),
    ]
)
def test__numpy_single_period_pca_cov(returns, num_factors, expected_loadings,
                                      expected_specific_var):
    returns = (returns - returns.mean()) / returns.std()
    loadings, specific_var = _numpy_compute_single_period_pca_cov(returns.values,
                                                                  num_factors)
    np.testing.assert_allclose(loadings, expected_loadings, atol=1e-6, rtol=0)
    np.testing.assert_allclose(specific_var, expected_specific_var,
                               atol=1e-6, rtol=0)
