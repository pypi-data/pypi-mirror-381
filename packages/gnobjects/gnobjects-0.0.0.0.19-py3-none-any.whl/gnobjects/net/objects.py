...
class CORSObject:
    def __init__(self,
                 ...
                 allow_client_types: List[Literal['proxy', 'client', 'server']] = ['proxy'],
        ...
        - `proxy` - Пользователи и другие службы сети `GN`

        - `client` - (TBD) Пользователи напрямую. Без использования прокси серверов сети `GN`

        - `server` - Сервера запрашивающие информацию у других серверов сети `GN`
    ...
    def deserialize(data: Dict[int, Any]) -> 'CORSObject':
        return CORSObject(
            ...
            allow_client_types=data.get(2, ['proxy']),
...
class GNRequest:
    ...
    class __client:
        ...
        @property
        def type(self) -> Literal['proxy', 'client', 'server']:
            """
            # Тип клиента
            ...
            :return: Literal['proxy', 'client', 'server']
            """
        ...
        def type_int(self) -> Literal[1, 4, 2]:
            ...
            - `1` - proxy - Пользователи и другие службы сети `GN`

            - `4` - client - Пользователи напрямую. Без использования прокси серверов сети `GN`

            - `2` - server - Сервера запрашивающие информацию у других серверов сети `GN`
    ...
    def setRoute(self, route: Optional[str] = None):
        ...
        :param route: Маршрут запроса (например, `gn:proxy:real`).

        Если не указан, используется `gn:proxy:real`.
        """
        if route is None:
            route = 'gn:proxy:real'
        self._route = route
