    ...
    for scen_id, entries in by_scenario.items():
        ...
        for entry in entries:
            ...
            if isinstance(ev, dict):
                ...
                error_msg = (
                    ev.get("error") if len(ev.get("error")) < 400 else ev.get("error")[:400] + "..."
                )
                if error_msg:
                ...
                if isinstance(result_obj, dict) and result_obj:
                    ...
                    eval_results = _json.dumps(result_obj, ensure_ascii=False, indent=4)
                    eval_results = (
                        eval_results if len(eval_results) < 400 else eval_results[:400] + "..."
                    )
...
def v3launch(
    ...
    rprint(f"[green]Agent[/green] {agent_id}")
    ...
    rprint(f"[green]Agent version[/green] {version_id}")
    if not version_id:
    ...
    if selected_scenario_set_id:
        ...
        if choice != 0:
            ...
            if status == "IN_PROGRESS":
                ...
                raise typer.Exit(code=1)
    rprint(f"[green]Scenario set[/green] {selected_scenario_set_id}")
    ...
    with Live(
        ...
        # Load agent_connection from .veris/config.json and add to payload
        config_path = project_dir / ".veris" / "config.json"
        if not config_path.exists():
            rprint("[red]Missing .veris/config.json. Please initialize your project.[/red]")
            raise typer.Exit(code=1)
        with config_path.open("r", encoding="utf-8") as f:
            config = _json.load(f)
        agent_connection = config.get("agent")
        if not agent_connection:
            rprint("[red]Missing 'agent' in .veris/config.json[/red]")
            raise typer.Exit(code=1)
        sim = api.v3_start_simulation(
            agent_id,
            {
                "version_id": version_id,
                "scenario_set_id": selected_scenario_set_id,
        ...
        if not run_id:
            ...
            raise typer.Exit(code=1)
        live.update(Text.from_markup(f"[green]Run[/green] {run_id}"))
    ...
    if watch:
        try:
            ...
            rprint(
                "[yellow]Stopped watching. Check progress with: "
                f"veris sim query --agent-id {agent_id} --run-id {run.run_id}[/yellow]"
            )
    else:
        query_run(agent_id, run_id)  # type: ignore[arg-type]
        rprint(
            f"[yellow]Check progress with: veris sim query --agent-id {agent_id} --run-id {run.run_id}[/yellow]"
        )
