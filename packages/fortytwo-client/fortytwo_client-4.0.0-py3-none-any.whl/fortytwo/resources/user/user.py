...
class User(Model):
    ...
    def __init__(self: Self, data: Any) -> None:
        ...
        self.anonymize_date: datetime           = parse_date(data["anonymize_date"])
        self.data_erasure_date: datetime        = parse_date(data["data_erasure_date"])
        self.created_at: datetime               = parse_date(data["created_at"])
        self.updated_at: datetime               = parse_date(data["updated_at"])
        self.alumnized_at: datetime | None      = (
            parse_date(data["alumnized_at"]) if data.get("alumnized_at") else None
        )
...
register_serializer(
    ...
    lambda u: {
        ...
        "location":             u.location,
        "wallet":               u.wallet,
        "anonymize_date":       u.anonymize_date.isoformat(),
        "data_erasure_date":    u.data_erasure_date.isoformat(),
