...
@cli.command()
@click.argument('path', type=click.Path(exists=True))
@click.option('--translator', '-t', type=click.Choice(['openai', 'deepl', 'google', 'mock']), default='openai', help='Translation service to use')
@click.option('--source-lang', '-s', default='auto', help='Source language code')
@click.option('--target-lang', '-l', default='en', help='Target language code')
...
@cli.command()
@click.option('--translator', '-t', type=click.Choice(['openai', 'deepl', 'mock']), default='openai', help='Translation service to check')
...
async def _translate_files(
    ...
    with Progress(
        ...
        task = progress.add_task(f"[cyan]Translating {len(files)} files...", total=len(files))
        ...
        async def translate_single_file(file_path: Path):
            ...
            async with semaphore:
                try:
                    ...
                    # Translate units
                    translated_units = await _translate_units(
                        translator, 
                        result.parse_result.units, 
                        result.parse_result.file_type, 
                        source_lang, 
                        target_lang
                    )
...
[bold]Cost per Character:[/bold] ${info['cost_per_character']:.6f}
...
def _display_info() -> None:
    ...
    info_text = """
[bold blue]LangLint v1.0.0[/bold blue]
