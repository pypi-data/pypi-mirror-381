...
class TestDDCAL(unittest.TestCase):
    ...
    # some additional edge cases to test
    # frequencies = np.load("./data/edge_case_one_data_element.npy.npy")  # parameters don't matter (1 element with value 10)
    # frequencies = np.load("./data/edge_case_constant_data.npy.npy")  # parameters don't matter (1000 elements with value 10)
    ...
    def test_parameters(self):
        ...
        self.nuc, self.sed, self.sv, self.msc = analyze_results(
            frequencies_result, self.cluster_size
        )
        ...
        assert self.nuc > 0
        assert self.sed > 0
        assert self.sv >= 0  # =0 if just 1 cluster
        assert self.msc >= -1
        assert self.msc <= 1
    ...
    def test_sizes(self):
        ...
        self.assertEqual(
            self.ddcal.sorted_data.size, self.ddcal.labels_sorted_data.size
        )
