...
class DDCAL:
    ...
    def fit(self, data: np.ndarray):
        ...
        # do some checks if execution of the algorithm is necessary
        if self.n_clusters == 1 and data_size > 1:
            labels_.fill(0)
        ...
        while clusters_array.size > 0 and data[data_min_idx:data_max_idx].size > 0:
            ...
            norm_frequencies = _normalize_min_max_feature(
                data[data_min_idx:data_max_idx]
            )
            ...
            boundaries = np.linspace(
                self.feature_boundary_min,
                self.feature_boundary_max,
                num=self.num_simulations,
            )
            for b in range(0, boundaries.size):
                ...
                def get_next_gap_q_lower_bound(
                    _data: np.ndarray, current_idx: int, min_idx: int, max_idx: int
                ) -> tuple[float, int]:
                    """
                    helper
                ...
                def get_next_gap_q_upper_bound(
                    _data: np.ndarray, current_idx: int, min_idx: int, max_idx: int
                ) -> tuple[float, int]:
                    """
                    helper
                ...
                while True:
                    if (
                        data[data_min_idx : data_min_idx + q_lower_bound].size == 0
                    ):  # skip if array is empty
                        break
                    std_dev_current = np.std(
                        data[data_min_idx : data_min_idx + q_lower_bound]
                    )
                    next_gap, idx_next_gap = get_next_gap_q_lower_bound(
                        data,
                        data_min_idx + q_lower_bound,
                        data_min_idx,
                        data_max_idx - 1,
                    )
                ...
                while True:  # O(log n)
                    if (
                        data[data_max_idx - q_upper_bound : data_max_idx].size == 0
                    ):  # skip if array is empty
                        break
                    std_dev_current = np.std(
                        data[data_max_idx - q_upper_bound : data_max_idx]
                    )
                    next_gap, idx_next_gap = get_next_gap_q_upper_bound(
                        data,
                        data_max_idx - q_upper_bound,
                        data_min_idx,
                        data_max_idx - 1,
                    )
            ...
            if not q_idx_best_found_lo and not q_idx_best_found_up:
                # both boundaries must be greater than min q_aim
                # print("no boundary found for all tested boundaries: extending q tolerance")
                q_tolerance_tmp = (
                    q_tolerance_tmp + q_tolerance_tmp * self.q_tolerance_increase_step
                )
                continue  # to start over slicing the boundary range and testing with extended q tolerance
            ...
            # add elements to upper bucket
                ...
                labels_[data_max_idx - q_upper_bound : data_max_idx] = clusters_array[
                    -1
                ]
