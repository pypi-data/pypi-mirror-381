...
# No need to change the settings below because you can change some of these values dynamically when defining the splits using the functions from neuroprobe_train_test_splits.py and the dataset from neuroprobe_datasets.py. For the standard evaluation on the Neuroprobe benchmark, you do not need to change these values.
...
# Define a global random seed for reproducibility. This number is used in the dataset class. Do not modify this!
...
# NOTE: Neuroprobe = Neutoprobe Lite. Lite is the default.
NEUROPROBE_MAX_SAMPLES = NEUROPROBE_LITE_MAX_SAMPLES
NEUROPROBE_N_FOLDS = NEUROPROBE_LITE_N_FOLDS
NEUROPROBE_SUBJECT_TRIALS = NEUROPROBE_LITE_SUBJECT_TRIALS
...
NEUROPROBE_FULL_SUBJECT_TRIALS = [ # structured as (subject_id, trial_id)
    (1, 0), (1, 1), (1, 2), 
...
NEUROPROBE_LONGEST_TRIALS_FOR_SUBJECT = { # structured as subject_id: [longest_trial_id, second_longest_trial_id]
    1: [0, 1],
    2: [4, 6],
    3: [2, 1],
    4: [2, 1],
...
# The subset of electrodes for every subject that is used in the Neuroprobe benchmark. Do not change this!
...
NEUROPROBE_ELECTRODES = NEUROPROBE_LITE_ELECTRODES # NOTE: Neuroprobe = Neuroprobe Lite. Lite is the default.