...
def process_step(folders, fac, n_imgs, n_obj):
    ##### Step variables ############
    files = os.listdir(folders['imgs_folder_A'])   # List of files in the aligned images folder
    t_imgs = len(files)                 # Total number of images
    ...
    while i < t_imgs:
        ...
        if i == 0:
            estado_global["prompts"] = td.leer_prompts(os.path.join(folders['root'], 'mask_prompts.csv'))
            ...
            for filename in sorted(os.listdir(folders['mask_folder'])):
                if filename.startswith(f'outmask_fr{i - 1}'):
                    if cnt >= n_obj:
                        ...
                        td.save_masks(folders['mask_folder'], n_imgs, i, estado_global["video_segments"], estado_global["cl"])
                    ...
                    td.add_object_mask(new_mask, id_obj, estado_global["predictor"], estado_global["inference_state"], 0)
                    cnt += 1
            td.actualizar_segmentos_video(estado_global["predictor"], estado_global["inference_state"], estado_global["video_segments"])
            td.save_masks(folders['mask_folder'], n_imgs, i, estado_global["video_segments"], estado_global["cl"])
            cnt = 0

        ### Delete images from auxiliary folder
...
def main():
    ...
    try:
        process_step(folders, args.fac, args.n_imgs, args.n_obj)
    except Exception as e:
