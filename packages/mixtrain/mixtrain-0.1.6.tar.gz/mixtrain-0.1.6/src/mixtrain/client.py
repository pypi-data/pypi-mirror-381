...
def detect_rule_changes(old_rules: List[Dict[str, Any]], new_rules: List[Dict[str, Any]]) -> str:
...
class MixClient:
    ...
    def __init__(self, workspace_name: Optional[str] = None, api_key: Optional[str] = None):
    ...
    def _detect_auth_method(self) -> AuthMethod:
        ...
        if env_api_key:
            if not env_api_key.startswith("mix-"):
                raise ValueError("MIXTRAIN_API_KEY environment variable must start with 'mix-'")
            return AuthMethod.API_KEY
    ...
    def _determine_workspace_name(self) -> str:
        ...
        if self._auth_method == AuthMethod.API_KEY:
            ...
            if not active_workspace:
                raise ValueError("No active workspace found. Please authenticate with 'mixtrain login'")
    ...
    def list_datasets(self) -> Dict[str, Any]:
        ...
        response = self._make_request("GET", f"/lakehouse/workspaces/{self._workspace_name}/tables")
        return response.json()

    def create_dataset_from_file(self, name: str, file_path: str, description: Optional[str] = None) -> Dict[str, Any]:
    ...
    def delete_dataset(self, name: str) -> httpx.Response:
        """Delete a dataset."""
        return self._make_request("DELETE", f"/lakehouse/workspaces/{self._workspace_name}/tables/{name}")
    ...
    def get_catalog(self) -> Any:
        ...
        try:
            provider_secrets = self._make_request(
                "GET", f"/workspaces/{self._workspace_name}/dataset-providers/6"
            ).json()

            if provider_secrets["provider_type"] != "apache_iceberg":
                raise Exception(f"Dataset provider {provider_secrets['provider_type']} is not supported")
            ...
            if (
                ...
                os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = f"/tmp/{self._workspace_name}/service_account.json"

                # Set up Google Cloud credentials (temporary file)
                os.makedirs(f"/tmp/mixtrain/{self._workspace_name}", exist_ok=True)
                with open(f"/tmp/mixtrain/{self._workspace_name}/service_account.json", "w") as f:
    ...
    def list_dataset_providers(self) -> Dict[str, Any]:
        ...
        response = self._make_request("GET", f"/workspaces/{self._workspace_name}/dataset-providers/")
        return response.json()

    def create_dataset_provider(self, provider_type: str, secrets: Dict[str, str]) -> Dict[str, Any]:
        """Onboard a new dataset provider for the workspace."""
        payload = {"provider_type": provider_type, "secrets": secrets}
        response = self._make_request("POST", f"/workspaces/{self._workspace_name}/dataset-providers/", json=payload)
        return response.json()

    def update_dataset_provider(self, provider_id: int, secrets: Dict[str, str]) -> Dict[str, Any]:
        """Update secrets for an existing dataset provider."""
        payload = {"secrets": secrets}
        response = self._make_request(
            "PUT", f"/workspaces/{self._workspace_name}/dataset-providers/{provider_id}", json=payload
        )
        return response.json()

    def delete_dataset_provider(self, provider_id: int) -> Dict[str, Any]:
        """Remove a dataset provider from the workspace."""
        response = self._make_request("DELETE", f"/workspaces/{self._workspace_name}/dataset-providers/{provider_id}")
        return response.json()

    # Model provider operations
    def list_model_providers(self) -> Dict[str, Any]:
        """List available and onboarded model providers."""
        response = self._make_request("GET", f"/workspaces/{self._workspace_name}/models/providers")
        return response.json()

    def create_model_provider(self, provider_type: str, secrets: Dict[str, str]) -> Dict[str, Any]:
        """Onboard a new model provider for the workspace."""
        payload = {"provider_type": provider_type, "secrets": secrets}
        response = self._make_request("POST", f"/workspaces/{self._workspace_name}/models/providers", json=payload)
        return response.json()

    def update_model_provider(self, provider_id: int, secrets: Dict[str, str]) -> Dict[str, Any]:
        """Update secrets for an existing model provider."""
        payload = {"secrets": secrets}
        response = self._make_request(
            "PUT", f"/workspaces/{self._workspace_name}/models/providers/{provider_id}", json=payload
        )
        return response.json()

    def delete_model_provider(self, provider_id: int) -> Dict[str, Any]:
        """Remove a model provider from the workspace."""
        response = self._make_request("DELETE", f"/workspaces/{self._workspace_name}/models/providers/{provider_id}")
        return response.json()

    def list_models(self) -> Dict[str, Any]:
        """List all internal models in the current workspace."""
        response = self._make_request("GET", f"/workspaces/{self._workspace_name}/models/")
        return response.json()
    ...
    def list_routing_configs(self, status: Optional[str] = None) -> Dict[str, Any]:
        ...
        response = self._make_request("GET", f"/workspaces/{self._workspace_name}/inference/configs", params=params)
        return response.json()

    def get_routing_config(self, config_id: int, version: Optional[int] = None) -> Dict[str, Any]:
        ...
        response = self._make_request(
            "GET", f"/workspaces/{self._workspace_name}/inference/configs/{config_id}", params=params
        )
        return response.json()
    ...
    def get_active_routing_config(self) -> Optional[Dict[str, Any]]:
        ...
        """
        response = self._make_request("GET", f"/workspaces/{self._workspace_name}/inference/active-config")
        result = response.json()
        if not result:
            raise Exception("No active routing configuration found in workspace")
        return result
    
    def get_routing_engine(self, config_id: Optional[int] = None, version: Optional[int] = None) -> Optional[Dict[str, Any]]:
    ...
    def create_routing_config(self, name: str, description: str, rules: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        response = self._make_request("POST", f"/workspaces/{self._workspace_name}/inference/configs", json=payload)
        return response.json()
    ...
    def update_routing_config(
        ...
        if rules is not None:
            ...
            try:
                ...
                _change_message = change_message or detect_rule_changes(old_rules, rules)
                payload["change_message"] = _change_message
            except Exception as e:
                # If we can't get the current config, fall back to generic message
                logger.warning(f"Failed to detect rule changes for config {config_id}: {e}")
                payload["change_message"] = "Configuration updated via CLI"

        response = self._make_request(
            "PUT", f"/workspaces/{self._workspace_name}/inference/configs/{config_id}", json=payload
        )
        return response.json()

    def activate_routing_config(self, config_id: int, version: Optional[int] = None) -> Dict[str, Any]:
        ...
        response = self._make_request(
            "POST", f"/workspaces/{self._workspace_name}/inference/configs/{config_id}/activate", json=payload
        )
        return response.json()
    ...
    def delete_routing_config(self, config_id: int) -> Dict[str, Any]:
        ...
        Returns:
            Deletion result
        """
        response = self._make_request("DELETE", f"/workspaces/{self._workspace_name}/inference/configs/{config_id}")
        return response.json()

    def test_routing_config(self, config_id: int, test_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self._make_request(
            "POST", f"/workspaces/{self._workspace_name}/inference/configs/{config_id}/test", json=payload
        )
        return response.json()
    ...
    def get_routing_config_versions(self, config_id: int) -> Dict[str, Any]:
        ...
        """
        response = self._make_request("GET", f"/workspaces/{self._workspace_name}/inference/configs/{config_id}/versions")
        return response.json()
    ...
    def authenticate_with_token(self, token: str, provider: str) -> str:
        ...
        return auth_utils.authenticate_with_token(token, provider, get_config, self._make_request_for_auth)

    def authenticate_github(self, access_token: str) -> str:
        """Authenticate using GitHub access token."""
        return auth_utils.authenticate_github(access_token, get_config, self._make_request_for_auth)

    def authenticate_google(self, id_token: str) -> str:
        """Authenticate using Google ID token."""
        return auth_utils.authenticate_google(id_token, get_config, self._make_request_for_auth)

    def _make_request_for_auth(self, method: str, path: str, **kwargs) -> httpx.Response:
...
def create_routing_config(name: str, description: str, rules: List[Dict[str, Any]]) -> Dict[str, Any]:
...
def activate_routing_config(config_id: int, version: Optional[int] = None) -> Dict[str, Any]:
