...
def apply_environment_api_key(arguments: Namespace, empty_ok=False):
    # API key
    env_api_key = environ[WOWOOL_PORTAL_KEY_ENV_NAME] if WOWOOL_PORTAL_KEY_ENV_NAME in environ else None
    ...
    if not empty_ok and arguments.api_key is None:
        raise PortalClientError(
            "MissingApiKeyError",
            f"An API key is required. Use the -k option or set the environment variable '{WOWOOL_PORTAL_KEY_ENV_NAME}'",
        )
