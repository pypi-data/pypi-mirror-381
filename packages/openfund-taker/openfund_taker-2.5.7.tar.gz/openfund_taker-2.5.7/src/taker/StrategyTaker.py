...
class StrategyTaker():
    ...
    def find_PDArrays(self, symbol, struct, side=None, start_index=-1,balanced=False, is_struct_body_break=True, pair_config=None) -> pd.DataFrame:
        """_summary_
            寻找PDArray
        Args:
            symbol (_type_): _description_
        ...
        Returns:
            _type_: _description_
        """
        return self.smcPDArray.find_PDArrays(struct=struct, side=side, start_index=start_index, balanced=balanced, is_struct_body_break=is_struct_body_break)  
    
    def get_latest_PDArray(self, symbol, data, side, start_index=-1, check_balanced=True, is_struct_body_break=True, mask=None) -> dict:
        """_summary_
            获取最新的PDArray
        Args:
            symbol (_type_): _description_
        ...
        Returns:
            _type_: _description_
        """
        return self.smcPDArray.get_latest_PDArray(data, side, start_index, check_balanced,  mask, is_struct_body_break,)
    
    def find_OBs(self, symbol, struct, side=None, start_index=-1, is_valid=True, is_struct_body_break=True, pair_config=None) -> pd.DataFrame:
        """_summary_
            识别OB
        Args:
            symbol (_type_): _description_
        ...
        return self.smcOB.find_OBs(struct=struct, side=side, start_index=start_index, is_valid=is_valid, is_struct_body_break=is_struct_body_break)
    ...
    def get_latest_struct(self, symbol, data, is_struct_body_break=True) -> dict:
        ...
        Returns:
            _type_: _description_
        """
        return self.smcStruct.get_latest_struct(data, is_struct_body_break=is_struct_body_break)
    ...
    def get_support_resistance_from_OBs(self, symbol, obs_df, struct_df) -> dict:
        ...
        if OBs_df is None or len(OBs_df) == 0:
            ...
            support_OB = self.get_latest_OB(symbol=symbol,data=OBs_df,trend=self.BULLISH_TREND)
            if support_OB :
                ...
                support_timestamp = struct_df.at[struct_df.index[-1], self.TIMESTAMP_COL]
        
            resistance_OB = self.get_latest_OB(symbol=symbol,data=OBs_df,trend=self.BEARISH_TREND)
            if resistance_OB :
                resistance_price = resistance_OB.get(self.OB_MID_COL)
                resistance_timestamp = resistance_OB.get(self.TIMESTAMP_COL)
            else:
                resistance_price = struct_df.at[struct_df.index[-1], self.STRUCT_HIGH_COL]
                resistance_timestamp = struct_df.at[struct_df.index[-1], self.TIMESTAMP_COL]
    ...
    def _round_price_to_tick(self, symbol, price:Decimal) -> Decimal:
        ...
        adjusted_price = self.format_price(round(price / tick_size) * tick_size)
        return self.toDecimal(adjusted_price)
    ...
    def calculate_tp_price_by_pct(self, symbol: str, position: dict, tp_pct: float, offset: int = 1) -> Decimal:
        """计算止盈价格
        Args:
            symbol: 交易对
            position: 持仓信息
        ...
        tp_pct_decimal = self.toDecimal(tp_pct) / 100
        ...
        base_price = entry_price * (
            1 + tp_pct_decimal if side == self.LONG_KEY 
            else 1 - tp_pct_decimal
        )
        ...
        # 空仓: 基础价格 + 偏移量
        take_profile_price = (
            base_price - price_offset if side == self.LONG_KEY 
    ...
    def monitor_total_profit(self):
        ...
        stop_loss_pct = float(self.strategy_config.get("all_stop_loss_pct",2))  # 全局止损百分比


        self.logger.info(f"策略时间框架 {htf}|{atf}|{etf} stop_loss_pct={stop_loss_pct}...\n")
