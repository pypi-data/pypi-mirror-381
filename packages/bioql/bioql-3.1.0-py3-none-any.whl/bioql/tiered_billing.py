...
def check_rate_limit(user_id: str, rate_limit: int) -> Tuple[bool, int]:
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (
            tracker_id, user_id, window_start.isoformat(),
            now.isoformat(), current_count + 1, now.isoformat()
        ))
...
def check_quota(user_id: str, backend_type: str, user_info: Dict) -> Tuple[bool, str]:
    ...
    if used >= quota:
        ...
        if overage_price > 0:
            ...
            return False, f"❌ Monthly {backend_name} quota exceeded ({used}/{quota}). Upgrade tier or wait for next billing cycle."
...
def calculate_backend_cost(backend: str, backend_type: str, shots: int, user_info: Dict) -> float:
    """Calculate cost based on backend type and pricing tier"""

    # Real 2025 pricing
    backend_costs = {
        'simulator': 0.00009 / 1000,  # $0.09 per 1000 jobs = $0.00009 per job
        'gpu': 0.0027,  # $2.70 per 1000 jobs = $0.0027 per job
        'quantum': 3.20,  # $3.20 per job (IBM Pay-As-You-Go)
    }

    base_cost = backend_costs.get(backend_type, 0.0)
    ...
    if used >= quota and quota < 999999:  # Over quota and not unlimited
        # Apply overage pricing
        overage_key = f'overage_{backend_type}'
        overage_cost = user_info.get(overage_key, base_cost)
        return overage_cost
    else:
        # Within quota - cost is covered by subscription
        return 0.0  # Internal accounting, not charged per-use

def log_usage(user_id: str, api_key_id: str, program: str, backend: str, shots: int,
              success: bool, execution_time: float, error_message: str = None,
              cost: float = 0.0, result_data: Dict = None, user_info: Dict = None) -> None:
    ...
    try:
        ...
        """, (
            usage_id, user_id, api_key_id, program[:1000], program_hash,
            qubits, 0, 0,
            algorithm, backend, backend, backend_type,
            shots, shots if success else 0, success, execution_time,
            error_message, str(real_cost), 1.0, 1.0,
            str(real_cost), False, now, now
        ))
...
def update_usage_analytics(user_id: str, backend_type: str, success: bool,
                          execution_time: float, cost: float) -> None:
