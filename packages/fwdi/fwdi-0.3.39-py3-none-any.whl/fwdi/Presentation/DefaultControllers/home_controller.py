...
class Controller(metaclass=MetaServiceFWDI):
    def __init__(self):
        super().__init__()
...
"""
    def index(self, message:str)->Response:
        Controller.__log__(f"{__name__}:{message}")
        page = f"{self.default_page}\n\t\tHello {message} !!! {datetime.now()}" if len(message) > 0 else self.default_page
