...
async def create_a_node(
    ...
    _logger.info(f"🚀 Starting create_a_node for {data.name} ({node_type})")
    ...
    await raise_if_node_exists(session, data.name)
    _logger.info(
        f"⏱️  Step 1 - raise_if_node_exists: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )

    # Step 2: Check for inactive nodes
    step_start = time.perf_counter()
    if recreated_node := await create_node_from_inactive(
        ...
        _logger.info(
            f"⚡ Recreated inactive node in {(time.perf_counter() - start_time) * 1000:.1f}ms",
        )
        return recreated_node  # pragma: no cover
    _logger.info(
        f"⏱️  Step 2 - create_node_from_inactive: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )

    # Step 3: Get/validate namespace
    step_start = time.perf_counter()
    namespace = get_namespace_from_name(data.name)
    await get_node_namespace(
        session=session,
    ...
    data.namespace = namespace
    _logger.info(
        f"⏱️  Step 3 - namespace validation: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )

    # Step 4: Create Node object
    step_start = time.perf_counter()
    node = Node(
        name=data.name,
        namespace=data.namespace,
    ...
    _logger.info(
        f"⏱️  Step 4 - create Node object: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )

    # Step 5: Create node revision (LIKELY EXPENSIVE)
    step_start = time.perf_counter()
    node_revision = await create_node_revision(data, node_type, session, current_user)
    _logger.info(
        f"⏱️  Step 5 - create_node_revision: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )

    # Step 6: Process primary key attributes
    step_start = time.perf_counter()
    ...
    _logger.info(
        f"⏱️  Step 6 - primary key processing: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )

    # Step 7: Save to database (LIKELY EXPENSIVE)
    step_start = time.perf_counter()
    await save_node(
        session,
        node_revision,
    ...
    _logger.info(
        f"⏱️  Step 7 - save_node: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )
    ...
    _logger.info(
        f"⏱️  Step 8 - background tasks: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )

    # Step 9: Final node loading (LIKELY EXPENSIVE)
    step_start = time.perf_counter()
    ...
    _logger.info(
        f"⏱️  Step 9 - final Node.get_by_name: {(time.perf_counter() - step_start) * 1000:.1f}ms",
    )

    total_time = (time.perf_counter() - start_time) * 1000
    _logger.info(f"✅ Completed create_a_node for {data.name} in {total_time:.1f}ms")
...
async def validate_dimension_link(
    ...
    skip_catalog_check: bool = False,
    ctx: CompileContext | None = None,
) -> ast.Join:
    if (
        not skip_catalog_check
...
async def create_or_update_dimension_link(
    session: AsyncSession,
    new_revision: NodeRevision,
    dimension_node: Node,
    node: Node,
    ...
    """
    Create a new node revision for a dimension link update and return the new dimension link
    and the new node revision.
    """
