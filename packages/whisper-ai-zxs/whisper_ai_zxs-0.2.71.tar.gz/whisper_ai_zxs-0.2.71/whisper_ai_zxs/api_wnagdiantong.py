    ...
    def create_trade(self, file_path):
        ...
        for i, row in df.iterrows():
            ...
            if tid not in trades:
                # 处理付款时间字段，兼容datetime和str类型
                pay_time_val = row['付款时间']
                if pd.notna(pay_time_val):
                    if hasattr(pay_time_val, 'strftime'):
                        pay_time_str = pay_time_val.strftime('%Y-%m-%d %H:%M:%S')
                    else:
                        pay_time_str = str(pay_time_val)
                else:
                    pay_time_str = '0000-00-00 00:00:00'
                trades[tid] = {
                    'tid': tid,
                    'shop_name': row['店铺名称'],
                    'trade_status': 30,
