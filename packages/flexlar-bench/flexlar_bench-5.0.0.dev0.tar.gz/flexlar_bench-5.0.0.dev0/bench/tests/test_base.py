...
FLEXLAR_BRANCH = "version-13-hotfix"
if PYTHON_VER.major == 3:
	if PYTHON_VER.minor >= 10:
		FLEXLAR_BRANCH = "develop"
...
class TestBenchBase(unittest.TestCase):
	...
	def assert_folders(self, bench_name):
		...
		self.assert_exists(bench_name, "apps", "flexlar")
	...
	def init_bench(self, bench_name, **kwargs):
		...
		flexlar_tmp_path = "/tmp/flexlar"

		if not os.path.exists(flexlar_tmp_path):
			exec_cmd(
				f"git clone https://github.com/flexlar/flexlar -b {FLEXLAR_BRANCH} --depth 1 --origin upstream {flexlar_tmp_path}"
			)
		...
		if not os.path.exists(os.path.join(self.benches_path, bench_name)):
			...
			exec_cmd(
				"git remote set-url upstream https://github.com/flexlar/flexlar",
				cwd=os.path.join(self.benches_path, bench_name, "apps", "flexlar"),
			)
