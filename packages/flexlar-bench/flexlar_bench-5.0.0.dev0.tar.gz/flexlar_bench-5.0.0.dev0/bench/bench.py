...
class Bench(Base, Validator):
	...
	def uninstall(self, app, no_backup=False, force=False):
		if app == "flexlar":
			raise ValidationError("You cannot uninstall the app `flexlar`")
		from bench.app import App
	...
	def build(self):
		# build assets & stuff
		run_flexlar_cmd("build", bench_path=self.name)
...
class BenchApps(MutableSequence):
	...
	def update_apps_states(
		...
		if self.apps and not os.path.exists(self.states_path):
			...
			# Keeping flexlar as the first app.
			if "flexlar" in self.apps:
				self.apps.remove("flexlar")
				self.apps.insert(0, "flexlar")
	...
	def initialize_apps(self):
		try:
			self.apps = [
				...
				if is_flexlar_app(os.path.join(self.bench.name, "apps", x))
			]
			self.apps.remove("flexlar")
			self.apps.insert(0, "flexlar")
...
class BenchSetup(Base):
	...
	def env(self, python="python3"):
		...
		- upgrade env pip
		- install flexlar python dependencies
		"""
		import bench.cli
		import click
		...
		flexlar = os.path.join(self.bench.name, "apps", "flexlar")
		...
		if os.path.exists(flexlar):
				env = None

				from bench.utils.app import get_current_flexlar_version
				if get_current_flexlar_version(self.bench.name) >= 16:
					check_pkg_config()
				...
				if os.environ.get("BENCH_USE_UV"):
					self.run(
						f"uv pip install {quiet_flag} --upgrade -e {flexlar} --python {self.bench.python}",
						cwd=self.bench.name, env=env,
					)
				else:
					self.run(
						f"{self.bench.python} -m pip install {quiet_flag} --upgrade -e {flexlar}",
						cwd=self.bench.name, env=env,
					)
	...
	def backups(self):
		...
		user = self.bench.conf.get("flexlar_user")
