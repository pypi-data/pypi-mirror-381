...
sudoers_file = "/etc/sudoers.d/flexlar"
UNSET_ARG = object()
...
def is_flexlar_app(directory: str) -> bool:
	is_flexlar_app = True

	for folder in paths_in_app:
		if not is_flexlar_app:
			break

		path = glob(os.path.join(directory, "**", folder))
		is_flexlar_app = is_flexlar_app and path

	return bool(is_flexlar_app)
...
def is_valid_flexlar_branch(flexlar_path: str, flexlar_branch: str):
	...
	:param flexlar_path: git url
	:type flexlar_path: str
	:param flexlar_branch: branch to check
	:type flexlar_branch: str
	...
	if flexlar_branch:
		try:
			res = g.ls_remote("--heads", "--tags", flexlar_path, flexlar_branch)
			if not res:
				raise InvalidRemoteException(
					f"Invalid branch or tag: {flexlar_branch} for the remote {flexlar_path}"
				)
		except GitCommandError as e:
			raise InvalidRemoteException(f"Invalid flexlar path: {flexlar_path}") from e
...
def check_latest_version():
	...
	if os.environ.get("FLEXLAR_DOCKER_BUILD"):
		return
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/flexlar-bench/json")
	except Exception:
...
def run_flexlar_cmd(*args, **kwargs):
	...
	p = subprocess.Popen(
		(f, "-m", "flexlar.utils.bench_helper", "flexlar") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_env_flexlar_commands(bench_path=".") -> List:
	"""Caches all available commands (even custom apps) via Flexlar
	...
	try:
		return json.loads(
			get_cmd_output(
				f"{python} -m flexlar.utils.bench_helper get-flexlar-commands", cwd=sites_path
			)
		)
...
def find_org(org_repo, using_cached: bool = False):
	...
	raise InvalidRemoteException(
		f"{org_repo} not found under flexlar or erpnext GitHub accounts"
	)
...
class _dict(dict):
	...
	# bench port of flexlar._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fix flexlar#14833 via DFP
...
def get_cmd_from_sysargv():
	...
	For Command: `bench --profile --site flexlarframework.com migrate --no-backup`
	sys.argv: ["/home/flexlar/.local/bin/bench", "--profile", "--site", "flexlarframework.com", "migrate", "--no-backup"]
	Actual command run: migrate

	"""
	# context is passed as options to flexlar's bench_helper
	from bench.bench import Bench

	flexlar_context = _dict(params={"--site"}, flags={"--verbose", "--profile", "--force"})
	cmd_from_ctx = None
	sys_argv = sys.argv[1:]
	skip_next = False
	...
	for arg in sys_argv:
		...
		if arg in flexlar_context.flags:
			continue

		elif arg in flexlar_context.params:
			skip_next = True
			continue
...
def get_app_cache_extract_filter(
	...
	def filter_function(member: TarInfo, dest_path: str) -> Optional[TarInfo]:
		...
		try:
			...
			# Links created by `flexlar` after extraction
			return None
