...
def can_enable_multi_queue_consumption(bench_path: str) -> bool:
	try:
		...
		flexlar_version = Version(get_current_version("flexlar", bench_path=bench_path))

		return flexlar_version > supported_version
	except Exception:
		return False
...
def check_supervisord_config(user=None):
	...
	# removed updating supervisord conf & reload in Aug 2022 - gavin@flexlar.io
	import configparser
