...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		name (str): This could look something like
		        1. https://github.com/flexlar/healthcare.git
		        2. git@github.com:flexlar/healthcare.git
		        3. flexlar/healthcare@develop
		        4. healthcare
		...
		class Healthcare(AppConfig):
		        dependencies = [{"flexlar/erpnext": "~13.17.0"}]
		"""
...
class App(AppMeta):
	...
	def install(
		...
		if not resolved and self.app_name != "flexlar" and not ignore_resolution:
			click.secho(
	...
	def validate_app_dependencies(self, throw=False) -> None:
		...
		deps: Optional[dict] = (
			pyproject.get("tool", {}).get("bench", {}).get("flexlar-dependencies")
		)
		if not deps:
			return
	...
	def prune_app_directory(self):
		app_path = self.get_app_path()
		if can_flexlar_use_cached(self):
...
def can_flexlar_use_cached(app: App) -> bool:
	min_flexlar = get_required_flexlar_version(app)
	if not min_flexlar:
		return False

	try:
		return sv.Version(min_flexlar) in sv.SimpleSpec(">=15.12.0")
	except ValueError:
	...
	try:
		...
		"""
		return sv.Version("15.12.0") not in sv.SimpleSpec(min_flexlar)
	except ValueError:
		click.secho(
			f"Bench app-cache: invalid value found for flexlar version '{min_flexlar}'",
			fg="yellow",
		)
		# Invalid expression
		return False
...
def validate_dependency(app: App, dep: str, req_version: str, throw=False) -> None:
	...
	if not dep_path.is_dir():
		click.secho(f"Required flexlar-dependency '{dep}' not found.", fg="yellow")
		if throw:
			sys.exit(1)
		return
	...
	if sv.Version(dep_version) not in sv.SimpleSpec(req_version):
		click.secho(
			f"Installed flexlar-dependency '{dep}' version '{dep_version}' "
...
def get_required_flexlar_version(app: App) -> Optional[str]:
	pyproject = app.get_pyproject() or {}

	# Reference: https://github.com/flexlar/bench/issues/1524
	req_flexlar = (
		pyproject.get("tool", {})
		.get("bench", {})
		.get("flexlar-dependencies", {})
		.get("flexlar")
	)

	if not req_flexlar:
		click.secho(
			"Required flexlar version not set in pyproject.toml, "
			"please refer: https://github.com/flexlar/bench/issues/1524",
			fg="yellow",
		)
...
def remove_unused_node_modules(app_path: Path) -> None:
	...
	Since most popular Flexlar apps use Vite to build their frontends,
...
def make_resolution_plan(app: App, bench: "Bench"):
	...
	for app_name in app._get_dependencies():
		...
		is_valid_flexlar_branch(dep_app.url, dep_app.branch)
...
def add_to_excluded_apps_txt(app, bench_path="."):
	if app == "flexlar":
		raise ValueError("Flexlar app cannot be excluded from update")
...
def get_app(
	...
	"""bench get-app clones a Flexlar App from remote (GitHub or any other git server),
	...
	flexlar_path, flexlar_branch = None, None
	...
	if resolve_deps:
		...
		if "flexlar" in resolution:
			# Todo: Make flexlar a terminal dependency for all flexlar apps.
			flexlar_path, flexlar_branch = resolution["flexlar"].url, resolution["flexlar"].tag
	...
	if not is_bench_directory(bench_path):
		...
		init(
			path=bench_path,
			flexlar_path=flexlar_path,
			flexlar_branch=flexlar_branch or branch,
		)
		os.chdir(bench_path)
		bench_setup = True
...
def new_app(app, no_git=None, bench_path="."):
	if bench.FLEXLAR_VERSION in (0, None):
		click.secho(
			f"{os.path.realpath(bench_path)} is not a valid bench directory.",
			fg="red",
		)
	...
	if no_git:
		if bench.FLEXLAR_VERSION < 14:
			click.secho("Flexlar v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_flexlar_cmd(*args, bench_path=bench_path)
...
def install_app(
	...
	# macOS needs a custom PKG_CONFIG_DIR for flexlar v16+
	from bench.utils.app import get_current_flexlar_version
	if app == "flexlar" and get_current_flexlar_version(bench_path) >= 16:
		check_pkg_config()
...
def use_rq(bench_path):
	...
	celery_app = os.path.join(bench_path, "apps", "flexlar", "flexlar", "celery_app.py")
