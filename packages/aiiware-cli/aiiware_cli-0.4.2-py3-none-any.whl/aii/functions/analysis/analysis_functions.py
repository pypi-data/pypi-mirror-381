...
class ResearchFunction(FunctionPlugin):
    ...
    async def execute(
        ...
        if use_web:
            progress.add_step("Searching web sources", icon="🔍")
            progress.add_step(f"Analyzing {max_sources} sources", icon="📊")
        ...
        try:
            if use_web and search_results:
                # Step 2: Analyze sources (already started web search)
                progress.start_step(step_index)
                # Just a quick step for user feedback
                progress.complete_step(step_index, success=True)
                step_index += 1
            ...
            if research_mode == "llm-only" and web_attempted:
                ...
                message += "   2. Add to `~/.config/aii/secrets.yaml`: `brave_api_key: your-key`\n"
                message += "   3. Update `~/.config/aii/config.yaml`: `web_search.provider: brave`\n\n"
