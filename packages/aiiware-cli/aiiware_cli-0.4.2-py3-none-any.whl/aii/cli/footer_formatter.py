...
class SessionFooterFormatter:
    ...
    def format_session_footer(self, session: SessionMetrics,
                             verbosity: VerbosityLevel = VerbosityLevel.STANDARD) -> str:
        """Generate footer from accumulated session data (synchronous version)"""
        if not session:
            return self._format_empty_footer()

        if verbosity == VerbosityLevel.MINIMAL:
            return self._format_minimal_footer(session)
        elif verbosity == VerbosityLevel.STANDARD:
            return self._format_standard_footer(session)
        else:  # VerbosityLevel.DETAILED
            return self._format_detailed_footer(session)
    ...
    async def format_intelligent_footer(self, session: SessionMetrics, user_input: str,
        ...
        return self._compose_footer_with_insights(session, insights, verbosity)

    def _format_minimal_footer(self, session: SessionMetrics) -> str:
    ...
    def _format_standard_footer(self, session: SessionMetrics) -> str:
    ...
    def _format_detailed_footer(self, session: SessionMetrics) -> str:
    ...
    def _compose_footer_with_insights(self, session: SessionMetrics,
                                     ...
                                     verbosity: VerbosityLevel) -> str:
        """Compose footer with semantic insights integration"""
        if verbosity == VerbosityLevel.MINIMAL:
            return self._format_minimal_footer(session)
        elif verbosity == VerbosityLevel.STANDARD:
            return self._format_standard_footer_with_insights(session, insights)
        else:  # VerbosityLevel.DETAILED
            return self._format_detailed_footer_with_insights(session, insights)

    def _format_standard_footer_with_insights(self, session: SessionMetrics,
                                             insights: Optional[SessionInsights]) -> str:
