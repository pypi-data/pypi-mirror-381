...
from dockreg.lib.misc import camel_to_snake, is_valid_hostname
from .exceptions import DockregError, DockregNotFoundError
...
def find_registry_container(registry: DockerRegistryRef, client: docker.DockerClient) -> Container:
    ...
    if not containers:
        raise DockregError(f'Could not find active container running registry for {registry}')

    # This really should not happen as it would mean 2 containers on one port!
    if len(containers) > 1:
        raise DockregError(f'Multiple containers running registry for {registry}')
...
class DockerImageInfo:
    ...
    def from_registry(cls, image: DockerObjectRef) -> DockerImageInfo:
        ...
        if resp.status_code == 404:
            raise DockregNotFoundError(image)
...
def list_images(repo: DockerObjectRef) -> list[DockerObjectRef]:
    ...
    if resp.status_code == 404:
        raise DockregNotFoundError(repo)
    resp.raise_for_status()
    data = resp.json()
