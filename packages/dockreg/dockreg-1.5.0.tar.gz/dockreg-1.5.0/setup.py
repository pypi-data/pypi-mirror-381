...
__version__ = (Path(__file__).parent / 'dockreg' / 'VERSION').read_text().strip()

REPO_URL = 'https://github.com/jin-gizmo/dockreg'
...
def looks_like_script(path: Path) -> bool:
    ...
    if not (path.is_file() and (stat := path.stat()).st_size > 2 and stat.st_mode & 0o100):  # noqa
        return False
