...
def calculate_continuous(X_test, y_true, y_pred, approach_name, calculate_entropy):
...
def evaluate_verification(
    ...
    return calculate_continuous(
        auth_data.X, y_true, y_pred, approach_name, calculate_entropy
    )
...
def evaluate_session_takeover_attack(
    attack_detector, attack_data, threshold, approach_name="attack_detector"
):
    y_true = np.argmax(attack_data.y, axis=1)
    y_scores = attack_detector.authentication(attack_data.X)
    y_scores = [x[0] for x in y_scores]
    y_pred = np.array([0 if x >= threshold else 1 for x in y_scores])

    attack_point = np.where(y_true == 1)[0][0]

    fig = go.Figure(data=go.Scatter(x=[i for i in range(len(y_scores))], y=y_scores))
    fig.add_hline(threshold, line_color="green")
    fig.add_vline(attack_point, line_color="red")
    fig.update_layout(
        {
