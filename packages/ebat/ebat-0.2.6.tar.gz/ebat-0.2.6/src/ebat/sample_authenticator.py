...
if __name__ == "__main__":
    ...
    attack_detector = SimpleMLP(
        input_nodes=INPUT_SIZE,
        output_nodes=config["user_num"],
        middle_layer_size=NN_SIZE,
    )
    attack_detector.training(attack_train)
    results = evaluate_verification(attack_detector, attack_test)
    results.metrics()
    results.visualise()
    evaluate_session_takeover_attack(
        attack_detector, attack_test, results.results["eer_threshold"]
    )
