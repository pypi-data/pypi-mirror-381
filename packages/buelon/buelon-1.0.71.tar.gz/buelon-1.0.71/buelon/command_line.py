...
def display_status(args):
    async def func():
        async with pete.hub.WorkerClient(settings.worker.host, settings.worker.port, settings.worker.scopes.split(',')) as client:
...
def cli():
    ...
    parser.add_argument('-v', '--version', action='version', version='Buelon 1.0.71')
