...
// Implementation
void fm_learn_mcmc_simultaneous::_learn(Data &train, Data &test)
{
  ...
  for (uint i = num_complete_iter; i < num_iter; i++)
  {
    ...
    if (task == TASK_REGRESSION)
    {
      ...
      if (train.verbose)
      {
        std::cout << "#Iter=" << std::setw(3) << i << "\tTrain=" << rmse_train << "\tValidation=" << rmse_test_all << std::endl;
      }
...
void fm_learn_mcmc_simultaneous::_evaluate(DVector<double> &pred, DVector<DATA_FLOAT> &target, double normalizer, double &rmse, double &mae, uint &num_eval_cases)
{
  _evaluate(pred, target, normalizer, rmse, mae, 0, num_eval_cases);
}

void fm_learn_mcmc_simultaneous::_evaluate_class(DVector<double> &pred, DVector<DATA_FLOAT> &target, double normalizer, double &accuracy, double &loglikelihood, uint &num_eval_cases)
{
