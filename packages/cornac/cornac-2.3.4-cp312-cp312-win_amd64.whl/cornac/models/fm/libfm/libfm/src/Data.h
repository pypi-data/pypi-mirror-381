...
public:
  ...
  void load(LargeSparseMatrixMemory<DATA_FLOAT> &data, DVector<DATA_FLOAT> &target);
  void debug();
...
void Data::load(LargeSparseMatrixMemory<DATA_FLOAT> &X, DVector<DATA_FLOAT> &target)
{
  // if (this->verbose)
  // {
  //   std::cout << "has x = " << has_x << std::endl;
  ...
  num_feature = this->data->getNumCols();
  num_values = this->data->getNumValues();
