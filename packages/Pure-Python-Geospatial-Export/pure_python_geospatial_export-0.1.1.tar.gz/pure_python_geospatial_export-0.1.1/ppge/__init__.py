...
def _validate_and_fix_multipoint_coordinates(shape: dict) -> dict:
    """
    Validate and fix MultiPoint coordinates to ensure they are properly formatted.

    Args:
        shape: GeoJSON-like shape dictionary

    Returns:
        dict: Fixed shape dictionary with valid coordinates
    """
    if shape["type"] == "MultiPoint":
        valid_coordinates = []
        for coord in shape["coordinates"]:
            # Ensure each coordinate is a proper [x, y] pair
            if isinstance(coord, (list, tuple)) and len(coord) >= 2:
                valid_coordinates.append([float(coord[0]), float(coord[1])])
            elif isinstance(coord, (int, float)):
                # Skip malformed single-value coordinates
                continue
            else:
                # Skip any other malformed coordinates
                continue

        # Only proceed if we have valid coordinates
        if valid_coordinates:
            shape = shape.copy()
            shape["coordinates"] = valid_coordinates
            return shape
        else:
            # Return a null geometry if no valid coordinates
...
def _determine_shapetype_from_geometry(geometry, geom_format: GeometryFormat) -> int:
    ...
    try:
        ...
        # Return NULL for malformed geometry
        return pyshp.NULL
        ...
        for row in all_rows:
            ...
            if geometry is None:
                ...
                try:
                    ...
                    # Skip rows with invalid geometry data
                    continue
            ...
            if shape is not None:
                if shape["type"] == "Point":
                    # Always use multipoint instead of point in shapefile output
                    shape["type"] = "Multi" + shape["type"]
                    shape["coordinates"] = [shape["coordinates"]]

                # Validate and fix MultiPoint coordinates before passing to pyshp
                shape = _validate_and_fix_multipoint_coordinates(shape)
