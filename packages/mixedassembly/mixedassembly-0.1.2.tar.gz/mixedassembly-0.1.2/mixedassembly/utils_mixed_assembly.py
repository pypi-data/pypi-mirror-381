#!/usr/bin/env python3
"""
Utility functions for the mixed assembly pipeline.
...
This module implements helper functions that:
 ...
 - detect and insert IRMA insertions into a reference-coordinate consensus,
...
logger = logging.getLogger("utils_mixed_assembly")
...
def qc_process(filtered_seqs: Dict[str, str],
    ...
    try:
        ...
        obs_vs_exp_subst = round(mixed_seq_substitutions - exp_substitutions, 3)

        n_insertions, length_insertions = process_insertions(insertions)

        res_dict = {
            "IRMA_COVERAGE": coverage_inicial,
            "MIXED_COVERAGE": coverage_final,
            "IRMA_SUBSTITUTIONS": irma_substitutions,
            "MIXED_SUBSTITUTIONS": mixed_seq_substitutions,
            "EXPECTED_SUBSTITUTIONS": exp_substitutions,
            "OBS-EXP_SUBSTITUTIONS": obs_vs_exp_subst,
            "N INSERTIONS": n_insertions,
...
def add_irma_insertions(irma_aln: str, ref_aln: str, final_seq: str) -> Tuple[str, List[Tuple[int, str]]]:
    """
    Detect insertions in `irma_aln` relative to `ref_aln` (both aligned strings, with '-' gaps),
    ...
    try:
        # Basic checks
        if len(irma_aln) != len(ref_aln):
            raise ValueError("irma_aln and ref_aln must have the same length (aligned with gaps).")
        ...
        # Collect insertions from irma relative to ref
        insertions: List[Tuple[int, str]] = []
        ...
        while pos < L:
            ...
            # If we find a run where ref has '-' but irma has bases -> insertion in irma
            if ref_aln[pos] == '-' and irma_aln[pos] != '-':
                ins_chars: List[str] = []
                # collect contiguous insertion characters while ref_aln is '-' and irma_aln is not '-'
                while pos < L and ref_aln[pos] == '-' and irma_aln[pos] != '-':
                    ins_chars.append(irma_aln[pos])
                    pos += 1
        ...
        logger.exception("add_irma_insertions failed")
        raise


def create_consensus(abacas_seq: str, irma_seq: str, window_df: pd.DataFrame) -> str:
    """
    ...
    otherwise the base from irma_seq is used. Windows may overlap; ABACAS takes priority where windows
    ...
    Args:
        ...
        irma_seq: IRMA sequence (reference-coordinate)
    ...
    """
    try:
        if len(abacas_seq) != len(irma_seq):
            raise ValueError("ABACAS and IRMA sequences must have the same length")
        ...
        for _, row in window_df.iterrows():
            ...
            selected_seq = abacas_seq if source else irma_seq
...
def create_windows_df(windows: List[Tuple[int, int]],
                      ...
                      prior_table: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for window in windows:
        ...
        window_dict = {
            "start": start,
            "end": end,
            "MISSING_IRMA": missing_irma,
            "MISSING_ABACAS": missing_abacas,
            "ABACAS_MORE_INFO": missing_irma > missing_abacas,
        ...
        try:
            ...
            if (abacas_fragments in [1, 2] and (missing_irma > missing_abacas)) and missing_abacas < 50:
