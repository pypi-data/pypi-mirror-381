...
class PressureVessel:
    def __init__(
        ...
        spread_sheet_name="Tankinator.xlsx",
        verbose=False,
    ):
        ########Key inputs##########
    ...
    def run(self):
        #####Run calculation########
        self.compressed_gas_function.func(
            Wind_avai=self.Wind_avai,
            H2_flow=self.H2_flow,
            cdratio=self.cdratio,
        ...
        self.capacity_max = (
            self.compressed_gas_function.capacity_max
        )  # This is the maximum equivalent H2 storage in kg
        self.t_discharge_hr_max = (
            self.compressed_gas_function.t_discharge_hr_max
        )  # This is tha maximum storage duration in kg
    ...
    def calculate_from_fit(self, capacity_kg):
        capex_per_kg = self.compressed_gas_function.exp_log_fit(
            [self.a_fit_capex, self.b_fit_capex, self.c_fit_capex], capacity_kg
        )
    ...
    def distributed_storage_vessels(self, capacity_total_tgt, N_sites):
        ...
        capex_distributed_total = (
            N_sites * capex_site
        )  # the cost for the total distributed storage facilities
        opex_distributed_total = (
            N_sites * opex_site
        )  # the cost for the total distributed storage facilities
...
if __name__ == "__main__":
    ...
    print(
        "tank footprint (upright):",
        storage.get_tank_footprint(capacity_req, upright=True)[0],
    )
    print(
        "tank footprint (flat):",
        storage.get_tank_footprint(capacity_req, upright=False)[0],
    )

    print("\nnumber of tanks req'd:", storage.get_tanks(capacity_req))
    print(
        "total footprint (upright):",
        storage.get_tank_footprint(capacity_req, upright=True)[1],
    )
    print(
        "total footprint (flat):",
        storage.get_tank_footprint(capacity_req, upright=False)[1],
    )
