...
def test_client_initialization_success(grpc_server, testing_certs, monkeypatch, mock_jwt_token):
	"""
    Verify the client can be initialized successfully with valid parameters.
    """
	# Mock for the OIDC discovery GET request
	monkeypatch.setattr(requests, "get", lambda *args, **kwargs: MagicMock(
		json=lambda: {"token_endpoint": "http://fake-keycloak/token"}
	))
	# Mock for the token fetching POST request
	monkeypatch.setattr(requests, "post", lambda *args, **kwargs: MagicMock(
		raise_for_status=lambda: None,
		json=lambda: {"access_token": mock_jwt_token}
	))
...
def test_client_initialization_auth_fails(monkeypatch):
    ...
    """
	# This mock correctly targets the 'get' call that fails first.
	monkeypatch.setattr(requests, "get", lambda *args, **kwargs: MagicMock(
		raise_for_status=lambda: exec('raise requests.exceptions.RequestException("DNS lookup failed")')
	))
...
def test_client_initialization_cert_not_found(monkeypatch, mock_jwt_token):
	"""
...
@pytest.fixture
def fully_initialized_client(grpc_server, testing_certs, monkeypatch, mock_jwt_token):
...
def test_client_list_tools(fully_initialized_client, mock_tool_registry):
    ...
    """
	tools = fully_initialized_client.list_tools(verbose=False)
...
def test_client_call_tool(fully_initialized_client):
    ...
    """
	chunks = list(fully_initialized_client.call_tool("text_tool", {}, verbose=False))
	assert len(chunks) == 2
