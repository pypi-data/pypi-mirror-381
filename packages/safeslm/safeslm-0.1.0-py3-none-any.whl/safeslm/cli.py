...
def main():
    ...
    if args.cmd == "train":
        ...
        client.train(output_dir=args.out_dir, num_train_epochs=args.epochs, per_device_train_batch_size=args.batch_size)

    elif args.cmd == "infer":
        client = SafeSLM(args.model, precision=args.precision, device=args.device, adapter_path=args.adapter)
        out = client.prompt(args.prompt, max_new_tokens=args.max_new_tokens)
        print("=== OUTPUT ===")
        print(out)
