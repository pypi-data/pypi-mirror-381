...
class SafeSLM:
    def __init__(
        ...
        **kwargs,
        ...
        if router is None:
            ...
            router = make_default_router(encoder)
        self.router = router
    ...
    def prompt(self, text: str, max_new_tokens: int = 128, **generate_kwargs) -> str:
        """Run a single prompt through the pipeline and return text."""
        out = self.pipeline.run(prompt=text, max_new_tokens=max_new_tokens, **generate_kwargs)
        return out
