...
class SafeSLMPipeline:
    def __init__(self, base_model, tokenizer, router=None, lora_model=None, device: str = "cpu"):
        self.base_model = base_model.to(device)
        self.lora_model = lora_model.to(device) if lora_model else None  # may be None until adapter loaded
        self.tokenizer = tokenizer
        self.router = router or make_default_router()
        self.device = device

    def run(self, prompt: str, max_new_tokens: int = 128, generation_kwargs=None) -> str:
