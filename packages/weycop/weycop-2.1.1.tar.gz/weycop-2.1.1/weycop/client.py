...
class WeycopClient:
    ...
    def chat_completions_create(
        ...
        Args:
            ...
            reasoning_effort: GPT-5 reasoning effort level ("minimal", "low", "medium", "high"). Only applies to GPT-5 models
            verbosity: GPT-5 verbosity level ("low", "medium", "high"). Only applies to GPT-5 models
...
class AsyncWeycopClient:
    ...
    async def chat_local(
        ...
        **kwargs
        ...
        """
        Create a chat completion using local models (Ollama) - async
        ...
        Returns:
            ChatCompletion object
        """
        return await self.chat_completions_create(model=model, messages=messages, provider="local", **kwargs)
    ...
    async def chat_openai(
        ...
        **kwargs
        ...
        """
        Create a chat completion using OpenAI models - async
        ...
        Returns:
            ChatCompletion object
        """
        return await self.chat_completions_create(model=model, messages=messages, provider="openai", **kwargs)
    ...
    async def chat_anthropic(
        ...
        **kwargs
        ...
        """
        Create a chat completion using Anthropic models - async
        ...
        Returns:
            ChatCompletion object
        """
        return await self.chat_completions_create(model=model, messages=messages, provider="anthropic", **kwargs)
    
    async def get_available_models(self) -> Dict[str, List[str]]:
        """Get available models organized by provider - async"""
        response_data = await self._make_request("GET", "/v1/chat/models")
        
        models_by_provider = {"local": [], "openai": [], "anthropic": []}
        
        for model_data in response_data["data"]:
            provider = model_data.get("owned_by", "local")
            model_id = model_data["id"]
            
            if provider in models_by_provider:
                models_by_provider[provider].append(model_id)
            else:
                models_by_provider["local"].append(model_id)
    ...
    async def get_model_info(self, model: str) -> Dict[str, Any]:
        ...
        try:
            response_data = await self._make_request("GET", f"/v1/chat/models/{model}")
            return response_data
        except APIError:
            # Fallback to generic info if specific model info not available
            return {
                "model": model,
                "provider": "unknown",
    ...
    async def get_health(self) -> HealthStatus:
        """Get service health status - async"""
        response_data = await self._make_request("GET", "/v1/chat/health")
    ...
    async def get_usage(self) -> Dict[str, Any]:
        """Get current usage and quota information - async"""
        return await self._make_request("GET", "/v1/chat/usage")