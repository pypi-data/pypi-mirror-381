...
def get_child_access(fRootProfile, fChildAccount, fRegion="us-east-1", fRoleList=None):
    ...
    if fChildAccount == ParentAccountId:
        explain_string = (
            "We're trying to get access to either the Root Account (which we already have access "
    ...
    for role in fRoleList:
        try:
            logging.info(
                "Trying to access account %s using %s profile assuming role: %s", fChildAccount, fRootProfile, role
            )
...
def get_child_access3(faws_acct, fChildAccount: str, fRegion: str = None, fRoleList: list = None):
    ...
    if fChildAccount == ParentAccountId:
        explain_string = (
            f"We're trying to get access to either the Root Account (which we already have access "
        ...
        logging.info(explain_string)
        # Handle both traditional credentials and SSO profiles
        try:
            # For SSO profiles, credentials might not be directly accessible
            if faws_acct.session.profile_name and (
                not faws_acct.creds.access_key or faws_acct.creds.access_key is None
            ):
                # SSO profile - use profile-based access
                account_credentials = {
                    "ParentAcctId": ParentAccountId,
                    "MgmtAccount": faws_acct.MgmtAccount,
                    "OrgType": org_type,
                    "AccessKeyId": None,  # SSO doesn't expose access keys
                    "SecretAccessKey": None,  # SSO doesn't expose secret keys
                    "SessionToken": None,  # SSO doesn't expose session tokens
                    "AccountNumber": fChildAccount,
                    "AccountId": fChildAccount,
                    "Region": fRegion,
                    "AccountStatus": faws_acct.AccountStatus,
                    "RolesTried": fRoleList,
                    "Role": "Use Profile",
                    "Profile": faws_acct.session.profile_name,
                    "AccessError": False,
                    "Success": True,
                    "ErrorMessage": None,
                }
            ...
            # Fallback for any credential issues
            logging.error(f"Error creating credentials for account {fChildAccount}: {e}")
            account_credentials = {
                "ParentAcctId": ParentAccountId,
                "MgmtAccount": faws_acct.MgmtAccount,
                "OrgType": org_type,
    ...
    for role in fRoleList:
        try:
            ...
            # If we were successful up to this point, then we'll short-cut everything and just return the credentials that worked
            logging.info(
                f"The credentials for account {fChildAccount} using parent account "
    ...
    # Returns a dict object since that's what's expected
    # It will only get to the part below if the child isn't accessed properly using the roles already defined
    logging.debug(f"Failure:\nRole list: {fRoleList}\naccount credentials: {account_credentials}")
    account_credentials = {
        "AccessError": True,
        "Success": False,
...
def enable_drift_on_stacks2(ocredentials: dict, fRegion: str, fStackName: str):
    ...
    logging.info(
        f"Enabling drift detection on Stack {fStackName} in Account {ocredentials['AccountNumber']} in region {fRegion}"
    )
...
def enable_drift_on_stackset2(ocredentials: dict, fStackSetName: str):
    ...
    client_cfn = session_cfn.client("cloudformation")
    logging.info(
        f"Enabling drift detection on Stack {fStackSetName} in "
...
def find_sns_topics2(ocredentials, fTopicFrag: str = None, fExact: bool = False):
    ...
    if "all" in fTopicFrag:
        logging.info(
            f"Looking for all SNS Topics in account {ocredentials['AccountNumber']} from Region {ocredentials['Region']}\n"
        ...
        logging.info(
            f"Looking for specific SNS Topics in account {ocredentials['AccountNumber']} from Region {ocredentials['Region']}"
        )
        topic_list2 = []
        for item in fTopicFrag:
            for topic in TopicList:
                ...
                if fExact:
                    ...
                    topic_list2.append(topic)
        logging.info(
            f"We found {len(topic_list2)} SNS Topics",
        )
        return topic_list2
...
def find_role_names2(ocredentials, fRegion, fRoleNameFrag=None):
    ...
    if fRoleNameFrag is None:
        fRoleNameFrag = ["all"]
    # Prefer SSO/profile-based session if available, else fall back to raw keys or default chain
    if "Profile" in ocredentials and ocredentials["Profile"]:
        ...
        session_iam = boto3.Session(
            aws_access_key_id=ocredentials["AccessKeyId"],
            aws_secret_access_key=ocredentials["SecretAccessKey"],
            aws_session_token=ocredentials.get("SessionToken"),
            region_name=fRegion,
        )
    else:
        session_iam = boto3.Session(region_name=fRegion)
    ...
    if "all" in fRoleNameFrag:
        logging.info(
            f"Looking for all RoleNames in account {ocredentials['AccountNumber']} from Region {fRegion}\n"
            f"RoleName Arns Returned: {RoleNameList}\n"
            f"We found {len(RoleNameList)} RoleNames"
        )
        return RoleNameList
    else:
        logging.info(f"Looking for specific RoleNames in account {ocredentials['AccountNumber']} from Region {fRegion}")
        RoleNameList2 = []
...
def find_cw_log_group_names2(ocredentials, fRegion, fCWLogGroupFrag=None):
    ...
    if "Profile" in ocredentials and ocredentials["Profile"]:
        ...
        session_cw = boto3.Session(region_name=fRegion)
    ...
    if "all" in fCWLogGroupFrag:
        logging.info(
            f"Looking for all Log Group names in account {ocredentials['AccountNumber']} from Region {fRegion}\n"
...
def find_org_services2(ocredentials, serviceNameList=None):
    ...
    if serviceNameList is None:
        serviceNameList = ["all"]
    # Prefer SSO/profile-based session if available, else fall back to raw keys or default chain
    region = ocredentials["Region"] if "Region" in ocredentials.keys() else "us-east-1"
    if "Profile" in ocredentials and ocredentials["Profile"]:
        session_org = boto3.Session(profile_name=ocredentials["Profile"], region_name=region)
    elif "AccessKeyId" in ocredentials and ocredentials["AccessKeyId"]:
        session_org = boto3.Session(
            aws_access_key_id=ocredentials["AccessKeyId"],
            aws_secret_access_key=ocredentials["SecretAccessKey"],
            aws_session_token=ocredentials.get("SessionToken"),
            region_name=region,
        ...
        session_org = boto3.Session(region_name=region)
    client_org = session_org.client("organizations")
    ...
    if "all" in serviceNameList or "All" in serviceNameList or "ALL" in serviceNameList:
        logging.info(
            f"Looking for all Org-Enabled services in account {ocredentials['AccountNumber']} from Region {ocredentials['Region']}\n"
        ...
        logging.info(
            f"Looking for specific enabled Org services in account {ocredentials['AccountNumber']} from Region {ocredentials['Region']}"
        )
...
def disable_org_service2(ocredentials, serviceName=None):
    ...
    try:
        ...
        client_org.exceptions.AccessDeniedException,
        client_org.exceptions.AWSOrganizationsNotInUseException,
        client_org.exceptions.ConcurrentModificationException,
        client_org.exceptions.ConstraintViolationException,
        client_org.exceptions.InvalidInputException,
        client_org.exceptions.ServiceException,
        client_org.exceptions.TooManyRequestsException,
        client_org.exceptions.UnsupportedAPIEndpointException,
    ) as my_Error:
...
def find_security_groups2(
    ocredentials, f_fragments: list = None, f_exact: bool = False, defaultOnly: bool = False
) -> list:
    """
    ocredentials is an object with the following structure:
            - ['AccessKeyId'] holds the AWS_ACCESS_KEY
    ...
    logging.info(
        f"Looking for default Security Groups in account {ocredentials['AccountNumber']} from Region {ocredentials['Region']}"
    )
    ...
    logging.info(
        f"We found {len(AllSecurityGroups)} {'default' if defaultOnly else ''} Security Groups in account {ocredentials['AccountNumber']} in Region {ocredentials['Region']}"
    )
...
def find_references_to_security_groups2(ocredentials, f_security_group: dict):
    ...
    logging.info(
        f"Looking for Security Groups in account {ocredentials['AccountNumber']} from Region {ocredentials['Region']}"
    )
    ...
    response = client_vpc.describe_network_interfaces(
        Filters=[
            {"Name": "group-id", "Values": [f_security_group["GroupId"]]},
        ]
    )
    for network_interface in response["NetworkInterfaces"]:
        ...
        network_interface["AttachmentId"] = (
            network_interface["Attachment"]["InstanceId"]
            if "InstanceId" in network_interface["Attachment"].keys()
            else None
        )
        if "InstanceId" in network_interface["Attachment"].keys():
            InstanceTags = client_vpc.describe_instances(InstanceIds=[network_interface["Attachment"]["InstanceId"]])[
                "Reservations"
            ][0]["Instances"][0]["Tags"]
    ...
    while "NextToken" in response.keys():
        ...
        for network_interface in response["NetworkInterfaces"]:
            ...
            if "InstanceId" in network_interface["Attachment"].keys():
                InstanceTags = client_vpc.describe_instances(
                    InstanceIds=[network_interface["Attachment"]["InstanceId"]]
            ...
            SecurityGroupReferences.append(network_interface)
    logging.info(
        f"We found {len(SecurityGroupReferences)} references to security groups in account {ocredentials['AccountNumber']} in Region {ocredentials['Region']}"
    )
    # This second lookup section handles the other Security Groups case.
    response_inbound = client_vpc.describe_security_groups(
        Filters=[
            {"Name": "ip-permission.group-id", "Values": [f_security_group["GroupId"]]},
        ]
    )
    ...
    response_outbound = client_vpc.describe_security_groups(
        Filters=[
            {"Name": "egress.ip-permission.group-id", "Values": [f_security_group["GroupId"]]},
        ]
    )
...
def find_account_vpcs2(ocredentials, defaultOnly=False):
    ...
    try:
        if defaultOnly:
            logging.info(
                f"Looking for default VPCs in account {ocredentials['AccountNumber']} from Region {ocredentials['Region']}"
            )
            ...
            logging.info(
                f"Looking for all VPCs in account {ocredentials['AccountNumber']} from Region {ocredentials['Region']}"
            )
            logging.info(f"defaultOnly: {str(defaultOnly)}")
            # TODO: Enable pagination
            response = client_vpc.describe_vpcs()
        logging.info(
            f"We found {len(response['Vpcs'])} VPCs in account {ocredentials['AccountNumber']} in Region {ocredentials['Region']}"
        )
    except ClientError as my_Error:
        if "UnauthorizedOperation" in str(my_Error):
            logging.warning(
                f"Authorization Failure accessing account {ocredentials['AccountNumber']} in {ocredentials['Region']} region"
            )
...
def del_config_recorder2(ocredentials, fRegion, fConfig_recorder_name):
    ...
    client_cfg = session_cfg.client("config")
    logging.error(
        "Deleting Config Recorder %s from Region %s in account %s",
        fConfig_recorder_name,
        fRegion,
...
def del_delivery_channel2(ocredentials, fRegion, fDelivery_channel_name):
    ...
    client_cfg = session_cfg.client("config")
    logging.error(
        "Deleting Delivery Channel %s from Region %s in account %s",
        fDelivery_channel_name,
        fRegion,
        ocredentials["AccountNumber"],
    )
...
def del_config_recorder_or_delivery_channel2(deletion_item):
    ...
    client_cfg = session_cfg.client("config")
    logging.error(
        f"Deleting {deletion_item['Type']} '{deletion_item['name']}' from Region {deletion_item['Region']} in account {deletion_item['AccountId']}"
    )
    response = {"Success": False, "ErrorMessage": None}
    try:
...
def find_cloudtrails2(ocredentials, fRegion, fCloudTrailnames=None):
    ...
    fullresponse = []
    if (
        fCloudTrailnames is None or len(fCloudTrailnames) == 0
    ):  # Therefore - they're really looking for a list of trails
        try:
...
def del_cloudtrails2(ocredentials, fRegion, fCloudTrail):
    ...
    client_ct = session_ct.client("cloudtrail")
    logging.info(
        "Deleting CloudTrail %s in account %s from Region %s", fCloudTrail, ocredentials["AccountNumber"], fRegion
    )
...
def find_gd_invites2(ocredentials, fRegion):
    ...
    logging.info(
        "Looking for GuardDuty invitations in account %s from Region %s", ocredentials["AccountNumber"], fRegion
    )
    try:
        ...
        if "AuthFailure" in str(my_Error):
            print(
                ocredentials["AccountNumber"]
                + ": Authorization Failure for account {}".format(ocredentials["AccountNumber"])
            )
        if str(my_Error).find("security token included in the request is invalid") > 0:
            print(
                "Account #:"
                + ocredentials["AccountNumber"]
                + f" - It's likely that the region you're trying ({fRegion}) isn't enabled for your account"
            )
        else:
            print(my_Error)
...
def delete_gd_invites2(ocredentials, fRegion, fAccountId):
    ...
    client_gd = session_gd.client("guardduty")
    logging.info("Looking for GuardDuty invitations in account %s in Region %s", ocredentials["AccountNumber"], fRegion)
    try:
...
def find_account_instances2(ocredentials=None):
    ...
    if ocredentials is None:
        ...
        if ProfileAccountNumber == ocredentials["AccountNumber"]:
            session_ec2 = boto3.Session(profile_name=ocredentials["Profile"], region_name=ocredentials["Region"])
        else:
            session_ec2 = boto3.Session(
                aws_access_key_id=ocredentials["AccessKeyId"],
                aws_secret_access_key=ocredentials["SecretAccessKey"],
    ...
    logging.info(
        f"Looking for instances in account # {ocredentials['AccountNumber']} in region {ocredentials['Region']}"
    )
...
def find_account_ecs_clusters_services_and_tasks2(ocredentials=None) -> list[dict[str, any]]:
    ...
    if ocredentials is None:
        ...
        if ProfileAccountNumber == ocredentials["AccountNumber"]:
            session_ecs = boto3.Session(profile_name=ocredentials["Profile"], region_name=ocredentials["Region"])
        else:
            session_ecs = boto3.Session(
                aws_access_key_id=ocredentials["AccessKeyId"],
                aws_secret_access_key=ocredentials["SecretAccessKey"],
    ...
    try:
        ...
        logging.info(
            f"Looking for clusters in account # {ocredentials['AccountNumber']} in region {ocredentials['Region']}"
        )
...
def find_account_rds_instances2(ocredentials=None, fRegion=None):
    ...
    if ocredentials is None:
        ...
        if ProfileAccountNumber == ocredentials["AccountNumber"]:
            session_rds = boto3.Session(profile_name=ocredentials["Profile"], region_name=ocredentials["Region"])
        else:
            session_rds = boto3.Session(
                aws_access_key_id=ocredentials["AccessKeyId"],
                aws_secret_access_key=ocredentials["SecretAccessKey"],
    ...
    logging.info(
        f"Looking for RDS instances in account #{ocredentials['AccountNumber']} in region {ocredentials['Region']}"
    )
...
def find_global_accelerators2(ocredentials=None):
    ...
    if ocredentials is None:
        ...
        if ProfileAccountNumber == ocredentials["AccountNumber"]:
            session_ga = boto3.Session(profile_name=ocredentials["Profile"], region_name="us-west-2")
        else:
            session_ga = boto3.Session(
                aws_access_key_id=ocredentials["AccessKeyId"],
                aws_secret_access_key=ocredentials["SecretAccessKey"],
                aws_session_token=ocredentials["SessionToken"],
    ...
    while "NextToken" in Subnets.keys():
        ...
        try:
            if fipaddresses is None:
                logging.info(
                    f"Looking for all subnets in account #{ocredentials['AccountNumber']} in region {ocredentials['Region']}"
                )
                Subnets = subnet_info.describe_subnets()
                AllSubnets = Subnets
            else:
                logging.info(
                    f"Looking for Subnets that match any of {fipaddresses} in account #{ocredentials['AccountNumber']} in region {ocredentials['Region']}"
                )
                Subnets = subnet_info.describe_subnets()
                ...
                for subnet in Subnets["Subnets"]:
                    for address in fipaddresses:
                        logging.info(
                            f"{address} in {subnet['CidrBlock']}: {ipaddress.ip_address(address) in ipaddress.ip_network(subnet['CidrBlock'])}"
                        )
                        if ipaddress.ip_address(address) in ipaddress.ip_network(subnet["CidrBlock"]):
                            AllSubnets["Subnets"].append(subnet)
        except ClientError as my_Error:
            logging.error(
                f"Error connecting to account {ocredentials['AccountNumber']} in region {ocredentials['Region']}\n"
...
def find_tgws2(ocredentials):
    ...
    while "NextToken" in TGWs.keys():
        ...
        try:
            logging.info(
                f"Looking for all tgws in account #{ocredentials['AccountNumber']} in region {ocredentials['Region']}"
            )
...
def find_attachments2(ocredentials):
    ...
    while "NextToken" in Attachments.keys():
        ...
        try:
            logging.info(
                f"Looking for all tgw attachments in account #{ocredentials['AccountNumber']} in region {ocredentials['Region']}"
            )
...
def find_account_enis2(ocredentials, fRegion=None, fipaddresses=None):
    ...
    while "NextToken" in ENIs.keys():
        ...
        try:
            logging.info(
                f"Looking for ENIs that match any of {fipaddresses} in account #{ocredentials['AccountNumber']} in region {fRegion}"
            )
                ...
                if fipaddresses is not None:
                    ...
                    for address in fipaddresses:
                        if address == interface["PrivateIpAddress"]:
                            logging.info(
                                f"Found it - {interface['PrivateIpAddress']} - ENI: {interface['NetworkInterfaceId']}"
                            )
                            return_this_result = True
                        elif (
                            "Association" in interface.keys()
                            and "PublicIp" in interface["Association"].keys()
                            and address == interface["Association"]["PublicIp"]
                        ):
                            logging.info(
                                f"Found it - {interface['Association']['PublicIp']} - ENI: {interface['NetworkInterfaceId']}"
                            )
                            return_this_result = True
                        else:
                            continue
                    ...
                    AllENIs.append(
                        {
                            ...
                            "InstanceId": interface["Attachment"]["InstanceId"]
                            if ("Attachment" in interface.keys() and "InstanceId" in interface["Attachment"].keys())
                            else "No instance association",
                            "AttachmentStatus": interface["Attachment"]["Status"]
                            if "Attachment" in interface.keys()
                            else "Not attached",
                            "PublicIp": interface["Association"]["PublicIp"]
                            if "Association" in interface.keys() and "PublicIp" in interface["Association"].keys()
                            else "No Public IP",
                        }
                    )
        except ClientError as my_Error:
            logging.error(
                f"Error connecting to account {ocredentials['AccountNumber']} in region {fRegion}\n"
...
def find_account_volumes2(ocredentials):
    ...
    while "NextToken" in Volumes.keys():
        ...
        try:
            logging.info(
                f"Looking for all volumes in account #{ocredentials['AccountNumber']} in region {ocredentials['Region']}"
            )
                    ...
                    for attachment in volume["Attachments"]:
                        if "InstanceId" in attachment.keys():
                            AttachmentList.append(
                                {"InstanceId": attachment["InstanceId"], "AttachmentStatus": attachment["State"]}
                            )
                AllVolumes.append(
                    {
                        "VolumeName": Name,
...
def find_account_policies2(
    ocredentials, fRegion="us-east-1", fFragments: list = None, fExact: bool = False, fCMP: bool = False
) -> list:
    """
    ocredentials is an object with the following structure:
            - ['AccessKeyId'] holds the AWS_ACCESS_KEY
    ...
    if "Profile" in ocredentials and ocredentials["Profile"]:
        ...
        # Fallback to default credential chain for SSO
        session_iam = boto3.Session(region_name=fRegion)
...
def find_account_policies3(faws_acct, fRegion="us-east-1", fFragments=None):
    ...
    while Policies["IsTruncated"]:
        ...
        try:
            ...
            logging.error(
                f"Error connecting to account {faws_acct.acct_number} in region {fRegion}\n"
                f"This is likely due to '{fRegion}' not being enabled for your account\n"
                f"Error Message: {my_Error}"
            )
            continue
...
def find_policy_action2(ocredentials, fpolicy, f_action):
    ...
    try:
        logging.debug(
            f"Getting policy statements for policy {fpolicy} that exists within account #{ocredentials['AccountNumber']}"
        )
        Policy_statements = policy_info.get_policy_version(
            PolicyArn=fpolicy["Arn"], VersionId=fpolicy["DefaultVersionId"]
        )["PolicyVersion"]
        # Run through each of the policies, and determine if the passed in action fits within any of them
        # If it does - then include that data within the array, otherwise next...
...
def find_iam_users2(ocredentials):
    ...
    # Prefer SSO/profile-based session if available, else fall back to raw keys or default chain
    region = ocredentials["Region"] if "Region" in ocredentials.keys() else "us-east-1"
    logging.info(f"Account ID #: {str(ocredentials.get('AccountId', ocredentials.get('AccountNumber', 'unknown')))}")
    if "Profile" in ocredentials and ocredentials["Profile"]:
        session_iam = boto3.Session(profile_name=ocredentials["Profile"], region_name=region)
    elif "AccessKeyId" in ocredentials and ocredentials["AccessKeyId"]:
        session_iam = boto3.Session(
            aws_access_key_id=ocredentials["AccessKeyId"],
            aws_secret_access_key=ocredentials["SecretAccessKey"],
        ...
        session_iam = boto3.Session(region_name=region)
    client_iam = session_iam.client("iam")
...
def find_idc_directory_id2(ocredentials) -> list:
    ...
    region = ocredentials["Region"] if "Region" in ocredentials.keys() else "us-east-1"
    if "Profile" in ocredentials and ocredentials["Profile"]:
        session_idc = boto3.Session(profile_name=ocredentials["Profile"], region_name=region)
    elif "AccessKeyId" in ocredentials and ocredentials["AccessKeyId"]:
        session_idc = boto3.Session(
            aws_access_key_id=ocredentials["AccessKeyId"],
            aws_secret_access_key=ocredentials["SecretAccessKey"],
            aws_session_token=ocredentials.get("SessionToken"),
            region_name=region,
        ...
        session_idc = boto3.Session(region_name=region)
    ...
    while "NextToken" in idc_info.keys():
        ...
        all_idc_info_list.extend(idc_info["Instances"])
    idc_directory_id_list = [
        instance["IdentityStoreId"] for instance in all_idc_info_list if "IdentityStoreId" in instance.keys()
    ]
...
def find_idc_users2(ocredentials, f_IdentityStoreId: str) -> list:
    ...
    if f_IdentityStoreId is None:
        logging.info(
            "No Identity Store ID was provided.\n"
        ...
        if "all" in fSearchStrings:
                ...
                AllFunctions.append(
                    {
                        ...
                        "SecurityGroups": function["VpcConfig"]["SecurityGroupIds"]
                        if "VpcConfig" in function.keys() and "SecurityGroupIds" in function["VpcConfig"].keys()
                        else None,
                    }
                )
            if fTagValueToFilter is not None:
                ...
                for lambda_item in AllFunctions:
                    ...
                    AllFilteredFunctions.append(
                        {
                            "Function": lambda_function["Configuration"],
                            "Tags": lambda_function["Tags"]
                            if "Tags" in lambda_function.keys()
                            and any(fTagValueToFilter in lambda_function["Tags"].values())
                            else [],
                        }
                    )
                # for i in t:
            ...
            if fTagValueToFilter is not None:
                ...
                for lambda_item in AllFunctions:
                    ...
                    AllFilteredFunctions.append(
                        {
                            "Function": lambda_function["Configuration"],
                            "Tags": lambda_function["Tags"] if "Tags" in lambda_function.keys() else [],
                        }
                    )
                AllFunctions = AllFilteredFunctions.copy()
            return AllFunctions
...
def find_directories2(ocredentials, fRegion="us-east-1", fSearchStrings=None, fExact: bool = False):
    ...
    try:
        # Prefer SSO/profile-based session if available, else fall back to raw keys or default chain
        region = ocredentials.get("Region", fRegion)
        if "Profile" in ocredentials and ocredentials["Profile"]:
            session_ds = boto3.Session(profile_name=ocredentials["Profile"], region_name=region)
        elif "AccessKeyId" in ocredentials and ocredentials["AccessKeyId"]:
            session_ds = boto3.Session(
                aws_access_key_id=ocredentials["AccessKeyId"],
                aws_secret_access_key=ocredentials["SecretAccessKey"],
                aws_session_token=ocredentials.get("SessionToken"),
                region_name=region,
            ...
            session_ds = boto3.Session(region_name=region)
    ...
    if fSearchStrings is None or "all" in fSearchStrings:
        ...
        for directory in all_directories:
            for searchitem in fSearchStrings:
                ...
                if Collectitem:
                    logging.info(
                        f"Found fragment {searchitem} in directory {directory['Name']} in account {ocredentials['AccountId']}"
                    )
                    response_dict = {
                        "DirectoryName": directory["Name"],
                        "DirectoryId": directory["DirectoryId"],
...
def find_load_balancers(fProfile, fRegion, fStackFragment="all", fStatus="all"):
    ...
    if fStackFragment.lower() == "all" and (fStatus.lower() == "active" or fStatus.lower() == "all"):
        logging.info(
            "Found all the lbs in Profile: %s in Region: %s with Fragment: %s and Status: %s",
            fProfile,
            fRegion,
            fStackFragment,
            fStatus,
        ...
        for load_balancer in load_balancers["LoadBalancers"]:
            if fStatus in load_balancer["State"]["Code"]:
                logging.info(
                    "Found lb %s in Profile: %s in Region: %s with Fragment: %s and Status: %s",
                    load_balancers["LoadBalancerName"],
                    fProfile,
                    fRegion,
                    fStackFragment,
                    fStatus,
...
def find_load_balancers2(ocredential=None, fStackFragments=None, fStatus=None) -> list:
    ...
    if ocredential is None:
        ...
        session_elb = boto3.Session(
            aws_access_key_id=ocredential["AccessKeyId"],
        ...
        logging.info(
            f"Account: {ocredential['AccountId']} | Profile: {ocredential['Profile']} | Region: {ocredential['Region']} | Fragment: {fStackFragments} | Status: {fStatus}"
        )
    lb_info = session_elb.client("elbv2")
    load_balancers = lb_info.describe_load_balancers()
    load_balancers_Copy = []
    if ("all" in fStackFragments or "All" in fStackFragments or "ALL" in fStackFragments) and (
        fStatus.lower() == "active" or fStatus.lower() == "all"
    ):
        if ocredential is None:
            logging.info(f"Found all the lbs with Fragment: {fStackFragments} and Status: {fStatus}")
        else:
            logging.info(
                f"Found all the lbs in Account: {ocredential['AccountId']} in Region: {ocredential['Region']} with Fragment: {fStackFragments} and Status: {fStatus}"
            )
        return load_balancers["LoadBalancers"]
        ...
        for load_balancer in load_balancers["LoadBalancers"]:
            if fStatus in load_balancer["State"]["Code"]:
                if ocredential is None:
                    logging.info(
                        f"Found lb {load_balancers['LoadBalancerName']} with Fragment: {fStackFragments} and Status: {fStatus}"
                    )
                else:
                    logging.info(
                        f"Found lb {load_balancers['LoadBalancerName']} in Account: {ocredential['AccountId']} in Region: {ocredential['Region']} with Fragment in {fStackFragments} and Status: {fStatus}"
                    )
                load_balancers_Copy.append(load_balancer)
    elif fStatus.lower() == "active":
        for load_balancer in load_balancers["LoadBalancers"]:
            for stack_fragment in fStackFragments:
                if stack_fragment in load_balancer["LoadBalancerName"]:
                    if ocredential is None:
                        ...
                        logging.info(
                            f"Found lb {load_balancers['LoadBalancerName']} in Account: {ocredential['AccountId']} in Region: {ocredential['Region']} with Fragment: {stack_fragment} and Status: {fStatus}"
                        )
                    load_balancers_Copy.append(load_balancer)
    return load_balancers_Copy
    ...
    if faws_acct.Success:
        ...
        if ("all" in fStackFragments or "All" in fStackFragments or "ALL" in fStackFragments) and (
            ...
            for load_balancer in load_balancers["LoadBalancers"]:
                if fStatus in load_balancer["State"]["Code"]:
                    logging.info(
                        f"Found lb {load_balancers['LoadBalancerName']} in Account: {faws_acct.acct_number} in Region: {fRegion} with Fragment in {fStackFragments} and Status: {fStatus}"
                    )
                    load_balancers_Copy.append(load_balancer)
        elif fStatus.lower() == "active":
            for load_balancer in load_balancers["LoadBalancers"]:
                for stack_fragment in fStackFragments:
                    if stack_fragment in load_balancer["LoadBalancerName"]:
                        logging.info(
                            f"Found lb {load_balancers['LoadBalancerName']} in Account: {faws_acct.acct_number} in Region: {fRegion} with Fragment: {stack_fragment} and Status: {fStatus}"
                        )
                        load_balancers_Copy.append(load_balancer)
        return load_balancers_Copy
    else:
        raise Exception(
            f"Profile: {faws_acct.Profile} in Region: {faws_acct.Region} didn't authenticate properly. Please check credentials"
        )
    ...
    if fStatus.lower() == "active" and not fStackFragment.lower() == "all":
        ...
        for stack in AllStacks:
            if fStackFragment in stack["StackName"]:
                ...
                logging.info(
                    "Found stack %s in Profile: %s in Region: %s with Fragment: %s and Status: %s",
                    stack["StackName"],
                    fProfile,
                    fRegion,
                    fStackFragment,
        ...
        stacks = client_cfn.list_stacks(
            StackStatusFilter=["CREATE_COMPLETE", "DELETE_FAILED", "UPDATE_COMPLETE", "UPDATE_ROLLBACK_COMPLETE"]
        )
        ...
        for stack in stacks["StackSummaries"]:
            ...
            logging.info(
                "Found stack %s in Profile: %s in Region: %s regardless of fragment and Status: %s",
                stack["StackName"],
                fProfile,
                fRegion,
                fStatus,
...
def find_stacks2(ocredentials: dict, fRegion: str, fStackFragment: list = None, fStatus: list = None):
    ...
    if desired_status == "active" and not (
        "all" in fStackFragment or "ALL" in fStackFragment or "All" in fStackFragment
    ):
        # Send back stacks that are active, check the fragment further down.
        ...
        for stack in allstacks:
            for fragment in fStackFragment:
                if fragment in stack["StackName"]:
                    # Check the fragment now - only send back those that match
                    logging.info(
                        f"1-Found stack {stack['StackName']} in Account: {ocredentials['AccountNumber']} in "
    ...
    # For all stacks, where we *did not* specify a fragment to find
        ...
        stacks = client_cfn.list_stacks(StackStatusFilter=fStatus)
        logging.info(
            f"Found {len(stacks['StackSummaries'])} stacks in Account: {ocredentials['AccountNumber']} in "
    ...
    # TODO: This case will never be triggered, since "all" stacks will be covered by the case above.
        ...
        for stack in stacks["StackSummaries"]:
            logging.info(
                f"Found stack {stack['StackName']} in Account: {ocredentials['AccountNumber']} in "
                ...
                for fragment in fStackFragment:
                    for status in fStatus:
                        if fragment in stack["StackName"] and status == stack["StackStatus"]:
                            # Check the fragment now - only send back those that match
                            logging.info(
                                f"Found stack {stack['StackName']} in Account: {ocredentials['AccountNumber']}"
        ...
        for stack in AllStacks:
            ...
            for fragment in fStackFragment:
                if stack["StackName"].find(fragment) > -1:
                    logging.info(
                        f"Found stack {stack['StackName']} in Account: {faws_acct.acct_number} in Region: {fRegion} with Fragment: {fragment}"
                    )
...
def find_stacks_in_acct3(faws_acct, fRegion, fStackFragment="all", fStatus="active"):
    ...
    StackStatusFilter_active = ["CREATE_COMPLETE", "UPDATE_COMPLETE", "UPDATE_ROLLBACK_COMPLETE"]
    if fStatus.lower() == "active" and not fStackFragment.lower() == "all":
        # Send back stacks that are active, check the fragment further down.
        ...
        for stack in stacks["StackSummaries"]:
            if fStackFragment in stack["StackName"]:
                # Check the fragment now - only send back those that match
                logging.info(
                    f"1-Found stack {stack['StackName']} in Account: {faws_acct.acct_number} in "
                    f"Region: {fRegion} with Fragment: {fStackFragment} and Status: {fStatus}"
                )
                stacksCopy.append(stack)
    elif fStatus.lower() == "active" and fStackFragment.lower() == "all":
        ...
        for stack in stacks["StackSummaries"]:
            logging.info(
                f"2-Found stack {stack['StackName']} in Account: {faws_acct.acct_number} in "
                f"Region: {fRegion} with Fragment: {fStackFragment} and Status: {fStatus}"
            )
            stacksCopy.append(stack)
    elif fStatus.lower() == "all" and fStackFragment.lower() == "all":
        ...
        logging.info(f"4-Found {len(stacks)} stacks in Account: {faws_acct.acct_number} in Region: {fRegion}")
        return stacks["StackSummaries"]
        ...
        for stack in stacks["StackSummaries"]:
            if fStackFragment in stack["StackName"]:
                # Check the fragment now - only send back those that match, regardless of status
                logging.info(
                    f"3-Found stack {stack['StackName']} in Account: {faws_acct.acct_number} in "
                    f"Region: {fRegion} with Fragment: {fStackFragment} and Status: {fStatus}"
                )
                stacksCopy.append(stack)
    elif not fStatus.lower() == "active":
        ...
        if "StackSummaries" in stacks.keys():
            for stack in stacks["StackSummaries"]:
                if fStackFragment in stack["StackName"] and fStatus in stack["StackStatus"]:
                    # Check the fragment now - only send back those that match
                    logging.info(
                        f"5-Found stack {stack['StackName']} in Account: {faws_acct.acct_number}"
...
def find_stacksets2(ocredentials: dict, fStackFragments: list = None, fStatus: str = None):
    ...
    logging.info(
        f"Account ID: {ocredentials['AccountId']} | Region: {ocredentials['Region']} | Fragment: {fStackFragments} | Status: {fStatus}"
    )
    session_aws = boto3.Session(
        region_name=ocredentials["Region"],
        aws_access_key_id=ocredentials["AccessKeyId"],
    ...
    try:
        logging.info(
            f"Looking for stack sets in account {ocredentials['AccountId']} matching fragment {fStackFragments} with status {fStatus}"
        )
    ...
    if ("all" in fStackFragments or "ALL" in fStackFragments or "All" in fStackFragments) and len(stacksets2) > 0:
        logging.info(
            f"Found all the stacksets in Account: {ocredentials['AccountNumber']} in Region: {ocredentials['Region']}"
        )
        return stacksets2
    else:
        stacksetsCopy = []
        for stack in stacksets2:
            for stackfrag in fStackFragments:
                if stackfrag in stack["StackSetName"]:
                    logging.info(
                        f"Found stackset {stack['StackSetName']} in account: {ocredentials['AccountId']} in Region: {ocredentials['Region']} with Fragment: {stackfrag}"
                    )
...
def find_stacksets3(
    faws_acct, fRegion: str = None, fStackFragmentList: list = None, fExact: bool = False, fGetHealth: bool = False
) -> dict:
    """
            ...
            def run(self):
                while True:
                    ...
                    try:
                        ...
                        stack_set_operations = client_cfn.list_stack_set_operations(
                            StackSetName=c_stackset["StackSetName"]
                        )
                        ...
                        if "NextToken" in stack_set_operations.keys():
                            while "NextToken" in stack_set_operations.keys():
                                stack_set_operations = client_cfn.list_stack_set_operations(
                                    StackSetName=c_stackset["StackSetName"], NextToken=stack_set_operations["NextToken"]
                                )
                                all_stack_set_operations.extend(stack_set_operations["Summaries"])
                        all_stack_instances = []
                        number_of_stack_instances_dict = client_cfn.list_stack_instances(
                            StackSetName=c_stackset["StackSetName"]
                        )
                        ...
                        if "NextToken" in number_of_stack_instances_dict.keys():
                            while "NextToken" in number_of_stack_instances_dict.keys():
                                number_of_stack_instances_dict = client_cfn.list_stack_set_operations(
                                    StackSetName=c_stackset["StackSetName"],
                        ...
                        sorted_operations = sorted(
                            all_stack_set_operations,
                            key=lambda d: (d["Status"], d.get("EndTimestamp", datetime.now())),
                            reverse=True,
                        )
                        logging.info(
                            f"Found {len(all_stack_set_operations)} operations within the StackSet {c_stackset['StackSetName']}"
                        )
                        ...
                        c_stackset.update(
                            {
                                ...
                                "EndTimestamp": operation_end_timestamp,
                                "OperationId": operation_id,
                                "Stack_Instances_number": len(all_stack_instances),
                            }
                        )
                    except ClientError as my_Error:
...
def find_stack_instances(fProfile, fRegion, fStackSetName, fStatus="CURRENT"):
    ...
    while "NextToken" in stack_instances.keys():  # Get all instance names
        stack_instances = client_cfn.list_stack_instances(
            StackSetName=fStackSetName, NextToken=stack_instances["NextToken"]
        )
...
def find_stack_instances3(
    ...
    logging.info(
        f"Account: {faws_acct.acct_number} | Region: {fRegion} | StackSetName: {fStackSetName} | account list: {faccountlist}"
    )
...
def delete_stack_instances(
    fProfile, fRegion, lAccounts, lRegions, fStackSetName, fRetainStacks=False, fOperationName="StackDelete"
):
    """
...
def delete_stack_instances3(
    ...
    try:
        if fPermissionModel == "SELF_MANAGED":
            response.update(
                client_cfn.delete_stack_instances(
                    StackSetName=fStackSetName,
                    Accounts=lAccounts,
                    Regions=lRegions,
                    RetainStacks=fRetainStacks,
...
def find_sc_products(fProfile, fRegion, fStatus="ERROR", flimit=100):
    ...
    if fStatus.lower() == "all":
        ...
        response = client_sc.search_provisioned_products(
            PageSize=flimit, Filters={"SearchQuery": [f"status:{fStatus}"]}
        )
        while "NextPageToken" in response.keys():
            response2.extend(response["ProvisionedProducts"])
            response = client_sc.search_provisioned_products(
                PageSize=flimit, Filters={"SearchQuery": [f"status:{fStatus}"]}, PageToken=response["NextPageToken"]
            )
    response2.extend(response["ProvisionedProducts"])
    return response2
...
def find_ssm_parameters(fProfile, fRegion):
    ...
    while "NextToken" in response.keys():
        ...
        TotalParameters = TotalParameters + len(response["Parameters"])
        logging.info(
            f"Found another {len(response['Parameters'])} parameters, bringing the total up to {TotalParameters}"
        )
        for param in response["Parameters"]:
            response2.append(
                {
        ...
        if (len(response2) % 500 == 0) and (logging.getLogger().getEffectiveLevel() > 20):
            print(
                f"{ERASE_LINE}Sorry this is taking a while - we've already found {len(response2)} parameters!", end="\r"
            )
...
def find_ssm_parameters2(ocredentials):
    ...
    if "Profile" in ocredentials and ocredentials["Profile"]:
        ...
        # Fallback to default credential chain for SSO
        session_ssm = boto3.Session(region_name=region)
    client_ssm = session_ssm.client("ssm")
    response = {}
    response2 = []
    TotalParameters = 0
    ...
    try:
        ...
        for param in response["Parameters"]:
            response2.append(
                {
                    "MgmtAcct": ocredentials["MgmtAccount"],
                    "AccountNumber": ocredentials["AccountNumber"],
                    "Region": region,
                    "Profile": session_ssm.profile_name,
...
def find_ssm_parameters3(faws_acct, fregion=None):
    ...
    try:
        ...
        for param in response["Parameters"]:
            response2.append(
                {
                    "MgmtAcct": faws_acct.MgmtAccount,
                    "AccountNumber": faws_acct.acct_number,
                    "Region": fregion,
                    "Profile": session_ssm.profile_name,
                    "Description": param["Description"] if "Description" in param.keys() else None,
...
def get_region_azs2(ocredentials):
    ...
    logging.info(
        f"Finding available availability zones for account {ocredentials['AccountNumber']} in Region {ocredentials['Region']}"
    )
    session_ec2 = boto3.Session(
        aws_access_key_id=ocredentials["AccessKeyId"],
        aws_secret_access_key=ocredentials["SecretAccessKey"],
...
def get_all_credentials(
    fProfiles: list = None,
    fTiming: bool = False,
    fSkipProfiles: list = None,
    fSkipAccounts: list = None,
    fRootOnly: bool = False,
    fAccounts: list = None,
    fRegionList: list = None,
    RoleList: list = None,
) -> list:
    ...
    if fProfiles is None:  # Default use case from the classes
        ...
        AllCredentials.extend(
            get_credentials_for_accounts_in_org(
                aws_acct, fSkipAccounts, fRootOnly, fAccounts, profile, RegionList, RoleList, fTiming
            )
        )
    else:
...
def get_credentials_for_accounts_in_org(
    faws_acct,
    fSkipAccounts=None,
    fRootOnly=False,
    accountlist=None,
    ...
    class AssembleCredentials(Thread):
        ...
        def run(self):
            while True:
                ...
                try:
                    logging.info(
                        f"Attempting to connect to {c_account_info['AccountId']} {'using a role from this list: ' + fRoleNames if fRoleNames is not None else 'using pre-configured role names'} "
                    )
                    faccount_credentials = get_child_access3(
                        faws_acct, c_account_info["AccountId"], c_region, fRoleNames
                    )
                    if faccount_credentials["Success"]:
                        logging.info(f"Successfully connected to account {c_account_info['AccountId']}")
                        faccount_credentials.update({"ParentProfile": c_profile, "RolesTried": fRoleNames})
                    else:
                        logging.error(
                            f"Error connecting to account {c_account_info['AccountId']} in region {c_region}.\n"
                            f"Parent Profile was {c_profile}\n"
                            f"Error Message: {faccount_credentials['ErrorMessage']}"
                        )
                        faccount_credentials.update(
                            {
                                "MgmtAccount": c_account_info["MgmtAccount"],
    ...
    logging.debug(
        f"Account Passed in: {faws_acct.acct_number} | Child Accounts: {ChildAccounts} | Account Type: {faws_acct.AccountType} | Account Status: {faws_acct.AccountStatus} | RoleNames to try: {fRoleNames}"
    )
