...
def mock_reducto_client():
    ...
    class SimpleHttpClient:
        ...
        def post(self, url: str, headers: dict | None = None, **kwargs):
            return httpx.post(url, headers=self._merge_headers(headers), **kwargs)

        def put(self, url: str, headers: dict | None = None, **kwargs):
            return httpx.put(url, headers=self._merge_headers(headers), **kwargs)
    ...
    with patch(
        "sema4ai_docint.extraction.reducto.sync.SyncExtractionClient._new_reducto_client"
    ) as mock:
...
def test_successful_upload(test_server, test_file, mock_reducto_client):
    ...
    # Create client
    client = SyncExtractionClient(api_key="test_key", base_url=base_url)
...
def test_upload_403_error(test_server, test_file, mock_reducto_client):
    ...
    with pytest.raises(UploadForbiddenError) as exc_info:
        client.upload(test_file)

    msg = str(exc_info.value)
    assert "HTTP 403" in msg or "403" in msg


def test_upload_401_error(test_server, test_file, mock_reducto_client):
...
def test_upload_500_error(test_server, test_file, mock_reducto_client):
...
def test_upload_with_inline_bytes(test_server, mock_reducto_client):
    ...
    client = SyncExtractionClient(api_key="test_key", base_url=base_url)
    result = client.upload(b"inline-bytes-content")
    assert result == "bytes_file_id_123"


def test_upload_with_file_like_bytesio(test_server, mock_reducto_client):
    ...
    client = SyncExtractionClient(api_key="test_key", base_url=base_url)
    result = client.upload(io.BytesIO(b"file-like-bytes"))
    assert result == "filelike_file_id_123"


def test_upload_missing_presigned_url(test_server, test_file, mock_reducto_client):
...
def test_upload_missing_file_id(test_server, test_file, mock_reducto_client):
...
def test_upload_presigned_url_success(test_server, test_file, mock_reducto_client):
...
def test_upload_empty_json_response(test_server, test_file, mock_reducto_client):
...
def test_upload_with_custom_headers(test_server, test_file, mock_reducto_client):
