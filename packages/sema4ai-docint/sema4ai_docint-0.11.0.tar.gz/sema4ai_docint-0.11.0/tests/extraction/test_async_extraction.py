...
async def async_client(async_test_server):
    ...
    with patch(
        ...
        mock_client._client.headers = {"X-API-Key": "test_key"}
        mock_client._client.post = AsyncMock()
        mock_client._client.put = AsyncMock()
        mock_client.close = AsyncMock()

        # Mock job API
        mock_client.parse = AsyncMock()
        mock_client.parse.run_job = AsyncMock(return_value=MagicMock(job_id="test_job_123"))
        mock_client.extract = AsyncMock()
        mock_client.extract.run_job = AsyncMock(return_value=MagicMock(job_id="test_job_123"))
        mock_client.split = AsyncMock()
        mock_client.split.run_job = AsyncMock(return_value=MagicMock(job_id="test_job_123"))
        mock_client.job = AsyncMock()
        mock_client.job.get = AsyncMock()
...
async def test_client_close(async_client):
    ...
    async_client._client.is_closed = MagicMock(return_value=False)  # Sync method, not async
    ...
    async_client._client.is_closed = MagicMock(return_value=True)  # Sync method, not async
...
async def test_upload_excludes_content_type_header(async_client, test_file):
    ...
    async_client._client._client.post = AsyncMock(
...
async def test_upload_with_path(async_client, test_file):
    ...
    # Mock successful PUT
    async_client._client._client.put = AsyncMock(
    ...
    with patch.object(async_client, "_create_aiohttp_session_sync", return_value=mock_session):
        ...
        async_client._client._client.post.assert_called_once()
        mock_session.put.assert_called_once()
...
async def test_upload_403_error(async_client, test_file):
    ...
    async_client._client._client.post = AsyncMock(return_value=MagicMock(status_code=403))
...
async def test_start_parse(async_client):
    ...
    async_client._client.parse.run_job.assert_called_once()
...
async def test_start_extract(async_client):
    ...
    async_client._client.extract.run_job.assert_called_once()
...
async def test_start_split(async_client):
    ...
    async_client._client.split.run_job.assert_called_once()
...
async def test_job_status(async_client):
    ...
    async_client._client.job.get = AsyncMock(return_value=MagicMock(status="Pending"))

    status = await async_client.get_job_status("job_123")

    assert status == "Pending"
    async_client._client.job.get.assert_called_once_with(job_id="job_123")
...
async def test_wait_for_job_success(async_client):
    ...
    # Mock job completion
    async_client._client.job.get = AsyncMock(
...
async def test_job_polling(async_client):
    ...
    async_client._client.job.get = mock_get
...
async def test_parse_convenience_method(async_client):
    ...
    async_client._client.parse.run_job = AsyncMock(return_value=MagicMock(job_id="parse_job"))
    async_client._client.job.get = AsyncMock(
        return_value=MagicMock(
            status="Completed",
    ...
    async_client._client.parse.run_job.assert_called_once()
    async_client._client.job.get.assert_called()
...
async def test_extract_convenience_method(async_client):
    ...
    async_client._client.extract.run_job = AsyncMock(return_value=MagicMock(job_id="extract_job"))
    async_client._client.job.get = AsyncMock(
...
async def test_split_convenience_method(async_client):
    ...
    async_client._client.split.run_job = AsyncMock(return_value=MagicMock(job_id="split_job"))
    async_client._client.job.get = AsyncMock(
    ...
    with patch(
        ...
        async with AsyncExtractionClient(
            ...
            with patch.object(client, "_create_aiohttp_session_sync", return_value=mock_session):
                ...
                client._client.job.get = AsyncMock(
