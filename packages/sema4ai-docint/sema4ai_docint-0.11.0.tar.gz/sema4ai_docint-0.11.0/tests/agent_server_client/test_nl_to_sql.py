...
@pytest.mark.skip(reason="Agent server bootstrapping is outdated and not working as desired")
class TestNLToSQL:
    ...
    @pytest.fixture
    def setup_test_data(self, test_case: str, postgres, request):
        """Setup test-specific data for each test case."""
        conn, props = postgres

        # implicitly skips the rest of the setup if this test is not enabled.
        TestNLToSQL.check_eval_marker(request)
    ...
    def test_generate_validation_rules(
        ...
        rules = agent_client.generate_validation_rules(
            test_data["use_case_description"],
            test_data["rules_description"],
            [view.model_dump() for view in views],
            view_reference_data=view_reference_data,
            database_name=mindsdb_db_name,
        )
