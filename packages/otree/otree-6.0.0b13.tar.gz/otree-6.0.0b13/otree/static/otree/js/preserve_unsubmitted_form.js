function preserveUnsavedInput(inp) {
    console.log("persisting input", inp);
    ...
    if (storedValue != null) {
        ...
        _otreeForm[inp.name][valueAttr] = storedValue;
    ...
    inp.addEventListener('input', function () {
        ...
        console.log("stored value", curValue);
    });
}
...
let _otreeUrlParts = window.location.pathname.split('/');
let _otreeLastIndex = _otreeUrlParts.length - 1
let _otreePageNumber = _otreeUrlParts[_otreeLastIndex];
// in case the path somehow has a trailing slash
if (_otreePageNumber === '') _otreePageNumber = _otreeUrlParts[_otreeLastIndex - 1];
let _otreeForm;


document.addEventListener("DOMContentLoaded", function (event) {
    _otreeForm = document.getElementById('form');
    for (let inp of _otreeForm.elements) {
        preserveUnsavedInput(inp);
    }
});
