...
from .admin import mk_participant_code_urls


def get_room_shared_context(room, request):
    """Get shared context data for room views."""

    return {
        'participant_urls': room.get_participant_urls(request),
        'room_wide_url': room.get_room_wide_url(request),
        'room': room,
        'collapse_links': True,
    }
...
class RoomWithoutSession(AdminView):
    ...
    def get_context_data(self, **kwargs):
        context = get_room_shared_context(self.room, self.request)
        context.update(
            {
...
class RoomWithSession(AdminView):
    ...
    def get_context_data(self, **kwargs):
        ...
        participant_code_urls = mk_participant_code_urls(
            self.room.get_session(), self.request
        )
        ...
        context.update(
            {
                ...
                'participant_code_urls': participant_code_urls,
            }
        )
        return super().get_context_data(**context, **kwargs)
