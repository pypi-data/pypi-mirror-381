...
class FormPageOrInGameWaitPage:
    ...
    # could also call it layout_from_scratch
    use_blank_layout = False
    ...
    def get_context_data(self, **context):
        ...
        powered_by_otree = core_gettext("Powered by {product_name}").format(
            product_name='<a href="https://www.otree.org/" target="_blank" class="link-secondary">oTree</a>'
        )
        context.update(
            view=self,
            ...
            use_standard_layout=not (self.has_trial() or self.use_blank_layout),
            powered_by_otree=powered_by_otree,
        )
        ...
        if has_trial:
            ...
            if use_roundtrip_mode:
                ...
                trials = [
                    otree.trials.encode_trial(t, self.trial_stimulus_fields)
                    for t in otree.trials.get_all_trials(
                        Trial, self.player, type(self).__name__
                    )
                ]
...
class Page(FormPageOrInGameWaitPage):
    ...
    preserve_unsubmitted_form = False
    ...
    def form_invalid(self, form):
        ...
        response = self.render_to_response(context)
        response.headers[otree.constants.redisplay_with_errors_http_header] = (
    ...
    def must_complete_trials(self):
        ...
        if self.has_trial():
            Trial = self.trial_model
            return bool(
                otree.trials.get_current_trial(
                    Trial, self.player, page_name=type(self).__name__
                )
            )
        return False
    ...
    def post(self):
        ...
        back_button_clicked = post_data.get(otree.constants.back_button)
        if back_button_clicked:
            prev_page_index = pp._index_in_pages - 1
            if prev_page_index < 1:
                raise BackButtonError(
                    "Back button cannot be used on the first page of a session."
                )
            lookup = get_page_lookup(pp._session_code, prev_page_index)
            PrevPage = lookup.page_class
            if isinstance(PrevPage, WaitPage):
                raise BackButtonError(
                    "Back button cannot be used if previous page is a wait page"
                )
            if not self.allow_back_button:
                raise BackButtonError(
                    "In order to use a back button, you must set `allow_back_button = True` on the Page class"
                )
            if lookup.app_name != self._lookup.app_name:
                raise BackButtonError(
                    "Back button cannot be used if previous page is in a different app"
                )
            # what about if previous page has is_displayed=False?
            pp._index_in_pages = prev_page_index
            return self._redirect_to_page_the_user_should_be_on()
...
class BackButtonError(Exception):
    pass
...
class GenericWaitPageMixin:
    ...
    def get_context_data(self):
        ...
        return dict(
            ...
            body_text=body_text,
            GBAT_INACTIVE_SECONDS_UNTIL_PROMPT=settings.GBAT_INACTIVE_SECONDS_UNTIL_PROMPT,
            GBAT_INACTIVE_SECONDS_TO_CONFIRM=settings.GBAT_INACTIVE_SECONDS_TO_CONFIRM,
        )
...
class WaitPage(FormPageOrInGameWaitPage, GenericWaitPageMixin):
    ...
    def _get_default_body_text(self):
        if self.group_by_arrival_time:
            return core_gettext("Waiting to form a group with other participants.")
