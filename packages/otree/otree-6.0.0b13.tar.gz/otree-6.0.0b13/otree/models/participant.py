...
from otree.database import MixinVars, CurrencyType, NoResultFound
...
class Participant(MixinVars, otree.database.SSPPGModel):
    ...
    # doesn't actually hold any data. just used because we need
    # a column in the monitor table for the presence icon,
    # and the code currently expects an attribute
    _presence = ''  # Column(st.String(1), default=True)
    ...
    def get_player(self, app_name, round_number):
        models_module = otree.common.get_models_module(app_name)
        PlayerClass = getattr(models_module, 'Player')
        try:
            return PlayerClass.objects_get(participant=self, round_number=round_number)
        except NoResultFound:
            msg = f"No player found for the given app_name={repr(app_name)} and round_number={repr(round_number)}"
