...
class Omega3TextConfig(Gemma2Config, PretrainedConfig):
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 262208):
            Vocabulary size of the Omega3Text model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`Omega3TextModel`]
        ...
        sliding_window (`int`, *optional*, defaults to 4096):
            In Omega3Text, every other layer uses sliding window attention. This is the size of the sliding window.
    ...
    >>> # Initializing a Omega3Text omega3-llm style configuration
    >>> configuration = Omega3TextConfig()
    >>> # Initializing a model from the omega3-llm style configuration
...
class Omega3Config(PretrainedConfig):
    ...
    Args:
        text_config (`Union[Omega3TextConfig, dict]`, *optional*):
    ...
    ```python
    >>> from transformers import Omega3ForConditionalGeneration, Omega3Config, SiglipVisionConfig, Omega3TextConfig
    ...
    def __init__(
        self,
        text_config: Optional[Union[Omega3TextConfig, dict[str, Any]]] = None,
        ...
        **kwargs,
        ...
        if text_config is None:
            text_config = Omega3TextConfig()
            logger.info("text_config is None, using default Omega3TextConfig text config.")
        elif isinstance(text_config, dict):
            text_config = Omega3TextConfig(**text_config)
...
class Omega3ModelOutputWithPast(PaligemmaModelOutputWithPast):
    pass


class Omega3CausalLMOutputWithPast(PaligemmaCausalLMOutputWithPast):
    pass


class Omega3TextScaledWordEmbedding(nn.Embedding):
    """
...
class Omega3Attention(Gemma2Attention):
    def __init__(self, config: Omega3TextConfig, layer_idx: int):
        ...
        self.q_norm = Omega3RMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
        self.k_norm = Omega3RMSNorm(dim=config.head_dim, eps=config.rms_norm_eps)
...
class Omega3DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Omega3TextConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.self_attn = Omega3Attention(config=config, layer_idx=layer_idx)
        self.mlp = Omega3MLP(config)
        self.input_layernorm = Omega3RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Omega3RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.pre_feedforward_layernorm = Omega3RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
        self.post_feedforward_layernorm = Omega3RMSNorm(self.hidden_size, eps=config.rms_norm_eps)
...
class Omega3TextModel(Gemma2Model):
    ...
    def __init__(self, config: Omega3TextConfig):
        ...
        # Omega3 downcasts the below to bfloat16, causing sqrt(3072)=55.4256 to become 55.5. See https://github.com/huggingface/transformers/pull/29402
        self.embed_tokens = Omega3TextScaledWordEmbedding(
        ...
        self.rotary_emb_local = Omega3RotaryEmbedding(config=config)
...
class Omega3ForCausalLM(Gemma2ForCausalLM):
    ...
    def __init__(self, config: Omega3TextConfig):
        super().__init__(config)
        self.model = Omega3TextModel(config)


class Omega3MultiModalProjector(nn.Module):
    def __init__(self, config: Omega3Config):
        ...
        self.mm_soft_emb_norm = Omega3RMSNorm(
...
class Omega3Model(PaliGemmaModel):
    ...
    def forward(
        ...
        **lm_kwargs,
    ) -> Union[tuple, Omega3ModelOutputWithPast]:
        if (input_ids is None) ^ (inputs_embeds is not None):
            raise ValueError("You must specify exactly one of input_ids or inputs_embeds")
        ...
        return Omega3ModelOutputWithPast(
...
class Omega3ForConditionalGeneration(PaliGemmaForConditionalGeneration):
    @auto_docstring
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        ...
        **lm_kwargs,
    ) -> Union[tuple, Omega3CausalLMOutputWithPast]:
        r"""
        ...
        return Omega3CausalLMOutputWithPast(
            loss=loss,
            logits=logits,
...
class Omega3ForSequenceClassification(Omega3PreTrainedModel):
...
__all__ = [
    "Omega3Config",
    "Omega3TextConfig",
    "Omega3PreTrainedModel",  # noqa: F822
    "Omega3TextModel",
    "Omega3ForCausalLM",
    "Omega3ForConditionalGeneration",
    "Omega3Model",
    "Omega3ForSequenceClassification",
]
