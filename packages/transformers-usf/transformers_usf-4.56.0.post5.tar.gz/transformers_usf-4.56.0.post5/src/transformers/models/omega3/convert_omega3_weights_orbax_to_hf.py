r"""Utility to convert Omega models from Orbax to HF Transformers checkpoint.

python -m transformers.models.omega3.convert_omega3_weights_orbax_to_hf \
    --variant='omega3_llm' \
    --tokenizer_path="$HOME/omega3/tokenizer/omega3_cleaned_262144_v2.spiece.model" \
    --checkpoint_path="$HOME/omega3/omega3_llm_pt_orbax/" \
    --output_path="$HOME/omega3/omega3_llm_pt_safetensors/"
"""
...
_VARIANT_OMEGA3_4B = "omega3_4b"
_VARIANT_OMEGA3_27B = "omega3_27b"
_VARIANTS = {
    _VARIANT_OMEGA3_4B: Omega3Config(
    ...
    _VARIANT_OMEGA3_27B: Omega3Config(
        text_config=Omega3TextConfig(
            vocab_size=262_208,
            hidden_size=42 * 128,
...
def convert(checkpoint_path: str, config: Omega3Config) -> dict[str, torch.Tensor]:
...
def main(*args):
    ...
    logging.info(
        "Converting Omega 3 (%s) @ %s (language) and %s (vision)",
        variant,
        _TRANSFORMER_DTYPE.value,
        _VISION_DTYPE.value,
    )
    state_tree = convert(_CHECKPOINT_PATH.value, config)
    logging.info("Converted Omega 3 (%s) state tree from Orbax to Hugging Face.", variant)

    with accelerate.init_empty_weights():
        model = Omega3ForConditionalGeneration(config)

    model.load_state_dict(state_tree, assign=True, strict=True)
    logging.info(
        "Loaded Omega 3 (%s) in Hugging Face Transformers as a %s instance.",
        variant,
        type(model).__name__,
    )
    model.save_pretrained(output_path, safe_serialization=True)
    logging.info(
        "Saved Omega 3 (%s) to SafeTensors in %s using %s",
        variant,
        output_path,
        type(model).__name__,
    )
    ...
    processor = Omega3Processor(
        image_processor=image_processor,
        tokenizer=tokenizer,
        chat_template=tokenizer.chat_template,
    )
    processor.save_pretrained(output_path)
    logging.info("Saved Omega3Processor for %s to %s", variant, output_path)
    del processor
