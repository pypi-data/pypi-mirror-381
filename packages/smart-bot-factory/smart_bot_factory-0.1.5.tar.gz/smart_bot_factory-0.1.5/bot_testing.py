...
ФАЙЛЫ СЦЕНАРИЕВ:
    configs/BOT_ID/tests/*.yaml - тестовые сценарии
    
ОТЧЕТЫ:
    configs/BOT_ID/reports/test_YYYYMMDD_HHMMSS.txt - подробные отчеты
...
from bot_utils import parse_ai_response
...
class ScenarioLoader:
    ...
    @staticmethod
    def load_all_scenarios_for_bot(bot_id: str) -> List[TestScenario]:
        """Загружает все сценарии для указанного бота"""
        tests_dir = Path(f"configs/{bot_id}/tests")
...
class BotTester:
    ...
    def _initialize_bot(self):
        ...
        try:
            ...
            # Проверяем, что файл запускалки существует
            bot_launcher_path = Path(__file__).parent / f"{self.bot_id}.py"
            logging.info(f"🔍 Проверка файла запускалки: {bot_launcher_path}")
            
            if not bot_launcher_path.exists():
                raise ValueError(f"Файл запускалки не найден: {bot_launcher_path}")
            logging.info(f"✅ Файл запускалки найден")
            
            # Повторяем точно ту же логику, что в <bot-id>.py, но без запуска main
            root_dir = Path(__file__).parent
            config_dir = root_dir / 'configs' / self.bot_id
            logging.info(f"📁 Каталог конфигурации: {config_dir}")
            
            if not config_dir.exists():
                raise ValueError(f"Папка конфигурации не найдена: {config_dir}")
            logging.info(f"✅ Каталог конфигурации найден")
            
            # Устанавливаем BOT_ID как в оригинальном файле
            os.environ['BOT_ID'] = self.bot_id
            logging.info(f"🏷️ BOT_ID установлен: {self.bot_id}")
            
            # Загружаем .env ДО импорта config, как в оригинале
            ...
            # Устанавливаем PROMT_FILES_DIR для Config
            prompts_dir = f"configs/{self.bot_id}/prompts"
            os.environ['PROMT_FILES_DIR'] = prompts_dir
            logging.info(f"📋 Каталог промптов: {prompts_dir}")
            
            # Теперь импортируем Config - он увидит правильные переменные
            logging.info(f"⚙️ Инициализация конфигурации...")
            ...
            logging.info(f"✅ OpenAI клиент создан (модель: {config.OPENAI_MODEL})")
            ...
            logging.info(f"✅ Загрузчик промптов создан (файлов: {len(config.PROMPT_FILES)})")
            
            # Инициализируем Supabase клиент
            logging.info(f"🗄️ Инициализация Supabase клиента...")
    ...
    async def get_welcome_file_caption_test(self) -> str:
        ...
        try:
            ...
            folder_value = Path(f'configs/{bot_id}/welcome_file')
            if not folder_value:
                return ""

            folder = Path(folder_value)
            if not folder.exists() or not folder.is_dir():
                logger.info(f"Директория приветственных файлов не найдена: {folder_value}")
                return ""
    ...
    async def process_user_message_test(self, user_message: str, session_id: str, system_prompt: str):
        ...
        # Используем локальный config вместо импорта из main
        config = Config()
        ...
        try:
            ...
            logging.info(f"📚 Получение истории сообщений (лимит: {config.MAX_CONTEXT_MESSAGES})...")
            chat_history = await self.supabase_client.get_chat_history(session_id, limit=config.MAX_CONTEXT_MESSAGES)
...
class TestRunner:
    ...
    def __init__(self, bot_id: str, max_concurrent: int = 5):
        self.bot_id = bot_id
        self.max_concurrent = max_concurrent
        self.bot_tester = BotTester(bot_id)
...
class ReportGenerator:
    ...
    @staticmethod
    def save_report(bot_id: str, results: List[TestResult]) -> str:
        """Сохраняет отчет в файл"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Создаем папку отчетов в конфиге бота
        reports_dir = f"configs/{bot_id}/reports"
        os.makedirs(reports_dir, exist_ok=True)
        
        # Очищаем старые отчеты перед созданием нового
        ReportGenerator.cleanup_old_reports(reports_dir, max_reports=10)
        
        # Лаконичное название файла
        report_filename = f"{reports_dir}/test_{timestamp}.txt"
...
async def main():
    ...
    try:
        ...
        if scenario_file:
            ...
            scenario_path = f"configs/{args.bot_id}/tests/{scenario_file}"
            scenarios = ScenarioLoader.load_scenarios_from_file(scenario_path)
            
            if not scenarios:
                print(f"❌ Файл сценариев '{scenario_file}' не найден или пуст")
                return 1
        else:
            scenarios = ScenarioLoader.load_all_scenarios_for_bot(args.bot_id)
        ...
        # Запускаем тесты
        test_runner = TestRunner(args.bot_id, args.max_concurrent)
        results = await test_runner.run_tests(scenarios)
        
        # Генерируем отчеты
        ReportGenerator.generate_console_report(args.bot_id, results)
        report_file = ReportGenerator.save_report(args.bot_id, results)
