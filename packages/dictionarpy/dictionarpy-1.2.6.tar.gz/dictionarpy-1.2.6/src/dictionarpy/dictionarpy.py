...
class DictionarPy:
    ...
    def _get_sim_words(self) -> set[str]:
        '''
        Return words in the database that contain the input string.
    ...
    def get_random_word(self, pos='any') -> str:
        '''
        Return a random word from the dictionary. Optionally constrained to a
    ...
    def show_ipa_guide(self, key:str='all') -> None:
        ...
        guide = {
            ...
            'æ':  [[u('a') + 'pple',
                   'b' + u('a') + 'g',
                   'b' + u('a') + 'n'], '[ɛə] b' + u('a') + 'n (pre-nasal vowel)'],
            'aɪ': [['b' + u('i') + 'ke',
                   'sk' + u('y'),
                   'p' + u('ie'),
                   'h' + u('igh')]],
