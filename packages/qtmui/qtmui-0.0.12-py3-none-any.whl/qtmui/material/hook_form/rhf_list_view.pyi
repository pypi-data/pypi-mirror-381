import uuid
from qtpy.QtCore import Qt, Property, Slot
from qtpy.QtWidgets import QWidget, QVBoxLayout, QSizePolicy, QLabel
from qtmui.hooks import useState
from ...material.spacer.hspacer import HSpacer
from ...material.form_label.form_label import FormLabel
from ...material.button.button import Button
from ..controller import Controller
from ..form_control_label import FormControlLabel
from ..box import Box
from ..form_helper_text import FormHelperText
from ..switch import Switch
from qtmui.material.styles import store, useTheme
from ..qss_name import *
class RHFListView:
    def __init__(self, name: str, value: bool, hightLight: bool, row: bool, label: str, list_view: object, spacing: int, helperText: str): ...
    def add_new_item(self): ...
    def setup_ui(self): ...
    def set_state(self, state): ...
    def set_value(self, value): ...
    def stateSignal(self): ...
    def stateSignal(self, value): ...
    def state(self, state): ...