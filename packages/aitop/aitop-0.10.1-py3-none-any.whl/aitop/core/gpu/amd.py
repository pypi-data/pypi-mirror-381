...
class AMDGPUMonitor(BaseGPUMonitor):
    """AMD GPU monitoring implementation with ROCm 6.3 specific parsing with batch query optimization."""
    ...
    def _get_rocm_version(self) -> str:
        ...
        if not version:
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    # Extract version from package version string (e.g., "6.3.0-1" -> "6.3.0")
                    match = re.search(r'(\d+\.\d+\.\d+)', result.stdout)
                    if match:
                        version = match.group(1)
                        self.logger.debug(f"Method 1a (dpkg rocm-core) found version: {version}")
                ...
                try:
                    ...
                    if result.returncode == 0 and result.stdout.strip():
                        ...
                        if match:
                            version = match.group(1)
                            self.logger.debug(f"Method 1b (rpm rocm-core) found version: {version}")
                except Exception as e:
            ...
            try:
                ...
                if version_header.exists():
                    ...
                    if major and minor and patch:
                        ...
                        self.logger.debug(f"Method 2 (rocm_version.h) found version: {version}")
            except Exception as e:
        ...
        if not version:
            try:
                ...
                if result.returncode == 0:
                    ...
                    if match:
                        version = match.group(1)
                        self.logger.debug(f"Method 3 (update-alternatives) found version: {version}")
            except Exception as e:
        ...
        if not version:
            try:
                ...
                if version_file.exists():
                    ...
                    if match:
                        version = match.group(1)
                        self.logger.debug(f"Method 4 (.info/version) found version: {version}")
            except Exception as e:
        ...
        if not version:
            try:
                ...
                if version_file.exists():
                    ...
                    if match:
                        version = match.group(1)
                        self.logger.debug(f"Method 5 (.info/version-dev) found version: {version}")
            except Exception as e:
        ...
        if not version:
            try:
                ...
                if version_file.exists():
                    ...
                    if match:
                        version = match.group(1)
                        self.logger.debug(f"Method 6 (VERSION) found version: {version}")
            except Exception as e:
    ...
    def _get_batch_metrics_json(self, gpu_index: int) -> Dict[str, Any]:
        ...
        cmd = [
            str(self.smi_path), '-d', str(gpu_index),
            '--showmeminfo', 'vram',
            '--showpower',
            '-t',
            '--showuse',
    ...
    def _parse_json_memory(self, json_data: Dict[str, Any], gpu_index: int) -> Tuple[float, float]:
        """Parse memory info from JSON batch query (best-effort, varies by ROCm version)."""
        try:
    ...
    def _parse_json_temperature(self, json_data: Dict[str, Any], gpu_index: int) -> float:
    ...
    def _parse_json_power(self, json_data: Dict[str, Any], gpu_index: int) -> float:
        ...
        try:
            card_key = f'card{gpu_index}'
            if card_key in json_data:
                power = json_data[card_key].get('Average Graphics Package Power (W)', 0.0)
                return float(power) if power else 0.0
            return 0.0
        except (KeyError, AttributeError, ValueError):
            return 0.0

    def _parse_json_utilization(self, json_data: Dict[str, Any], gpu_index: int) -> float:
    ...
    def get_gpu_info(self) -> List[GPUInfo]:
        """Get comprehensive AMD GPU information for all devices with optimized batch queries."""
        ...
        try:
            ...
            for device in devices:
                ...
                batch_data = self._get_batch_metrics_json(index) if self._json_supported else {}
                ...
                if batch_data and self._json_supported:
                    ...
                    try:
                        ...
                        memory_used, memory_total = self._parse_json_memory(batch_data, index)
                        ...
                        self.logger.warning(f"JSON parsing incomplete for GPU {index}: {e}, falling back to text")
