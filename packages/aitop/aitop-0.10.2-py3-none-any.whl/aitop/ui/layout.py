...
class LayoutRect:
    ...
    def inner(self, margin: int = 0, left: int = None, right: int = None,
              top: int = None, bottom: int = None) -> 'LayoutRect':
        """
        ...
        r = right if right is not None else margin
        t = top if top is not None else margin
        b = bottom if bottom is not None else margin

        return LayoutRect(
            y=self.y + t,
            x=self.x + l,
            height=max(0, self.height - t - b),
            width=max(0, self.width - l - r)
        )
...
class LayoutCalculator:
    ...
    def _calculate_layout(self) -> Dict[str, LayoutRect]:
        ...
        tabs = LayoutRect(
            y=header.bottom,
            x=0,
            height=self.TABS_HEIGHT,
            width=width
        )

        # Footer (bottom, fixed height)
        footer = LayoutRect(
            y=height - self.FOOTER_HEIGHT,
            x=0,
        ...
        overview_left, overview_right, separator_x = self._calculate_overview_split(content)
    ...
    def _calculate_overview_split(
        ...
        available_width = content.width - self.CONTENT_MARGIN_LEFT - self.CONTENT_MARGIN_RIGHT
    ...
    def get_content_area(self) -> LayoutRect:
        ...
        return layout['content'].inner(
            left=self.CONTENT_MARGIN_LEFT,
            right=self.CONTENT_MARGIN_RIGHT
        )
