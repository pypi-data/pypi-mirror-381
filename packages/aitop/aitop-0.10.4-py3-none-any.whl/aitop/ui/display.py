...
class Display:
    ...
    def __init__(self, stdscr: Any, theme_file: Optional[Path] = None) -> None:
        ...
        try:
            ...
            # Cursor control may not be available on all terminals
        ...
        self._screen_state: "OrderedDict[str, bool]" = OrderedDict()
        ...
        self._bar_cache: "OrderedDict[str, Tuple[str, int]]" = OrderedDict()
    ...
    def safe_addstr(
        self, y: int, x: int, text: str, attr: Optional[int] = None
    ) -> bool:
    ...
    def handle_resize(self) -> None:
        ...
        try:
            ...
            if new_height > self.actual_height or new_width > self.actual_width:
                ...
                self.logger.debug(
                    f"Terminal resized to {self.width}x{self.height}, created new pad"
                )
            ...
            self.logger.debug(
                f"Visible area: {self.visible_width}x{self.visible_height}"
            )
    ...
    def refresh(self) -> None:
        ...
        try:
            ...
            self.pad.noutrefresh(
                0, 0, 0, 0, self.visible_height - 1, self.visible_width - 1
            )
            curses.doupdate()
