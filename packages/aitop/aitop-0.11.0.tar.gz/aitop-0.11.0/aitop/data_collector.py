...
class DataCollector:
    ...
    def __init__(
        ...
        self._thread = threading.Thread(
            target=self._collection_loop, daemon=True, name="AITop-Collector"
        )
        ...
        try:
            ...
            vendor_str = ", ".join(self.vendors) if self.vendors else "None"
            self.logger.debug(f"Detected GPU vendors: {vendor_str}")
        ...
        self.executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="aitop_collector"
        )
    ...
    def _collection_loop(self):
        ...
        while self.running.is_set():
            try:
                ...
                if current_time >= next_collection:
                    ...
                    if self._adaptive_timing:
                        ...
                        if cpu_load > self._load_threshold:
                            ...
                            throttled_interval = (
                                self.update_interval * self._throttle_factor
                            )
                            self.logger.debug(
                                f"High system load ({cpu_load:.2f}), "
                                f"throttling collection interval to "
                                f"{throttled_interval:.2f}s"
                            )
                    ...
                    if collection_success:
                        ...
                        if consecutive_errors > self._error_threshold:
                            # Cap at 30s
                            backoff_time = min(30.0, self._error_backoff * 2)
                            self.logger.warning(
                ...
                now = time.time()
                # At least 10ms sleep
                remaining = max(0.01, next_collection - now)

                # Use shorter sleep for more responsiveness when close
                # to next collection
                if remaining < 0.1:
                    # Very short sleep when close to next collection
                    time.sleep(0.01)
                else:
                    # Sleep at most 100ms
                    time.sleep(min(remaining / 2, 0.1))
                ...
                with self._cache_lock:
                    ...
                    if len(self._cache["errors"]) > self._max_errors:
                        self._cache["errors"] = self._cache["errors"][
                            -self._max_errors :
                        ]
        ...
        try:
            with self._cache_lock:
                ...
                if self._next_runs["system"] <= current_time:
                    ...
                    if self.memory_monitor:
                        try:
                            futures["memory"] = self.executor.submit(
                                self.memory_monitor.get_memory_stats
                            )
                            futures["memory_types"] = self.executor.submit(
                                self.memory_monitor.get_memory_by_type
                            )
                        except Exception as e:
                            self.logger.error(
                                f"Error submitting memory stats task: {e}"
                            )
                ...
                if self._next_runs["gpu_quick"] <= current_time and self.gpu_monitors:
                    try:
                        futures["gpu_quick"] = self.executor.submit(
                            self._collect_quick_gpu_metrics
                        )
                        self._next_runs["gpu_quick"] = (
                            current_time + self.update_interval
                        )
                    except Exception as e:
                        self.logger.error(
                            f"Error submitting quick GPU metrics task: {e}"
                        )
                ...
                if self._next_runs["gpu_full"] <= current_time and self.gpu_monitors:
                    try:
                        futures["gpu_full"] = self.executor.submit(
                            self._collect_full_gpu_info
                        )
                        self._next_runs["gpu_full"] = (
                            current_time + self.gpu_info_interval
                        )
                    except Exception as e:
                ...
                if self._next_runs["processes"] <= current_time and self.ai_monitor:
                    try:
                        ...
                        self._next_runs["processes"] = (
                            current_time + self.process_interval
                        )
                    except Exception as e:
                ...
                if self._next_runs["error_cleanup"] <= current_time:
                    self._cache["errors"] = [
                        e
                        for e in self._cache["errors"]
                        if current_time - e["timestamp"]
                        < 300  # Keep errors for 5 minutes
                    ]
                    self._next_runs["error_cleanup"] = (
                        current_time + 60
            ...
            for key, future in futures.items():
                ...
                try:
                    ...
                    if (
                        key in ["cpu", "gpu_full"]
                        and len(self._cache.get("errors", [])) > 3
                    ):
    ...
    def _record_error(self, component: str, message: str) -> None:
        ...
        with self._cache_lock:
            self._cache["errors"].append(
                {"timestamp": time.time(), "component": component, "error": message}
            )
        ...
        try:
            ...
            for monitor in self.gpu_monitors:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Error collecting quick metrics from {monitor_name}: {e}"
                    )
    ...
    def _collect_full_gpu_info(
        ...
        try:
            ...
            self.logger.error(
                f"Critical error in full GPU info collection: {e}", exc_info=True
            )
    ...
    def stop(self):
        ...
        try:
            ...
            # Python 3.8 doesn't, so we need to handle both
            import sys

            if sys.version_info >= (3, 9):
                self.executor.shutdown(wait=False, cancel_futures=True)
            else:
                # Python 3.8: shutdown without cancel_futures
        ...
        if self._thread.is_alive():
            ...
            if self._thread.is_alive():
                self.logger.warning(
                    "DataCollector thread did not stop gracefully within 5 seconds"
                )
