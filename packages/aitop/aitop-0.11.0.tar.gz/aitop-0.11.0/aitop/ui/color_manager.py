...
class ColorManager:
    ...
    Includes enhanced compatibility and LRU pair recycling.
    """
    ...
    def __init__(self) -> None:
        ...
        self.max_pairs = (
            max(32, min(curses.COLOR_PAIRS - 10, 200))
            if hasattr(curses, "COLOR_PAIRS")
            else 190
        )
        ...
        self.logger.debug(
            f"Color manager initialized with {self.max_colors} colors "
            f"and {self.max_pairs} color pairs"
        )

    def detect_terminal_capabilities(self) -> None:
        ...
        self.can_change = (
            curses.can_change_color() if hasattr(curses, "can_change_color") else False
        )
        ...
        self.logger.debug("Terminal color capabilities detected:")
        self.logger.debug(f"  TERM={term}, COLORTERM={colorterm}")
        self.logger.debug(
            f"  max_colors={self.max_colors}, can_change={self.can_change}"
        )
        self.logger.debug(f"  has_truecolor={self.has_truecolor}")

    def get_color_index(self, color_spec: str) -> int:
        """Convert a color specification to a curses color index.
        ...
        if (
            self.can_change
            and self.has_truecolor
            and self.next_color_index < self.max_colors
        ):
            try:
    ...
    def get_color_pair(self, fg: str, bg: str = "-1") -> int:
        ...
        try:
            ...
            if len(self.pair_pool) >= self.max_pairs:
                ...
                self.logger.debug(
                    f"Recycling color pair {pair_number} "
                    f"(was {evicted_key}, now {pair_key})"
                )
            else:
