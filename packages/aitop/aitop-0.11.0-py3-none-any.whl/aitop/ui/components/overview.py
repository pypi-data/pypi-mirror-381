...
class OverviewPanel:
    ...
    def _render_gpu_overview(
        self, gpu: GPUInfo, vendor: str, y: int, x: int, width: int
    ) -> int:
        ...
        self.display.safe_addstr(
            y, x, "GPU Status", curses.color_pair(1) | curses.A_BOLD
        )
        y += 1
        ...
        self.display.safe_addstr(
            y, x, f"GPU {gpu.index}: {vendor_prefix}{gpu.name}", curses.color_pair(5)
        )
        y += 1
        ...
        # VRAM line format: "VRAM   [bar]  12.5/ 24.0GB ( 52.1%)"
        # = 7 + 2 + bar + 2 + 24 = 35 + bar
        ...
        line = (
            f"VRAM   [{bar}] {mem_used_gb:5.1f}/{mem_total_gb:5.1f}GB "
            f"({mem_percent:5.1f}%)"
        )
        self.display.safe_addstr(y, x, line, color)
        y += 1
    ...
    def _render_memory_overview(
        self, memory_stats: MemoryStats, y: int, x: int, width: int
    ) -> int:
        ...
        # Title
        self.display.safe_addstr(
            y, x, "System Memory", curses.color_pair(1) | curses.A_BOLD
        )
        y += 1

        # RAM usage
        # RAM line format: "RAM    [bar]  18.5/ 62.7GB ( 29.5%)"
        # = 7 + 2 + bar + 2 + 24 = 35 + bar
        ...
        line = (
            f"RAM    [{bar}] {ram_used_gb:5.1f}/{ram_total_gb:5.1f}GB "
            f"({memory_stats.percent:5.1f}%)"
        )
        self.display.safe_addstr(y, x, line, color)
        y += 1
        ...
        if memory_stats.swap_total > 0:
            ...
            bar, color = self.display.create_bar(
                memory_stats.swap_percent, swap_bar_width
            )
            line = (
                f"Swap   [{bar}] {swap_used_gb:5.1f}/{swap_total_gb:5.1f}GB "
                f"({memory_stats.swap_percent:5.1f}%)"
            )
            self.display.safe_addstr(y, x, line, color)
            y += 1
    ...
    def _render_process_overview(
        ...
        Args:
            ...
            selected_index: Index of selected process for highlighting
        ...
        # Title
        self.display.safe_addstr(
            y, x, "Top Processes", curses.color_pair(1) | curses.A_BOLD
        )
        y += 2

        # Calculate dynamic widths based on available space
        total_fixed_width = sum(self.base_headers.values()) + len(
            self.base_headers
        )  # Add 1 for each separator
        name_width = max(
            self.min_name_width, width - total_fixed_width - 2
        )  # -2 for margins
        ...
        for idx, proc in enumerate(processes[:max_processes]):
                ...
                for gpu in gpus:
                    for gpu_proc in gpu.processes:
                        if gpu_proc["pid"] == proc["pid"]:
                            ...
                            current_util = 0.0
                            if (
                                "cu_occupancy" in gpu_proc
                                and gpu_proc["cu_occupancy"] is not None
                            ):
                                try:
                                    current_util = float(
                                        gpu_proc["cu_occupancy"].rstrip("%")
                                    )
                            ...
                            # All memory values in GPUInfo should be in MB,
                            # but some GPU backends might provide process
                            # memory in different units
                            # AMD case: Memory might be in bytes
                            # Heuristic to detect bytes
                            ...
                            current_vram = (
                                (proc_memory / gpu.memory_total * 100)
            ...
            status_color = (
                curses.color_pair(2) if status == "running" else curses.color_pair(6)
            )
            ...
            # Determine if this process is selected
            is_selected = idx == selected_index

            # Write with different colors for status and selection highlighting
            if is_selected:
                # Entire line with reverse video
                line = base_metrics + status_field + name_field
                self.display.safe_addstr(y, x, line, curses.A_REVERSE)
            else:
                # Normal rendering with different colors
                self.display.safe_addstr(y, x, base_metrics, curses.color_pair(6))
                self.display.safe_addstr(
                    y, x + len(base_metrics), status_field, status_color
                )
                self.display.safe_addstr(
                    y,
                    x + len(base_metrics) + len(status_field),
                    name_field,
                    curses.color_pair(6),
                )
    ...
    def _render_cpu_overview(
        self, cpu_stats: CPUStats, y: int, x: int, width: int
    ) -> int:
        ...
        # Title
        self.display.safe_addstr(
            y, x, "CPU Status", curses.color_pair(1) | curses.A_BOLD
        )
        y += 1

        # CPU model
        self.display.safe_addstr(
            y, x, f"Model: {cpu_stats.model}", curses.color_pair(5)
        )
        y += 2

        # CPU utilization
        # CPU line format: "CPU    [bar]  28.0%"
        # = 7 + 2 + bar + 2 + 5 = 16 + bar
        ...
        if cpu_stats.temperature > 0:
            ...
            self.display.safe_addstr(
                y, x, line, self.display.get_color(cpu_stats.temperature)
            )
            y += 1

        # Load averages
        line = (
            f"Load   {cpu_stats.load_1min:5.2f} "
            f"{cpu_stats.load_5min:5.2f} {cpu_stats.load_15min:5.2f}"
        )
    ...
    def render(
        ...
        if separator_x >= 0:
            ...
            for gpu, vendor in gpu_info:
                y = self._render_gpu_overview(
                    gpu, vendor, y, left_inner.x, left_inner.width
                )
                y += 1

            y = self._render_memory_overview(
                memory_stats, y, left_inner.x, left_inner.width
            )
    ...
    def get_selected_process(
        self, processes: List[Dict[str, Any]], selected_index: int
    ) -> Optional[Dict[str, Any]]:
        """Get the selected process from the overview top processes list.

        Args:
            processes: List of all process information
            selected_index: Index of selected process (0-4 for top 5)
        ...
        # Sort processes by CPU usage (same as render)
        ...
        if 0 <= selected_index < len(top_processes):
            return top_processes[selected_index]
