...
class CPUPanel:
    ...
    def render(self, cpu_stats: CPUStats, start_y: int = 3) -> None:
        ...
        self.display.safe_addstr(
            y, x, "CPU Information", curses.color_pair(1) | curses.A_BOLD
        )
        y += 2

        # CPU identification
        self.display.safe_addstr(
            y, x, f"Vendor: {cpu_stats.vendor}", curses.color_pair(5)
        )
        y += 1
        self.display.safe_addstr(
        ...
        self.display.safe_addstr(y, x, "Per-Core Usage:", curses.color_pair(5))
        y += 1
        ...
        if cpu_stats.current_freq > 0:
            self.display.safe_addstr(y, x, "Frequency:", curses.color_pair(5))
            y += 1
            freq_percent = (
                (cpu_stats.current_freq - cpu_stats.min_freq)
                / (cpu_stats.max_freq - cpu_stats.min_freq)
                * 100
                if cpu_stats.max_freq > cpu_stats.min_freq
                else 0
            )
            bar, color = self.display.create_bar(freq_percent, bar_width)
            line = (
                f"Current  [{bar}] {cpu_stats.current_freq/1000:4.1f} GHz "
        ...
        # Load averages
        self.display.safe_addstr(y, x, "Load Averages:", curses.color_pair(5))
        y += 1
        line = (
