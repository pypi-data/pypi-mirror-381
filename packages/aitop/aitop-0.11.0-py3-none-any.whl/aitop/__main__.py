...
class AITop:
    ...
    def handle_input(self) -> None:
        ...
        try:
            ...
            if key == ord("q"):
                ...
                if self.selected_tab == 0 and self.system_data:  # Overview tab
                    self.handle_overview_kill_request()
                elif self.selected_tab == 1 and self.system_data:  # AI Processes tab
                ...
                if self.selected_tab == 0:  # Overview tab
                ...
                elif self.selected_tab == 1:  # AI Processes tab
                    ...
                    gpu_processes = self.gpu_panel.get_all_gpu_processes(
    ...
    def handle_gpu_kill_request(self) -> None:
        ...
        try:
            ...
            selected_process = self.gpu_panel.get_selected_process_by_index(
                self.system_data.gpu_info, self.selected_gpu_process_index
            )
            ...
            if result["warnings"]:
                if not self.modal.render_warning_list(
                    result["warnings"], "GPU Process Warning"
                ):
                    # User chose not to continue
                    return
    ...
    def _get_footer_controls(self) -> list:
        ...
        # Add kill control for Overview, AI Processes, and GPU tabs
        if self.selected_tab == 0:  # Overview tab
...
def main():
    ...
    # Display options
    parser.add_argument(
        "--theme", type=str, help="Override theme selection (e.g., monokai_pro, nord)"
    )
    parser.add_argument(
        "--no-adaptive-timing",
        action="store_true",
