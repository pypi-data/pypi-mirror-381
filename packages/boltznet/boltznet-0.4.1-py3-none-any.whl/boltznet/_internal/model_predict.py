...
class baseSeqPredict:
    ...
    def __get_annots_in_range__(self,seqid_list,pos,type_list=None,name_list=None):
        ...
        # if we are here, then now entries in annot to any seqid in list
        return None
    ...
    def __load_annot_gff__(self,gff_file):
        ...
        with open(gff_file) as f:
            for line in f:
                ...
                row = {
                    ...
                    "symbol": attr_dict.get("Name", None),                    
                    "start": int(start),
                    "stop": int(stop),
                    "strand": strand
                }
        ...
        seqid_to_df = {k: pd.DataFrame(v) for k, v in seqid_to_rows.items()}
        
        self.annots=seqid_to_df
        return
    ...
    def __load_seqs_from_fasta__(self, seqfile):
        ...
        for record in SeqIO.parse(seqfile, "fasta"):
            ...
            if match:
                seqpos.append((int(match.group(1)), int(match.group(2))))
    ...
    def __get_topN_seqs__(self,seq_inds=None, tf_names=None,perc_th=0.2,mode=1, savefilename=None):
    ...
    def __analyze_seqs__(self):
        ...
        # winSum
        x_flat = np.max(self.winSum,axis=(1)) # max over every position of each sequence => (Nseqs,ntfs)
        ranks = np.apply_along_axis(rankdata, 0, x_flat, method='average') # max is max rank
        self.winSumRanks=x_flat.shape[0] - ranks +1 # need to covert so that 1 is max rank
    ...
    def predictionRanking(self,mode = 0,rank_type = 0,layer = None,seq_inds = None,**kwargs):
        '''Ranking Function that returns a numpy array of ranks of the best sequences for each model 
    ...
    def plotSeqResponseByInds(self,inds=None, seqnames=None, annots=None, fig=None, tf_names=None, highlights=None, savefilename=None, title=None, maxN = 10, row_titles=None, dpi=300, perc_th=0.1, ranking_mode=1, **kwargs):
        ...
        If only one TF in the model, then put all responses in one figure
        Otherwise, one figure per ind with all TFs for each ind
        ...
        ######################################
        # single plot for one TF and all sequence inds
        # In this case, for each sequence, we rely on plotSeqPredict
        # to plot the annotation - each subfigure of the full fig will have
        # annot, seqlogo, heatmap, baseseq (iff set)
        ######################################
        if self.ntfs==1:
            #create the fig and subfigs if fig not given
            if fig is None:
                fig=plt.figure(figsize=(figwidth,figheightperrow*len(inds)), layout='constrained')
        ...
        # get rankings for the tfs for each sequence
        ...
        for ii,i in enumerate(inds):
            ...
            # or return none
                ...
                annot=self.__get_annots_in_range__([self.seqnames[i],'default'], self.seqpos[i])

            ######################################
            # create a new plot for each sequence i if more then one TF
            # if we have annot:
            ...
            for tt,tf_ind in enumerate(tf_inds):
                ...
                if self.ntfs==1:
                    if row_titles is None:
                        row_title='%s: %s'%(self.tf_list[tf_ind],title)
                    else:
                        row_title=row_titles[tt]
                else:
                    if row_titles is None:
                        row_title=self.tf_list[tf_ind]
                    else:
                        row_title=row_titles[tt]
                ...
                if self.ntfs>1:
                    ...
                    plotfig=subfigs[ii] # one seq per subfig
                ...
                if highlights is not None:
                    ...
                    df=self.__get_annots_in_range__([self.seqnames[i],'default'], self.seqpos[i],type_list=['TF_binding_site'],name_list=[self.tf_list[tf_ind]])
                    if df is not None:    
            ...
            if self.ntfs>1 and savefilename is not None:
        ...
        # only one plot with all sequences for a single tf
        # we do not expect savefilename to be a list,but if it is, take the first entry
        if self.ntfs==1 and savefilename is not None:
            if isinstance(savefilename,list):
...
class seqPredict(baseSeqPredict):
    ...
    def load_annot_gff(self,filename):
        '''Load annotations from a gff
        
        Annotations are associated with specific sequences through their seqid
        If seq is "default" then the annotation will be applied to all sequences
...
class baseGenomePredict(baseSeqPredict):
    ...
    def __init__(self, model_name_list=None, model_location=CONFIG.model_json_dir,genome='ecoli'):
        ...
        # TODO create a factory fuction to return object of appropriate type
        assert genome=='ecoli', 'Only supporting ecoli genome for now'
        
        # load genome object and do genomewide prediction
        self.genome=genomes.ecoliGenome()
        self.__genomewidePredict__()
        
        # load gene locations and annotations - this is ecoli only for now
        print("Getting known genes and binding sites....")
        self.genes=self.genome.getGenes()
        self.known_sites=self.genome.getTFKnownPeaks()
                
    
    def getGenesInRegion(self,start,stop):
        '''Return a data frame of genes that overlap the region start-stop'''
    
        overlapping = self.genes[(self.genes['start'] <= stop) & (self.genes['stop'] >= start)]
    ...
    def __genomewidePredict__(self):
        ...
        self.full_FRSumConv=layers[1].numpy()
        ...
        # the sum of the predictions on both strands in kernel size windows
        (mfit,mfitc,biases)=tf_util.getModelMotifs(self.predict_model)
        self.ksize=mfit[0].shape[1]
...
class genomePredict(baseGenomePredict):
    ...
    def plotRegionSeqResponse(self, tf_names=None, savefilename=None,**kwargs):
        ...
        TODO - scale hits to max over genome
        TODO - pull out common code between this and seqPredict.plotSeqResponse and put in base class
        TODO - support stacked model - one plot per TF
        TODO - support taking a list of starts and stops as long as 
        all the regions are the same length - then make plots with N subfigs
        TODO - support for saving fig
        ...
        if tf_names is not None:
            ...
            tf_inds=list(range(0,self.ntfs))
        ...
        genes=self.getGenesInRegion(start, stop)
        ...
        for tt,tf_ind in enumerate(tf_inds):
            ...
            # get the max and min conv values over all sequences for this TF
            vmax=np.max(self.conv[:,:,:,tf_ind:tf_ind+1])
            vmin=np.min(self.conv[:,:,:,tf_ind:tf_ind+1])
            ...
            plotSeqPredict(self.conv[0:1,:,:,tf_ind:tf_ind+1],self.seqs[0],self.predict_model,seqpos=self.seqpos[0],
                           ...
                           kernel_ind=tf_ind,annot=genes, highlights=highlights, **kwargs)
