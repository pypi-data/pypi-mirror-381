...
class IcepapSnapshot:
    ...
    def __init__(self, icepap: IcePAPController=None):
        log_name = '{0}.IcepapSnapshot'.format(__name__)
        self.log = logging.getLogger(log_name)
        self.ipap = icepap
        self.snapshot = {'Date': '',
                         'System': {'VER': '', 'HOST': '', 'PORT': ''},
                         'AxesErrors': [],
                         'Axes': {},
                         }
        self.done = 0
    ...
    def __getitem__(self, item):
        return self.snapshot['Axes'][item]

    def __setitem__(self, key, value):
        self.snapshot['Axes'][key] = value

    @property
    def axes(self):
        return self.snapshot['Axes'].keys()
    ...
    @property
    def system_ver(self):
        return self.snapshot['System']['VER']

    @system_ver.setter
    def system_ver(self, value):
        self.snapshot['System']['VER'] = value

    @property
    def host(self):
        return self.snapshot['System']['HOST']

    @host.setter
    def host(self, value):
        self.snapshot['System']['HOST'] = value

    @property
    def port(self):
        return self.snapshot['System']['PORT']

    @port.setter
    def port(self, value):
        self.snapshot['System']['PORT'] = value

    @property
    def axes_errors(self):
        return self.snapshot['AxesErrors']

    def add_axis_with_error(self, axis):
        self.snapshot['AxesErrors'].append(axis)

    def create_snapshot(self, filename=None):
        factor = 100 / (len(self.ipap.axes) + 2)
        _done = 0
        self.date = time.strftime('%Y/%m/%d %H:%M:%S +%z')
        self.host = self.ipap.host
        self.port = self.ipap.port
        self.system_ver = self.ipap.ver['SYSTEM']['VER'][0]
        _done += 1
        self.done = _done * factor

        for axis in self.ipap.find_axes():
            axis_snapshot = AxisSnapshot(self.ipap[axis])
            error = axis_snapshot.create_snapshot()
            if error:
                self.add_axis_with_error(axis)
            self[axis] = axis_snapshot.snapshot
            _done += 1
            self.done = _done * factor
        if filename is not None:
            self.save(filename)
        self.done = 100
        return self.axes_errors

    def check(self, snapshot):
        if self.host != snapshot.host:
            raise ValueError(f'The snapshots hosts do not match '
                             f'{snapshot.host} != {self.host}')

        diff = {'SystemVer': None,
                'NoCommonAxes': list(self.axes ^ snapshot.axes),
                'AxesWithDiff': []}
        if self.system_ver != snapshot.system_ver:
            diff['SystemVer'] = (self.system_ver, snapshot.system_ver)

        common_axes = self.axes & snapshot.axes
        for axis in common_axes:
            axis_snap = AxisSnapshot()
            axis_snap.load(self[axis])
            axis_diff = axis_snap.check(snapshot[axis])
            if axis_diff:
                diff[axis] = axis_diff
                diff['AxesWithDiff'].append(axis)
        return diff
...
def create(ctx, icepap, output, axes_str):
    ...
    if output is None:
        ...
        snapshots_folder = config_manager.snapshots_folder
...
@click.option('-a', '--all', is_flag=True,
              help='Show all position register difference. By defualt it '
                   ...
                   'Control encoder ')
@click.option('-r', '--raw', is_flag=True, help='Show raw data')
def check(first, second, all, raw):
    """
    Check two snapshot files

    \b
    The script will check the axes on the snapshots
    It will show:
    ...
    * Final results
       - System
       - Axes with important change
       - Axes with change in position/encoder registers not used
       - Axes with error on reading Fist snap
       - Axes with error on reading Second snap
       - Axes no common between two snapshots.
    * Optional:
        - Raw results: if the flag -r/--raw is set it shows raw data
        - All results: if the flag -a/--all is set it shows all changes
    """
    first_snap = IcepapSnapshot()
    first_snap.load_snapshot(first)
    second_snap = IcepapSnapshot()
    second_snap.load_snapshot(second)
    try:
        diff = first_snap.check(second_snap)
    except Exception as e:
        echo('*' * 79)
        echo('Results:')
    ...
    for axis, change in diff.items():
        if axis in ['SystemVer', 'NoCommonAxes', 'AxesWithDiff']:
            # Skipped no axes keys
            continue
        # Configuration Change
        if 'Ver' in change:
            color = 'red'
        elif axis not in diff['AxesWithDiff']:
            color = 'green'
        else:
            color = 'yellow'
    ...
    echo('*' * 79)
    echo('To see all changes use the flag "-a" ')
    echo('To see the raw changes use the flag "-r" ')
    echo('*' * 79)
    echo('Resume ')

    echo(f'System: {first_snap.host}')
    if (not axes_changed and not first_snap.axes_errors and not
    axes_change_noise and not diff['NoCommonAxes']):
        echo(f'Results:\nSystem OK')
        echo('*' * 79)
        return
    if diff['SystemVer'] is not None:
    ...
    if axes_change_noise:
        echo(f'Axes with change on position register not used:'
             f' {axes_change_noise}',
             color='blue')
    if first_snap.axes_errors:
        echo(f'Axes with error on reading first snap:'
             f' {first_snap.axes_errors}',
             color='yellow')
    if second_snap.axes_errors:
        echo(f'Axes with error on reading second snap:'
             f' {second_snap.axes_errors}',
             color='yellow')
    if diff['NoCommonAxes']:
        echo(f'Axes no common: {diff["NoCommonAxes"]}',
             color='red')
