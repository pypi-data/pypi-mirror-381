...
class IcepapCMS(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.ui = self
        with path('icepapcms.gui.ui','icepapcms.ui') as f:
            uic.loadUi(f, baseinstance=self.ui)

        self.config = ConfigManager()

        default_user = 'NotValidated'
        self.config.username = default_user
        if os.name == 'posix':  # this works for linux and macOSX
            self.config.username = os.getenv('USER', default_user)
            self._os_nt = False
        elif os.name == 'nt':  # win NT, XP... (and Vista?)
            self.config.username = os.getenv('USERNAME', default_user)
            self._os_nt = True
        ...
        if self.config.use_ldap:
            ...
            # SIGNATURES
            login = DialogLdapLogin(self.config.ldap_servers,
                                    self.config.ldap_user_template,
                                    self.config.ldap_not_allowed,
                                    self)
            accepted = login.exec()
            if not accepted:
                sys.exit(-1)
            self.config.username = login.username
    ...
    def buildLocationCombo(self):
        ...
        fixed_location = self.config.fixed_location
        if fixed_location == "*":
            fixed_location = None
    ...
    def actionTemplatesMth(self):
        ...
        with path('icepapcms.templates','catalog.xml') as f:
            dlg = TemplatesCatalogWidget(str(f), self.ui.pageiPapDriver, self)
        dlg.show()

    @loggingInfo
    def actionHelpMth(self):
        with path('icepapcms.doc', 'IcepapCMSUserManual.pdf') as f:
            webbrowser.open(str(f))

    @loggingInfo
    def actionUser_ManualMth(self):
        with path('icepapcms.doc', 'IcePAP_UserManual.pdf') as f:
            webbrowser.open(str(f))

    @loggingInfo
    def actionHardware_ManualMth(self):
        with path('icepapcms.doc', 'IcePAP_HardwareManual.pdf') as f:
            webbrowser.open(str(f))
