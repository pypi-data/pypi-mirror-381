...
class HistoricCfgWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.ui = self
        with path('icepapcms.gui.ui','historiccfgwidget.ui') as f:
    ...
    def update_data(self):
        ...
        for cfg in self.icepap_driver.historic_cfgs:
            ...
            # Note:
            # Old configuration use as default name date.now() which is passed
            # as on the IcepapDriverCfg creation and after the date is set
            # again to date.now(), thats way the name and the date are
            # diferent in few seconds/milliseconds.
            # New version change this to have the same date.now()
            default_name = datetime.strftime("%Y-%m-%d %H:%M")
            if cfg.description or not cfg.name.startswith(default_name):
