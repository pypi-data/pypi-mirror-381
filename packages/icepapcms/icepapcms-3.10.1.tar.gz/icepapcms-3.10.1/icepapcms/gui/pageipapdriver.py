...
class PageiPapDriver(QtWidgets.QWidget):
    ...
    def __init__(self, mainwin, test_mode=False):
        ...
        self.ui = self
        with path('icepapcms.gui.ui', 'pageipapdriver.ui') as f:
            uic.loadUi(f, baseinstance=self.ui, package='icepapcms.gui')

        self.signalMapper = QtCore.QSignalMapper(self)

        self.axis = QtWidgets.QDialog()
        with path('icepapcms.gui.ui', 'axis.ui') as f:
            uic.loadUi(f, baseinstance=self.axis, package='icepapcms.gui')

        self.motor = QtWidgets.QDialog()
        with path('icepapcms.gui.ui', 'motor.ui') as f:
            uic.loadUi(f, baseinstance=self.motor, package='icepapcms.gui')

        self.encoders = QtWidgets.QDialog()
        with path('icepapcms.gui.ui', 'encoders.ui') as f:
            uic.loadUi(f, baseinstance=self.encoders, package='icepapcms.gui')

        self.closedloop = QtWidgets.QDialog()
        with path('icepapcms.gui.ui', 'closedloop.ui') as f:
            uic.loadUi(f, baseinstance=self.closedloop,
                       package='icepapcms.gui')

        self.homing = QtWidgets.QDialog()
        with path('icepapcms.gui.ui', 'homing.ui') as f:
            uic.loadUi(f, baseinstance=self.homing, package='icepapcms.gui')

        self.io = QtWidgets.QDialog()
        with path('icepapcms.gui.ui', 'io.ui') as f:
            uic.loadUi(f, baseinstance=self.io, package='icepapcms.gui')
    ...
    def _setWidgetToolTips(self):
        ...
        with path('icepapcms.templates', 'driverparameters.xml') as f:
    ...
    def setDescription(self):
        ...
        cfg_db = self.config.database
        desc_cfg_hwversion = hwversion[0][1]
        desc_cfg_dbhost = self.config.db_server
        if cfg_db == DataBasesTypes.SQLite:
            desc_cfg_dbhost = self.config.db_folder
    ...
    def doImport(self):
        filename = ""
        try:
            folder = self.config.configs_folder
