...
class IcepapsManager(Singleton):
    ...
    def configDriverToDefaults(self, icepap_name, driver_addr):
        try:
            self.iPaps[icepap_name][driver_addr].send_cmd('CONFIG')
    ...
    def host_in_same_subnet(self, host):
        if self.config.run_all_networks or self.config.use_all_networks:
            return True
