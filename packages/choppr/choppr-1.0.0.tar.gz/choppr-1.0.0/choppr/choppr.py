...
from choppr.utils.files import compress_directory, extract_archive, output_list, version_existing_file
...
class Choppr:
    ...
    def __init__(
        self, mode: OperatingMode, config_file: Path, log: logging.Logger | None = None, output_sbom: Path | None = None
    ) -> None:
        ...
        Arguments:
            ...
            log: Logger to use (default None)
        ...
        try:
            ...
            config.output_files.sbom = output_sbom or config.input_files.sbom.parent.joinpath(
        ...
        log.info(f"SBOM: {Shares.input_files.sbom.resolve()}")
        if Shares.input_files.strace_results:
            log.info(f"strace Results: {Shares.input_files.strace_results.resolve()}")
        if Shares.input_files.cache_archive:
            log.info(f"Cache Archive: {Shares.input_files.cache_archive.resolve()}")
    ...
    def _resolve_allowlist_denylist(self) -> None:
        log_header("Resolving Allowlist and Denylist")

        threads, _ = self._run_handler_methods_concurrently("resolve_allowlist_denylist", purl_types=set(PurlType))
        for thread in threads:
            thread.join()
    ...
    def _get_component_scope(self, component: Component) -> tuple[ComponentScope, bool] | None:
        ...
        if Shares.options.keep_essential_os_components and component.type == ComponentType.OPERATING_SYSTEM:
            return ComponentScope.REQUIRED, False
        ...
        if purl_type and purl_type not in self.unimplemented_purl_types_logged:
            ...
            Shares.log.debug(f"Unsupported component type in SBOM: {component.type}")
