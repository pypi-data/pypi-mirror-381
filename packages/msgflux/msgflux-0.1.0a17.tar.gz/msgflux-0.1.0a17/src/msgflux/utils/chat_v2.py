...
def hint_to_schema(type_hint) -> dict:
    ...
    # Union (inclui Optional)
    if origin is Union:
        args = get_args(type_hint)
        # separa None (null) de outros
        has_none = any(a is type(None) for a in args)
        non_none_args = [a for a in args if a is not type(None)]
        schemas = [hint_to_schema(a) for a in non_none_args]

        # se só tiver um tipo não-none, simplifica
        if len(schemas) == 1:
            base = schemas[0]
            if has_none:
                # aceita null também
                # JSON Schema usa "anyOf" or "type": ["null", ...] — aqui usamos anyOf para compatibilidade
                return {"anyOf": [base, {"type": "null"}]}
            return base

        # múltiplos tipos -> anyOf
        anyof = schemas
        if has_none:
...
def generate_json_schema(cls: type) -> Dict[str, Any]:
    ...
    for param, type_hint in annotations.items():
        ...
        # Decide se é required:
        # Só NÃO é required quando o Union contém None (i.e., Optional)
        origin = get_origin(type_hint)
        is_optional = False
        if origin is Union:
            args = get_args(type_hint)
