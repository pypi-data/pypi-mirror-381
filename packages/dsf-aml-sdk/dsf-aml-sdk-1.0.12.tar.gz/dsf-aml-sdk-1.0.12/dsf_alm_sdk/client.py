...
class AMLSDK:
    BASE_URL = 'https://dsf-gbc0i4q3m-jaime-alexander-jimenezs-projects.vercel.app/'
    ...
    def _make_request(self, endpoint: str, data: Dict) -> Dict:
        ...
        try:
            ...
            # --- INICIO DEL CÓDIGO AJUSTADO ---
            elif response.status_code >= 400:
                try:
                    # Intenta obtener el error del JSON, que es el formato esperado
                    error_msg = response.json().get('error', 'API error')
                except Exception:
                    # Si falla (ej. es HTML), crea un mensaje de error con el texto plano
    ...
    def pipeline_identify_seeds(self, dataset: List[Dict], config: Union[Dict, Config], 
        ...
        """Identify informative seeds at decision boundaries"""
        if self.tier == 'community':
    ...
    def pipeline_generate_critical(self, config: Union[Dict, Config], seeds: Optional[List[Dict]] = None,
                                  original_dataset: Optional[List[Dict]] = None, **kwargs) -> Dict:
        """Generate critical variants from seeds"""
        if self.tier == 'community':
            raise LicenseError("Pipeline requires premium tier")
        ...
        if original_dataset:
            request_data['original_dataset'] = original_dataset
