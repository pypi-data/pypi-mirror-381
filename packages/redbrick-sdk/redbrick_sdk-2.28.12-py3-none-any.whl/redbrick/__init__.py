...
from redbrick.common.context import RBContextImpl, RBContextLocal
...
def version() -> str:
    ...
    version_check(__version__, False)  # config.check_version)
...
def get_org_local(
    org_id: str, schema, context, url: str = DEFAULT_URL
) -> RBOrganization:
    """
    Get an existing redbrick organization object.
    ...
    url: str = DEFAULT_URL
        Should default to https://api.redbrickai.com
    """
    return RBOrganizationImpl(
        RBContextLocal(schema=schema, context=context, url=url), org_id
    )


def get_dataset_local(
    org_id: str, dataset_name: str, schema, context, url: str = DEFAULT_URL
) -> RBDataset:
    """Get an existing RedBrick dataset object.
    ...
    url: str = DEFAULT_URL
        Should default to https://api.redbrickai.com
    """
    return RBDatasetImpl(
        RBContextLocal(schema=schema, context=context, url=url), org_id, dataset_name
    )


def get_workspace_local(
    org_id: str, workspace_id: str, schema, context, url: str = DEFAULT_URL
) -> RBWorkspace:
    """
    Get an existing RedBrick workspace object.
    ...
    """
    return RBWorkspaceImpl(
        RBContextLocal(schema=schema, context=context, url=url), org_id, workspace_id
    )


def get_project_local(
    org_id: str, project_id: str, schema, context, url: str = DEFAULT_URL
) -> RBProject:
    """
    Get an existing RedBrick project object.
    ...
    return RBProjectImpl(
        RBContextLocal(schema=schema, context=context, url=url), org_id, project_id
    )
