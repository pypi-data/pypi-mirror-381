...
async def test_congress_name_extraction():
    ...
    try:
        ...
        real_names = sum(1 for d in (house_disclosures + senate_disclosures) 
                        if d.raw_data.get('politician_name') and 
                        len(d.raw_data.get('politician_name', '')) > 3)
        
        print(f"\n📈 RESULTS:")
        print(f"Total disclosures found: {total_disclosures}")
        print(f"Disclosures with names: {real_names}")
        print(f"Success rate: {real_names/total_disclosures*100:.1f}%" if total_disclosures > 0 else "No data")
