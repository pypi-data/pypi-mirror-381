...
class DynamoDB:
    ...
    def query_items(self, table_name:str, partition_key_value:any, partition_key_name:str="phone_id"):
    ...
    def scan_table(self, table_name:str, filter_column:str=None, filter_value=None):
        ...
        if filter_column and filter_value is not None:
            params["FilterExpression"] = "#col = :val"
            params["ExpressionAttributeNames"] = {"#col": filter_column}

            if isinstance(filter_value, str):
                params["ExpressionAttributeValues"] = {":val": {"S": filter_value}}
            elif isinstance(filter_value, (int, float)):
                params["ExpressionAttributeValues"] = {":val": {"N": str(filter_value)}}
            elif isinstance(filter_value, bool):
                params["ExpressionAttributeValues"] = {":val": {"BOOL": filter_value}}
            else:
                raise TypeError(f"Tipo não suportado para filter_value: {type(filter_value)}")

        return self.ajust_return_data(self.dynamo_resource.scan(**params)["Items"])
