...
"""
__version__ = "0.0.2"
print("loading pydiodon - version du Nouvel An")
...
def coa(X, k=-1, meth="svd", transpose=False, withloops=False):
	"""
	**what it does**
	...
	*af, revised 21.02.21, 22.11.05*   
	...
	w_row		= np.sqrt(1.0/margin_row)	# inverse of square roots of row margins
	w_col		= np.sqrt(1.0/margin_col)	# inverse of square roots of column margins 	
	Dr			= np.diag(w_row)			# Dr^{-1/2}; metric on rows
	Dc			= np.diag(w_col)			# Dc^{-1/2}; metric on columns
	
	if not withloops:
		# Computation of matrix for SVD
		RC			= np.outer(margin_row, margin_col)	# expected rank one matrix of products of marginals, r \otimes c
		SG			= Dr @ (X-RC)						# S in Nenadic & Greenacre
		SG			= SG @ Dc							# SG = Dr^{-1/2}.(X- r \otimes c).Dc^{-1/2} 
		...
		if meth=="grp":
			if k==-1:
				print("\n!!! warning: k = -1 is not acceptable for svd_grp ...")
				print("please select a positive dimension")
				exit("program terminated, sorry ... !!!")
			else:
				U, S, V = svd_grp(A, k)
		#
		L	= S*S
		...
		#
		print("\ncomparing loopd and diagonal matrices")
		if withloop:
			SG	= np.zeros((n,p))
			for i in range(n):
				for j in range(p):
					SG[i,j]			= (X[i,j]-RC[i,j])/(sqrt_margin_row[i]*sqrt_margin_col[j])
			U_a, S_a, VT_a	= np.linalg.svd(SG, full_matrices=False)
			V_a		= VT_a.T
			La		= S_a*S_a
			if k > 0:
				U_a   	= U_a[:,0:k]
			...
			for i in range(n):
				for j in range(k):
					Y_ra[i,j]	= U[i,j]*S_a[j]/sqrt_margin_row[i]
			for i in range(p):
				for j in range(k):
					Y_ca[i,j]	= V[i,j]*S_a[j]/sqrt_margin_col[i] 
...
def load_dataset(dataset, datadir="../../data4tests/"):
	"""
	"""
	#
	if dataset=="diatoms_sweden":
		...
		A, rownames, colnames = loadfile(infile, delimiter="\t", rownames=True, colnames=True)
	#
	elif dataset == "example_coa":
		datafile	=  "CoA_LMF82.tsv"
		infile  	= datadir + datafile
		A, rownames, colnames = loadfile(infile, fmt="ascii", delimiter="\t", rownames=True, colnames=True)
	#
	#
...
def plot_eig(L, k=-1, frac=True, cum=False, Log=False, dot_size=-1, col="red", title=None, pr=False, x11=True, plotfile=None, fmt="png"):
	"""
...
def plot_components_splines(Y, n_axis=-1, v_col="red", title=None, x11=True, imfile=None, fmt="png"):
	"""
...
def plot_cumulated_quality_per_item(Y,r=2, col="blue", title=None, x11=True, plotfile=None, fmt="png"):
	"""
...
def plot_components_quality(Y, Qual, axis_1=1, axis_2=2, r=2, cmap="ocean", diam=50, title=None, x11=True, plotfile=None, fmt="png"):
	"""
	**what it does**
...
def plot_var_heatmap(V, varnames, cmap="viridis", title=None, x11=True, plotfile=None, fmt="png"):
	"""
	**what it does**
...
def plot_coa(Y_r, Y_c, axis_1=1, axis_2=2, col_dsize=20, row_dsize=20, row_col="blue", col_col="red", rownames=[], colnames=None, title=None, x11=True, plotfile=None, fmt="png"):
	"""
	**what it does**
