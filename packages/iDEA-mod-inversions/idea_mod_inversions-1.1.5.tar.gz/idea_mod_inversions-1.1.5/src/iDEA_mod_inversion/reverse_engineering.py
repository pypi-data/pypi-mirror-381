...
def reverse(
    ...
    **kwargs
    ...
    |     scheme: int, Set to 1 to use godby scheme set to 2 to use wang-parr scheme. (default = 1)
    |     occs_calc: bool, Set to True to use the thermal Wang-Parr inversion.  (default = False)
    |     tau: float, Electronic temperature for use with thermal Wang-Parr inversion. 
    |     chem_u: float, chemical potential for use with thermal Wang-Parr inversion. 
    |     placeholder: temp, XXXXXXXX for use with thermal Wang-Parr inversion. 
    ...
    if scheme == 1:
        while convergence > tol:
            if silent is False:
                print(
                    r"iDEA.Godby_inversion: convergence = {0:.5}, tolerance = {1:.5}".format(
                        convergence, tol
                    ),
                    end="\r",
                )
    ...
    if scheme == 2: 
        if occs_calc == False:
            while convergence > tol:
                if silent is False:
                    print(
                        r"iDEA.Wang-Parr_inversion: convergence = {0:.5}, tolerance = {1:.5}".format(
                            convergence, tol
                        ),
                        end="\r",
                    )
                ...
                denom = np.zeros(len(n))
                for i in range(int(s_fictitious.count)):
                    denom += (state.up.orbitals[:,i]**2 + state.down.orbitals[:,i]**2)*(1/(state.up.energies[i]+state.down.energies[i]))
                update_term = ((target_n - n)/(denom))*step_size
                s_fictitious.v_ext += update_term
                convergence = np.sum(abs(n - target_n)) * s.dx
            if silent is False:
                print()
    return s_fictitious
