...
def deconvolute(
    ...
    device: str = "cuda",
    n_epoch: int = 8000,
    adam_params: Optional[Dict] = None,
    batch_prior: float = 2,
    plot: bool = False,
    fig_size: Tuple[float, float] = (4.8, 3.6),
    dpi: int = 200,
):
    ...
    assert (
        X.shape[1] == sc_ref.shape[1]
    ), "Spatial data and SC reference data must have the same number of genes."
    ...
    def model(sc_ref, X, batch_prior):
        ...
        with pyro.plate("spot", n_spot, dim=-1):
            q = pyro.sample(
                "Proportion",
                dist.Dirichlet(
                    torch.full((n_type,), 3.0, device=device, dtype=torch.float64)
                ),
            )
            ...
            pyro.sample(
                "Spatial RNA", dist.Multinomial(total_count=max_exp, probs=rho), obs=X
            )
    ...
    def guide(sc_ref, X, batch_prior):
        ...
        alpha_loc = pyro.param(
            "alpha_loc", torch.zeros(n_gene, device=device, dtype=torch.float64)
        )
        alpha_scale = pyro.param(
            "alpha_scale",
            torch.ones(n_gene, device=device, dtype=torch.float64),
            constraint=constraints.positive,
        )
        base_dist = dist.Normal(alpha_loc, alpha_scale)
        sigmoid_trans = dist.transforms.SigmoidTransform()  # Transforms to [0, 1]

        alpha_dist = pyro.distributions.TransformedDistribution(
            base_dist, [sigmoid_trans]
        )
        ...
        with pyro.plate("spot", n_spot):
            sigma = pyro.param(
                "sigma",
                lambda: torch.full(
                    (n_spot, n_type), 3.0, device=device, dtype=torch.float64
                ),
                constraint=constraints.positive,
            )
...
def estimation_proportion(
    ...
    mean_exp = np.array(
        [
            np.mean(np.sum(Y[adata_sc.obs[key_type] == type_list[i]], axis=1))
...
def plot_proportion(
    ...
    spot_location: np.ndarray,
    radius: float,
    cmap_name: str = "viridis",
    alpha: float = 0.4,
    save_path: Optional[str] = "proportion.png",
    vmax: float = 0.98,
    spot_scale: float = 1.3,
    show_figure: bool = False,
    int_ticks: bool = False,
    bar_location: Tuple[int, int] = (5800, 8100),
) -> None:
    """
    ...
    img[c[0] : c[0] + cbar_array.shape[0], c[1] : c[1] + cbar_array.shape[1]][
        np.sum(cbar_array, axis=2) < 252 * 3
...
def proportion_to_count(
    p: np.ndarray, n: Union[int, np.ndarray], multiple_spots: bool = False
) -> np.ndarray:
    """
...
def simulation(
    ...
    adata_sc: anndata.AnnData,
    key_type: str,
    cell_proportion: np.ndarray,
    n_cell: Union[int, str] = 10,
    batch_effect_sigma: float = 0.1,
    zero_proportion: float = 0.3,
    additive_noise_sigma: float = 0.05,
    save: bool = True,
    out_dir: str = "",
    filename: str = "ST_Simulated.h5ad",
    verbose: int = 0,
) -> anndata.AnnData:
    """
    ...
    if verbose:
        print(
            "Prepared the ground truth. Time use {:.2f}".format(
    ...
    for i in range(n_spot):
        for j in range(n_type):
            if cell_count[i, j] > 0:
                X_temp = np.array(
                    np.sum(Y[np.random.choice(type_index[j], cell_count[i, j])], axis=0)
                )
                if X_temp.ndim > 1:
                    X[i] += X_temp[0]
                else:
    ...
    if verbose:
        print(
            "Constructed the ground truth. Time use {:.2f}".format(
                time.time() - time_start
            )
        )
        time_start = time.time()
    ...
    if verbose:
        print(
            "Added batch effect, zero reads, and additive noise. Time use {:.2f}".format(
                time.time() - time_start
            )
        )
        time_start = time.time()
    ...
    if save:
        ...
        if verbose:
            print(
                "Saved the simulated data to file. Time use {:.2f}".format(
                    time.time() - time_start
                )
            )
...
class Evaluation:
    def __init__(
        self,
        proportion_truth: np.ndarray,
        proportion_estimated_list: List[np.ndarray],
        methods: List[str],
        out_dir: str = "",
        ...
        self.function_map = {
            ...
            "Square error": self.square_error,
    ...
    def absolute_error(
        proportion_truth: np.ndarray,
        proportion_estimated: np.ndarray,
        metric_type: str = "Spot",
    ) -> np.ndarray:
    ...
    def cosine(
        ...
        if metric_type == "Spot":
            cosine_similarity = (
                np.sum(proportion_truth * proportion_estimated, axis=1)
                / np.linalg.norm(proportion_estimated, axis=1)
                / np.linalg.norm(proportion_truth, axis=1)
            )
            return cosine_similarity
        elif metric_type == "Cell type":
            cosine_similarity = (
                np.sum(proportion_truth * proportion_estimated, axis=0)
                / np.linalg.norm(proportion_estimated, axis=0)
                / np.linalg.norm(proportion_truth, axis=0)
            )
            return cosine_similarity
        else:
            raise ValueError(f"Invalid metric type {metric_type}")
    ...
    def correlation(
        ...
        if metric_type == "Cell type":
            proportion_truth_centered = proportion_truth - np.mean(
                proportion_truth, axis=0
            )
            proportion_estimated_centered = proportion_estimated - np.mean(
                proportion_estimated, axis=0
            )
            correlation_values = np.sum(
                proportion_truth_centered * proportion_estimated_centered, axis=0
            ) / np.sqrt(
                np.sum(proportion_truth_centered**2, axis=0)
                * np.sum(proportion_estimated_centered**2, axis=0)
            )
            return correlation_values
        elif metric_type == "Spot":
            proportion_truth_centered = proportion_truth - np.mean(
                proportion_truth, axis=1, keepdims=True
            )
            proportion_estimated_centered = proportion_estimated - np.mean(
                proportion_estimated, axis=1, keepdims=True
            )
            correlation_values = np.sum(
                proportion_truth_centered * proportion_estimated_centered, axis=1
            ) / np.sqrt(
                np.sum(proportion_truth_centered**2, axis=1)
    ...
    def evaluate_metric(
        self, metric="Cosine similarity", metric_type="Spot", region=None
    ):
        """
        ...
        for i in range(self.n_method):
            metric_values.append(
                func(
                    self.proportion_truth[select],
                    self.proportion_estimated_list[i][select],
                    metric_type,
                )
            )
    ...
    def plot_metric(
        ...
        if region is not None:
            region_name = (
                "_" + "+".join(region) if isinstance(region, list) else "_" + region
            )

        if metric_type == "Cell type" and cell_types is not None:
            idx = np.array(
                [np.where(self.type_list == type_)[0][0] for type_ in cell_types]
            )
            ...
            methods_name = np.repeat(
                self.methods, len(self.metric_dict[metric + " " + metric_type][0])
            )
        ...
        if show:
            ...
            ax = sns.boxplot(
                data=df, y=metric, x="Method", showfliers=False, palette=palette
            )
            if metric_type == "Spot":
                ax = sns.stripplot(
                    data=df,
            ...
            if save:
                plt.savefig(
                    f"{self.out_dir}figures/{metric}_{metric_type}{region_name}{suffix}.jpg",
                    dpi=500,
                    bbox_inches="tight",
                )
            plt.show()
    ...
    def plot_metric_spot_type(self, save=False, metric="Absolute error"):
        ...
        df = pd.DataFrame(
            {"metric": metric_values, "Method": methods_name, "Cell type": cell_type}
        )

        for cell_type in self.type_list:
            ax = sns.boxplot(
                data=df[df["Cell type"] == cell_type],
                y="metric",
                x="Method",
                showfliers=False,
            )
            ...
            if save:
                ...
                plt.savefig(
                    f"{self.out_dir}figures/{metric} {cell_type}.jpg",
                    dpi=500,
                    bbox_inches="tight",
                )
            plt.show()
    ...
    def plot_metric_all(self, save=False, metric="Absolute error", region=None):
        ...
        if save:
            plt.savefig(
                f"{self.out_dir}figures/{metric}{region_name}.tiff",
                dpi=800,
                bbox_inches="tight",
            )
        plt.show()
...
def decomposition(
    ...
    if verbose:
        print(
            "Prepared proportion data. Time use {:.2f}".format(time.time() - time_start)
        )
        time_start = time.time()

    adata_sc_temp, adata_st_temp = sc_reference.initialization(
        adata_sc, adata_st, filtering=filtering_gene
    )
    if verbose:
        print(
            "Initialized scRNA and ST data. Time use {:.2f}".format(
                time.time() - time_start
            )
        )
        time_start = time.time()
    ...
    X = (
        adata_st_temp.X
        if type(adata_st_temp.X) is np.ndarray
        else adata_st_temp.X.toarray()
    )
    # X = np.array(adata_st_temp.X)
    if verbose:
        print(
            "Processed scRNA and ST data. Time use {:.2f}".format(
                time.time() - time_start
            )
        )
        time_start = time.time()
    ...
    if verbose:
        print(
            "Decomposition complete. Time use {:.2f}".format(time.time() - time_start)
        )
        time_start = time.time()
    ...
    ST_decompose = (
        ST_decompose / (np.sum(ST_decompose, axis=1, keepdims=True) + 1e-10) * 1e6
    )
    ...
    if verbose:
        print(
            "Constructed ST decomposition data file. Time use {:.2f}".format(
                time.time() - time_start
            )
        )
        time_start = time.time()
    ...
    if save:
        ...
        if verbose:
            print(
                "Saved file to output folder. Time use {:.2f}".format(
                    time.time() - time_start
                )
            )
...
def assign_type_out(
    ...
    smooth = np.sum(weights[:, :, np.newaxis] * cell_proportion, axis=1) / np.sum(
...
def archive_assign_type_out_gp(
    nucleus_df: pd.DataFrame,
    cell_proportion: np.ndarray,
    spot_centers: np.ndarray,
    type_list: List[str],
...
) -> Union[pd.DataFrame, Tuple[pd.DataFrame, List[GaussianProcessRegressor]]]:
    """Assign the cell type to the cells outside the spot.
    ...
    for i in tqdm(range(len(type_list))):
        ...
        gp_list += [gp]
        interpolation[:, i] = (
            gp_list[i].predict(nucleus_centers, return_std=False) * std[i] + mean[i]
        )
