...
def test():
    ...
    material_name = 'TOLUENE_PROPANE_BUTANE'
    composition_moles = [0.2, 0.2, 0.6]

    # Define the material
    material = Material(name = material_name, components = [MaterialComponent(name = 'TOLUENE', mole_fraction=0.2), 
                                                            MaterialComponent(name = 'PROPANE', mole_fraction= 0.2),
                                                            MaterialComponent(name = 'N-BUTANE', mole_fraction= 0.6)], component_count= 3)
    ...
    if resultCode == ResultCode.SUCCESS:
        # check whether mass composition is a number and not zero
        if not isinstance(convert_comp_mole_to_mass_calculation.composition_mass[0], float) or convert_comp_mole_to_mass_calculation.composition_mass[0] == 0.0:
            assert False, f'Regression failed with convert_comp_mole_to_mass_calculation.composition_mass[0] = {convert_comp_mole_to_mass_calculation.composition_mass[0]}'
