...
def test():
    ...
    """
    Distances and footprints to concentration levels calculation test case with the following properties:
    ...
    # Set up the distances and footprints to concentration levels calculation.
    ...
    print('Running distances_and_footprints_to_concentration_levels_calculation...')
    resultCode = distances_and_footprints_to_concentration_levels_calculation.run()

    # Print any messages.
    if len(distances_and_footprints_to_concentration_levels_calculation.messages) > 0:
        print('Messages:')
        for message in distances_and_footprints_to_concentration_levels_calculation.messages:
            print(message)

    if resultCode == resultCode.SUCCESS:
        # Check whether the number of contour_points is greater than 0
        if (len(distances_and_footprints_to_concentration_levels_calculation.contour_points) == 0):
            assert False, f'Regression failed with len(distances_and_footprints_to_concentration_levels_calculation.contour_points) = {len(distances_and_footprints_to_concentration_levels_calculation.contour_points)}'
        
        print(f'SUCCESS: distances_and_footprints_to_concentration_levels_calculation ({distances_and_footprints_to_concentration_levels_calculation.calculation_elapsed_time}ms)')
    else:
        assert False, f'FAILED distances_and_footprints_to_concentration_levels_calculation with result code {resultCode}'
