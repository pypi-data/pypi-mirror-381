...
def test():
    ...
    """
    Max distance to conc calculation test case with the following properties:
    ...
    # Set up the entities required by the max distance to conc calculation.
    dispersion_output_config = DispersionOutputConfig(concentration = end_point_concentration)

    # Set up the max distance to conc calculation.
    max_distance_to_conc_calculation = MaxDistanceToConcCalculation(scalar_udm_outputs= dispersion_calculation.scalar_udm_outputs, weather= weather, dispersion_records= dispersion_calculation.dispersion_records, dispersion_record_count= len(dispersion_calculation.dispersion_records), substrate=substrate, dispersion_output_config=dispersion_output_config, material=vessel_catastrophic_rupture_calculation.exit_material, dispersion_parameters=dispersion_parameters)

    print('Running max_distance_to_conc_calculation...')
    resultCode = max_distance_to_conc_calculation.run()

    # Print any messages.
    if len(max_distance_to_conc_calculation.messages) > 0:
        print('Messages:')
        for message in max_distance_to_conc_calculation.messages:
            print(message)

    if resultCode == resultCode.SUCCESS:
        # check whether the max distance to concentration is a number and not zero
        if not isinstance(max_distance_to_conc_calculation.distance, (int, float)) or max_distance_to_conc_calculation.distance == 0:
            assert False,f'Regression failed with max_distance_to_conc_calculation.distance = {max_distance_to_conc_calculation.distance}'
        
        print(f'SUCCESS: max_distance_to_conc_calculation ({max_distance_to_conc_calculation.calculation_elapsed_time}ms)')
    else:
        assert False, f'FAILED max_distance_to_conc_calculation with result code {resultCode}'
