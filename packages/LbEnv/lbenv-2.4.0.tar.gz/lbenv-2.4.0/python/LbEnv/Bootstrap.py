...
def collect_roots(root):
    ...
    if isdir(root):
        ...
        try:
            ...
            yield from chain.from_iterable(map(collect_roots, deps))
...
def main():
    ...
    from LbEnv import DEFAULT_SITEROOT, __version__
    ...
    parser.add_argument(
        "-m", "--mysiteroot", dest="siteroot", default=DEFAULT_SITEROOT, help=SUPPRESS
    )
    ...
    if {"-h", "--help"}.intersection(sys.argv) and {
        "--sh",
        "--csh",
        "--py",
    ...
    host_os = "{}-{}".format(os.uname()[4], LbPlatformUtils.host_os().split("-")[1])

    control.set("LCG_hostos", host_os)

    mysiteroot = args.siteroot if args.siteroot else LbEnv.computeDefaultSiteRoot()
    ...
    # Set X509 variables
    control.set("X509_VOMS_DIR", "/cvmfs/lhcb.cern.ch/etc/grid-security/vomsdir")
    control.set("X509_VOMSES", "/cvmfs/lhcb.cern.ch/etc/grid-security/vomses")
    control.set("X509_CERT_DIR", "/cvmfs/lhcb.cern.ch/etc/grid-security/certificates")
    ...
    if args.shell == "py":
        ...
        lines = [
            template.format(name, shlex.quote(value))
            for name, value in sorted(env.items())
        ]
        lines.append('source "$LBENV_ALIASES"')
        print("\n".join(lines))
