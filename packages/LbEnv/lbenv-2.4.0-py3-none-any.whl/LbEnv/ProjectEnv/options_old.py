...
from LbEnv import DEFAULT_SITEROOT
from LbEnv.ProjectEnv import SearchPath, SearchPathEntry
...
def addSearchPath(parser):
    ...
    def nightly_base(option, opt_str, value, parser):
        ...
        if not os.path.isdir(value):
            raise OptionValueError(f'"{value}" is not a directory')
    ...
    def user_area_option(_option, _opt_str, value, parser):
        from LbEnv.ProjectEnv.lookup import force_paths_from_slot

        print("in user_area_option:", value)
        force_paths_from_slot(value)
        parser.values.user_area = value

    parser.add_option(
        "--user-area",
        action="callback",
        callback=user_area_option,
        type="string",
        nargs=1,
