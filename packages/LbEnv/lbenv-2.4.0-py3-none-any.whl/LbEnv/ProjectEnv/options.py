...
from LbEnv import computeDefaultSiteRoot
from LbEnv.ProjectEnv import EnvSearchPathEntry, SearchPath, SearchPathEntry
...
"""
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}({self.base!r}, {self.slot!r}, {self.day!r})"
...
def addSearchPath(parser):
    ...
    class NightlyBaseAction(argparse.Action):
        ...
        def __call__(self, parser, namespace, values, option_string=None):
            if namespace.nightly:
                raise parser.error(f"{option_string} specified after --nightly")

            if not os.path.isdir(values):
                raise parser.error(f'"{values}" is not a directory')
    ...
    class NightlySlotAction(argparse.Action):
        def __call__(self, parser, namespace, values, option_string=None):
            ...
            from LbEnv.ProjectEnv.lookup import (
                ...
                findNightlyDir,
                force_paths_from_slot,
            )

            try:
                slot_dir = findNightlyDir(slot, day, nightly_bases)
                force_paths_from_slot(slot_dir)
    ...
    class UserAreaAction(argparse.Action):
        def __call__(self, parser, namespace, values, option_string=None):
            from LbEnv.ProjectEnv.lookup import force_paths_from_slot

            force_paths_from_slot(values)
            setattr(namespace, self.dest, values)

    parser.add_argument(
        "--user-area",
        action=UserAreaAction,
    ...
    parser.add_argument(
        "-r",
        "--siteroot",
        default=computeDefaultSiteRoot(),
...
def addPlatform(parser):
    ...
    parser.add_argument(
        "-c",
        "--platform",
        help="runtime platform. To require one of several platforms a regex can be used "
        'wrapped in forward slashes, e.g. "/.*clang.*detdesc.*/". With "best", the most '
        "appropriate is chosen [default: %(default)s]",
        default=None,
    )
    parser.add_argument(
        "--force-platform",
    ...
    if not platform:
        ...
        if btag and cconf and btag != cconf:
            parser.error(
                ...
                f"({btag} != {cconf}), please unset CMTCONFIG or fix "
                "the values"
            )
