...
from LbEnv import computeDefaultSiteRoot, fixProjectCase
...
class TemporaryEnvironment:
    ...
    def gen_script(self, shell_type):
        ...
        if shell_type not in shells:
            raise RuntimeError(
                f"Shell type '{shell_type}' unknown. Available: {shells}"
            )
        out = ""
        ...
        while old_vars:
            ...
            if key not in self.env:
                ...
                if can_be_added:
                    # set variable
                    if shell_type == "csh":
                        out += f'setenv {key} "{self.env[key]}"\n'
                    elif shell_type == "sh":
                        out += f'export {key}="{self.env[key]}"\n'
                    elif shell_type == "bat":
                        out += f"set {key}={self.env[key]}\n"
                    done_vars.append(key)
                else:
                    old_vars.append(key)
        return out
...
def FindProjectVersions(
    ...
    if requested_version:
        ...
        special_project_dirs.append(f"{project}_{requested_version}")

        prj = fixProjectCase(project)
        if prj != project:
            # same as above, but with canonical project name (if applicable)
            special_project_dirs.append(f"{prj}_{requested_version}")

        # release style project name+version
        special_project_dirs.append(
            os.path.join(PROJECT, f"{PROJECT}_{requested_version}")
        )
...
def SortVersionTuples(versions, reverse=False):
    ...
    """
    vers_map = {t[1]: t for t in versions[::-1]}
...
class ProjectInfo:
    ...
    def __init__(self, project, version, realName, path):
        ...
        if self.version:
            self._projectenv_cmt_dir = os.path.join(
                self.path, f"{self.name}Env", self.version, "cmt"
            )
    ...
    def __str__(self):
        if self.version:
            return f"{self.name} {self.version} from {self.project_dir}"
        else:
            return f"{self.name} from {self.project_dir}"
...
class GetNightlyCMTPROJECTPATH(ContentHandler):
    ...
    def __init__(self, slot, day=None):
        ...
        if day in (None, "Today"):
            ...
            raise TypeError(f"What do you mean with day = {day!r}?")
...
class SetupProject:
    ...
    def check_environment(self):
        ...
        if "CMTCONFIG" not in self.environment:
    ...
    def _prepare_parser(self):
        from optparse import IndentedHelpFormatter as BasicHelpFormatter
        ...
        def external_version_option(_option, opt_str, value, parser):
            if len(parser.largs) < 2:
                raise OptionValueError(
                    f"{opt_str} must be preceded by the name of the external"
                )
        ...
        def runtime_project_option(_option, opt_str, _value, parser):
            if len(parser.rargs) < 1:
                raise OptionValueError(
                    f"{opt_str} must be followed by the project name and optionally by the version"
                )
        ...
        parser.add_option(
            ...
            action="store_false",
            dest="auto_override",
            help=f"Do not automatically prepend the projects {auto_override_projects}",
        )
        ...
        def nightly_base(option, opt_str, value, parser):
            ...
            if parser.values.nightly:
                raise OptionValueError(f"{option} specified after --nightly")
            ...
            if not os.path.isdir(path):
                raise OptionValueError(f'"{path}" is not a directory')
        ...
        def nightly_option(_option, opt_str, _value, _parser):
            ...
            try:
                ...
                raise OptionValueError(
                    f"{opt_str} must be followed by the slot of the "
                    "nightlies and optionally by the build id"
                )
            ...
            if not slot_dir:
                raise OptionValueError(
                    f"Cannot find slot {slot} in {nightly_bases}. Check the values of the option {opt_str}"
                )
            path = os.path.join(slot_dir, day)
            if not os.path.isdir(path):
                raise OptionValueError(
                    f"The directory {path} does not exists. Check the values of the option {opt_str}"
                )
    ...
    def _ask_version(self, versions):
        ...
        while not ans:
            sys.stderr.write(
                "Please enter your choice ({} q[uit] [{}]): ".format(
            ...
            if ans == "":
                ...
                ans = None
        if ans:
            self._always(f"Trying version '{ans}'")
    ...
    def _prepare_tmp_local_project(self):
        ...
        self._debug(f"Using temporary directory '{tmp_dir}'")
        ...
        if not self.external_only and self.project_info.sys:
            ...
            # we use it
            req.write(f"use {self.project_info.sys} *\n")

        # add user specified tags
        for t in self.tag_add:
            req.write(f"apply_tag {t}\n")
        ...
        for e in (
            ...
            if e != "CASTOR" or not self.disable_CASTOR:  # skip CASTOR
                req.write(f"use {e} v* LCG_Interfaces\n")
        ...
        for e in self.ext_versions:
            req.write(f'macro {e}_config_version "{self.ext_versions[e]}"\n')
        ...
        for p in self.overriding_projects + [self.project_info] + self.runtime_projects:
            prj.write("use {}\n".format(p.realName.replace(os.sep, " ")))
        prj.flush()
        del prj
    ...
    def _gen_setup(self, root_dir):
        ...
        if "command not found" in out:
            ...
            self._debug("----- using CMT {} -----".format(out.strip().split("\n")[-1]))
        ...
        if out and not self.force:  # non empty output means error
            ...
            inc_notice = re.compile(
                r"^("
                r"# Required version.*incompatible with|"
                r"#CMT---> Warning: apply_tag with empty name"
                r").*"
            )
        ...
        if self.context_path:
            if not self.opts.quiet:
                lines.append(f"echo Using CMTUSERCONTEXT = '{self.context_path}'")
    ...
    def _prepend_prompt(self):
        ...
        if (
            ...
            if self.shell == "sh":
                ...
                new_prompt += f"    PS1=$'%{{\\e[1;34m%}}[{self.project_info.name}]%{{\\e[m%}}'$PS1\n"
                new_prompt += "  else\n"
                ...
                new_prompt += f'  set prompt="%B%{{\\033[34m%}}[{self.project_info.name}]%{{\\033[0m%}}%b$prompt"\n'
                new_prompt += "endif\n"
        else:
            if self.shell == "sh":
                new_prompt = 'if [[ -n "$PS1" ]]; then\n'
                # assume that ZSH_VERSION is always set in zsh and never in bash
                new_prompt += "  if [[ -n $ZSH_VERSION ]]; then\n"
                new_prompt += f"    PS1=$'%{{\\e[1;34m%}}[{self.project_info.name} {self.project_info.version}]%{{\\e[m%}}'$PS1 \n"
                ...
                new_prompt = "if ( $?prompt ) then\n"
                new_prompt += f'  set prompt="%B%{{\\033[34m%}}[{self.project_info.name} {self.project_info.version}]%{{\\033[0m%}}%b$prompt"\n'
                new_prompt += "endif\n"
    ...
    def parse_args(self, args=sys.argv[1:]):
        ...
        # It's not possible to use callbacks with default values, so we need to
        # handle the default value for the --siteroot option here.
        if self.opts.siteroot is None:
            self.parser.get_option("--siteroot").callback(
                self.parser.get_option("--siteroot"),
                "--siteroot",
                computeDefaultSiteRoot(),
                self.parser,
            )
    ...
    def prepare(self, args=None):
        ...
        self._verbose(f"SetupProject {self.parser.get_version()}")
        ...
        self._debug(
            f"Look for all versions of '{self.project_name}' in {self.search_path}"
            + ((self.user_area and (f" with user area in '{self.user_area}'")) or "")
        )
        versions = FindProjectVersions(
            self.project_name,
            self.search_path,
        ...
        if not versions:
            self._error(f"Cannot find project '{self.project_name}'")
            return 1
        ...
        if self.project_version:
            for v in versions:
                ...
                self._error(
                    f"Cannot find version '{self.project_version}' of {self.project_name}. Try with --list."
                )
                return 1
        ...
        if required_root_version:
            ...
            # version of ROOT
            self._debug(
                f"Looking for the LCGCMT version providing ROOT {required_root_version}"
            )
            ...
            if lcg_version:
                ...
                self._error(f"Cannot find ROOT {required_root_version}")
                return 1
        ...
        if self.args and not self.project_version:
            ...
            if pattern.lower() == "latest":
                ...
                if found:
                    ...
                    self._error(
                        f"Cannot find version '{self.args[0]}' of {self.project_name}. Try with --list-versions."
                    )
                    return 1
        ...
        if not self.project_info:
            ...
            self._error(
                f"PANIC: Cannot find version '{self.project_version}' of {self.project_name} after initial check"
            )
            return 1
        ...
        for p, v in self.opts.runtime_projects:
            ...
            if not vv:
                self._error(f"Cannot find project '{p}'")
                return 1
            pi = makeProjectInfo(
                version=v,
                versions=vv,
            ...
            if not pi:
                self._error(
                    f"Cannot find version '{v}' of {p}. Try with --list-versions."
                )
                return 1
        ...
        for p in self.overriding_projects + [self.project_info] + self.runtime_projects:
            if not p.supportsPlatform(platform, self.user_area):
                self._error(
                    f"Project {p.name} {p.version} is not available for platform {platform}"
                )
                return 1
            self._verbose(f"Project {p.name} {p.version} uses {p.policy} policy")
    ...
    def run(self, args=None):
        ...
        if not self.opts.quiet:
            self._always(f"Configuring {self.project_info}")
        ...
        if "CMTPROJECTPATH" in self.environment:
            messages.append(
                "Using CMTPROJECTPATH = '{CMTPROJECTPATH}'".format(**self.environment)
            )
            if self.keep_CMTPROJECTPATH:
                messages.append("(as defined by the user)")
        else:
            messages.append(f"Using CMTPATH = '{CMTPATH}'")
        tmps = self.project_info.name
        if self.project_info.version:
            tmps += f" {self.project_info.version}"
        messages.append(f"Environment for {tmps} ready.")
        ...
        if len(lines) > 1:
            lines = (
                [f"({lines[0]},"] + [f" {l}," for l in lines[1:-1]] + [f" {lines[-1]})"]
            )
        else:
            lines[0] = f"({lines[0]})"
        messages += lines
        ...
        if self.project_info.policy == "old":
            ...
            if varname in self.environment:
                ...
                if os.path.isdir(exedir):
                    ...
                    messages.append(f"Appending {exedir} to the path.")
                    self.environment["PATH"] = os.pathsep.join(
...
class Comp(SetupProject):
    def _prepare_parser(self):
        ...
        parser.add_option("--disallow-containers", action="store_true", help="ignored")
        parser.add_option("--prefer-container", action="store_true", help="ignored")
        parser.add_option("--bind", action="append", help="ignored")
        return parser
...
def expandAllVars(env, iterations=10):
    ...
    new_env = {key: expandVars(value, env) for key, value in env.items()}
