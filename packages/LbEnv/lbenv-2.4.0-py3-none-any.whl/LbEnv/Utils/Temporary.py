        ...
        if self.name and not self._closed:
            try:
                ...
                if "None" not in str(ex):
                    raise
                _sys.stderr.write(f"ERROR: {ex!r} while cleaning up {self!r}")
                return
            ...
            if _warn:
                ...
                self._warn(f"Implicitly cleaning up {self!r}", UserWarning)
...
class TempDir(TemporaryDirectory):
    ...
    def __init__(self, suffix="", prefix="tmp", base_dir=None, keep_var="KEEPTEMPDIR"):
        ...
        """
        super().__init__(suffix, prefix, base_dir)
