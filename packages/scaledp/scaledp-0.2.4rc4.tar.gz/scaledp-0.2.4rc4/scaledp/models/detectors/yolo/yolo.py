...
class YOLO:
    ...
    def rescale_image_with_padding(self, image: np.ndarray, target_size: Tuple[int, int]) -> np.ndarray:
        """
        ...
        padded_image[self.pad_y:end_y, self.pad_x:end_x] = resized_image[:actual_height, :actual_width]
    ...
    def restore_coordinates(self, boxes: np.ndarray) -> np.ndarray:
        ...
        restored_boxes[:, [0, 2]] = np.clip(restored_boxes[:, [0, 2]], 0, self.original_width)
    ...
    def prepare_input(self, image):
        ...
        print(
            f"Input width: {self.input_width}, Input height: {self.input_height}")

        # Rescale image with padding instead of simple resize
        input_img = self.rescale_image_with_padding(input_img, (self.input_width, self.input_height))
