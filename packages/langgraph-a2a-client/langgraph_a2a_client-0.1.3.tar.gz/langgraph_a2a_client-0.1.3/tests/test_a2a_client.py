...
async def test_discover_agent_success(a2a_client, mock_agent_card):
    ...
    with patch.object(a2a_client, "_discover_agent_card", new_callable=AsyncMock) as mock_discover:
        ...
        mock_discover.assert_called_once_with("https://example.com/new-agent")
...
async def test_send_message_success(a2a_client, mock_agent_card):
    ...
    mock_message = MagicMock(
        model_dump=MagicMock(return_value={"content": "Response from agent"})
    )
    ...
    with patch.object(a2a_client, "_discover_agent_card", new_callable=AsyncMock) as mock_discover, \
        ...
        mock_client.send_message = AsyncMock(return_value=iter([mock_message]))
        ...
        result = await a2a_client.a2a_send_message(
            message_text="Hello",
            target_agent_url="https://example.com/agent",
            message_id="test-123"
        )
...
async def test_send_message_error(a2a_client):
    ...
    with patch.object(a2a_client, "_discover_agent_card", new_callable=AsyncMock) as mock_discover:
        ...
        result = await a2a_client.a2a_send_message(
            message_text="Hello",
            target_agent_url="https://example.com/bad-agent"
        )
