...
"""pyrugged Class PolynomialRotationTest"""
...
def test_identity(raw):
    ...
    for _index in range(20):
        ...
        builder.add_los_transform(
            PolynomialRotation("identity", Vector3D(rvg.nextVector()), AbsoluteDate.J2000_EPOCH, [0.0])
        )
        ...
        assert tdl.get_parameters_drivers()[0].name == "identity[0]"
...
def test_fixed_combination(raw):
    ...
    for _ in range(20):
        ...
        builder.add_los_transform(PolynomialRotation("r1", axis_1, AbsoluteDate.J2000_EPOCH, [angle_1]))
        ...
        builder.add_los_transform(PolynomialRotation("r2", axis_2, AbsoluteDate.J2000_EPOCH, [angle_2]))
        ...
        builder.add_los_transform(PolynomialRotation("r3", axis_3, AbsoluteDate.J2000_EPOCH, [angle_3]))
        ...
        assert driver_3.name == "r3[0]"
        assert driver_3.min_value == float("inf") or driver_3.min_value == float("-inf")
        assert driver_3.min_value < 0
        assert driver_3.max_value == float("inf") or driver_3.max_value == float("-inf")
        assert driver_3.max_value > 0
