...
"""pyrugged Class CheckedPatternElevationUpdater"""
...
class CheckedPatternElevationUpdater(TileUpdater):
    """Elevation Updater for pyrugged/raster tests"""

    def __init__(self, size, n_val, elevation_1, elevation_2):
        """Builds a new instance."""

        self.size = size
        self.n_val = n_val
        self.elevation_1 = elevation_1
        self.elevation_2 = elevation_2
    ...
    def update_tile(self, latitude, longitude, tile):
        ...
        for i in range(self.n_val):
            p_val = int(np.floor((min_latitude + i * step) / step))

            for j in range(self.n_val):
                q_val = int(np.floor((min_longitude + j * step) / step))
                tile.set_elevation(i, j, self.elevation_1 if (((p_val ^ q_val) & 1) == 0) else self.elevation_2)
