...
class BuildingBlock:
    ...
    def __repr__(self):
        return (
            "BuildingBlock(symmetry={}, core={}, connector={}, funcGroups={})".format(
                self.symmetry, self.core, self.conector, self.funcGroups
            )
        )
    ...
    def from_file(self, path, file_name):
        ...
        read_func_dict = {"xyz": read_xyz, "gjf": read_gjf}
    ...
    def from_name(self, name):
        ...
        symm_check, core_check, conector_check, funcGroup_check = self.check_existence(
            name
        )

        error_msg = "Building Block name is invalid!\n"
        error_msg += "Symm: {}, Core: {}, Connector: {}, Functional Group:{}".format(
            symm_check, core_check, conector_check, funcGroup_check
        )
        ...
        self.create_BB_structure(
            self.symmetry, self.core, self.conector, *possible_funcGroups
        )
        if self.save_bb:
            self.save()
    ...
    def rotate_around(
        self,
        rotation_axis: list | NDArray = [1, 0, 0],
        angle: float = 0.0,
        degree: bool = True,
    ):
        """
    ...
    def add_connection_group(self, conector_name):
        ...
        self.smiles = self.smiles.replace(
            "[Q]", f"{connector.properties['smiles'].replace('[Q]', '')}"
        )
        ...
        for i in range(len(location_Q_struct[0])):
            ...
            close_Q_struct = find_closest_atom(
                "Q", location_Q_struct[1][i], self.atom_types, self.atom_pos
            )[1]
            ...
            close_Q_connector = find_closest_atom(
                "Q", location_Q_connector[1][0], n_conector_label, n_conector_pos
            )[1]
            ...
            n_conector_pos = np.delete(
                n_conector_pos,
                find_index(np.array([0.0, 0.0, 0.0]), n_conector_pos),
                axis=0,
            )
            ...
            rotated_translated_group = (
                np.dot(n_conector_pos, -np.transpose(Rot_m)) + location_Q_struct[1][i]
            )
    ...
    def add_connection_group_symm(self, conector_name):
        ...
        # Remove the Q atoms from structure
        self.atom_types = [
            self.atom_types[i] for i in range(len(self.atom_types)) if i not in Q_idx
        ]
        self.atom_labels = [
            self.atom_labels[i] for i in range(len(self.atom_labels)) if i not in Q_idx
        ]

        self.atom_pos = np.array(
            [self.atom_pos[i] for i in range(len(self.atom_pos)) if i not in Q_idx]
        )
        ...
        conector_pos = R.from_rotvec(
            -90 * unit_vector(conector_pos[1] - conector_pos[0]), degrees=True
        ).apply(conector_pos)
    ...
    def add_R_group(self, R_name, R_type):
        ...
        self.smiles = self.smiles.replace(
            f"[{R_type}]", f"{rgroup.properties['smiles'].replace('[Q]', '')}"
        )
        ...
        for i, _ in enumerate(location_R_struct):
            ...
            close_R_struct = closest_atom_struc(
                R_type, location_R_struct[i], self.atom_types, self.atom_pos
            )[1]
            ...
            close_R_group = find_closest_atom(
                "R", pos_R_group[0], n_group_label, n_group_pos
            )[1]
            ...
            n_group_pos = np.delete(
                n_group_pos, find_index(np.array([0.0, 0.0, 0.0]), n_group_pos), axis=0
            )

            # Rotate and translade the R group to R position in the strucutre
            rotated_translated_group = (
                np.dot(n_group_pos, -np.transpose(Rot_m)) + location_R_struct[i]
            )
            ...
            # Remove the R atoms from structure
            self.atom_pos = np.delete(
                self.atom_pos, find_index(location_R_struct[i], self.atom_pos), axis=0
            )
    ...
    def get_available_core(self):
        ...
        for symm in self.available_symmetry:
            ...
            available_cores[symm] = [
                i.rstrip(".cjson") for i in os.listdir(symm_path) if ".cjson" in i
            ]
    ...
    def check_existence(self, name):
        ...
        if symm in self.available_symmetry:
            ...
            print(
                "ERROR!: Building Block symmetry be one of the following:",
                ",".join(self.available_symmetry),
            )
            symm_check = False
    ...
    def get_buildingblock_list(self, shape, connector_group):
        ...
        return [
            i.rstrip(".xyz")
            for i in files_list
            if shape == i.split("_")[0] and connector_group in i.split("_")[2]
        ]
