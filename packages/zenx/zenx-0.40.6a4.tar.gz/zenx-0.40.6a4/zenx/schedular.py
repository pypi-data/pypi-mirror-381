...
class Schedular(ABC):
    ...
    async def execute_task(self, task_func: Callable[[], Coroutine]) -> None:
        loop = asyncio.get_running_loop()
        try:
            self.logger.debug("executing task", at=loop.time())
            await task_func()
    ...
    async def _dispatcher(self, task_func: Callable[[], Coroutine], start_time: float) -> None:
        ...
        while not self.shutdown_event.is_set():
            ...
            if delay:
                ...
                self.logger.debug("sleeping", target_time=target_time, monotonic_clock=loop.time(), sleep_time=delay, should_wake_at=loop.time() + delay)
                await asyncio.sleep(delay)
            self.logger.debug("waked up early", target_time=target_time, monotonic_clock=loop.time())
            ...
            self.logger.debug("time has come", target_time=target_time, monotonic_clock=monotonic_clock, drift=monotonic_clock - target_time)
