...
def crawl(
    ...
    if len(spiders) > 1:
        ...
        engine.run_spiders(spiders_to_run)
...
def solveragent():
    ...
    from zenx.solver_agent import SolverAgent

    logger = configure_logger("solver_agent", settings)
    agent = SolverAgent(logger=logger, settings=settings)
    logger.info("Starting solver agent")
    uvloop.run(agent.run())
