...
async def consume_target(target: str):
    ...
    while True:
        ...
        try:
            blueprints = await solver_redis.rpop(target, count=100)
            if not blueprints:
                logger.debug("check", target=target, blueprints=len(blueprints))
                continue
            await local_redis.lpush(target, *blueprints)
            logger.debug("got", target=target, blueprints=len(blueprints))
        except Exception:
