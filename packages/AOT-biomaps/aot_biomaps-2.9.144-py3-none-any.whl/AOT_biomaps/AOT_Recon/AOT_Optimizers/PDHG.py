...
from AcoustoOpticTomography.AOT_biomaps.AOT_Recon.ReconEnums import NoiseType
import torch
...
def CP(
    ...
    """
    Primal-Dual Hybrid Gradient (PDHG) method for image reconstruction.
    Supports Total Variation (TV) and Kullback-Leibler (KL) divergence regularization.
    ...
    Args:
        ...
        isSavingEachIteration: If True, returns all intermediate reconstructions
        method: 'TV' for Total Variation, 'KL' for Kullback-Leibler divergence
        L: Lipschitz constant (estimated if None)
        withTumor: Boolean for description only
        device: Torch device (auto-selected if None)
    """
    if noiseModel == NoiseType.GAUSSIAN:
    ...
    elif noiseModel == NoiseType.POISSON:
        return chambolle_pock_KL(
            SMatrix,
            y,
            alpha,
            theta,
        ...
        raise ValueError("Noise model must be specified: Gaussian (NoiseType.GAUSSIAN) or Poisson (NoiseType.POISSON)")
    else:
        raise ValueError(f"Unknown noise model: {noiseModel}. Use Gaussian (NoiseType.GAUSSIAN) or Poisson (NoiseType.POISSON)")
...
def chambolle_pock_TV(
    ...
    # Return results
    if isSavingEachIteration:
        return [tensor.cpu().numpy() for tensor in I_reconMatrix]
    else:
        return (x.reshape(Z, X) * (norm_y / norm_A)).cpu().numpy()

def chambolle_pock_KL(
    SMatrix,
    y,
    alpha=1e-9,
    theta=1.0,
