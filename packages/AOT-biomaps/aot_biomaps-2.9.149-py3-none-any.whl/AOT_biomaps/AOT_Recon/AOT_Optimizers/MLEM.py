    ...
    try:
        ...
        if use_gpu:
            if use_multi_gpu and torch.cuda.device_count() > 1:
                return _MLEM_multi_GPU(SMatrix, y, numIterations, isSavingEachIteration, tumor_str)
            else:
                return _MLEM_single_GPU(SMatrix, y, numIterations, isSavingEachIteration, tumor_str, device)
        else:
            if use_numba:
                return _MLEM_CPU_numba(SMatrix, y, numIterations, isSavingEachIteration, tumor_str)
            else:
                return _MLEM_CPU_opti(SMatrix, y, numIterations, isSavingEachIteration, tumor_str)

    except Exception as e:
        print(f"Error in MLEM: {type(e).__name__}: {e}")
        return None, []

def _MLEM_single_GPU(SMatrix, y, numIterations, isSavingEachIteration, tumor_str, device):
    try:
...
def _MLEM_multi_GPU(SMatrix, y, numIterations, isSavingEachIteration, tumor_str):
    try:
...
def _MLEM_CPU_numba(SMatrix, y, numIterations, isSavingEachIteration, tumor_str):
    try:
...
def _MLEM_CPU_opti(SMatrix, y, numIterations, isSavingEachIteration, tumor_str):
    try:
        T, Z, X, N = SMatrix.shape
