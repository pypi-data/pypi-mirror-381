...
'''Notes: for imported() and modx_imported(), dependencies of modx itself will
not show up as this feature will be added in the future if the users want it.
...
import ast, tokenize, io, importlib.util
from packaging import version
...
def vcompat(module_name, python_version=None):
    ...
    # --- Tokenize: identifiers only (ignores comments/strings) ---
    token_names = set()
    for tok in tokenize.generate_tokens(io.StringIO(content).readline):
        if tok.type == tokenize.NAME:
            token_names.add(tok.string)

    # --- AST parse: catch syntax constructs ---
    try:
        tree = ast.parse(content)
    except Exception:
        tree = None

    def add_warning(key, rule, label):
    ...
    for key, rule in COMPAT_RULES.items():
        ...
        if key in token_names:
            # Check if this feature is problematic for ANY of our target versions
            problematic_for_any = False
            earliest_version = None
            
            if "removed_in" in rule:
                removed_ver = version.parse(rule["removed_in"])
                # Check if any target version is >= removal version
                for target in targets:
                    if target >= removed_ver:
                        problematic_for_any = True
                        if earliest_version is None or removed_ver < earliest_version:
                            earliest_version = removed_ver
                        break
            
            elif "added_in" in rule:
                added_ver = version.parse(rule["added_in"])
                # Check if any target version is < added version
                for target in targets:
                    if target < added_ver:
                        problematic_for_any = True
                        if earliest_version is None or added_ver < earliest_version:
                            earliest_version = added_ver
                        break
            
            if problematic_for_any and earliest_version:
                if "removed_in" in rule:
                    add_warning(key, rule, (str(earliest_version), f"REMOVED → {rule['note']}"))
                elif "added_in" in rule:
                    add_warning(key, rule, (str(earliest_version), f"NOT AVAILABLE before {rule['added_in']} → {rule['note']}"))
    ...
    if tree:
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.NamedExpr):
                ...
                if problematic:
                    add_warning("walrus", COMPAT_RULES["walrus"],
            ...
            if hasattr(ast, "Match") and isinstance(node, ast.Match):
                ...
                if problematic:
                    add_warning("match", COMPAT_RULES["match"],
            ...
            if isinstance(node, ast.AsyncFunctionDef):
                ...
                if problematic:
                    add_warning("async_kw", COMPAT_RULES["async"],
                                ("3.5", f"CHANGED since 3.5 → {COMPAT_RULES['async']['note']}"))

            if isinstance(node, ast.Await):
                problematic = any(target < version.parse("3.5") for target in targets)
                if problematic:
                    add_warning("await_kw", COMPAT_RULES["await"],
                                ("3.5", f"CHANGED since 3.5 → {COMPAT_RULES['await']['note']}"))

            # F-strings (Python 3.6+) - FIXED: Only warn if checking against Python < 3.6
            if isinstance(node, ast.JoinedStr):
                problematic = any(target < version.parse("3.6") for target in targets)
                if problematic:
                    add_warning("fstrings", COMPAT_RULES["fstrings"],
