...
class A2AServer:
    ...
    def agent_skills(self) -> list[AgentSkill]:
        ...
        return self._agent_skills if self._agent_skills is not None else []
    ...
    def serve(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if app_type == "fastapi":
                uvicorn.run(
                    self.to_fastapi_app(), host=host or self.host, port=port or self.port, **kwargs
                )
            else:
                uvicorn.run(
                    self.to_starlette_app(), host=host or self.host, port=port or self.port, **kwargs
                )
