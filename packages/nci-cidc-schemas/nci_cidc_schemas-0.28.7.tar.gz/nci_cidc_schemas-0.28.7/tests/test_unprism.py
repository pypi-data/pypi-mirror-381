...
def test_derive_files_CyTOF_analysis():
    ...
    # remove 'output_files' from first cytof assay (simulate not included in analysis)
    ct["assays"]["cytof"][0]["records"][0].pop("output_files", None)
    # ensure still able to successfully retrieve result even if cytof assay output_files do not exist
    result = derive_files(DeriveFilesContext(ct, "cytof_analysis", fetch_artifact))
    assert len(result.artifacts) == 3
