...
from labels.parsers.cataloger.alpine.utils import new_alpine_package
...
def parse_apk_db(
    ...
    apk_db_entries = _collect_apk_db_entries(content)
    packages = _collect_packages(_env, apk_db_entries, location)
...
def _parse_package_content(content: str) -> ApkDBEntry | None:
...
def _construct_apk_db_entry(data: dict[str, str]) -> ApkDBEntry | None:
    package = data.get("P")
    version = data.get("V")
...
def _collect_packages(
    _env: Environment | None, apk_db_entries: list[ApkDBEntry], location: Location
) -> list[Package]:
    packages: list[Package] = []
    environment = _env.linux_release if _env else None

    for apk_db_entry in apk_db_entries:
        package = new_alpine_package(apk_db_entry, environment, location)
        if package:
...
def _collect_relationships(packages: list[Package]) -> list[Relationship]:
    ...
    for package in packages:
        if not isinstance(package.ecosystem_data, ApkDBEntry):
            continue
...
def _build_lookup_table(packages: list[Package]) -> dict[str, list[Package]]:
    ...
    for package in packages:
        if package.ecosystem_data is None:
            continue
        if not isinstance(package.ecosystem_data, ApkDBEntry):
            continue
