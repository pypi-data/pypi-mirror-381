...
class Byneuron:
    ...
    def get_numberevents(self, item, size):
        ...
        starts at current timestamp and returns in time
        """
        ...
        f_time = {  # added to return only historical events.
            "type": "range",
            "to": self._data_timestamp(self.now),
            "maxInclusive": True
        }
        indexsets = indexset_keys
        data = self._numberevents(indexsets=indexsets, filters=[f_item, f_time], size=size, order='DESC')
        if data:
            for i in data:
