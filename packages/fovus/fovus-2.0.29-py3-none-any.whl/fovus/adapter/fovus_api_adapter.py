...
class FovusApiAdapter:
        ...
        list_runs = self.get_list_runs(
            {
                ...
                "filterOptions": {
                    "runStatus": "Running",
                    "runNames": [task_name] if task_name else None,
                },
            }
        )

        self._log_debug(job_id, f"Found {len(list_runs.get('runList', []))} running tasks", "debug")
    ...
    def terminate_task(self, job_id: str, run_id: str):
        headers = self.fovus_cognito_adapter.get_authorization_header()
        response = requests.post(
            FovusApiUtil.get_api_address(Api.JOB, ApiMethod.TERMINATE_TASK),
            json={"workspaceId": self.workspace_id, "jobId": job_id, "runId": run_id},
            headers=headers,
            timeout=TIMEOUT_SECONDS,
        )
        return FovusApiUtil.confirm_successful_response(response.json(), response.status_code, self.__class__.__name__)
    ...
    @cache_result(cache_key="get_pipeline", ttl=600, persistent=True)
