...
def create_response_model(model: type[BaseModel], security_policy: Optional[SecurityPolicy] = None) -> type[BaseModel]:
    """
    ...
    if not filtered_fields:
        raise ValueError(
            f"Security policy resulted in no available fields for model {model.__name__}. "
            "Check allowed_fields and blocked_fields configuration."
        )

    # Create optional fields
    optional_fields = {name: (Optional[field.annotation], None) for name, field in filtered_fields.items()}
...
def _filter_fields_by_policy(fields, security_policy: Optional[SecurityPolicy]):
    """
...
def _get_allowed_fields_set(security_policy: SecurityPolicy) -> Optional[Set[str]]:
