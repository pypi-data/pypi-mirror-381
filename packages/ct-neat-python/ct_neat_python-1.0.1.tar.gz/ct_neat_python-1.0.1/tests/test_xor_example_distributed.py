...
import ctneat
from ctneat.distributed import MODE_PRIMARY, MODE_SECONDARY
...
def run_primary(addr, authkey, generations):
    ...
    checkpointer = ctneat.Checkpointer(max(1, int(generations / 4)), 10)
    ...
    if (checkpointer.last_generation_checkpoint >= 0) and (checkpointer.last_generation_checkpoint < 100):
        ...
        p2 = ctneat.checkpoint.Checkpointer.restore_checkpoint(filename)
        p2.add_reporter(ctneat.StdOutReporter(True))
        stats2 = ctneat.StatisticsReporter()
