...
def token_create(payload: dict, key: str, minutes: int = 5, algorithm: Algorithms = ALGORITHMS.RS256, encryption_config: Optional[EncDecConfig] = None) -> str:
    ...
    try:
        ...
        signed_jwt = sign(claims=claims_dict, key=key, algorithm=algorithm)
...
def token_verify(token: str, key: str, algorithms: List[Algorithms] = [ALGORITHMS.RS256], decryption_config: Optional[EncDecConfig] = None) -> dict:
    ...
    try:
        ...
        return verify(token=token, key=key, algorithms=algorithms)
