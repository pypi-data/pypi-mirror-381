...
# -------------------------
# NEW: Deduped overview counts for DATA (one count per dataset)
# -------------------------
def _overview_counts_data_dedup(state: dict, owner_sel: Optional[str] = None) -> Dict[str, int]:
    """
    Count each DATA dataset once, using the WORST status across all its stages/chunks.
    This fixes the inflation where summing per-stage statuses double-counts datasets.
    """
    ...
    # Filter owners if owner_sel is used (owner label or raw owner key)
    ...
    # best-per-dataset (actually "worst" severity)
    ...
    for owner in owners_to_scan:
        for mode, name_map in (jobs.get(owner, {}) or {}).items():
            for dataset, node in (name_map or {}).items():
                # collect worst status across all four stages for this dataset
                worst_dataset = None
                worst_i = 10**9
                for stg in DATA_STAGES:
                    ...
                    if i < worst_i:
                        worst_i = i
                        worst_dataset = w
                # if no counts, fall back to chunks inspection (rare)
                ...
                if worst_dataset is None:
                    worst_dataset = "other"
                # record one status per dataset; if same dataset appears across owners/modes,
                # keep the worst among them
                prev = ds_worst.get(dataset)
                if prev is None:
                    ds_worst[dataset] = worst_dataset
                else:
                    if rank.get(worst_dataset, 9999) < rank.get(prev, 9999):
                        ds_worst[dataset] = worst_dataset

    out: Dict[str, int] = {s: 0 for s in vocab}
    for s in ds_worst.values():
        out[s] = out.get(s, 0) + 1
    # keep unknowns in 'other'
    known = set(vocab)
    other_extra = 0
...
def register_callbacks(app, cfg, host):
    ...
    def refresh(_n, tab,
        ...
        if tab in ("data", "all"):
            stage_opts = [{"label": st.title(), "value": st} for st in DATA_STAGES]
        else:
            stage_opts = []
        ...
        if tab == "data":
            # use DEDUPED overview counts for Data
            data_counts_overview = _overview_counts_data_dedup(state, owner_sel)
            fig_ext_overview = pie.figure("data", "Overview",  data_counts_overview, labels_order=status_vocab)
            ...
            # sum four tabs, but use DEDUPED counts for data part
            dedup_data = _overview_counts_data_dedup(state, owner_sel)
            tot_counts: Dict[str, int] = {}
            tabs_to_sum = {
                "data": dedup_data,
                "features": cached_aggregate_counts(state, "features"),
                "alphas": cached_aggregate_counts(state, "alphas"),
                "strategies": cached_aggregate_counts(state, "strategies"),
            }
            for _t, part in tabs_to_sum.items():
        ...
        if tab == "all":
            dedup_data = _overview_counts_data_dedup(state, owner_sel)
            tot_counts: Dict[str, int] = {}
            for part in (dedup_data,
                         cached_aggregate_counts(state, "features"),
                         cached_aggregate_counts(state, "alphas"),
                         cached_aggregate_counts(state, "strategies")):
                for k, v in (part or {}).items():
                    tot_counts[k] = tot_counts.get(k, 0) + int(v or 0)

            pies_for_kpi = [
                pie.figure("all", "Overview",  tot_counts,                          labels_order=status_vocab),
                pie.figure("data", "Data",     dedup_data,                          labels_order=status_order_for_tab("data")),
            ...
            data_counts_overview = _overview_counts_data_dedup(state, owner_sel)
            pies_for_kpi = [
                pie.figure("data", "Overview",  data_counts_overview, labels_order=status_vocab),
        ...
        # Respect configured refresh_ms (stop 1s flicker)
