    ...
    def render(self, cfg):
        return dbc.Card(
            dbc.CardBody([
                ...
                dcc.Dropdown(
                    id="rows-per-page",
                    options=[{"label": str(n), "value": n} for n in (20, 40, 60, 80, 100, 150, 200)] + [{"label": "All", "value": 999999}],
                    value=200,
                    clearable=False,
                    style={"width": "120px"},
                ),
                html.Div(id="selection-tools"),
                html.Div(id="kpi-container", className="mt-3"),
            ]),
            style={"margin": "0"},
        )
# dataset_dashboard/components/compute_kpi.py
...
class KpiStrip:
    ...
    def render(self, tab: str, vocab: List[str], counts: Dict[str, int], per_row: int = 3):
        ...
        return html.Div(
            ...
            style={
                "display": "grid",
                "gridTemplateColumns": "repeat(3, minmax(160px, 1fr))",  # exactly 3 per row
                "gap": "12px",
                "maxWidth":  "100%",
                "text-align"    : "center",
                "marginTop": "8px",
