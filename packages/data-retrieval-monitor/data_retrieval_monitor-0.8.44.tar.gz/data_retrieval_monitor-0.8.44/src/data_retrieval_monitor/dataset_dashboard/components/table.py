...
class TableComponent:
    ...
    def _chunk_inline_links(
        ...
        *,
        ...
        # Build #pick?… for the selection panel (dataset, idx, tab, stage)
        qs_parts = [f"dataset={quote_plus(str(dataset))}", f"idx={idx}"]
        if source_tab:
            qs_parts.append(f"tab={quote_plus(source_tab)}")
        if source_tab == "data" and source_stage:
            qs_parts.append(f"stage={quote_plus(source_stage)}")
        qs = "&".join(qs_parts)
    ...
    def _chunk_block(
        ...
        *,
        ...
        """
        Lightweight inline rendering of all chunks:
           c0 [p][📋?] c1 [p][📋?] c2 ...
        """
        if not chunks:
        ...
        return html.Span(nodes, style={"whiteSpace": "nowrap"})
    ...
    def _build_section_table(
        ...
        for row_groups in _chunked(entries_sorted, gpr):
            ...
            for _, own, md, dn, d_map in row_groups:
                if is_data:
                    # Shade by WORST chunk status per stage (not best)
                    stage_stat = {}
                    for stg in vis_stages:
                        leaf = self._safe(d_map.get(stg))
                        worst = self._worst_status_from_chunks("data", list(self._safe(leaf.get("chunks"))))
                        stage_stat[stg] = worst
                    ...
                    for stg in vis_stages:
                        ...
                        cells.append(
                            html.Td(
                                self._chunk_block(
                                    "data",
                                    list(self._safe(leaf.get("chunks"))),
                                    chunks_per_line,
                                    prefix="c",
                                    dataset_name=dn,
                                    compact=compact,
                                    source_tab="data",
                                    source_stage=stg,  # so selection knows which stage list contains chunks
                                ),
                                style={
                                    "verticalAlign": "top",
                                    "padding": "6px 10px",
                    ...
                    # Shade by WORST status among this cell's chunks
                    worst = self._worst_status_from_chunks(section_tab, list(self._safe(leaf.get("chunks"))))
