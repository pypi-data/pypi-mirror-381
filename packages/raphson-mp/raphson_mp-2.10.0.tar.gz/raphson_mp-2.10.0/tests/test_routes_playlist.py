...
async def test_share_playlist_no_permission(http_client: T_client, playlist: Playlist, user_id: int, csrf_token: str):
    ...
    with db.MUSIC.connect() as conn:
        assert conn.execute("SELECT * FROM user_playlist_write WHERE user = ?", (user_id,)).fetchone() is None
