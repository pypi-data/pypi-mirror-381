...
async def test_user_edit(http_client: T_client, csrf_token: str, user_id: int, conn: Connection):
    ...
    with user_admin(conn):
        ...
        assert not await auth.verify_password(conn, user_id, TEST_PASSWORD)
        # Try new password
        assert await auth.verify_password(conn, user_id, new_password)
        user = auth.User.get(conn, user_id=user_id)
        # Check username
        assert user.username == new_username

        # Restore username and password
        await user.update_username(TEST_USERNAME)
        await user.update_password(TEST_PASSWORD)
