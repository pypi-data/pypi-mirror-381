...
async def test_add_remove(
    client: RaphsonMusicClient, http_client: T_client, track: Track, conn: Connection, user_id: int, csrf_token: str
):
    conn.execute("DELETE FROM dislikes WHERE user = ? AND track = ?", (user_id, track.path))
