...
def main():
    ...
    projectfile = toml.loads(Path("pyproject.toml").read_text())
    ...
    subprocess.check_call(["podman", "build", "-t", "-f", "Containerfile.nginx", f"codeberg.org/raphson/music-server-nginx:{version}", "--target", "prod", "."])
