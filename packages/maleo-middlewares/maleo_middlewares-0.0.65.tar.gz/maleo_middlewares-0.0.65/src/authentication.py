...
from maleo.enums.organization import Role as OrganizationRole
...
class Backend(AuthenticationBackend):
    ...
    def _build_authentication_component(
        ...
        scopes = ["authenticated"] + [f"{domain}:{role}" for role in roles]
    ...
    async def _authenticate_api_key(
        ...
        *,
        ...
        return self._build_authentication_component(user, user_organization)
    ...
    async def _validate_impersonation(
        ...
        impersonated_user, impersonated_user_organization = await self._get_credentials(
        ...
        if is_system(authentication):
            ...
            if (
                authentication.credentials.organization_id
                != impersonation.organization_id
            ):
                raise AuthenticationError(
